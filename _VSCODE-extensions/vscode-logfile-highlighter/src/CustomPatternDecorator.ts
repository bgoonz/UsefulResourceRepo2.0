...
class CustomPatternDecorator {
  ...
  public updateConfiguration(): void {
    const configPatterns = vscode.workspace
      .getConfiguration("logFileHighlighter")
      .get("customPatterns") as Array<{ pattern: string; foreground: string }>;
    ...
    for (const item of configPatterns) {
      if (item.foreground !== undefined && item.pattern !== undefined) {
        this._configPattern.push(
          new CustomPattern(item.pattern, item.foreground)
        );
      }
    }
  }
  ...
  public decorateDocument(changedEvent: vscode.TextDocumentChangeEvent): void {
    if (
      this._configPattern.length === 0 ||
      changedEvent.contentChanges.length === 0
    ) {
      return;
    }
    ...
    const change = changedEvent.contentChanges.sort((a, b) =>
      Math.abs(a.range.start.line - b.range.start.line)
    )[0];
    ...
    const contentToEnd: string = doc.getText(
      new vscode.Range(startPos, doc.lineAt(doc.lineCount - 1).range.end)
    );
