...
class TimePeriodCalculator {
  ...
  public getTimePeriod(data: string): moment.Duration {
    ...
    const datesPattern =
      "(" + isoDatePattern + "|" + cultureDatesPattern + "){1}";
    ...
    if (matches.length >= 2) {
      ...
      if (firstMoment.isValid() && lastMoment.isValid()) {
        ...
        if (
          moment.isDuration(firstDuration) &&
          moment.isDuration(lastDuration)
        ) {
          // Used for non ISO dates like '13:12:11.001'
          timePeriod = moment.duration(
            lastDuration.asMilliseconds() - firstDuration.asMilliseconds()
          );
        }
      }
    }
