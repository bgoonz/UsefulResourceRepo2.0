...
export class SessionManager implements Middleware {
  ...
  constructor(
    ...
    this.hostVersion = vscode.extensions.getExtension(
    ...
    this.log.write(
      ...
      `Operating System: ${
        OperatingSystem[this.platformDetails.operatingSystem]
      } ${osBitness}`
    );
  ...
  public start() {
    ...
    if (
      os.platform() === "darwin" &&
      this.getPowerShellVersionLabel() === "alpha"
    ) {
      if (
        !(
        ...
        const thenable = vscode.window.showWarningMessage(
        ...
        thenable.then((s) => {
          if (s === "Show Documentation") {
            cp.exec(
              "open https://github.com/PowerShell/vscode-powershell/blob/master/docs/" +
    ...
    if (this.powerShellExePath) {
      this.bundledModulesPath = path.resolve(
        __dirname,
        this.sessionSettings.bundledModulesPath
      );
      ...
      if (this.inDevelopmentMode) {
        ...
        if (
          fs.existsSync(
            path.join(devBundledModulesPath, "PowerShellEditorServices/bin")
          )
        ) {
      ...
      this.editorServicesArgs =
        ...
        `-BundledModulesPath '${PowerShellProcess.escapeSingleQuotes(
  ...
  public createDebugSessionProcess(
    sessionPath: string,
    sessionSettings: Settings.ISettings
  ): PowerShellProcess {
  ...
  public getPowerShellExePath(): string {
    ...
    if (
      ...
      const powerShellExePaths = getAvailablePowerShellExes(
        this.platformDetails,
        this.sessionSettings
      );
      const powerShellDefaultVersion = powerShellExePaths.find(
        (item) =>
          item.versionName === this.sessionSettings.powerShellDefaultVersion
      );
  ...
  private onConfigurationUpdated() {
    ...
    this.focusConsoleOnExecute =
      settings.integratedConsole.focusConsoleOnExecute;

    // Detect any setting changes that would affect the session
    if (
      !this.suppressRestartPrompt &&
      (settings.useX86Host !== this.sessionSettings.useX86Host ||
        settings.powerShellExePath.toLowerCase() !==
  ...
  private registerCommands(): void {
    this.registeredCommands = [
      vscode.commands.registerCommand("PowerShell.RestartSession", () => {
        this.restartSession();
      }),
      vscode.commands.registerCommand(this.ShowSessionMenuCommandName, () => {
      ...
      vscode.commands.registerCommand(
        ...
        (isExecute?: boolean) => {
          this.showSessionConsole(isExecute);
        }
      ),
    ];
  }
  ...
  private startPowerShell() {
    ...
    const sessionFilePath = utils.getSessionFilePath(
      Math.floor(100000 + Math.random() * 900000)
    );

    this.languageServerProcess = new PowerShellProcess(
      this.powerShellExePath,
      this.bundledModulesPath,
    ...
    this.languageServerProcess.start("EditorServices").then(
      (sessionDetails) => {
  ...
  private startLanguageClient(
    sessionDetails: utils.IEditorServicesSessionDetails
  ) {
    ...
    try {
      this.log.write(
        `Connecting to language service on pipe ${sessionDetails.languageServicePipeName}...`
      );
      ...
      this.languageServerClient.onReady().then(
        () => {
          ...
          this.languageServerClient.onNotification(
            ...
            (runspaceDetails) => {
              this.setStatusBarVersionString(runspaceDetails);
            }
          );
        },
        (reason) => {
  ...
  private changePowerShellExePath(exePath: string) {
    this.suppressRestartPrompt = true;
    Settings.change("powerShellExePath", exePath, true).then(() =>
      this.restartSession()
    );
  }
  ...
  private resolvePowerShellPath(powerShellExePath: string): string {
    ...
    if (!utils.checkIfFileExists(resolvedPath)) {
      this.setSessionFailure(
        "powershell.exe cannot be found or is not accessible at path " +
          resolvedPath
      );
  ...
  private showSessionMenu() {
    ...
    const availablePowerShellExes = getAvailablePowerShellExes(
      this.platformDetails,
      this.sessionSettings
    );

    if (this.sessionStatus === SessionStatus.Running) {
      const currentPowerShellExe = availablePowerShellExes.find(
        (item) => item.exePath.toLowerCase() === currentExePath
      );
      ...
      menuItems = [
        new SessionMenuItem(`Current session: ${powerShellSessionName}`, () => {
    ...
    const powerShellItems = availablePowerShellExes
      ...
      .map((item) => {
        return new SessionMenuItem(`Switch to ${item.versionName}`, () => {
    ...
    menuItems.push(
      new SessionMenuItem("Open Session Logs Folder", () => {
...
export const RunspaceChangedEventType = new NotificationType<
  IRunspaceDetails,
  void
