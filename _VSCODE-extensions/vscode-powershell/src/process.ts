...
export class PowerShellProcess {
  ...
  public start(
    logFileName: string
  ): Thenable<utils.IEditorServicesSessionDetails> {
    return new Promise<utils.IEditorServicesSessionDetails>(
      (resolve, reject) => {
        try {
          ...
          this.startArgs +=
            `-LogPath '${PowerShellProcess.escapeSingleQuotes(
              editorServicesLogPath
            )}' ` +
            `-SessionDetailsPath '${PowerShellProcess.escapeSingleQuotes(
              this.sessionFilePath
            )}' ` +
            `-FeatureFlags @(${featureFlags})`;

          const powerShellArgs = ["-NoProfile", "-NonInteractive"];
          ...
          if (this.sessionSettings.developer.powerShellExeIsWindowsDevBuild) {
            ...
            const batScriptPath = path.resolve(
              __dirname,
              "../../sessions/powershell.bat"
            );
            fs.writeFileSync(
              batScriptPath,
              `@set DEVPATH=${path.dirname(
                powerShellExePath
              )}\r\n@${powerShellExePath} %*`
            );
          ...
          this.consoleTerminal.processId.then((pid) => {
            this.log.write(`powershell.exe started, pid: ${pid}`);
          });
        } catch (e) {
          reject(e);
        }
