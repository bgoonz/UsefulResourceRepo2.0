...
export const System32PowerShellPath =
  getWindowsSystemPowerShellPath("System32");
export const SysnativePowerShellPath =
  getWindowsSystemPowerShellPath("Sysnative");
export const SysWow64PowerShellPath =
  getWindowsSystemPowerShellPath("SysWow64");
...
export function fixWindowsPowerShellPath(
  ...
  if (
    (platformDetails.isProcess64Bit &&
      lowerCasedPath === powerShell64BitPathOn32Bit) ||
    (!platformDetails.isProcess64Bit &&
      lowerCasedPath === powerShell32BitPathOn64Bit)
  ) {
...
export function getAvailablePowerShellExes(
  platformDetails: IPlatformDetails,
  sessionSettings: Settings.ISettings | undefined
  ...
  if (platformDetails.operatingSystem === OperatingSystem.Windows) {
    ...
    exePaths.forEach((exePath) => {
      if (fs.existsSync(exePath)) {
        paths.push({
          versionName:
            "PowerShell Core" + (/-preview/.test(exePath) ? " Preview" : ""),
          exePath,
        });
      }
    });
