...
export const ScriptRegionRequestType = new RequestType<any, any, void, void>(
  "powerShell/getScriptRegion"
);
...
class DocumentLocker {
  ...
  public unlockAll(): void {
    Object.keys(this.lockedDocuments)
      .slice()
      .forEach((documentKey) => this.unlockInternal(documentKey));
  }
    ...
    OnTypeFormattingEditProvider
{
  ...
  private static showStatusBar(
    document: TextDocument,
    hideWhenDone: Thenable<any>
  ): void {
    const statusBar = AnimatedStatusBar.showAnimatedStatusBarMessage(
      "Formatting PowerShell document",
      hideWhenDone
    );
  ...
  private static disposeAllStatusBars() {
    Object.keys(this.statusBarTracker)
      .slice()
      .forEach((key) => this.disposeStatusBar(key));
  }
  ...
  public provideDocumentRangeFormattingEdits(
    ...
    this.logger.writeVerbose(
      `Formatting document range ${JSON.stringify(range)} - ${document.uri}...`
    );
  ...
  public provideOnTypeFormattingEdits(
    ...
    this.logger.writeVerbose(
      `Formatting on type at position ${JSON.stringify(position)} - ${
        document.uri
      }...`
    );

    return this.getScriptRegion(document, position, ch).then(
      (scriptRegion) => {
      ...
      (err) => {
        this.logger.writeVerbose(
          `Error while requesting script region for formatting: ${err}`
        );
      }
    );
  }
  ...
  private logAndReturnTextEdits(
    textEdits,
    getFormattingDuration: () => number
  ): vscode.TextEdit[] | Thenable<vscode.TextEdit[]> {
    return textEdits.then(
      (edits) => {
        this.logger.writeVerbose(
          `Document formatting finished in ${getFormattingDuration()}s`
        );
        return edits;
      },
      (err) => {
        this.logger.writeVerbose(
          `Document formatting failed in ${getFormattingDuration()}: ${err}`
        );
      }
    );
  }

  private getScriptRegion(
    document: TextDocument,
    position: Position,
    ch: string
  ): Thenable<IScriptRegion> {
    const oneBasedPosition = toOneBasedPosition(position);
    return this.languageClient
      .sendRequest(ScriptRegionRequestType, {
  ...
  private lockDocument(
    ...
    PSDocumentFormattingEditProvider.documentLocker.lock(
      document,
      unlockWhenDone
    );
  }
...
export class DocumentFormatterFeature implements IFeature {
  ...
  constructor(private logger: Logger, documentSelector: DocumentSelector) {
    ...
    this.formattingEditProvider =
      vscode.languages.registerDocumentFormattingEditProvider(
        documentSelector,
        this.documentFormattingEditProvider
      );
    this.rangeFormattingEditProvider =
