...
export class CustomViewsFeature implements IFeature {
  ...
  public setLanguageClient(languageClient: LanguageClient) {
    languageClient.onRequest(NewCustomViewRequestType, (args) => {
      this.contentProvider.createView(args.id, args.title, args.viewType);
    });

    languageClient.onRequest(ShowCustomViewRequestType, (args) => {
      this.contentProvider.showView(args.id, args.viewColumn);
    });

    languageClient.onRequest(CloseCustomViewRequestType, (args) => {
      this.contentProvider.closeView(args.id);
    });

    languageClient.onRequest(SetHtmlContentViewRequestType, (args) => {
      this.contentProvider.setHtmlContentView(args.id, args.htmlContent);
    });

    languageClient.onRequest(AppendHtmlOutputViewRequestType, (args) => {
...
class PowerShellContentProvider implements vscode.TextDocumentContentProvider {
  ...
  private didChangeEvent: vscode.EventEmitter<vscode.Uri> =
...
class HtmlContentView extends CustomView {
  ...
  public getContent(): string {
    ...
    if (
      this.htmlContent.styleSheetPaths &&
    ...
    if (
      this.htmlContent.javaScriptPaths &&
...
export const NewCustomViewRequestType = new RequestType<
  INewCustomViewRequestArguments,
  void,
  void,
  void
...
export const ShowCustomViewRequestType = new RequestType<
  IShowCustomViewRequestArguments,
  void,
  void,
  void
...
export const CloseCustomViewRequestType = new RequestType<
  ICloseCustomViewRequestArguments,
  void,
  void,
  void
...
export const SetHtmlContentViewRequestType = new RequestType<
  ISetHtmlContentViewRequestArguments,
  void,
  void,
  void
...
export const AppendHtmlOutputViewRequestType = new RequestType<
  IAppendHtmlOutputViewRequestArguments,
  void,
  void,
  void
