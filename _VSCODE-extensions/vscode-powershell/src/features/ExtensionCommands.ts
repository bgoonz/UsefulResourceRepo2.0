...
export const InvokeExtensionCommandRequestType = new RequestType<
  InvokeExtensionCommandRequestArguments,
  void,
  void,
  void
...
export const ExtensionCommandAddedNotificationType = new NotificationType<
  IExtensionCommandAddedNotificationBody,
  void
...
function asCodeRange(value: Range): vscode.Range {
  ...
  return new vscode.Range(
    asCodePosition(value.start),
    asCodePosition(value.end)
  );
}
...
export class ExtensionCommandsFeature implements IFeature {
  ...
  constructor(private log: Logger) {
    ...
    this.command2 = vscode.commands.registerCommand(
      ...
      (param: IInvokeRegisteredEditorCommandParameter) => {
        ...
        if (commandToExecute) {
          this.languageClient.sendRequest(InvokeExtensionCommandRequestType, {
  ...
  public setLanguageClient(languageclient: LanguageClient) {
    ...
    if (this.languageClient !== undefined) {
      ...
      this.languageClient.onRequest(GetEditorContextRequestType, (details) =>
        this.getEditorContext()
      );

      this.languageClient.onRequest(InsertTextRequestType, (details) =>
        this.insertText(details)
      );

      this.languageClient.onRequest(SetSelectionRequestType, (details) =>
        this.setSelection(details)
      );

      this.languageClient.onRequest(NewFileRequestType, (filePath) =>
        this.newFile()
      );

      this.languageClient.onRequest(OpenFileRequestType, (filePath) =>
        this.openFile(filePath)
      );

      this.languageClient.onRequest(CloseFileRequestType, (filePath) =>
        this.closeFile(filePath)
      );

      this.languageClient.onRequest(SaveFileRequestType, (saveFileDetails) =>
      ...
      this.languageClient.onRequest(ShowErrorMessageRequestType, (message) =>
        this.showErrorMessage(message)
      );

      this.languageClient.onRequest(ShowWarningMessageRequestType, (message) =>
  ...
  private addExtensionCommand(command: IExtensionCommandAddedNotificationBody) {
    ...
    this.extensionCommands.sort((a: IExtensionCommand, b: IExtensionCommand) =>
      a.name.localeCompare(b.name)
    );
  }

  private showExtensionCommands(
    client: LanguageClient
  ): Thenable<InvokeExtensionCommandRequestArguments> {
  ...
  private onCommandSelected(
    ...
    if (chosenItem !== undefined) {
      client.sendRequest(InvokeExtensionCommandRequestType, {
  ...
  ): Thenable<EditorOperationResponse> {
  ...
  private async saveFile(
    ...
    switch (currentFileUri.scheme) {
      case "file":
        ...
        if (
          !this.findTextDocument(this.normalizeFilePath(currentFileUri.fsPath))
        ) {
          this.log.writeAndShowError(
      ...
      case "untitled":
        ...
        if (path.isAbsolute(saveFileDetails.newPath)) {
          ...
          if (
            ...
            this.log.writeAndShowWarning(
              "Cannot save file to relative path: no workspaces are open. " +
    ...
    await this.saveDocumentContentToAbsolutePath(
      currentFileUri,
      newFileAbsolutePath
    );
    return EditorOperationResponse.Completed;
