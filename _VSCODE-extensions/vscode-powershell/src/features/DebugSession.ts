...
export const StartDebuggerNotificationType = new NotificationType<void, void>(
  "powerShell/startDebugger"
);

export class DebugSessionFeature
  implements IFeature, DebugConfigurationProvider
{
  ...
  constructor(
    ...
    context.subscriptions.push(
      vscode.debug.registerDebugConfigurationProvider("PowerShell", this)
    );
  }
  ...
  public setLanguageClient(languageClient: LanguageClient) {
    languageClient.onNotification(StartDebuggerNotificationType, () =>
  ...
  public resolveDebugConfiguration(
    ...
    if (this.sessionManager.getSessionStatus() !== SessionStatus.Running) {
      const msg =
        "Cannot debug or run a PowerShell script until the PowerShell session has started. " +
    ...
    let createNewIntegratedConsole =
    ...
    if (config.request === "attach") {
      ...
      if (platformDetails.operatingSystem !== OperatingSystem.Windows) {
        const msg =
          "Attaching to a PowerShell Host Process is supported only on Windows.";
    ...
    if (generateLaunchConfig) {
      ...
      config.script = currentDocument.isUntitled
      ...
      if (settings.debugging.createTemporaryIntegratedConsole) {
        ...
        config.cwd = currentDocument.isUntitled
    ...
    if (config.request === "launch") {
      ...
      if (debugCurrentScript) {
        if (currentDocument === undefined) {
          const msg =
            'To debug the "Current File", you must first open a ' +
        ...
        if (currentDocument.isUntitled) {
          ...
          if (extIndex !== -1) {
            const ext = currentDocument.fileName
              .substr(extIndex + 1)
              .toUpperCase();
          ...
          if (
            currentDocument.languageId !== "powershell" ||
            !isValidExtension
          ) {
            let path = currentDocument.fileName;
            const workspaceRootPath = vscode.workspace.rootPath;
            if (currentDocument.fileName.startsWith(workspaceRootPath)) {
              path = currentDocument.fileName.substring(
                vscode.workspace.rootPath.length + 1
              );
            }

            const msg =
              "PowerShell does not support debugging this file type: '" +
              path +
              "'.";
            vscode.window.showErrorMessage(msg);
            return;
    ...
    if (createNewIntegratedConsole) {
      ...
      this.tempDebugProcess = this.sessionManager.createDebugSessionProcess(
        sessionFilePath,
        settings
      );
...
export const GetPSHostProcessesRequestType = new RequestType<
  any,
  IGetPSHostProcessesResponseBody,
  string,
  void
...
export class PickPSHostProcessFeature implements IFeature {
  ...
  private getLanguageClientResolve: (
    value?: LanguageClient | Thenable<LanguageClient>
  ) => void;
  ...
  constructor() {
    this.command = vscode.commands.registerCommand(
      ...
      () => {
        return this.getLanguageClient().then(
          (_) => this.pickPSHostProcess(),
          (_) => undefined
        );
      }
    );
      ...
      return new Promise<LanguageClient>((resolve, reject) => {
        ...
        vscode.window
          .showQuickPick(
            ...
            {
              placeHolder:
                "Attach to PowerShell host process: Please wait, starting PowerShell...",
            },
  ...
  private pickPSHostProcess(): Thenable<string> {
    return this.languageClient
      .sendRequest(GetPSHostProcessesRequestType, null)
      .then((hostProcesses) => {
        ...
        for (const p in hostProcesses) {
          if (hostProcesses.hasOwnProperty(p)) {
            ...
            items.push({
              ...
              description: `PID: ${hostProcesses[
                p
              ].processId.toString()}${windowTitle}`,
        ...
        if (items.length === 0) {
          return Promise.reject(
            "There are no PowerShell host processes to attach to."
          );
        }
