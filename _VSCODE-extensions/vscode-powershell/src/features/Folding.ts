...
class LineNumberRange {
  ...
  constructor(rangeKind: vscode.FoldingRangeKind) {
   ...
   * @returns         Built LineNumberRange object
   */
  public fromLinePair(startLine: number, endLine: number): LineNumberRange {
...
export class FoldingProvider implements vscode.FoldingRangeProvider {
  ...
  constructor(powershellGrammar: IGrammar) {
  ...
  public async provideFoldingRanges(
    ...
    for (let i = 0; i < document.lineCount; i++) {
      const result = this.powershellGrammar.tokenizeLine(
    ...
    const foldableRegions = this.extractFoldableRegions(
      tokens,
      document
    ...
    return (
      foldableRegions
        ...
        .filter(
          (item, index, src) =>
            index === 0 || item.startline !== src[index - 1].startline
        )
   ...
   */
  public currentSettings(): Settings.ISettings {
  ...
  private matchScopeElements(
    ...
    tokens.forEach((token) => {
      ...
      if (tokenStack.length > 0 && token.scopes.indexOf(endScopeName) !== -1) {
        result.unshift(
          new LineNumberRange(matchType).fromTokenPair(
            tokenStack.pop(),
            token,
            document
          )
    ...
    tokens.forEach((token, index) => {
      if (token.scopes.indexOf(scopeName) !== -1) {
        ...
        if (
          ...
          tokens[index + 1].scopes.indexOf(scopeName) === -1
        ) {
          result.push(
            new LineNumberRange(matchType).fromTokenPair(
              startToken,
  ...
  private matchBlockCommentScopeElements(
    ...
    tokens.forEach((token) => {
      if (
        token.scopes.indexOf("punctuation.definition.comment.powershell") !== -1
      ) {
        const line: vscode.TextLine = this.lineAtOffset(
          token.startIndex,
          document
        );
        ...
        if (this.lineCommentText.test(line.text)) {
          ...
          if (startLine === -1) {
            ...
            result.push(
              new LineNumberRange(vscode.FoldingRangeKind.Comment).fromLinePair(
                startLine,
                nextLine - 1
              )
            );
   ...
   */
  private addTokenScope(token: IToken, scopeName: string): IToken {
  ...
  private extractFoldableRegions(
    ...
    this.matchBlockCommentScopeElements(tokens, document).forEach((match) => {
      matchedTokens.push(match);
    });
...
export class FoldingFeature implements IFeature {
  ...
  constructor(private logger: ILogger, documentSelector: DocumentSelector) {
    ...
    this.loadPSGrammar(logger).then(
      (grammar) => {
  ...
  public loadPSGrammar(logger: ILogger): Thenable<IGrammar> {
    ...
    logger.writeDiagnostic(
      `PowerShell grammar file specified as ${grammarPath}`
    );
