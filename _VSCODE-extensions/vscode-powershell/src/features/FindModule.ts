...
export const FindModuleRequestType = new RequestType<any, any, void, void>(
  "powerShell/findModule"
);

export const InstallModuleRequestType = new RequestType<
  string,
  void,
  void,
  void
>("powerShell/installModule");
...
export class FindModuleFeature implements IFeature {
      ...
      () => {
        ...
        vscode.window
          .showQuickPick(
            ...
            {
              placeHolder:
                "Please wait, retrieving list of PowerShell modules. This can take some time...",
            },
  ...
  private pickPowerShellModule(): Thenable<string> {
    return this.languageClient
      .sendRequest(FindModuleRequestType, null)
      .then((modules) => {
        ...
        for (const item in modules) {
          if (modules.hasOwnProperty(item)) {
            items.push({
              label: modules[item].name,
              description: modules[item].description,
            });
          }
        }
