...
export class NewFileOrProjectFeature implements IFeature {
  ...
  private showProjectTemplates(includeInstalledModules: boolean = false): void {
    vscode.window
      .showQuickPick(this.getProjectTemplates(includeInstalledModules), {
  ...
  ): Thenable<ITemplateQuickPickItem[]> {
    return this.languageClient
      ...
      .then((response) => {
        if (response.needsModuleInstall) {
          ...
          return Promise.reject<ITemplateQuickPickItem[]>(
            "Plaster needs to be installed"
          );
        } else {
  ...
  private createProjectFromTemplate(template: ITemplateDetails): void {
    vscode.window
      .showInputBox({
        placeHolder:
          "Enter an absolute path to the folder where the project should be created",
        ignoreFocusOut: true,
      })
      .then((destinationPath) => {
        if (destinationPath) {
          ...
          this.languageClient
            .sendRequest(NewProjectFromTemplateRequestType, {
...
export const GetProjectTemplatesRequestType = new RequestType<
  IGetProjectTemplatesRequestArgs,
  IGetProjectTemplatesResponseBody,
  string,
  void
...
export const NewProjectFromTemplateRequestType = new RequestType<
  any,
  INewProjectFromTemplateResponseBody,
  string,
  void
>("powerShell/newProjectFromTemplate");
