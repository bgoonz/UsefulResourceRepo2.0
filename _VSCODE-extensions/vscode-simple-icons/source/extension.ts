...
export async function activate(context: vscode.ExtensionContext) {
  context.subscriptions.push(
    vscode.workspace.onDidChangeConfiguration(() => probeConfiguration(context))
  );
...
async function probeConfiguration(
  ...
  const somethingChanged =
    (await toggleArrows(context)) || (await changeFolderColor(context));

  if (somethingChanged) {
    await vscode.window
      .showInformationMessage(
        "The window must be reloaded for changes to take effet",
        "Reload"
      )
      .then((choice) =>
        choice
...
async function toggleArrows(
  ...
  const jsonFiles = ["simple", "minimalistic"].map(
    (name) => name + "-icons.json"
  );
  ...
  for (const file of jsonFiles) {
    await changeFile(context, file, async (content) => {
      ...
      const conf = vscode.workspace
        .getConfiguration("simpleIcons")
        .get("hideArrows", false);
...
async function changeFolderColor(
  ...
  let color = vscode.workspace
    .getConfiguration("simpleIcons")
    .get<string>("simple.folder.color", null);
  ...
  if (!color) {
    ...
    await vscode.window.showWarningMessage(
      "Folder color not in hex format, color not changed"
    );
    return false;
  }

  const folders = ["folder", "folder.expanded"].map(
    (p) => path.join("icons", "simple-icons", p) + ".svg"
  );
  let somethingChanged = false;
...
async function changeFile(
  context: vscode.ExtensionContext,
  name: string,
  dg: (content: string) => Promise<string>
): Promise<void> {
  ...
  if (newContent) {
    return await new Promise<void>((resolve) =>
      fs.writeFile(file, newContent, (err) => resolve())
    );
  }
}
