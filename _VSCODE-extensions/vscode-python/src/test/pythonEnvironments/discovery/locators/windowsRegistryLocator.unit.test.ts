...
import { HKCU, HKLM, Options, REG_SZ } from 'winreg';
...
suite('Windows Registry', () => {
    ...
    async function getExpectedDataFromKey({ arch, hive, key }: Options, org: string): Promise<PythonEnvInfo> {
        ...
        const expectedEnvs: PythonEnvInfo[] = (
            await Promise.all([
                getExpectedDataFromKey(
                    { arch: 'x64', hive: HKLM, key: '\\SOFTWARE\\Python\\PythonCore\\3.9' },
                    'PythonCore'
                ),
                getExpectedDataFromKey(
                    { arch: 'x64', hive: HKLM, key: '\\SOFTWARE\\Python\\ContinuumAnalytics\\Anaconda38-64' },
                    'ContinuumAnalytics'
                ),
                getExpectedDataFromKey(
                    { arch: 'x64', hive: HKCU, key: '\\SOFTWARE\\Python\\PythonCore\\3.7' },
                    'PythonCore'
                ),
                getExpectedDataFromKey(
                    { arch: 'x86', hive: HKCU, key: '\\SOFTWARE\\Python\\PythonCodingPack\\3.8' },
                    'PythonCodingPack'
                )
            ])
