...
export class RevealServer {
  ...
  public start() {
    if (this.state === RevealServerState.Stopped) {
      ...
      console.log(
        `Reveal-server started, opening at http://${this.host}:${
  ...
  initExpressServer = () => {
    ...
    this.app.use(
      "/",
      this.staticDir(path.dirname(this.editor.document.fileName))
    );

    let highlightPath = path.resolve(
    ...
    this.app.use(
      `/lib/css/`,
      this.staticDir(path.join(highlightPath, "styles"))
    );
