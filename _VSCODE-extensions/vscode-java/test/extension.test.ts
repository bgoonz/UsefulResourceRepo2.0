...
suite("Java Language Extension", () => {
	...
	test("should register all java commands", function () {
		return vscode.commands.getCommands(true).then((commands) => {
			...
			let foundJavaCommands = commands.filter(function (value) {
				return (
					JAVA_COMMANDS.indexOf(value) >= 0 ||
					value.startsWith("java.")
				);
			});
			assert.equal(
				foundJavaCommands.length,
				JAVA_COMMANDS.length,
				"Some Java commands are not registered properly or a new command is not added to the test"
			);
		});
	});
	...
	test("should parse VM arguments with spaces", function () {
		let userArgs =
			'-javaagent:"C:\\Program Files\\Java\\lombok.jar" -Xbootclasspath/a:"C:\\Program Files\\Java\\lombok.jar" -Dfoo="Some \\"crazy\\" stuff"';
		let vmArgs = [];
		...
		assert.equal(
			vmArgs[0],
			"-javaagent:C:\\Program Files\\Java\\lombok.jar"
		);
		assert.equal(
			vmArgs[1],
			"-Xbootclasspath/a:C:\\Program Files\\Java\\lombok.jar"
		);
