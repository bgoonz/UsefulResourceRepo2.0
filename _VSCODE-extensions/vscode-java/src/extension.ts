			...
			return window.withProgress(
				...
				(p) => {
					return new Promise((resolve, reject) => {
						...
						let item = window.createStatusBarItem(
						...
						if (!port) {
							serverOptions = prepareExecutable(
								requirements,
								workspacePath,
								getJavaConfiguration()
							);
						} else {
							// used during development
							serverOptions = awaitServerConnection.bind(
								null,
								port
							);
						}
						...
						languageClient.onReady().then(() => {
							...
							languageClient.onNotification(
								...
								(notification) => {
									...
									const titles = notification.commands.map(
										(a) => a.title
									);
							...
							commands.registerCommand(
								...
								(
									...
									commands.executeCommand(
										...
										Uri.parse(uri),
										languageClient.protocol2CodeConverter.asPosition(
											position
										),
										locations.map(
											languageClient
							...
							commands.registerCommand(
								...
								async () => {
									...
									if (
										activeEditor.document.uri instanceof Uri
									) {
										await (<any>(
											commands.executeCommand(
											...
											return new Promise(
												(resolve, reject) => {
													setTimeout(() => {
														...
														if (
															res ===
															CompileWorkspaceStatus.SUCCEED
														) {
															resolve(res);
														} else {
															reject(res);
						...
						commands.registerCommand(Commands.OPEN_SERVER_LOG, () =>
							openServerLogFile(workspacePath)
						);
						...
						function applyWorkspaceEdit(obj) {
							let edit =
								languageClient.protocol2CodeConverter.asWorkspaceEdit(
									obj
								);
							if (edit) {
...
function projectConfigurationUpdate(languageClient: LanguageClient, uri?: Uri) {
	...
	if (!resource) {
		return window
			.showWarningMessage("No Java project to update!")
			.then(() => false);
	}
	if (isJavaConfigFile(resource.path)) {
		languageClient.sendNotification(
			ProjectConfigurationUpdateRequest.type,
			{
...
function toggleItem(editor: TextEditor, item) {
	if (
		...
		(editor.document.languageId === "java" ||
			isJavaConfigFile(editor.document.uri.path))
	) {
		item.show();
	} else {
		item.hide();
...
function onConfigurationChange() {
	return workspace.onDidChangeConfiguration((params) => {
		...
		if (hasJavaConfigChanged(oldConfig, newConfig)) {
			let msg =
				"Java Language Server configuration changed, please restart VS Code.";
...
function openServerLogFile(workspacePath): Thenable<boolean> {
	...
	if (!serverLogFile) {
		return window
			.showWarningMessage("Java Language Server has not started logging.")
			.then(() => false);
	}
		...
		.then((didOpen) => {
			if (!didOpen) {
				window.showWarningMessage(
					"Could not open Java Language Server log file"
				);
			}
			return didOpen;
		});
