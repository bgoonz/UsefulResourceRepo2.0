...
gulp.task("build_server", () => {
	cp.execSync(mvnw() + " -Pserver-distro clean package", {
		cwd: server_dir,
		stdio: [0, 1, 2],
	});
...
gulp.task("dev_server", () => {
	...
	if (isLinux()) {
		command +=
			"-Denvironment.os=linux -Denvironment.ws=gtk -Denvironment.arch=x86_64";
	} else if (isMac()) {
		command +=
			"-Denvironment.os=macosx -Denvironment.ws=cocoa -Denvironment.arch=x86_64";
	} else if (isWin()) {
		command +=
			"-Denvironment.os=win32 -Denvironment.ws=win32 -Denvironment.arch=x86_64";
	}
...
gulp.task("watch_server", () => {
	gulp.watch(server_dir + "/org.eclipse.jdt.ls.core/**/*.java", [
		"dev_server",
	]);
});
