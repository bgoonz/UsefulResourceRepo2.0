...
export function activate(context: vscode.ExtensionContext) {
  ...
  vscode.workspace.onDidChangeConfiguration(() => {
    const settings = vscode.workspace
      .getConfiguration("previewServer")
      .get("isWatchConfiguration") as boolean;
    if (settings) {
      resumeServer();
      if (!ignoreNotification) {
        vscode.window.showInformationMessage(
          localize("resumeServer.text", "Resume the Web Server.")
        );
      }
    }
  });
  ...
  let disposable: any = vscode.commands.registerCommand(
    "extension.preview",
    () => {
      // set ViewColumn
  ...
  let disposable2: any = vscode.commands.registerCommand(
    "extension.launch",
    () => {
      const uri = Utility.getUriOfActiveEditor();
      const options = vscode.workspace.getConfiguration("previewServer");
      const browsers = options.get("browsers") as string[];
      const ignoreDefaultBrowser = options.get(
      ...
      if (browsers === null && !ignoreDefaultBrowser) {
        ...
        return vscode.window.showErrorMessage(
          localize(
            "launchError.text",
            "You should set browser option or change ignoreDefultBrowser to true."
          )
        );
      }
    }
  ...
  let disposable3: any = vscode.commands.registerCommand(
    "extension.stop",
    () => {
      Server.stop();
      if (!ignoreNotification) {
        vscode.window.showInformationMessage(
          localize("stopServer.text", "Stop the Web Server successfully.")
        );
      }
    }
  );

  let disposable4: any = vscode.commands.registerCommand(
    "extension.resume",
    () => {
      resumeServer();
      if (!ignoreNotification) {
        vscode.window.showInformationMessage(
          localize("resumeServer.text2", "Resume the Web Server.")
        );
      }
    }
  );
  ...
  context.subscriptions.push(
    vscode.workspace.onDidChangeWorkspaceFolders(() => resumeServer())
  );
  context.subscriptions.push(
    disposable,
    disposable2,
...
function startServer() {
  ...
  if (space === Space.File) {
    rootPath = Utility.getOpenFilePath(
      vscode.window.activeTextEditor.document.fileName
    );
  } else if (
    space === Space.Folder ||
    (space === Space.Workspace && !startupProject)
  ) {
    rootPath = vscode.workspace.workspaceFolders[0].uri.fsPath;
  } else {
    for (let folder of vscode.workspace.workspaceFolders) {
      const workspaceName = vscode.workspace.getWorkspaceFolder(
        folder.uri
      ).name;
    ...
    if (rootPath === "" && !ignoreNotification) {
      vscode.window.showErrorMessage(
        localize(
          "startupProjectError.text",
          "startupProject option is null or invalide value."
        )
      );
    }
  }
