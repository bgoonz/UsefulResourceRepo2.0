...
export const getFileIconDefinitions = (
  ...
  allFileIcons.forEach((icon) => {
    ...
    if (icon.highContrast) {
      config = merge(
        {},
        config,
        setIconDefinition(icon.name, highContrastVersion)
      );
    }
  ...
  if (fileIcons.defaultIcon.light) {
    config = merge(
      {},
      config,
      setIconDefinition(fileIcons.defaultIcon.name, lightVersion)
    );
  ...
  if (fileIcons.defaultIcon.highContrast) {
    config = merge(
      {},
      config,
      setIconDefinition(fileIcons.defaultIcon.name, highContrastVersion)
    );
