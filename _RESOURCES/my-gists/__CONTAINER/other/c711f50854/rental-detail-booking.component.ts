...
export class RentalDetailBookingComponent implements OnInit {
  ...
  private isOneDay() {
    const dateRangeLength = this.daterange.length;
    if (dateRangeLength > 0) {
      return this.daterange[0] === this.daterange[dateRangeLength - 1];
    }
  ...
  openConfirmModal(content) {
    ...
    if (this.newBooking.guests <= 0) {
      this.newBooking.guests = 1;
    }
    this.newBooking.totalPrice =
      this.newBooking.days * this.rental.dailyRate * this.newBooking.guests;
