...
var wavesurfer = (function () {
  ...
  var createWavesurfer = function (song) {
    ...
    $.each(startTimes, function () {
      ...
      $("#sample" + song.id + "Span" + sampleNumber).width(
        parseFloat(song.duration) * ((pixelsPer4 * bpm) / 60)
      );
      canvas.width = parseFloat(song.duration) * ((pixelsPer4 * bpm) / 60);
      canvas.height = 80;
      $("#sample" + song.id + "Span" + sampleNumber).attr(
        "data-startTime",
        song.startTime[sampleNumber]
      );
      $("#sample" + song.id + "Span" + sampleNumber).css(
        "left",
        "" + parseInt(currentStartTime * pixelsPer16) + "px"
      );
      $("#sample" + song.id + "Span" + sampleNumber).css(
        "position",
        "absolute"
      ...
      $("#sample" + song.id + "Span" + sampleNumber).draggable({
        ...
        stop: function () {
          ...
          $(this).attr(
            "data-startTime",
            parseInt($(this).css("left")) / pixelsPer16
          );
...
$(document).ready(function () {
  ...
  $("#addTrackButton").click(function () {
    ...
    trackMasterGains[newTrackNumber] = {
      node: trackMasterGainNode,
      isMuted: false,
      isSolo: false,
    };
...
function createTrack(trackNumber) {
  $("#tracks").append(
    '<div class="row-fluid" id="selectTrack' +
      trackNumber +
      '"><div class="span2 trackBox" style="height: 84px;"><p style="margin: 0 0 0 0;" id="track' +
      trackNumber +
      'title">Track' +
      trackNumber +
      '</p><div style="margin: 5px 0 5px 0;" id="volumeSlider' +
      trackNumber +
      '"></div><div class="btn-toolbar" style="margin-top: 0px;"><div class="btn-group"><button type="button" class="btn btn-mini" id = "solo' +
      trackNumber +
      '"><i class="icon-headphones"></i></button><button type="button" class="btn btn-mini" id = "mute' +
      trackNumber +
      '"><i class="icon-volume-off"></i></button></div><div class="btn-group"><button type="button" class="btn btn-mini" data-toggle="button" id = "record' +
      trackNumber +
      '"><i class="icon-plus-sign"></i></button></div></div></div><div id="track' +
      trackNumber +
  ...
  $("#selectTrack" + trackNumber).click(function () {
    ...
    $.each(effects[activeTrack - 1], function () {
      ...
      if (currentEffect.type == "Compressor") {
        $("#compressorThresholdKnob")
          .val(currentEffect.threshold)
          .trigger("change");
        $("#compressorRatioKnob").val(currentEffect.ratio).trigger("change");
        $("#compressorAttackKnob")
          .val(currentEffect.attack * 1000)
          .trigger("change");
      }
  ...
  $("#record" + trackNumber).click(function () {
    ...
    if ($(this).hasClass("active")) {
      ...
      activeRecorder.getBuffer(function (recordingBuffer) {
        ...
        var newBuffer = ac.createBuffer(
          2,
          recordingBuffer[0].length,
          ac.sampleRate
        );
        ...
        $("#recording" + recordingCount + "Span").width(
          parseFloat(recordingDuration) * ((pixelsPer4 * bpm) / 60)
        );
        $("#recording" + recordingCount + "Span").attr(
          "data-startTime",
          startBar
        );
        $("#recording" + recordingCount + "Span").css(
          "left",
        ...
        $("#recording" + recordingCount + "Span").draggable({
          ...
          stop: function () {
            //get rid of old entry in table
            var currentRecordingCount = parseInt(
              $(this).attr("id").split("recording")[1]
            );
            var currentStartBar = $(this).attr("data-startTime");
            times[currentStartBar] = jQuery.removeFromArray(
