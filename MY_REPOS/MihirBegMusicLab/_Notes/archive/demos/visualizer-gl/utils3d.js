...
function initWebGL(
  ...
  // Bind attributes
  for (var i in attribs) gl.bindAttribLocation(gl.program, i, attribs[i]);
...
function makeSphere(ctx, radius, lats, longs) {
  ...
  ctx.bufferData(
    ctx.ARRAY_BUFFER,
    new WebGLFloatArray(normalData),
    ctx.STATIC_DRAW
  );
  ...
  ctx.bufferData(
    ctx.ARRAY_BUFFER,
    new WebGLFloatArray(texCoordData),
    ctx.STATIC_DRAW
  );
  ...
  ctx.bufferData(
    ctx.ARRAY_BUFFER,
    new WebGLFloatArray(geometryData),
    ctx.STATIC_DRAW
  );
  ...
  ctx.bufferData(
    ...
    new WebGLUnsignedShortArray(indexData),
    ctx.STREAM_DRAW
  );
...
function doLoadObj(obj, text) {
  ...
  obj.ctx.bufferData(
    obj.ctx.ARRAY_BUFFER,
    new WebGLFloatArray(normalArray),
    obj.ctx.STATIC_DRAW
  );
  ...
  obj.ctx.bufferData(
    obj.ctx.ARRAY_BUFFER,
    new WebGLFloatArray(textureArray),
    obj.ctx.STATIC_DRAW
  );
  ...
  obj.ctx.bufferData(
    obj.ctx.ARRAY_BUFFER,
    new WebGLFloatArray(vertexArray),
    obj.ctx.STATIC_DRAW
  );
  ...
  obj.ctx.bufferData(
    obj.ctx.ELEMENT_ARRAY_BUFFER,
    new WebGLUnsignedShortArray(indexArray),
    obj.ctx.STREAM_DRAW
  );
...
Framerate.prototype.updateFramerate = function () {
  var tot = 0;
  for (var i = 0; i < this.framerates.length; ++i) tot += this.framerates[i];
...
Framerate.prototype.snapshot = function () {
  if (this.renderTime < 0) this.renderTime = new Date().getTime();
  else {
    var newTime = new Date().getTime();
    var t = newTime - this.renderTime;
    var framerate = 1000 / t;
    this.framerates.push(framerate);
    while (this.framerates.length > this.numFramerates) this.framerates.shift();
