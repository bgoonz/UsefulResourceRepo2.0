...
function reverseBuffer(buffer) {
  ...
  if (newBuffer) {
    ...
    for (var channel = 0; channel < buffer.numberOfChannels; channel++) {
      ...
      for (var i = 0; i < length; i++) newBuf[length - i - 1] = oldBuf[i];
    }
  }
  return newBuffer;
}
...
Track.prototype.postLoadTasks = function () {
  ...
  this.lastBufferTime = 0.0;
  for (var i = 0; i < 4; i++) this.cues[i] = null;
  // TODO: need to clear MIDI cue lights

  drawBuffer(
    this.bufferCanvas.width,
    this.bufferCanvas.height,
    this.bufferCanvas.getContext("2d"),
    this.buffer
  );
  this.nameElement.innerHTML +=
    " (" + this.buffer.duration.toFixed(1) + " sec)";

  this.waveformDisplayCache = createRunningDisplayCache(
    this.buffer,
    this.isLeftTrack
  );
  drawRunningDisplay(
    runningDisplayContext,
    this.waveformDisplayCache,
    this.lastBufferTime
  );
};
...
Track.prototype.setCuePointAtCurrentTime = function (index) {
  ...
  if (index == 0) this.cueButton.classList.add("active");
...
Track.prototype.clearCuePoint = function (index) {
  ...
  if (index == 0) this.cueButton.classList.remove("active");
};
...
Track.prototype.jumpToPoint = function (time) {
  ...
  if (wasPlaying) this.togglePlayback();
...
Track.prototype.playSnippet = function () {
  ...
  sourceNode.buffer =
    this.currentPlaybackRate > 0 ? this.buffer : this.revBuffer;
  var startTime =
    this.currentPlaybackRate > 0
      ? this.lastBufferTime
      : sourceNode.buffer.duration - this.lastBufferTime;
...
Track.prototype.skip = function (ticks) {
  ...
  if (this.lastBufferTime < 0.0) this.lastBufferTime = 0.0;
  if (restart) this.togglePlayback();
  else {
    this.playSnippet();
  }
};
...
Track.prototype.togglePlaybackSpinUpDown = function () {
  ...
  if (this.isPlaying) {
    ...
    if (this.sourceNode) {
      // we may not have a sourceNode, if our PBR is zero.
...
Track.prototype.updatePlatter = function (drawOnScreen) {
  ...
  if (!this.isPlaying) {
    ...
    } else bufferTime = this.lastBufferTime;
  } else if (this.restartTime + 1 > now) {
    // we're still in "spin-up"
  ...
  if (drawOnScreen) {
    ...
    if (this.buffer) {
      ...
      runningDisplayContext.fillRect(
        RUNNING_DISPLAY_HALF_WIDTH,
        isTop ? 0 : RUNNING_DISPLAY_HALF_HEIGHT,
        1,
        RUNNING_DISPLAY_HALF_HEIGHT
      );
      ...
      for (var i = 0; i < 4; i++) {
        ...
        if (cue && cue.time > begin && cue.time < end) {
          var x =
            ((cue.time - begin) * RUNNING_DISPLAY_WIDTH) /
            SECONDS_OF_RUNNING_DISPLAY;
          ctx.fillStyle = cueColors[i];
          ctx.fillRect(
            x,
            isTop ? 0 : RUNNING_DISPLAY_HALF_HEIGHT,
          ...
          ctx.fillText(
            ...
            x + 2,
            isTop
              ? RUNNING_DISPLAY_HALF_HEIGHT / 2
              : RUNNING_DISPLAY_HALF_HEIGHT * 1.5
          );
        }
      }
    }
...
Track.prototype.changePlaybackRate = function (rate) {
  ...
  if (this.lastTimeStamp > now) return; // TODO: for now, we don't deal with changing pbr before the
  ...
  if (this.lastBufferTime > this.buffer.duration) {
    ...
    if (rate >= 0) return;
    else this.lastBufferTime = this.buffer.duration;
  }
  ...
  if (!this.sourceNode) {
    ...
    this.sourceNode = sourceNode;
  } // if I replace "now" with "0" below, Firefox works.
  else this.sourceNode.playbackRate.setValueAtTime(Math.abs(rate), now);
  this.currentPlaybackRate = rate;
};
