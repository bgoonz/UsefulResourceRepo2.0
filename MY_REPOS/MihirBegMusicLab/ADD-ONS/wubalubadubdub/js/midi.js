...
function turnOffLeftPlayButton() {
  if (midiOut) midiOut.send([0x80, 0x3b, 0x01]);
}

function turnOffRightPlayButton() {
  if (midiOut) midiOut.send([0x80, 0x42, 0x01]);
}
  ...
  if (cmd == 8 || (cmd == 9 && velocity == 0)) {
    ...
    switch (noteNumber) {
      ...
      case 0x5c:
        ...
        if (leftTrack.cueDeleteMode) {
          ...
          if (midiOut) midiOut.send([0x80, noteNumber, 0x01]);
        } else {
          if (leftTrack.cues[i]) {
            // jump to cuePoint
            ...
            // cue point wasn't set - set it.
            leftTrack.setCuePointAtCurrentTime(i);
            // light up the appropriate cue button
            if (midiOut) midiOut.send([0x90, noteNumber, 0x01]);
          }
        }
        break;
      ...
      case 0x46: // Deck B pitch bend -
        ...
        if (midiOut) midiOut.send([0x80, 0x3c, 0x01]);
        break;
      default:
        console.log(
          "MIDI: NoteOn: 0x" + noteNumber.toString(16) + " value=" + velocity
        );
    }
  } else if (cmd == 11) {
    ...
    switch (noteNumber) {
      ...
      case 0x0a: // Crossfader
        if (isMixTrack) velocity = 127 - velocity; // Mixtrack is flipped
      ...
      case 0x0d: // Deck A Pitch:  range of 0.92 - 1.08 (+/- 8%)
        ...
        if (midiOut)
          // light up the pitch light if centered
      ...
      case 0x0c: // Mixtrack: Cue Mix - filter frequency
        if (filterTrack)
          filterTrack.filter.frequency.value =
            logResponse(velocity / 127.0) * 10000.0;
        break;
      ...
      case 0x0b: // headphone/cue mix gain - filter resonance
        if (filterTrack) filterTrack.filter.Q.value = (velocity / 127.0) * 20;
        break;
      ...
      case 0x19: // Deck A wheel (shuttle)
        ...
        if (isMixTrack) {
          // skips 3 of every four ticks
          if (tickCount < 4) tickCount++;
          else tickCount = 0;
      ...
      default:
        console.log(
          "MIDI: CC: 0x" + noteNumber.toString(16) + " value=" + velocity
        );
    }
  }
}
...
function onMIDIInit(midi) {
  ...
  if (typeof midiAccess.inputs == "function") {
    ...
    for (var i = 0; i < list.length; i++)
      if (list[i].name.toString().indexOf("Numark") != -1) preferredIndex = i;

    if (list.length) {
      for (var i = 0; i < list.length; i++)
        selectMIDIIn.options[i] = new Option(
          list[i].name,
          list[i].fingerprint,
          i == preferredIndex,
          i == preferredIndex
    ...
    if (list.length) {
      for (var i = 0; i < list.length; i++)
        selectMIDIOut.options[i] = new Option(
          list[i].name,
          list[i].fingerprint,
          i == preferredIndex,
          i == preferredIndex
    ...
    for (var input of midiAccess.inputs.values()) {
      if (input.name.toString().indexOf("Numark") != -1) {
        ...
        selectMIDIIn.add(
          new Option(input.name, input.fingerprint, false, false)
        );
    }
    selectMIDIIn.onchange = changeMIDIIn;
    ...
    for (var output of midiAccess.outputs.values()) {
      if (output.name.toString().indexOf("Numark") != -1) {
        ...
        midiIn.onmidimessage = midiMessageReceived;
        selectMIDIOut.add(
          new Option(output.name, output.fingerprint, true, true)
        );
      } else
        selectMIDIOut.add(
          new Option(output.name, output.fingerprint, false, false)
        );
    }
    selectMIDIOut.onchange = changeMIDIOut;
  }
