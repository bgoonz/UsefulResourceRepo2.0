...
function drawRunningDisplay(context, cache, centerInSeconds) {
  var center = Math.floor(
    (centerInSeconds * RUNNING_DISPLAY_WIDTH) / SECONDS_OF_RUNNING_DISPLAY
  );
  //    var centerOffset = RUNNING_DISPLAY_HALF_WIDTH - center;
  var left = center - RUNNING_DISPLAY_HALF_WIDTH;

  var leftEdgeIndex = Math.floor(
    (center - RUNNING_DISPLAY_HALF_WIDTH) / MAX_CANVAS_WIDTH
  );
  if (leftEdgeIndex < 0) leftEdgeIndex = 0;
  var rightEdgeIndex = Math.floor(
    (center + RUNNING_DISPLAY_HALF_WIDTH) / MAX_CANVAS_WIDTH
  );
  if (rightEdgeIndex >= cache.length) rightEdgeIndex = cache.length - 1;
  ...
  for (var i = leftEdgeIndex; i <= rightEdgeIndex; i++) {
    context.drawImage(
      cache[i],
      RUNNING_DISPLAY_HALF_WIDTH - center + MAX_CANVAS_WIDTH * i,
      0
    );
    //        if (center!=0)
...
function createRunningDisplayCache(buffer, isTop) {
  var step =
    (SECONDS_OF_RUNNING_DISPLAY * buffer.sampleRate) / RUNNING_DISPLAY_WIDTH;
  var newLength = Math.floor(
    (buffer.duration / SECONDS_OF_RUNNING_DISPLAY) * RUNNING_DISPLAY_WIDTH
  );
  var data = buffer.getChannelData(0);
  ...
  for (var j = 0; j < newLength; j += MAX_CANVAS_WIDTH) {
    ...
    for (var i = 0; i < width; i++) {
      ...
      if (isTop) drawCTX.fillRect(i, 0, 1, max);
      else drawCTX.fillRect(i, RUNNING_DISPLAY_HEIGHT, 1, -max);
    }
