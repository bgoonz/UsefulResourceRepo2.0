...
function whileDraggingConnector(event) {
  ...
  // If this is a text node, use its parent element.
  if (toElem.nodeType == 3) toElem = toElem.parentNode;
  ...
  if (toElem.classList) {
    ...
    if (toElem == dragObj.elemNode) return;
...
function connectNodes(src, dst) {
  ...
  if (!src.outputConnections) src.outputConnections = new Array();
  ...
  if (!dst.inputConnections) dst.inputConnections = new Array();
  ...
  if (src.audioNode) src.audioNode.connect(dst.audioNode);

  if (dst.onConnectInput) dst.onConnectInput();
...
function deleteConnection() {
  ...
  breakSingleInputConnection(
    connections,
    connections.indexOf(this.inputConnection)
  );
}
...
function breakSingleInputConnection(connections, index) {
  ...
  src.outputConnections.splice(
    src.outputConnections.indexOf(connector.destination),
    1
  );
  if (src.audioNode) {
    // they may not have an audioNode, if they're a BSN or an Oscillator
...
function disconnectNode(nodeElement) {
  ...
  if (nodeElement.audioNode)
    // we may not have an audioNode, if we're a BSN or an Oscillator
