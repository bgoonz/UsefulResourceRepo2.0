...
function onUpdateDetune(event) {
  ...
  if (e.audioNode) e.audioNode.detune.value = e.oscillatorDetune;
}

function onUpdateOscillatorFrequency(event) {
  var e = updateSlider(event);
  e.oscillatorFrequency = event.target.value;
  if (e.audioNode) e.audioNode.frequency.value = e.oscillatorFrequency;
}
...
function updateSlider(event) {
  ...
  output.innerHTML =
    "" + event.target.value + " " + output.getAttribute("units");
...
function onPlayOscillator(event) {
  ...
  if (playButton.isPlaying) {
    ...
    while (e && !e.classList.contains("module")) e = e.parentNode;
    if (e && e.audioNode) {
      e.audioNode.stop(0);
      e.audioNode = null;
    }
...
function onToggleNormalize(event) {
  ...
  var e = checkbox.parentNode;
  while (e && !e.audioNode) e = e.parentNode;
  if (e) e.audioNode.normalize = checkbox.checked;
}
...
function switchOscillatorTypes(event) {
  ...
  if (e) {
    ...
    if (e.audioNode) e.audioNode.type = oscTypes[select.selectedIndex];
  }
}
...
function stopABSource(playButton) {
  ...
  if (e.audioNode) e.audioNode.stop(0);
}

function onPlayABSource(event) {
  var playButton = event.target;
  if (playButton.isPlaying) stopABSource(playButton);
  else {
    playButton.isPlaying = true;
    playButton.src = "img/ico-stop.gif";
...
function createAudioBufferSource(buffer) {
  ...
  if (buffer) {
    // TODO:  NASTY HACK!  USING A GLOBAL! Should also add dropped files for all buffers.
...
function createAnalyser() {
  ...
  canvas.style.webkitBoxReflect =
    "below 0px -webkit-gradient(linear, left top, left bottom, from(transparent), color-stop(0.9, transparent), to(white))";
...
function createBiquadFilter() {
  ...
  var gainSlider = addModuleSlider(
    module,
    "gain",
    1.0,
    0.0,
