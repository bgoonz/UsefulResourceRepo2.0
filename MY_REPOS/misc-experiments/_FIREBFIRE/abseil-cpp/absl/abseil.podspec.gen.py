...
def get_spec_name(label):
    ...
    assert label.startswith("//absl/"), "{} doesn't start with //absl/".format(label)
...
def write_podspec_rule(f, rule, depth):
    ...
    write_indented_list(
        f, "{indent}{var}.source_files = ".format(indent=indent, var=spec_var), srcs
    )
...
def main():
    ...
    parser.add_argument("-v", "--version", help="The version of podspec", required=True)
    parser.add_argument(
        "-t", "--tag", default=None, help="The name of git tag (default: version)"
    )
    parser.add_argument(
        "-o",
        "--output",
