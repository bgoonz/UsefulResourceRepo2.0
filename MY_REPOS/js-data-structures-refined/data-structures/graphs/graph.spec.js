...
describe("Graph", () => {
  ...
  const getValues = (node) =>
    Array.isArray(node) ? node.map((a) => getValues(a)) : node.value;
  ...
  describe("with (people) edges on undigraph", () => {
    ...
    describe("#findPath", () => {
      ...
      it("should get connecting path", () => {
        expect(graph.findPath("you", "barbara").map(getValues)).toEqual([
          "you",
          "mary",
          "barbara",
        ]);
      ...
      it("should get adjacent connecting path", () => {
        expect(graph.findPath("mary", "barbara").map(getValues)).toEqual([
          "mary",
          "barbara",
        ]);
