  ...
  return function domainMiddleware(req, res, next) {
    ...
    d.on("error", function (err) {
      ...
      if (d._throwErrorCount > 1) {
        console.error(
          "[domain-middleware] %s %s throw error %d times",
          req.method,
          req.url,
          d._throwErrorCount
        );
      ...
      var killtimer = setTimeout(function () {
        console.log(
          "[%s] [worker:%s] kill timeout, exit now.",
          new Date(),
          process.pid
        );
      ...
      if (cluster.worker) {
        try {
          ...
          console.error(
            "[%s] [worker:%s] Error on server close or worker disconnect!\n%s",
            new Date(),
            process.pid,
            er2.stack
          );
