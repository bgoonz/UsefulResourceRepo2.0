...
jQuery(document).ready(function () {
  if (
    ...
    jQuery("div.follow-button").length
  ) {
  ...
  jQuery(".rotatingtweets").each(function () {
    ...
    var rt_target_container_width = Math.min(
      rt_parent.innerWidth() -
        parseFloat(rt_parent.css("padding-left")) -
        parseFloat(rt_parent.css("padding-right")),
      rt_grandparent.innerWidth() -
        parseFloat(rt_grandparent.css("padding-left")) -
        parseFloat(rt_grandparent.css("padding-right")) -
        parseFloat(rt_parent.css("padding-left")) -
        parseFloat(rt_parent.css("padding-right")) -
        parseFloat(rt_parent.css("margin-left")) -
        parseFloat(rt_parent.css("margin-right"))
    );
    // Get the size of the parent box and subtract any padding
    var rt_target_width =
      rt_target_container_width -
      parseFloat(jQuery(this).css("padding-left")) -
      parseFloat(jQuery(this).css("padding-right")) -
      parseFloat(jQuery(this).css("margin-left")) -
      parseFloat(jQuery(this).css("margin-right")) -
      parseFloat(jQuery(this).css("border-left")) -
      parseFloat(jQuery(this).css("border-right"));
    var rt_fit = 1;
    ...
    if (rt_official_num > 0) {
      ...
      if (jQuery(rt_icon_id).length > 0) {
        if (rotate_wp_debug) {
          ...
          console.log(
            "- Width of Rotating Tweets container: " + jQuery(this).width()
          );
          console.log(
            "- Width of the icon container: " +
              jQuery(rt_icon_id).show().width()
          );
        }
        var rt_icon_width = 0;
...
*/
      ...
      if (rotate_wp_debug) {
        ...
        console.log(
          "- leftpadding - text : " +
            jQuery(rt_block_id).css("padding-left") +
            " and value: " +
            parseFloat(jQuery(rt_block_id).css("padding-left"))
        );
      }
    ...
    jQuery(window).resize(function () {
      ...
      if (rotate_wp_debug) {
        ...
        console.log(
          "New target width: " + (rt_resize_target_width + rt_resize_change)
        );
        console.log(
          "rt_max_width: " + (rt_resize_target_width + rt_resize_change)
        );
      }
      ...
      if (rt_resize_change != 0) {
        ...
        jQuery(rotate_id + " .rtw_main").width(
          rt_resize_target_main + rt_resize_change
        );
        jQuery(rotate_id + " .rotatingtweet").width(
          rt_resize_target_tweet + rt_resize_change
        );
        jQuery(rotate_id + " .rtw_meta").width(
        ...
        if (rt_newheight > 0) {
          jQuery(rotate_id).height(
            Math.max(
              rt_oldcontainerheight + rt_newheight - rt_oldheight,
              rt_newheight
            )
          );
        }
  ...
  jQuery(".rtw_intents a").hover(
    function () {
      ...
      jQuery(this)
        .find("img")
        .attr("src", rtw_src.replace(clearOutHovers, ".png"));
      var rtw_src = jQuery(this).find("img").attr("src");
      var srcReplacePattern = /.png$/;
      jQuery(this)
        .find("img")
        .attr("src", rtw_src.replace(srcReplacePattern, "_hover.png"));
    },
    function () {
      var rtw_src = jQuery(this).find("img").attr("src");
      var clearOutHovers = /_hover.png/;
