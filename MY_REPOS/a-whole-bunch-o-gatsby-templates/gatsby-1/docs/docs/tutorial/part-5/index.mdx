...
7. When you run your query, the `body` field for each node should look something like the data shown below. That's a lot of information! The `body` field actually contains the _compiled MDX content_ for your file. It might not be readable for humans, but it's the format that the `MDXRenderer` component understands. (You'll get to `MDXRenderer` in a moment.)
   ```json
...
<Notification
  ...
  content={
    <div>
      <p>
        <strong>Note:</strong> We{"'"}re still working on updating this Tutorial
        to use Gatsby v3. You've reached the end of the new content we've
        released so far. Subsequent parts will be added as we finish them. Part
        6 has a tentative release date of July 28, 2021.
      </p>
      <p style={{ marginBottom: 0 }}>
        If you{"'"}re looking for a full experience in the meantime, you can
        check out the (slightly outdated){" "}
