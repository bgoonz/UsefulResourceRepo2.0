...
async function run() {
  ...
  try {
    ...
    console.error(
      "Could not open package.json. Are you sure the plugin directory is correct?"
    )
    return
  }
  ...
  if (!fs.existsSync(gatsbyNodePath)) {
    console.error(
      `Could not find gatsby-node.js in ${gatsbyNodePath}. Are you sure this is a plugin directory?`
    )
    return
  }
  ...
  try {
    ...
    console.error(
      `Could not load gatsby-node.js. You may need to build the plugin first.`
    )
  ...
  const schemataPath = path.resolve(
    __dirname,
    "..",
    "src",
    "plugin-schemas.json"
