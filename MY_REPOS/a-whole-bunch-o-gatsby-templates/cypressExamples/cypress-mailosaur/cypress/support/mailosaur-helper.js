 ...
 */
export const createEmail = (randomName) =>
...
export const postMessageToMailService = (query) => {
  return cy.request({
    method: "POST",
    url: `${Cypress.env("MAILOSAUR_API")}/messages/await?server=${Cypress.env(
      "MAILOSAUR_SERVERID"
    )}`,
...
export const getEmailList = () => {
  return cy
    .request({
      method: "GET",
      url: `${Cypress.env("MAILOSAUR_API")}/messages?server=${Cypress.env(
        "MAILOSAUR_SERVERID"
      )}`,
      headers: commonHeaders,
      auth: commonAuthProps,
    ...
    .its("body")
    .its("items")
    .as("getEmailList") // this is the email list in an array
    .its("length")
...
export const waitUntilUserEmail = (userEmail) => {
  ...
  cy.waitUntil(() => getEmailList(), waitUntilOptions);

  // waitUntil expects a boolean result, if the email we need exists, we return true
  return cy.waitUntil(() => getUserEmail(userEmail), waitUntilOptions);
};
...
export const deleteEmail = (email) =>
  getEmailId(email).then((id) => deleteEmailById(id));
...
export const getEmailBody = (email) => {
  getEmailId(email).then((id) => retrieveMessage(id));
};
