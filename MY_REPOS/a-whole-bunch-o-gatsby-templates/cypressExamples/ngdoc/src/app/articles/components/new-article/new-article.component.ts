...
export class NewArticleComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.tagService.getTags().subscribe((tags: any) => {
      this.tagList = tags.map((tag) => tag.tag).sort();
    });
    ...
    this.articleForm = this.fb.group({
      ...
      url: [
        ...
        [
          ...
          NoTrackingTokenValidator(),
        ],
        this.dupCheck.checkDupUrl,
      ],
      author: [""],
  ...
  searchAuthors(event) {
    ...
    this.authorResults = this.authorList
      .filter((author) => {
  ...
  public searchTags(event) {
    ...
    this.tagResults = this.tagList.filter((tag) => {
      return (
        tag.toLocaleLowerCase().indexOf(event.query.toLocaleLowerCase()) > -1
      );
    });
  }
  ...
  showResponse(response) {
    this.authService.verifyCaptcha(response.response).subscribe((res: any) => {
      if (res.success) {
  ...
  save() {
    ...
    if (this.articleForm.valid) {
      this.checkDupInfo().subscribe((isDup: Boolean) => {
        if (!isDup) {
          this.articleSvc.createArticle(this.articleForm.value).subscribe(
            () => {
              this.msgs.push({
                severity: "success",
                summary: "Success!",
                detail: "Article Created. It will be made live after review.",
              });
              this.resetForm();
            },
            (rsp: any) => {
  ...
  checkDupInfo() {
    ...
    this.articleSvc
      .checkDup(this.articleForm.value)
