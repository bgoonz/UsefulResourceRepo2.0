...
export const selectError = createSelector(selectArticleState, getError);
...
export const selectAllArticleTags = createSelector(
  selectAllArticles,
  (articles: IArticle[]) => articles.map((article) => article.tags)
);
...
export const selectAllTags = createSelector(selectAllArticles, (articles) => {
  return articles.reduce(combineArrays, []);
});

export const selectUniqueTags = createSelector(selectAllTags, (tags) => {
