...
export const selectTagStrings = createSelector(selectAllTags, (tags) => {
...
export const selectDefaultTags = createSelector(
  ...
  (tags, filter) =>
    tags.filter((tag) => {
      return defaultTags.indexOf(tag) > -1 && filter.tags.indexOf(tag) === -1;
    })
);
...
export const selectFilteredTags = createSelector(
  ...
  selectTagSearch,
  selectTags,
  (filter, searchString, tags) =>
    tags.filter((tag) => {
      return (
        (!searchString || tag.indexOf(searchString) !== -1) &&
