...
const SearchResult = styled(InfiniteHits)`
  ul {
    list-style: none;
  }
  ...
  button {
    width: 16rem;
    margin: ${props => props.theme.spaces.xl} auto 0;
    border-radius: 4px;
...
const SearchInput = styled.input`
  ...
  color: white;
  flex-grow: 1;
  padding: 0 ${props => props.theme.spaces[`3xl`]} 0;
`;
...
const SearchBox = connectSearchBox(
  ({ currentRefinement, refine, searchPhrase, setSearchPhrase }) => {
    ...
    return (
      <SearchSection>
        <SearchForm onSubmit={updateSearch}>
          ...
          />
          {currentRefinement && (
            <SearchReset type="reset" onClick={resetSearch}>
              <FiX />
            </SearchReset>
          )}
        </SearchForm>
...
export default ({ closeModal }) => {
  ...
  return (
    <Search>
      ...
      >
        <SearchBox
          defaultRefinement={searchPhrase}
          searchPhrase={searchPhrase}
          setSearchPhrase={setSearchPhrase}
        />
        {searchPhrase && <SearchResult hitComponent={Hit} />}
      </InstantSearch>
    </Search>
  );
};
