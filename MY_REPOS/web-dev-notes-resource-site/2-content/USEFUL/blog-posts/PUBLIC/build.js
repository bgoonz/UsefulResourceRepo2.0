...
const generateMetadata = filename => {
  ...
  const addMembers = name => {
    metadata[ name ] = obj[ name ].map( member => ( {
      name: member.name,
      onname: member.onname
    } ) );
  };
  ...
  fs.writeFileSync( `./tmp/metadata/${obj.name}.json`, json, {
    flag: 'wx'
  } );
...
const createPage = ( obj ) => {
  ...
  const body = template.render( 'object', {
    obj
  } );

  const title = obj.name + ' JavaScript API';
  const html = template.render( 'page', {
    title,
    body,
    obj
  } );
...
makeCustomPage( 'PUBLIC', 'js' );

fs.readdirSync( './static' ).map( file => {
  fs.copyFile( `./static/${file}`, `./docs/${file}`, err => {
    if ( err ) {
      throw err
    }
  } );
