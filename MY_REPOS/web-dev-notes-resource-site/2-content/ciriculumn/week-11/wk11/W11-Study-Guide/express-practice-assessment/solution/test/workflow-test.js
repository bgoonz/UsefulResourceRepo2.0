		...
		it('with a hairColorId select field', (done) => {
			...
			request(app)
				...
				.expect((res) => {
					...
					expect(options).to.have.length(7, 'Did not have seven options as specified in the requirements');
...
describe('The submission page', () => {
	...
	it('can accept a valid submission with firstName, lastName, biography, and hairColorId and get redirected', (
		done
	) => {
		if (!app) {
			return done('Cannot read "app" from app.js');
	...
	it('can accept a valid submission with firstName, lastName, biography, age, and hairColorId and get redirected', (
		done
	) => {
		if (!app) {
			return done('Cannot read "app" from app.js');
