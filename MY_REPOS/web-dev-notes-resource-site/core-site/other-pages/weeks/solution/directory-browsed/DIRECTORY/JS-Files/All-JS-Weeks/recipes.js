...
async function getIngredientsData( id, data ) {
  ...
  if ( ingredientsSql.length === 0 ) {
    ...
    try {
      ( {
        rows
      } = await pool.query( ingredientsSql, [ id ] ) );
    } catch ( e ) {
...
async function getInstructionsData( id, data ) {
  ...
  if ( instructionsSql.length === 0 ) {
    ...
    try {
      ( {
        rows
      } = await pool.query( instructionsSql, [ id ] ) );
    } catch ( e ) {
...
async function getUnitsOfMeasureData( data ) {
  ...
  if ( unitsOfMeasureSql.length === 0 ) {
    ...
    try {
      ( {
        rows
      } = await pool.query( unitsOfMeasureSql ) );
    } catch ( e ) {
...
router.post( '/', async ( req, res ) => {
  ...
  if ( newRecipeSql.length === 0 ) {
    ...
    try {
      ( {
        rows
      } = await pool.query( newRecipeSql, [ req.body.title ] ) );
      id = rows[ 0 ].id;
    } catch ( e ) {
...
router.get( '/', async ( req, res ) => {
  ...
  try {
    ...
    if ( sql.length === 0 ) {
      recipes = {
        error: `No SQL in ${sqlFileName}`
      };
    } else {
      ( {
        rows
      } = await pool.query( sql, [ `%${term}%` ] ) );
      if ( rows.length ) {
        const row = rows[ 0 ];
