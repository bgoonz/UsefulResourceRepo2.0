...
(function ($) {
  $.fn.CommandBar = function () {
    var createMenuItem = function (text) {
      var item =
        '<li class="ms-ContextualMenu-item"><a class="ms-ContextualMenu-link"" href="#">';
      item += text;
      item += "</a></li>";
    ...
    var saveCommands = function ($commands, $commandWidth, $commandarea) {
      ...
      $commands.each(function () {
        ...
        var $rightOffset =
          ...
          $commandWidth +
          10 -
          $commandarea.position().left; // Added padding of 10
    ...
    var processOverflow = function (overFlowCommands, $oCommand, $menu) {
      ...
      if (overFlowCommands.length > 0) {
        ...
        for (var i = 0; i < overFlowCommands.length; i++) {
          ...
          var commandBarItemText = $Item
            .find(".ms-CommandBarItem-commandText")
            .text();
    ...
    return this.each(function () {
      ...
      var $CommandBarItems = $CommandMainArea
        .find(".ms-CommandBarItem")
        .not(".ms-CommandBarItem-overflow");
      var $OverflowCommand = $CommandBar.find(".ms-CommandBarItem-overflow");
      var $OverflowCommandWidth = $CommandBar
        .find(".ms-CommandBarItem-overflow")
        .outerWidth();
      ...
      allCommands = saveCommands(
        $CommandBarItems,
        $OverflowCommandWidth,
        $CommandMainArea
      );

      // Initiate process commands and add commands to overflow on load
      overFlowCommands = processCommands(
        allCommands,
        $CommandMainArea.innerWidth(),
        $OverflowCommandWidth
      );
      ...
      $SearchBox
        .find(".ms-CommandBarSearch-iconClearWrapper")
        .click(function () {
      ...
      $SearchBox
        .parent()
        .find(".ms-CommandBarSearch-input")
        .blur(function () {
          var $input = $(this);
          $input.val("");
          $input.parent().removeClass("is-active");
