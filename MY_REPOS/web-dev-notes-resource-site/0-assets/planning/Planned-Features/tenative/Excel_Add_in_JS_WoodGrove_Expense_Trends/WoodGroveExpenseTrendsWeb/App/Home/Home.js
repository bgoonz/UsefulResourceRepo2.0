...
(function () {
    ...
    $(document).ready(function () {
      ...
      if (!Office.context.requirements.isSetSupported("ExcelApi", "1.2")) {
        app.showNotification(
          "Need Office 2016 or greater",
          "Sorry, this add-in only works with newer versions of Excel."
        );
        return;
      }
  ...
  function createWelcomeSheet() {
    ...
    Excel.run(function (ctx) {
      ...
      return ctx.sync().then(function () {
        ...
        var sheetDesc1 =
          "1.  Import expense transactions into the workbook using the WoodGrove Trends task pane.";
        var sheetDesc2 = "2.  Create dashboards and trackers.";
        var sheetDesc3 = "3.  View and analyze trends.";
        var sheetDesc4 =
          "4.  Select a transaction in the Transactions sheet and add it as a charitable donation or a follow up item.";
  ...
  function importTransactions() {
    ...
    Excel.run(function (ctx) {
      ...
      var sheetDesc2 =
        "Filter transactions using the task pane to get insights.";
      ...
      var masterTableAddress =
        ...
        startRowNumber +
        ":G" +
        (startRowNumber + myBankTransactions.length);
      ...
      masterTable.getHeaderRowRange().values = [
        ["DATE", "AMOUNT", "MERCHANT", "CATEGORY", "TYPEOFDAY", "MONTH"],
      ];
      ...
      addContentToWorksheet(
        dataSheet,
        "B" + startRowNumber + ":G" + startRowNumber,
        "",
        "TableHeaderRow"
      );
      addContentToWorksheet(
      ...
      var chartDataRangeColumn1 = masterTable.columns
        .getItemAt(0)
        .getDataBodyRange();
      var chartDataRangeColumn2 = masterTable.columns
        .getItemAt(1)
        .getDataBodyRange();
      var chartDataRange = chartDataRangeColumn1.getBoundingRect(
        chartDataRangeColumn2
      );
      var chart = dataSheet.charts.add(
        "Line",
        chartDataRange,
  ...
  function showSelectedCategories() {
    ...
    var selectedCategoriesArray = $("input:checkbox:checked.category")
      .map(function () {
        return this.value;
      })
      .get();
  ...
  function createDashboard() {
    ...
    Excel.run(function (ctx) {
      ...
      var sheetDesc1 =
        "See a summary of your spending activity by category and by month.";
      ...
      var expensesByCategoryTable = ctx.workbook.tables.add(
        "Dashboard!B29:C29",
        true
      );
      ...
      // Queue a command to set the header row
      expensesByCategoryTable.getHeaderRowRange().values = [
        ["EXPENSE CATEGORY", "TOTAL SPENT"],
      ];

      // Queue a command to add the Expenses by Month table
      var expensesByMonthTable = ctx.workbook.tables.add(
        "Dashboard!F29:I29",
        true
      );
      ...
      // Queue a command to set the header row
      expensesByMonthTable.getHeaderRowRange().values = [
        ["MONTH", "WEEKEND SPEND", "WEEKDAY SPEND", "TOTAL SPENT"],
      ];
      ...
      var transactionsTable = ctx.workbook.tables.getItem(
        "TransactionsTable",
        true
      );

      // Queue commands to extract the Category and Month column values from the Transactions table and load the column values
      var categoryColumn = transactionsTable.columns
        .getItem("CATEGORY")
        .getDataBodyRange()
        .load("values");
      var monthColumn = transactionsTable.columns
        .getItem("MONTH")
        .getDataBodyRange()
        .load("values");
      ...
      return ctx.sync().then(function () {
        ...
        for (var i in uniqueCategories) {
          expensesByCategoryTable.rows
            .add(null, [[uniqueCategories[i], 5]])
            .getRange()
            .getLastColumn().formulas =
        ...
        expensesByCategoryTable.getTotalRowRange().getLastCell().values = [
          ["=SUM([TOTAL SPENT]"],
        ];
        ...
        expensesByMonthTable.getTotalRowRange().getLastCell().values = [
          ["=SUM([TOTAL SPENT]"],
        ];
        ...
        rangeTotalSpent.formulas = [
          ["=SUM(ExpensesByCategoryTable[TOTAL SPENT])"],
        ];
        ...
        var categoryChart = dashboardSheet.charts.add(
          "3dpie",
          categoryChartDataRange,
          Excel.ChartSeriesBy.auto
        );
        ...
        var monthChart = dashboardSheet.charts.add(
          "ColumnClustered",
          monthChartDataRange,
          Excel.ChartSeriesBy.auto
        );
  ...
  function createDonationsTracker() {
    ...
    Excel.run(function (ctx) {
      ...
      var sheetDesc1 =
        "Track your charitable contributions throughout the year.";
      var sheetDesc2 =
        "Use this data at the end of the year to report your tax deductions.";
      var sheetHeading2 = "Summary";
      ...
      // Queue a command to set the header row
      donationsTable.getHeaderRowRange().values = [
        ["DATE", "AMOUNT", "ORGANIZATION", "MONTH"],
      ];

      // Queue a command to add the Donations by Organization table
      var donationsByOrgTable = ctx.workbook.tables.add(
        "Donations!B12:C12",
        true
      );
      ...
      donationsByOrgTable.getHeaderRowRange().values = [
        ["ORGANIZATION", "AMOUNT"],
      ];
      donationsByOrgTable.showTotals = true;
      donationsByOrgTable.getTotalRowRange().getLastCell().values = [
        ["=SUM([AMOUNT]"],
      ];

      // Queue a command to add the Summary Donations by Month table
      var donationsByMonthTable = ctx.workbook.tables.add(
        "Donations!E12:F12",
        true
      );
      ...
      donationsByMonthTable.getTotalRowRange().getLastCell().values = [
        ["=SUM([AMOUNT]"],
      ];
  ...
  function createFollowupItemsTracker() {
    ...
    Excel.run(function (ctx) {
      ...
      var sheetDesc1 =
        "Track the transactions that need follow up in this tracker.";
      var sheetDesc2 =
        "After follow up, mark it as Complete so it is filtered out of the view.";
      var sheetHeading2 = "Summary";
      ...
      // Queue a command to set the header row
      followupTable.getHeaderRowRange().values = [
        ["TRANSACTION DATE", "ORGANIZATION", "AMOUNT", "STATUS"],
      ];
      ...
      rangetotalItems.formulas = [
        ['=COUNTIF(FollowUpTable[STATUS], "pending")'],
      ];
    ...
    Excel.run(function (ctx) {
      ...
      return (
        ctx
          .sync()
          .then(function () {
            rowToAdd = sourceRange
              .getEntireRow()
              .getIntersection(transactionsTable.getRange());
  ...
  function updateDonationTrackerSummaryTables() {
    ...
    Excel.run(function (ctx) {
      // Get the tables
      var donationsTable = ctx.workbook.tables.getItem("DonationsTable");
      var donationsByOrgTable = ctx.workbook.tables.getItem(
        "DonationsByOrgTable"
      );
      var donationsByMonthTable = ctx.workbook.tables.getItem(
        "DonationsByMonthTable"
      );
      ...
      var monthColumn = donationsByMonthTable.columns
        .getItemAt(0)
        .load("values");
      var lastRowAdded = donationsTable
        .getDataBodyRange()
        .getLastRow()
        .load("values");
      ...
      return (
        ctx
          ...
          .then(function () {
            ...
            var orgColumnValueArray = orgColumnValueArrays.map(function (item) {
              return item[0];
            });
            ...
            var indexOfOrg = orgColumnValueArray.indexOf(
              lastRowAdded.values[0][2]
            );

            // If the organization is new, add a new row to the summary table
            if (indexOfOrg === -1) {
              donationsByOrgTable.rows.add(null, [
                [lastRowAdded.values[0][2], lastRowAdded.values[0][1]],
              ]);
              // return promise here as well
            }
            // If this organization has been donated to already, update the existing row
            else {
              var amountColumn = donationsByOrgTable.columns
                .getItemAt(1)
                .load("values");
              return ctx.sync().then(function () {
                donationsByOrgTable
                  .getDataBodyRange()
                  .getCell(indexOfOrg - 1, 1).values = [
                  [
                    amountColumn.values[indexOfOrg][0] +
                      lastRowAdded.values[0][1],
                  ],
                ];
              });
            }
          ...
          .then(function () {
            ...
            var monthColumnValueArray = monthColumnValueArrays.map(function (
              item
            ) {
              return item[0];
            });

            var indexOfMonth = monthColumnValueArray.indexOf(
              lastRowAdded.values[0][3]
            );

            if (indexOfMonth === -1) {
              donationsByMonthTable.rows.add(null, [
                [lastRowAdded.values[0][3], lastRowAdded.values[0][1]],
              ]);
            } else {
              var amountColumn = donationsByMonthTable.columns
                .getItemAt(1)
                .load("values");
              return ctx.sync().then(function () {
                donationsByMonthTable
                  .getDataBodyRange()
                  .getCell(indexOfMonth - 1, 1).values = [
                  [
                    amountColumn.values[indexOfMonth][0] +
                      lastRowAdded.values[0][1],
                  ],
                ];
              });
  ...
  function addContentToWorksheet(
    sheetObject,
    rangeAddress,
    displayText,
    typeOfText
