...
function _requireDirAll(originalModule, absDir, options) {
  ...
  for (var length = files.length, i = 0; i < length; ++i) {
    ...
    if (
      fs.statSync(reqModule.filepath).isDirectory() &&
      options.recursive &&
      ...
      //  modules = {};
      //}
      modules[reqModule.name] = _requireDirAll(
        originalModule,
        reqModule.filepath,
        options
      );
    } else if (
      !isExcludedFileRe(reqModule, options.includeFiles) &&
      !isExcludedFileParent(reqModule, originalModule)
    ) {
      ...
      var target =
        reqModule.name === "index" && options.indexAsParent
          ? modules
          : modules && modules[reqModule.name];
