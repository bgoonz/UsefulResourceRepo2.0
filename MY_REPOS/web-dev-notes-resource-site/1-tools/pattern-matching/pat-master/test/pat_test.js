...
describe("caseof", function () {
  it("matches primitives", function () {
    ...
    var matchInt = function () {
      return "matchInt";
    };
    var matchStr = function () {
      return "matchStr";
    };
    var matchBool = function () {
      return "matchBool";
    };
    var matchArr = function () {
      return "matchArr";
    };
  ...
  it("matches multiple caseofs", function () {
    ...
    f1 = pat(f1).caseof(0, 1, match1).caseof(1, 1, match2);
  ...
  it("throws, if no match", function () {
    ...
    var f1 = pat(fn).caseof(_.isNumber, match);

    var f2 = pat().caseof(_.isNumber, match);

    var f3 = pat().caseof(_.isNumber, match).otherwise(fn);
  ...
  it("matches all rest", function () {
    ...
    var f1 = pat(fn).caseof(pat.rest(_.isNumber), match);

    var f2 = pat(fn).caseof("foo", "bar", pat.rest(_.isNumber), match);
  ...
  it("passes if rest doesn't have matcher", function () {
    ...
    var fooMatch = function () {
      return "foo match";
    };

    var f1 = pat(fn).caseof(pat.rest(), match);

    var f2 = pat(fn).caseof("foo", "bar", pat.rest(), fooMatch);
  ...
  it("packs all rest args to array", function () {
    ...
    var match = function (first, second, rest) {
      return rest;
    };
    var matchAll = function (rest) {
      return rest;
    };

    var f1 = pat(fn).caseof(Number, Number, pat.rest(_.isNumber), match);
    var f2 = pat(fn).caseof(pat.rest(), matchAll);
  ...
  it("matches all at once", function () {
    var matchThreeWords = function (all) {
      return all.join("") === "one two three";
    };
    ...
    var fn = pat().caseof(pat.all(matchThreeWords), matched);
  ...
  it("packs all args to array", function () {
    ...
    var match = function (all) {
      return all;
    };

    var f1 = pat(fn).caseof(pat.all(matchThreeWords), match);
  ...
  it("allows matcher function to return different number of arguments", function () {
    ...
    expect(f1("test1", [1, 5], "test2")).to.equal(
      "test1",
      1,
      2,
      3,
  ...
  it("allows nested pattern", function () {
    ...
    var f1 = pat(fn).caseof(pat(Number, Number), function (a, b) {
  ...
  it("allows nested pattern inside nested pattern", function () {
    ...
    var f1 = pat(fn).caseof(
      pat(pat(Number, Number), pat(Number, Number), pat(Number, Number)),
    ...
    ).to.equal("got points (1, 2), (3, 4), (5, 6)");
