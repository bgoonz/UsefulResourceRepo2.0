...
describe("EpicEditor.load", function () {
  ...
  describe("check if the DOM is in place", function () {
    editorIframe = document
      .getElementById(testEl)
    ...
    it("check to make sure the editor frame exists", function () {
      expect(editorInnards.getElementById("epiceditor-editor-frame")).toNot(
        beNull
      );
    });

    it("check to make sure the previewer frame exists", function () {
      expect(editorInnards.getElementById("epiceditor-previewer-frame")).toNot(
        beNull
      );
    });
...
describe("EpicEditor.load.options", function () {
  ...
  describe("when setting the container element", function () {
    it("allows the value to be a string of an ID to an element", function () {
      ...
      expect(
        document.getElementById(testEl).getElementsByTagName("iframe").length
      ).to(be, 1);
    });
    ...
    it("check that the localStorage key is correctly named when there's no fallback name", function () {
      ...
      expect(JSON.parse(localStorage.epiceditor)["__epiceditor-untitled-1"]).to(
        beTruthy
      );
    });
  });
  ...
  describe("when setting clientSideStorage", function () {
    ...
    it("check that when TRUE data IS saved to localStorage", function () {
      ...
      expect(JSON.parse(localStorage["epiceditor"])[testEl]).toNot(
        be,
        undefined
      );
    });
...
describe("EpicEditor.getElement", function () {
  ...
  it('check that "container" is the element given at setup', function () {
    expect(editor.getElement("container")).to(
      be,
      document.getElementById(testEl)
    );
  });
  ...
  it('check that "editor" is #document of the editor iframe', function () {
    expect(editor.getElement("editor")).to(
      be,
      _getIframeInnards(innerWrapper.getElementById("epiceditor-editor-frame"))
    );
  });

  it('check that "editorIframe" is <iframe> containing the editor', function () {
    expect(editor.getElement("editorIframe").id).to(
      be,
      "epiceditor-editor-frame"
    );
  });

  it('check that "previewer" is #document of the previewer iframe', function () {
    expect(editor.getElement("previewer")).to(
      be,
      _getIframeInnards(
        innerWrapper.getElementById("epiceditor-previewer-frame")
      )
    );
  });

  it('check that "previewerIframe" is <iframe> containing the previewer', function () {
    expect(editor.getElement("previewerIframe").id).to(
      be,
...
describe("EpicEditor.open", function () {
  ...
  before(function () {
    ...
    editor
      .importFile(openMeLaterFile, "open me later")
      .importFile(openMeFile, "open this file");
...
describe("EpicEditor.remove", function () {
  ...
  before(function () {
    ...
    editor
      .importFile(removeMeFile, "hello world")
      .importFile(dontRemoveMeFile, "foo bar");
  });
...
describe("EpicEditor.save", function () {
  ...
  before(function () {
    ...
    editor = new EpicEditor({
      basePath: "/epiceditor/",
      container: testEl,
      file: { defaultContent: "foo", autoSave: false },
    }).load();
  ...
  it("check to make sure new file contents are saved after value is changed in the editor and save is called", function () {
    ...
    editor.save();
    expect(JSON.parse(localStorage["epiceditor"])[testEl].content).to(
      be,
      "bar"
    );
  });
  ...
  it("check that the timestamp is updated when the content is modified", function () {
    var currentModifiedDate = JSON.parse(localStorage["epiceditor"])[testEl]
      .modified;
    editor.on("update", function () {
      eventWasFired = true;
    });
    editor.getElement("editor").body.innerHTML = "bar";
    editor.save();
    expect(currentModifiedDate).toNot(
      be,
      JSON.parse(localStorage["epiceditor"])[testEl].modified
    );
  });
});
