...
namespace("lint", function () {
  ...
  task(
    "all",
    ["lint:editor", "lint:docs", "lint:spec", "lint:util"],
    function () {
      complete();
    },
    { async: true }
    ...
    function () {
      ...
      var cwd = process.cwd(),
        ...
        cmds = [
          "jshint " +
            files.join(" ") +
            " --config .jshintrc --extra-ext .jshintrc",
        ];
...
task(
  ...
  function () {
    ...
    var destDir = path.join(process.cwd() + "/epiceditor/js/"),
      ...
      srcPaths = [srcDir + "editor.js", parser],
...
namespace("build", function () {
  ...
  task("force", [], function () {
    console.log(
      colorize(
        "--> Warning: Force build skips build pre-reqs. This build should not be commited.",
        "magenta"
      )
    );
...
task(
  ...
  function () {
    ...
    var destDir = path.join(process.cwd() + "/"),
      ...
      srcPaths = [srcDir + "header.html", tempPath, srcDir + "footer.html"],
...
task(
  ...
  function () {
    console.log(
      colorize("--> Test suite is now running (CTRL+C to quit)", "magenta")
    );
    console.log(
      colorize("--> http://localhost:5057/spec/runner.html", "yellow")
    );
    jake.exec(
      ["foounit serve"],
      function () {
...
task("ascii", [], function () {
  ...
  console.log(
    colorize("EpicEditor - An Embeddable JavaScript Markdown Editor", "yellow")
  );
});
