...
function get_api(argv) {
  ...
  return get("https://api.github.com" + path, query).then(function (
    contributors
  ) {
    return Promise.all(
      contributors
...
if ((argv.f && argv.m) || (argv.u && argv.r)) {
  (argv.f ? get_file(argv.f) : get_api(argv))
            ...
            list
              .map(function (contributor) {
                return (
                  ...
                  "</sub>](" +
                  contributor.url +
                  ")<br>[commits](https://github.com/jcubic" +
                  "/jquery.terminal/commits?author=" +
                  contributor.login +
                  ")"
                );
              })
