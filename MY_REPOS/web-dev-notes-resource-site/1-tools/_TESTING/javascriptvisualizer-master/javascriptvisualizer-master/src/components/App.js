...
class App extends Component {
  ...
  getColor = () => {
    ...
    const availableColors = Object.keys(flatColors).filter(
      (color) => !this.chosenColors.includes(color)
    );
  ...
  handleClear = (clearCode = true) => {
    ...
    // SHIPIT
    document.getElementById("execution-context").style = `background: #fff`;
  };
  handleSerialize = () => {
    this.props.history.push(
  ...
  setScopeVariables = (scope, scopeHash, forThis) => {
    ...
    if (frame && this.closuresToCreate[scopeHash] === true && !forThis) {
      return;
    }
    ...
    if (forThis) {
      ...
      return this.setState(({ scopes }) => {
        ...
        const newThis = removeQuotesFromKeys(
          JSON.stringify(
            {
              ...oldThis,
    ...
    this.setState(({ stack, scopes }) => {
      return {
        ...
        scopes: {
          ...scopes,
          [hash]: {
            arguments: formatValue(
              "Arguments",
              scope.properties.arguments.properties
            ),
  ...
  handleStep = () => {
    ...
    if (
      createNewExecutionContext(this.previousHighlight.node, highlighted.node)
    ) {
      ...
      get(highlighted, "node.type") === "AssignmentExpression"
        ? get(highlighted, "node.left.object.type") === "ThisExpression"
          ? {
              ...
              value:
                get(highlighted, "node.right.type") === "FunctionExpression"
                  ? "fn()"
  ...
  render() {
    const { code, currentOperation, stack, scopes, running, disableButtons } =
      this.state;
