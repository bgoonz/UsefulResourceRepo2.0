  ...
  $.fn.ajaxlivesearch = function (options) {
    ...
    function get_minimum_option_value(page_range) {
      ...
      for (i = 0; i < all_options.length; i += 1) {
        if (
          minimumOptionValue === undefined &&
          parseInt(all_options[i].value) !== 0
        ) {
          minimumOptionValue = all_options[i].value;
        } else {
          if (
            parseInt(all_options[i].value) < parseInt(minimumOptionValue) &&
            parseInt(all_options[i].value) !== 0
          ) {
            minimumOptionValue = all_options[i].value;
          }
    ...
    function search(
      ...
      form,
      options,
      bypass_check_last_value,
      reset_current_page
    ) {
      var result = getFormInfo(form, "result", options);

      if (
        $.trim(search_object.value).length &&
        $.trim(search_object.value).length >= options.min_chars_to_search
      ) {
        /**
        ...
        if (
          ...
          if (reset_current_page) {
            ...
            var current_page_lbl = getFormInfo(
              form,
              "current_page_lbl",
              options
            );
                ...
                success: function (response) {
                  if (response.status === "success") {
                    ...
                    if (responseResultObj.number_of_results === 0) {
                      ...
                      var minimumOptionValue =
                        get_minimum_option_value(page_range);

                      // if is visible
                      if (footer.is(":visible")) {
                        // if number of results is less than minimum range except 0: Hide
                        if (
                          parseInt(responseResultObj.number_of_results) <=
                          parseInt(minimumOptionValue)
                        ) {
                          remove_footer(footer, result);
                        } else {
    ...
    function generateFormHtml(elem, ls) {
      ...
      var paginationHtml =
        ...
        ls.page_limit +
        '">' +
        '<select name="ls_items_per_page" class="' +
        ls.page_range_class +
        '">' +
        optionsHtml +
        "</select>" +
        ...
        '">' +
        '<label class="' +
        ls.current_page_lbl_class +
        '">1</label> / ' +
        '<label class="' +
        ls.last_page_lbl_class +
        '"></label>' +
        "</div>" +
        '<div class="col ' +
        ls.navigation_class +
      ...
      var hidden_inputs =
        '<input type="hidden" name="ls_anti_bot" class="' +
        ls.form_anti_bot_class +
        '" value="">' +
        '<input type="hidden" name="ls_token" class="' +
        ls.token_class +
        '" value="' +
        ls.token +
        '">' +
        '<input type="hidden" name="ls_page_loaded_at" class="' +
        ls.loaded_at_class +
        '" value="' +
        ls.loaded_at +
        '">' +
        '<input type="hidden" name="ls_current_page" class="' +
        ls.current_page_hidden_class +
        '" value="1">' +
        '<input type="hidden" name="ls_query_id" value="' +
        elem_id +
        '">';

      var result =
        '<div class="' +
        ls.result_wrapper_class +
        '">' +
        '<div class="' +
        ls.result_class +
        '">' +
    ...
    this.each(function () {
      ...
      query.on("keyup", function (event) {
        ...
        if ($.trim(query.val()).length && keycode === 13) {
          if (
            !(result.is(":visible") || result.is(":animated")) ||
            parseInt(result.find("tr").length) === 0
          ) {
            show_result(result, ls);
          } else {
            if (query.selected_row !== undefined) {
              var data = {
                selected: $(query.selected_row),
                this: this,
                searchField: query,
              };
      ...
      query.on("focus", function () {
        ...
        if (
          $.trim(query.val()).length &&
          (result.is(":hidden") || result.is(":animated")) &&
      ...
      arrow.on("click", function () {
        ...
        if ($(this).hasClass(ls.next_page_class)) {
          ...
          if (
            parseInt(current_page.val(), 10) + 1 <=
            parseInt(total_page_lbl.html(), 10)
          ) {
      ...
      $(document)
        ...
        .bind("touchend", function (event) {
          ...
          if (distance < 10 && distance > -10) {
             ...
             */
            if (
              !$(event.target).closest(result).length &&
              !$(event.target).is(query) &&
              $(result).is(":visible")
            ) {
