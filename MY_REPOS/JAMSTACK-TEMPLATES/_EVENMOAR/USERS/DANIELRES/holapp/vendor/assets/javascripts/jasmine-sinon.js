...
(function (jasmine, beforeEach) {
  var sinon =
      typeof require === "function" && typeof module === "object"
        ...
        : window.sinon,
    spyMatchers =
      "called calledOnce calledTwice calledThrice calledBefore calledAfter calledOn alwaysCalledOn calledWith alwaysCalledWith calledWithExactly alwaysCalledWithExactly calledWithMatch alwaysCalledWithMatch".split(
        " "
      ),
    ...
    getMatcherFunction = function (sinonName, matcherName) {
      ...
      return function () {
        ...
        for (var i = 0; i < args.length; i++) {
          if (
            args[i] &&
            (typeof args[i].jasmineMatches === "function" ||
              args[i] instanceof jasmine.Matchers.ObjectContaining)
          ) {
            args[i] = createCustomMatcher(args[i]);
          }
        }

        return typeof sinonProperty === "function"
          ? sinonProperty.apply(this.actual, args)
          : sinonProperty;
      };
    };
  ...
  while (i--) {
    var sinonName = spyMatchers[i],
      matcherName =
        "toHaveBeen" + sinonName.charAt(0).toUpperCase() + sinonName.slice(1);

    spyMatcherHash[matcherName] = getMatcherFunction(sinonName, matcherName);
  }

  for (var j in unusualMatchers) {
    spyMatcherHash[unusualMatchers[j]] = getMatcherFunction(
      j,
      unusualMatchers[j]
    );
  }
