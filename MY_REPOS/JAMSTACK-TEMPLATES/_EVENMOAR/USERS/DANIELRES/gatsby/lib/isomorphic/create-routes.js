...
module.exports = (files, pagesReq) => {
  ...
  // template
  const templatesWithoutRoot = _.filter(
    files,
    (file) =>
      // eslint-disable-next-line comma-dangle
  ...
  staticFileTypes.forEach((type) => {
    try {
      ...
      if (
        ...
        e.message &&
        !e.message.match(/^Cannot find module/) &&
  ...
  pages.forEach((p) => {
    ...
    const parentTemplates = _.filter(
      templatesWithoutRoot,
      (templateFile) =>
        // eslint-disable-next-line comma-dangle
    ...
    const sortedParentTemplates = _.sortBy(
      parentTemplates,
