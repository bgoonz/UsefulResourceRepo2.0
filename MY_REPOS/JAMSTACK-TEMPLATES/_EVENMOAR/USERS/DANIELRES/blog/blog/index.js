...
const _Entry = (props) => {
  return (
    <div className={props.className}>
      <LinkIf if={!props.page.data.noLink} to={prefixLink(props.page.path)}>
        <div>
        ...
        {props.page.data.noLink && (
          <div dangerouslySetInnerHTML={{ __html: props.page.data.body }} />
        )}
      </LinkIf>
    </div>
  );
...
class BlogIndex extends React.Component {
  render() {
    ...
    const posts = sortedPages.filter(
      (page) =>
        access(page, "file.ext") === "md" &&
        !include(page.path, "/404") &&
        !include(page.path, "_drafts")
    );
    ...
    return (
      <div>
          ...
          meta={[
            {
              name: "description",
              content:
                "A blog about programming for the web, and various interesting things.",
            },
            {
              name: "keywords",
              content:
                "blog, articles, programming, drawing, frontend, backend",
            },
          ]}
        />
        ...
        <div style={{ marginBottom: 40 }}>{entries}</div>
