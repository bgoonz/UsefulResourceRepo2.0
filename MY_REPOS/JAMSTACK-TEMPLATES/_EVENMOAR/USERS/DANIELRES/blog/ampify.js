...
const addAmpVersionLink = function (html, filePath) {
  ...
  ampVersionUrl =
    ampVersionUrl.substring(0, ampVersionUrl.lastIndexOf("/")) + "/";
...
const ampify = function (html, filePath) {
  ...
  var css =
    sass
      .renderSync({ file: "css/zenburn.css", outputStyle: "compressed" })
      .css.toString() +
    sass
      .renderSync({ file: "css/amponly.scss", outputStyle: "compressed" })
      .css.toString();
  ...
  $("img:not([width]), img:not([height])").each(function () {
    ...
    if (src.includes("//")) {
      ...
      if (src.startsWith("/static")) {
        imageUrl = `${process.cwd()}/${inputDir}/${
          src.substr(1).split("?")[0]
        }`;
        // Any other images will come from the public directory
      } else {
        imageUrl = `${process.cwd()}/${inputDir}${minimalPath}${
          src.substr().split("?")[0]
        }`;
      }
