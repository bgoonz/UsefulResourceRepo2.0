...
const Img = styled((p) => <img title={p.alt} className={p.className} {...p} />)`
  border-radius: 10px;
  cursor: pointer;
  margin-bottom: 20px;
  display: block;
...
const images = [
  ...
  {
    src: "insect-communication.jpg",
    alt: "Photo: Insects communication macro",
  },
...
const FlickrLink = styled((p) => (
  <p className={p.className}>
    {" "}
    <a href="https://www.flickr.com/photos/pixarea/albums" target="_blank">
      <FaFlickr
        style={{
          fontSize: "130%",
          color: "#555",
  ...
  a {
    ...
    &:hover {
      background: white;
      box-shadow: -30px 30px 25px rgba(0, 0, 0, 0.15);
    }
    border-radius: 10px;
    span {
...
class Gallery extends React.Component {
  ...
  render() {
    ...
    const closeLightbox = () => this.setState({ lightbox: { isOpen: false } });
