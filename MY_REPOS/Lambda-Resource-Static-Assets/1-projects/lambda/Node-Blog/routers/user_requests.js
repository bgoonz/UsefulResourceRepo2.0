  ...
  userDb
    ...
    .then((users) => {
      if (users.length > 0) {
        ...
        res.status(404).json({
          message:
            "There are currently no Hobbits in the Shire. Check back later or go create a new one.",
        });
      }
    })
    .catch((err) => {
  ...
  userDb
    ...
    .then((user) => {
      if (user) {
        ...
        res.status(404).json({
          message:
            "That id doesn't correspond to any Hobbits here. Are you sure that was the right one?",
        });
      }
    })
    .catch((err) => {
      res.status(500).json({
        message:
          "This Hobbit is stronger than you, puny human. You cannot vanquish it with a mere click.",
      });
    });
});
...
router.put("/:id", customMiddleware.uppercase, (req, res) => {
  ...
  userDb
    ...
    .catch((err) => {
      res.status(500).json({
        message:
          "There was an error updating this Hobbit. It didn't like your new suggested name very much. Try again.",
      });
    });
});
