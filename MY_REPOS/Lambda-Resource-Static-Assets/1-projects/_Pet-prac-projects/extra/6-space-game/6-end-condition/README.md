...
1. **Implement game rules**. Implement the following game rules:
   ...
   1. **Restart logic**. When all lives are lost or the player won the game, display that the game can be restarted. Additionally restart the game when the _restart_ key is hit (you can decide what key should be mapped to restart).
