...
(function () {
  ...
  p.init = function (spriteSheet) {
    ...
    if (!this._inited) {
      ...
      bg.graphics
        ...
        .beginFill("#f55351")
        .drawRect(0, this.height - 170 * s, this.width, 170 * s);
    ...
    c.Tween.get(this.stats)
      .wait(1000)
      .to({ x: 40, alpha: 1 }, 1000, c.Ease.easeOut);
    ...
    c.Tween.get(this.hero)
      .wait(1000)
      .to({ x: 160 * s }, 3000, c.Ease.get(1));
  ...
  p.processSprite = function (sprite) {
    ...
    if (
      ...
      this.shot.visible &&
      !sprite.dead &&
      Math.abs(this.shot.x - sprite.x) < 50 * this.scale
    ) {
      ...
      !this.dead &&
      sprite.trap &&
      Math.abs(this.hero.x - sprite.x) < 80 * this.scale
    ) {
      ...
      !this.dead &&
      !sprite.jumped &&
      sprite.x < this.hero.x - 80 * this.scale
    ) {
      this.hazards++;
      sprite.jumped = true;
    }
  ...
  p.getSprite = function (label, speed, bounce, containerIndex, type) {
    ...
    var sprite = this.spritePool.length
      ? this.spritePool.pop()
      : new c.Sprite(this.spriteSheet);
    sprite.set({ speed: speed, type: type, dead: false });
    //sprite.gotoAndStop(0);
    sprite.gotoAndPlay(label);
    if (bounce) {
      c.Tween.get(sprite, { loop: true })
        .to({ scaleY: 0.9 }, 400)
        .to({ scaleY: 1 }, 400);
    }
  ...
  p.die = function (label) {
    ...
    var tween = c.Tween.get(this, { override: true })
      ...
      .to({ speed: 0 }, 4000, c.Ease.get(-0.5))
      .wait(1000)
  ...
  p.updateStats = function () {
    ...
    scoreFld.innerText = String(
      this.kills * 100 +
        this.hazards * 10000 +
        Math.round(this.distance / 10) * 100
    );
  };
