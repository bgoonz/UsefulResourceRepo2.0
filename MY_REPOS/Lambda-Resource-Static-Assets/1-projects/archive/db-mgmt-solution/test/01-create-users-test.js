...
describe("When 01-create-users.sql is run", () => {
  ...
  const checkSuper =
    "SELECT rolsuper AS is_super FROM pg_roles WHERE rolname = $1";
  ...
  async function testNormalUserExistence(user, password, isSuper) {
    ...
    const {
      rows: [{ is_super }],
    } = await pool.query(checkSuper, [user]);
    expect(is_super).to.equal(
      isSuper,
      `User should ${!isSuper ? "not " : ""}be superuser`
    );
  }
