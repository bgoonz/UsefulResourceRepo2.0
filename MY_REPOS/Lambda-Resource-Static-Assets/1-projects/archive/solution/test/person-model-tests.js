...
describe("The Person model", () => {
  ...
  it("retrieves peoples courses", async () => {
    ...
    expect(people[0].Courses.length).to.equal(
      0,
      `Expected ${people[0].lastName} to have 0 courses`
    );
    expect(people[1].Courses.length).to.equal(
      4,
      `Expected ${people[1].lastName} to have 4 courses`
    );
    expect(people[2].Courses.length).to.equal(
      4,
      `Expected ${people[2].lastName} to have 4 courses`
    );
  });
  ...
  context("Queries", function () {
    ...
    it("can lookup a list of Courses of a Person by email", async () => {
      const {
        lookupCoursesByPersonEmail,
      } = require("../queries/personLookup.js");

      const records = await lookupCoursesByPersonEmail(
        "ligula@velitduisemper.ca"
      );
