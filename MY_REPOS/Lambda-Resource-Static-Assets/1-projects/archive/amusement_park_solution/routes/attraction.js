...
const attractionValidators = [
  ...
  check("ridersPerVehicle")
    .isInt({ min: 0 })
    .withMessage(
      "Please provide a valid positive integer for Riders per Vehicle"
    ),
];

router.get(
  "/park/:parkId(\\d+)/attraction/add",
  csrfProtection,
  asyncHandler(async (req, res) => {
...
router.post(
  "/park/:parkId(\\d+)/attraction/add",
  csrfProtection,
  attractionValidators,
  asyncHandler(async (req, res) => {
    const parkId = parseInt(req.params.parkId, 10);
    const park = await db.Park.findByPk(parkId);

    const { attractionName, theme, opened, ridersPerVehicle } = req.body;
