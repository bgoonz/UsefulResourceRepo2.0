...
class Hash(object):
    ...
    def __init__(self, *args, **kwargs):
        ...
        )  # https://docs.python.org/3.5/library/hashlib.html#hashlib.hash.name
        self.byteorder = "little"
        self.digest_size = (
            0
        )  # https://docs.python.org/3.5/library/hashlib.html#hashlib.hash.digest_size
        self.block_size = (
            0
        )  # https://docs.python.org/3.5/library/hashlib.html#hashlib.hash.block_size
...
for name, s in zip(
    ...
    print(
        "For {:<8} : the block size is {:<3} and the digest size is {:<2}.".format(
            name, s.block_size, s.digest_size
        )
    )
        ...
        for offset in range(0, len(data), 64):
            ...
            for i in range(64):
                ...
                to_rotate = (
                    ...
                    + F
                    + K[i]
                    + int.from_bytes(chunks[4 * g : 4 * g + 4], byteorder="little")
                ) & 0xFFFFFFFF
...
hash_SHA2("")
assert (
    hash_SHA2("") == "e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855"
)
...
assert hash_SHA224("The quick brown fox jumps over the lazy dog") == true_hash_SHA224(
...
class SHA512(Hash):
    ...
    def update(self, arg):
        ...
        for offset in range(0, len(data), 128):
            ...
            for i in range(16, 80):
                s0 = (
                    rightrotate_64(w[i - 15], 1)
                    ^ rightrotate_64(w[i - 15], 8)
                    ^ rightshift(w[i - 15], 7)
                ) & 0xFFFFFFFFFFFFFFFF
                s1 = (
                    rightrotate_64(w[i - 2], 19)
                    ^ rightrotate_64(w[i - 2], 61)
                    ^ rightshift(w[i - 2], 6)
                ) & 0xFFFFFFFFFFFFFFFF
...
assert hash_SHA512("The quick brown fox jumps over the lazy dog") == true_hash_SHA512(
    "The quick brown fox jumps over the lazy dog"
)
...
class SHA384(Hash):
    ...
    def digest(self):
        ...
        return sum(
            leftshift(x, 64 * i) for i, x in enumerate(hash_pieces_without_67[::-1])
        )
...
assert hash_SHA384("The quick brown fox jumps over the lazy dog") == true_hash_SHA384(
    "The quick brown fox jumps over the lazy dog"
)
