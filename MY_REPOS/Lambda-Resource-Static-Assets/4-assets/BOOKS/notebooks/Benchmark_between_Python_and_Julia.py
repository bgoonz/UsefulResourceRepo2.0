...
def romberg_rec(f, xmin, xmax, n=8, m=None):
    ...
    if n == 0 and m == 0:
        ...
        return (1.0 / ((4 ** m) - 1)) * (
            (4 ** m) * romberg_rec(f, xmin, xmax, n, m - 1)
            - romberg_rec(f, xmin, xmax, n - 1, m - 1)
        )
...
def romberg(f, xmin, xmax, n=8, m=None):
    ...
    for j in range(1, m + 1):
        for i in range(j, n + 1):
            try:
                r[(i, j)] = (((4 ** j) * r[(i, j - 1)]) - r[(i - 1, j - 1)]) / float(
                    (4 ** j) - 1
                )
            except:
                raise ValueError(
                    "romberg() with n = {}, m = {} and i = {}, j = {} was an error.".format(
                        n, m, i, j
                    )
                )
...
def romberg_better(f, xmin, xmax, n=8, m=None):
    ...
    # First value:
    r = np.zeros((n + 1, m + 1))
    r[0, 0] = (xmax - xmin) * (f(xmax) + f(xmin)) / 2.0

    # One side of the triangle:
    for i in range(1, n + 1):
        h_i = (xmax - xmin) / 2.0 ** i
        r[i, 0] = (0.5 * r[i - 1, 0]) + h_i * math.fsum(
            f(xmin + ((2 * k - 1) * h_i)) for k in range(1, 1 + 2 ** (i - 1))
        )

    # All the other values:
    for j in range(1, m + 1):
        for i in range(j, n + 1):
            r[i, j] = (((4.0 ** j) * r[i, j - 1]) - r[i - 1, j - 1]) / (
                (4.0 ** j) - 1.0
            )
