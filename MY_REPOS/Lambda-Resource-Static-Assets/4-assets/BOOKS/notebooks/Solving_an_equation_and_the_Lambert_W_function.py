...
def one_solution(a, x0=0, verb=False):
    ...
    if sol.success:
        ...
        raise ValueError(
            f"No solution was found for a = {a:.3g} (and starting at x0 = {x0:.3g})."
        )
...
def upper_bound(a):
    up_b = (
        np.log(2 * a)
        - np.log(np.log(2 * a))
        + (e / (e - 1)) * (np.log(np.log(2 * a)) / np.log(2 * a))
    )
...
def lower_bound(a):
    lo_b = (
        np.log(2 * a)
        - np.log(np.log(2 * a))
        + np.log(np.log(2 * a)) / (2 * np.log(2 * a))
    )
...
plt.legend()
plt.xlabel("Parameter $a$")
plt.ylabel(r"Solution $x(a) = \sqrt{\frac{W(2a)}{2a}}$")
