...
def debug_view_for_str(frame, obj):
    if isinstance(obj, str):
        ...
        if "<stdin>" in info.filename or "<ipython-" in info.filename:
            msg = "- Access to {!r} (@{}) at {}:{}:{}"
            print(
                msg.format(obj, hex(id(obj)), info.filename, info.lineno, info.function)
            )
    return obj
...
print(1j)
with Noisy(
    lambda obj: obj + np.random.normal(10, 0.1) + np.random.normal(10, 0.1) * 1j
):
    print(1j)
print(1j)
