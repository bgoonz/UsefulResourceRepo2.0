...
def setup_prrint():
    ...
    latex_formatter.for_type_by_name(
        "numpy.polynomial.polynomial", "Polynomial", Polynomial_to_LaTeX
    )
...
matrice_Phi = [
    [np.round(produit_scalaire(Phi(P_n_s[i]), P_n_s[j]), 8) for i in range(1, 1 + 8)]
...
matrice_Phi_normalise = [
    [
        np.round(
            produit_scalaire(Phi(P_n_s_normalises[i - 1]), P_n_s_normalises[j - 1]), 8
        )
        for i in range(1, 1 + 8)
    ]
    for j in range(1, 1 + 8)
...
A = np.array([[1, 0, 0, 0], [p, 0, 1 - p, 0], [0, p, 0, 1 - p], [0, 0, 0, 1]])
etats = [0, 1, 2, 3]
