...
computed_means = pd.DataFrame(
    data={"mean_rating": train.groupby("work_id").mean()["rating"]}, index=works_id
)
computed_means[:5]
len(computed_means)
...
mapping_strrating_probwillsee = {"dislike": 0, "neutral": 0.50, "like": 0.75, "love": 1}
...
all_train[
    (all_train.work_id == 8025) & (all_train.strrating == "dislike")
].rating.mean()
...
all_train[
    (all_train.work_id == 8025) & (all_train.strrating == "neutral")
].rating.mean()
...
for cat in categories:
    print(
        "There is {:>5} work(s) in category '{}'.".format(
            sum(titles.category == cat), cat
        )
    )
...
categories = {"anime": 0, "album": 0, "manga": 1}
