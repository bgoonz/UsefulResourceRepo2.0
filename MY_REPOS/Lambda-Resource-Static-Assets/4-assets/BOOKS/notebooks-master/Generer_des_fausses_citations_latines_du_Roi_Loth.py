...
corpus = ["je mange des cerises", "je mange des bananes", "je conduis des camions"]
start = "je"
length = 4
...
print(
    "Il y a {:.3%} chance de commencer une citation par une majuscule.".format(
        proba_title
    )
)
...
def markov_try_while_failing(
    corpus, starts, length_min, length_max, proba_title, nb_max_trial=100
):
...
for _ in range(10):
    words = markov_try_while_failing(
        corpus, starts, length_min, length_max, proba_title
    )
    print(" ".join(words))
