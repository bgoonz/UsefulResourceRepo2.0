...
get_ipython().run_line_magic(
    "watermark",
    '-v -m -a "Lilian Besson (Naereen)" -p scipy,numpy,matplotlib,sympy,seaborn -g',
)
...
class Bernoulli:
    ...
    def __init__(self, probabilite):
        assert (
            0 <= probabilite <= 1
        ), "Erreur, probabilite doit être entre 0 et 1 pour un bras de Bernoulli."
...
class UCBV(MoyenneEmpirique):
    ...
    def choix(self):
        ...
        # 2nd cas : tous les bras ont été essayé
            ...
            moyennes_empiriques = self.recompenses / self.tirages
            variance = (
                self.recompensesCarrees / self.tirages
            ) - moyennes_empiriques ** 2  # Estimee de la variance

            ucb = (
                np.sqrt(self.alpha * np.log(self.t) * variance / self.tirages)
                + 3.0 * np.log(self.t) / self.tirages
            )
...
class KLUCB(MoyenneEmpirique):
    ...
    def choix(self):
        ...
        # 2nd cas : tous les bras ont été essayé
            ...
            indices = self.klucb(
                self.recompenses / self.tirages,
                f(self.t, self.c) / self.tirages,
                self.tolerance,
            )
            k = np.argmax(indices)
        return k
...
def affiche_selections(choix, noms, kstar=0):
    ...
    plt.title(
        "Sélection du meilleur bras #{} pour différents algorithmes".format(1 + kstar)
    )
    plt.show()
...
def affiche_hist_regret(recompenses, noms, horizon, mustar=1):
    ...
    fig.suptitle(
        "Histogramme du regret à $t = T = {}$ pour différents algorithmes".format(
            len(choix[0])
        )
    )
...
print(
    "Dimension de 'recompenses_moy' =", np.shape(recompenses_moy)
)  # juste pour vérifier
...
print(
    "Dimension de 'recompenses_fin' =", np.shape(recompenses_fin)
)  # juste pour vérifier
...
class ApproximatedFHGittins(MoyenneEmpirique):
    ...
    def __init__(self, K, horizon, alpha=1):
        """Crée l'instance de l'algorithme. Par défaut, alpha=1."""
        super(ApproximatedFHGittins, self).__init__(
            K
        )  # On laisse la classe mère faire le travaille
    ...
    def choix(self):
        ...
        # 2nd cas : tous les bras ont été essayé
            ...
            terme_sup = np.sqrt(
                self.alpha * np.log(m_sur_Nk / logdemi) / (2.0 * self.tirages)
            )
