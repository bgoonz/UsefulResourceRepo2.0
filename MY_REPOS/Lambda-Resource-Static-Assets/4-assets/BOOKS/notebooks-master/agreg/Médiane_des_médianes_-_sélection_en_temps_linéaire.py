...
def selection_naive(tableau: List[T], k: int) -> T:
    ...
    assert (
        1 <= k <= len(tableau)
    ), f"Erreur : k = {k} doit être entre 1 et n = {len(tableau)} = len(tableau)"
    ...
    if k == 1:  # cas simple
        ...
        for valeur in tableau:
            if valeur == min_tableau:
                if (
                    min_deja_vu
                ):  # on a déjà vu UNE occurrence de min, on l'ajoute encore
...
def mediane_cinq_valeurs(tableau: List[T]) -> T:
    assert (
        1 <= len(tableau) <= 5
    ), f"Erreur : tableau a longueur = {len(tableau)} mais devrait etre 1 <= .. <= 5"
...
[tableau3[i : i + 5] for i in range(0, len(tableau3), 5)]
...
[mediane_cinq_valeurs(tableau3[i : i + 5]) for i in range(0, len(tableau3), 5)]
...
for k in range(1, 1 + len(tableau1)):
    print(
        f"    Le {k}-ième élément du tableau est {selection_par_medianes(tableau1, k)}"
    )
...
for k in range(1, 1 + len(tableau2)):
    print(
        f"    Le {k}-ième élément du tableau est {selection_par_medianes(tableau2, k)}"
    )
...
def selection(
    tableau: List[T], k: int, left: Optional[int] = None, right: Optional[int] = None
) -> int:
...
for tab in (tableau2, tableau2[::-1]):
    print(f"Pour le tableau {tab} :")
    for k in range(1, 1 + len(tab)):
        print(f"    Le {k}-ième élément du tableau est {selection(tab[:], k)}")
...
def find_i_th_smallest(
    tableau: List[T], k: Optional[int] = None, items_per_column: int = 5
) -> T:
    if k is None:
        k = (len(tableau) - 1) // 2
    n = len(tableau)
    if n <= items_per_column:
        ...
        median_of_medians = find_i_th_smallest(
            medians
        )  #  with k=None, computes middle value = median
...
for number_of_numbers in tqdm([100, 1000, 10000]):
    ...
    for test in tqdm(range(number_of_tests)):
        print(
            f"Test {test + 1} / {number_of_tests} avec une liste de taille {number_of_numbers} :"
        )
        # create a list of random positive integers
        tableau = [
            random.randint(0, number_of_numbers) for i in range(0, number_of_numbers)
        ]
        for L in [tableau, tableau * 2]:
            ...
            get_ipython().run_line_magic(
                "timeit", "-n1 find_i_th_smallest(L, random.randint(0, len(L) - 1))"
            )
