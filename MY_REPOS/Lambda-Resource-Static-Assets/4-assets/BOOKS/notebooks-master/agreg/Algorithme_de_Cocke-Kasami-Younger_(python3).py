...
class Grammaire(object):
    ...
    - Symbole initial : '{}',
    - Règles : {}.""".format(
            self.nom, set(self.sigma), set(self.v), self.s, str_regles
        )
...
def estBienFormee(self: Grammaire) -> bool:
    ...
    tests = [
        ...
        all(
            ...
            and all(  # Les membres droits de règles sont des variables ou des lettres
                r in sigma | v for r in regle[1]
            )
            for regle in regles
        ),
    ]
...
def estChomsky(self: Grammaire) -> bool:
    ...
    sigma, v, s, regles = set(self.sigma), set(self.v), self.s, self.r
    estBienChomsky = all(
        (regle[0] == s and not regle[1])  # S -> epsilon
        or (len(regle[1]) == 1 and regle[1][0] in sigma)  # A -> a  # a in Sigma
        or (  # A -> B C
            len(regle[1]) == 2
...
def cocke_kasami_younger(self, w):
    ...
    assert estChomsky(
        self
    ), "Erreur : {} n'est pas en forme de Chomsky, l'algorithme de Cocke-Kasami-Younger ne fonctionnera pas.".format(
        self.nom
    )
    sigma, v, s, regles = set(self.sigma), set(self.v), self.s, self.r
    n = len(w)
...
def miseChomsky(self):
    ...
    """
    assert estBienFormee(
        self
    ), "Erreur : {} n'est pas en bien formée, la mise en forme normale de Chomsky ne fonctionnera pas.".format(
        self.nom
    )
    sigma, v, s, regles = set(self.sigma), set(self.v), self.s, self.r
    if estChomsky(self):
        print(
            "Info : la grammaire {} est déjà en forme normale de Chomsky, il n'y a rien à faire.".format(
                self.nom
            )
        )
    ...
    ), "Erreur : {} n'a pas ses lettres de production Sigma dans 'a'..'z' ...".format(
        self.nom
    )
    assert v < set(
        chr(i) for i in range(ord("A"), ord("Z") + 1)
    ), "Erreur : {} n'a pas ses lettres de travail V dans 'A'..'Z' ...".format(self.nom)
    ...
    for a in sigma:
        ...
        regles2.append(
            [V_(a), [a]]
        )  # Ajout de la règle V_a -> a (production de la lettre correspondante)
    ...
    for k, regle in enumerate(regles2):
        ...
        n = len(w)
        if n > 2:
            prime = lambda Si: "%s'_%d" % (
                Si,
                k,
            )  # Ajouter le k dans le nom assure que les nouvelles variables de travail sont toutes uniques
