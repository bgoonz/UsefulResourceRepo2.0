...
def test_estSousMot(u, y):
    ...
    if i >= 0:
        print(
            "Le mot u = '{}' est sous-mot de y = '{}', a partir de l'indice i = {}.".format(
                u, y, i
            )
        )
    else:
...
def aSousMotDeLongueur(x, y, k):
    ...
    # Pas trouvé !
    raise ValueError(
        "Aucun sous-mot de longueur k = {} de x = '{}' n'est dans y = '{}'.".format(
            k, x, y
        )
    )
...
def test_aSousMotDeLongueur(x, y, k):
    ...
    try:
        ...
        print(
            "==> Sous-mot de longueur k = {} de x = '{}' dans y = '{}' : u = '{}' en indice i = {} pour x et j = {} pour y.".format(
                k, x, y, u, i, j
            )
        )
    except ValueError:
...
def _sousMotMax(x, y, verb=True):
    ...
    while k >= 0:
        if verb:
            print(
                "  On essaie de trouver un sous-mot commun à x = '{}' et y = '{}' de taille k = {} ...".format(
                    x, y, k
                )
            )
        try:
            ...
            if verb:
                print(
                    "  ==> On a un sous-mot commun à x = '{}' et y = '{}' de taille k = {} : u = '{}' (i = {}, j = {}) !".format(
                        x, y, k, u, i, j
                    )
                )
            return i, j, u
            ...
            if verb:
                print(
                    "  ==> Aucun sous-mot commun à x = '{}' et y = '{}' de taille k = {} ...".format(
                        x, y, k
                    )
                )
        k -= 1
...
def sousMotMaxGlouton(x, y, verb=False, verifie=True):
    ...
    m = len(y)
    message(
        "\n1. Initialisation de la look-up matrice longueurs de taille (n+1, m+1) : n = {} m = {} ...".format(
            n, m
        )
    )
    ...
    for i in range(1, n + 1):
        for j in range(1, m + 1):
            if x[i - 1] == y[j - 1]:
                ...
                message(
                    "      Correspondance en cours, u[{}] = {} et v[{}] = {}, de longueur courante {} ...".format(
                        i - 1, x[i - 1], j - 1, y[j - 1], longueurs[i, j]
                    )
                )
    ...
    message(
        "4. On a obtenu iDebut = {} et jDebut = {} et longueur_max = {} ...".format(
            iDebut, jDebut, longueur_max
        )
    )
    ...
    if verifie:
        ...
        message(
            "      Sous-mot dans x = {}, et sous-mot dans y = {} ...".format(
                xSous, ySous
            )
        )
...
def sousMotMaxGloutonCreux(x, y, verb=False, verifie=True):
    ...
    m = len(y)
    message(
        "\n1. Initialisation de la look-up matrice longueurs, creuse de taille (n+1, m+1) : n = {} m = {} ...".format(
            n, m
        )
    )
...
def utiliseTableOrigines_x(x, origines, i, j, accu=""):
    ...
    if origines[i, j] == "↖":  # Vraie correspondance, on ajoute une lettre !
        ...
        raise ValueError(
            "Erreur dans la table 'origines', invalide pour le mot x = {} ...".format(x)
        )
...
def utiliseTableOrigines_y(x, origines, i, j, accu=""):
    ...
    if origines[i, j] == "↖":  # Vraie correspondance, on ajoute une lettre !
        ...
        raise ValueError(
            "Erreur dans la table 'origines', invalide pour le mot y = {} ...".format(y)
        )
...
def sousMotSeqGlouton(x, y, verb=False, verifie=True):
    ...
    m = len(y)
    message(
        "\n1. Initialisation des look-up matrices longueurs et origines de taille (n+1, m+1) : n = {} m = {} ...".format(
            n, m
        )
    )
    ...
    for i in range(1, n + 1):
        for j in range(1, m + 1):
            if x[i - 1] == y[j - 1]:
                ...
                origines[i, j] = "↖"
                message(
                    "      Vraie correspondance en cours, u[{}] = {} et v[{}] = {}, de longueur courante {} ...".format(
                        i - 1, x[i - 1], j - 1, y[j - 1], longueurs[i, j]
                    )
                )
                ...
                origines[i, j] = "↑"
                message(
                    "      Correspondance gauche (u) en cours, u[{}] = {} et v[{}] = {}, de pseudo-longueur courante {} ...".format(
                        i - 1, x[i - 1], j - 1, y[j - 1], longueurs[i, j]
                    )
                )
            else:
                ...
                origines[i, j] = "←"
                message(
                    "      Correspondance haute (v) en cours, u[{}] = {} et v[{}] = {}, de pseudo-longueur courante {} ...".format(
                        i - 1, x[i - 1], j - 1, y[j - 1], longueurs[i, j]
                    )
                )
    ...
    if verifie:
        ...
        if not res_x:
            raise ValueError(
                "Erreur: La séquence {} de x (aux indices {}) n'est pas une bonne sous-séquence de x = {}.".format(
                    laSousSeq, indices_x, x
                )
            )
        ...
        if laSousSeq != laSousSeq_y:
            raise ValueError(
                "Erreur: La sous-séquence {} de x est différente de celle de y : {} ...".format(
                    laSousSeq, laSousSeq_y
                )
            )
        ...
        if not res_y:
            raise ValueError(
                "Erreur: La séquence {} de y (aux indices {}) n'est pas une bonne sous-séquence de y = {}.".format(
                    laSousSeq, indices_y, y
                )
            )
...
x = "ABCBDAB"
y = "BDCABA"
laSousSeq, longueurs, origines, indices_x, indices_y = sousMotSeqGlouton(
    x, y, verifie=True
)
