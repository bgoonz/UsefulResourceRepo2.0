...
def mise_en_page_paragraphe_gloutonne(
    longueur_max: int, mots: List[str]
) -> List[List[str]]:
    print(f"Longueur maximum de la ligne = {longueur_max}")
    print(f"Longueur des mots = {longueurs_mots}")

    assert all(1 <= len(mot) <= longueur_max for mot in mots)
    ...
    assert all(longueur_ligne(ligne) == longueur_max for ligne in paragraphes)
    return paragraphes
...
def cout_paragraphes(paragraphes: List[List[str]], cout: Callable[[int], int]) -> int:
    ...
    espaces_de_fin = [len(ligne) - len(ligne.rstrip()) for ligne in lignes]
...
def print_couts(paragraphes):
    print(
        "- Nombre d'espaces en fin de lignes =",
        cout_paragraphes(paragraphes, cout=lambda i: i),
    )
    print(
        "- Somme des carrés des nombres d'espaces en fin de lignes =",
        cout_paragraphes(paragraphes, cout=lambda i: i ** 2),
    )
    print(
        "- Somme des cubes des nombres d'espaces en fin de lignes =",
        cout_paragraphes(paragraphes, cout=lambda i: i ** 3),
    )
...
def mise_en_page_paragraphe(
    longueur_max: int, mots: Tuple[str], choix_cout: str = "cubique"
) -> List[List[str]]:
    print(f"Longueur maximum de la ligne = {longueur_max}")
    mots = list(mots)
