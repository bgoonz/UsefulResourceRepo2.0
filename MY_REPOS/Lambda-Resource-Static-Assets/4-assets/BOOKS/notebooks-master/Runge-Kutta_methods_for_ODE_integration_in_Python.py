...
def rungekutta2(f, y0, t, args=()):
    ...
    for i in range(n - 1):
        h = t[i + 1] - t[i]
        y[i + 1] = y[i] + h * f(
            y[i] + f(y[i], t[i], *args) * h / 2.0, t[i] + h / 2.0, *args
        )
    return y
...
def test_1(n=101):
    ...
    plt.legend(loc="best")
    plt.title(
        "Comparison of different ODE integration methods for $n={}$ points".format(n)
    )
    plt.xlabel("$t = [0, 10]$")
    plt.grid()
    plt.show()
...
def f(y, t):
    return np.array(
        [y[1], y[2], 12 * y[0] ** (4 / 5.0) + np.cos(y[1]) ** 3 - np.sin(y[2])]
    )
...
def f(y, t):
    return np.array([y[1], y[2], y[3], y[0] ** (-5 / 3.0)])
...
for n in [20, 100, 1000]:
    ...
    for method in methods:
        print(
            "Time of solving this ODE for {} points with {} method...".format(
                n, method.__name__
            )
        )
