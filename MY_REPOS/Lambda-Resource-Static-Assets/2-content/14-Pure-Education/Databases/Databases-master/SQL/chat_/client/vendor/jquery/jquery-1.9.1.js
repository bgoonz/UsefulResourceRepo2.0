...
((window, undefined) => {
    ...
    const // List of deleted data cache ids, so we can reuse them
    core_deletedIds = [];
    ...
    const // Define a local copy of jQuery
    jQuery = (selector, context) => {
		// The jQuery object is actually just the init constructor 'enhanced'
		return new jQuery.fn.init( selector, context, rootjQuery );
	};

    const // Used for matching numbers
    core_pnum = /[+-]?(?:\d*\.|)\d+(?:[eE][+-]?\d+|)/.source;

    const // Used for splitting on whitespace
    core_rnotwhite = /\S+/g;

    const // Make sure we trim BOM and NBSP (here's looking at you, Safari 5.0 and IE)
    rtrim = /^[\s\uFEFF\xA0]+|[\s\uFEFF\xA0]+$/g;
    ...
    const // Matches dashed string for camelizing
    rmsPrefix = /^-ms-/;

    const rdashAlpha = /-([\da-z])/gi;

    const // Used by jQuery.camelCase as callback to replace()
    fcamelCase = (all, letter) => {
		return letter.toUpperCase();
	};

    const // The ready event handler
    completed = ({type}) => {
    ...
    const // Clean-up method for dom ready events
    detach = () => {
    ...
    jQuery.Callbacks = options => {
        ...
        let // End of the loop when firing
        firingLength;

        let // Index of currently firing callback (modified by remove if needed)
        firingIndex;

        let // First callback to fire (used internally by add and fireWith)
        firingStart;

        let // Actual callback list
        list = [];

        let // Stack of fire calls for repeatable lists
        stack = !options.once && [];
    ...
    jQuery.extend({
        ...
        when(subordinate /* , ..., subordinateN */) {
            ...
            let // the count of uncompleted subordinates
            remaining = length !== 1 || ( subordinate && jQuery.isFunction( subordinate.promise ) ) ? length : 0;

            const // the master Deferred. If resolveValues consist of only a single Deferred, just use that.
            deferred = remaining === 1 ? subordinate : jQuery.Deferred();

            const // Update function for both resolve and progress values
    ...
    function internalRemoveData( elem, name, pvt ) {
        ...
        const // See jQuery.data for more information
        cache = isNode ? jQuery.cache : elem;
    ...
    jQuery.extend({
        valHooks: {
            option: {
                get({attributes, value, text}) {
                    // attributes.value is undefined in Blackberry 4.7 but
                    // uses .value. See #6932
                    const val = attributes.value;
                    return !val || val.specified ? value : text;
                }
            },
            select: {
                get(elem) {
    ...
    if ( !jQuery.support.style ) {
        jQuery.attrHooks.style = {
            get({style}) {
                ...
                return style.cssText || undefined;
            },
            set({style}, value) {
    ...
    jQuery.event = {
        ...
        handlers({target, button, type}, handlers) {
            let sel;
            let handleObj;
            let matches;
            let i;
            ...
            if ( delegateCount && cur.nodeType && (!button || type !== "click") ) {

                for ( ; cur != this; cur = cur.parentNode || this ) {

                    // Don't check non-elements (#13208)
                    // Don't process clicks on disabled elements (#6911, #8165, #11382, #11764)
                    if ( cur.nodeType === 1 && (cur.disabled !== true || type !== "click") ) {
                        matches = [];
        ...
        special: {
            ...
            beforeunload: {
                postDispatch({result, originalEvent}) {
                    ...
                    if ( result !== undefined ) {
                        originalEvent.returnValue = result;
                    }
                }
            }
        },
    ...
    if ( !jQuery.support.submitBubbles ) {
        ...
        jQuery.event.special.submit = {
            setup() {
                ...
                jQuery.event.add( this, "click._submit keypress._submit", ({target}) => {
    ...
    if ( !jQuery.support.changeBubbles ) {
        ...
        jQuery.event.special.change = {
            ...
            setup() {
                ...
                if ( rformElems.test( this.nodeName ) ) {
                    ...
                    if ( this.type === "checkbox" || this.type === "radio" ) {
                        jQuery.event.add( this, "propertychange._change", function({originalEvent}) {
                            if ( originalEvent.propertyName === "checked" ) {
                ...
                jQuery.event.add( this, "beforeactivate._change", ({target}) => {
                    const elem = target;
            ...
            handle({target, isSimulated, isTrigger, handleObj}) {
                ...
                if ( this !== elem || isSimulated || isTrigger || (elem.type !== "radio" && elem.type !== "checkbox") ) {
    ...
    ((window, undefined) => {
        ...
        const // Use a stripped-down indexOf if we can't use a native one
        ...
        const // http://www.w3.org/TR/css3-syntax/#characters
        characterEncoding = "(?:\\\\.|[\\w-]|[^\\x00-\\xa0])+";

        const // Loosely modeled on CSS identifier characters
        // An unquoted value should be a CSS identifier http://www.w3.org/TR/css3-selectors/#attribute-selectors
        // Proper syntax: http://www.w3.org/TR/CSS21/syndata.html#value-def-identifier
        identifier = characterEncoding.replace( "w", "w#" );

        const // Acceptable operators http://www.w3.org/TR/selectors/#attribute-selectors
        operators = "([*^$|!~]?=)";

        const attributes = `\\[${whitespace}*(${characterEncoding})${whitespace}*(?:${operators}${whitespace}*(?:(['"])((?:\\\\.|[^\\\\])*?)\\3|(${identifier})|)|)${whitespace}*\\]`;

        const // Prefer arguments quoted,
        ...
        const // CSS escapes http://www.w3.org/TR/CSS21/syndata.html#escaped-characters
        runescape = /\\([\da-fA-F]{1,6}[\x20\t\r\n\f]?|.)/g;

        const funescape = (_, escaped) => {
        ...
        Expr = Sizzle.selectors = {
                ...
                "ATTR": function( name, operator, check ) {
                    return elem => {
                        ...
                        return operator === "=" ? result === check :
                            ...
                            operator === "~=" ? ` ${result} `.includes(check) :
            ...
            pseudos: {
                ...
                "selected": function({parentNode, selected}) {
                ...
                // Element/input types
                "header": function({nodeName}) {
                    return rheader.test( nodeName );
                },

                "input": function({nodeName}) {
                    return rinputs.test( nodeName );
                },

                "button": function({nodeName, type}) {
        ...
        function matcherFromGroupMatchers( elementMatchers, setMatchers ) {
            ...
            const superMatcher = (seed, context, xml, results, expandContext) => {
                let elem;
                let j;
                let matcher;
                let setMatched = [];
                ...
                const // We must always have either seed elements or context
    ...
    const // methods guaranteed to produce a unique set when starting from a unique set
    guaranteedUnique = {
        children: true,
        contents: true,
        next: true,
    ...
    jQuery.each({
        ...
        children({firstChild}) {
            return jQuery.sibling( firstChild );
        },
        contents(elem) {
    ...
    function winnow(elements, qualifier = 0, keep) {
        if ( jQuery.isFunction( qualifier ) ) {
            ...
            const filtered = jQuery.grep(elements, ({nodeType}) => {
                return nodeType === 1;
            });
    ...
    const // checked="checked" or checked
    ...
    const // We have to close these tags to support XHTML (#13200)
    wrapMap = {
    ...
    if ( window.getComputedStyle ) {
        ...
        curCSS = (elem, name, _computed) => {
            ...
            let // getPropertyValue is only needed for .css('filter') in IE9, see #12537
    ...
    if ( !jQuery.support.opacity ) {
        jQuery.cssHooks.opacity = {
            get({currentStyle, style}, computed) {
                // IE uses filters for opacity
                return ropacity.test( (computed && currentStyle ? currentStyle.filter : style.filter) || "" ) ?
    ...
    const // Avoid comment-prolog char sequence (#10098); must appease lint and evade compression
    allTypes = "*/".concat("*");
    ...
    jQuery.extend({
        ...
        ajax(url, options) {
            ...
            let // Response headers as string
    ...
    jQuery.ajaxTransport( "script", ({crossDomain, scriptCharset, url}) => {
    ...
    jQuery.ajaxPrefilter( "json jsonp", (s, {jsonpCallback}, jqXHR) => {
        let callbackName;
        let overwritten;
    ...
    function Animation( elem, properties, options ) {
        ...
        const tick = () => {
            ...
            const // archaic crash bug won't allow us to use 1 - ( 0.5 || 0 ) (#12497)
