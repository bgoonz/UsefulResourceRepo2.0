  ...
  if (member.type === "Function" || member.type === "Indexer") {
    ...
    if (!member.parameters && !member.returnType) {
      console.log(
        "warning: " +
          res +
          " is missing parameters - probably defined as foo : Function instead of foo(bar)"
      );
    ...
    if (!member.parameters || !member.returnType) {
      ...
      throw Error(
        res +
          " is missing parameters/returnType - probably defined as foo : Function instead of foo(bar)"
      );
...
const getPageDescription = function (obj) {
  let pageDesc =
    "Interactive API reference for the JavaScript " +
    obj.name +
    " Object. " +
    obj.description;
...
const render = (filename, locals) => {
  const template =
    templates[filename] ||
    fs.readFileSync("templates/" + filename + ".ejs", "utf8");
