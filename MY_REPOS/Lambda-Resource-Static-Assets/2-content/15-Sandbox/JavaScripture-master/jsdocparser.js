...
var processFileContents = function (contents) {
  ...
  if (fragments.length) {
    ...
    var nameComparison = function (m1, m2) {
      ...
      var comparison = m1.name
        .toLowerCase()
        .localeCompare(m2.name.toLowerCase());
      return comparison;
    };
...
var createHtmlExample = function (code, obj) {
  obj.hasExample = true;
  return template.render("htmlexample", {
    code: code,
...
var parseMember = function (member) {
  ...
  if (!member.match(/:/)) {
    ...
    if ((res = member.match(/\s*(\w*)\s*\<(.*)\>/))) {
      ...
      return { name: name, templates: parseParamList(res[2]) };
    }
    return member;
  }
  ...
  if (defaultValueSplit !== -1) {
    ...
    return { type: "Object", properties: parseParamList(res[1]) };
  } else {
  ...
  if (type.length > 2 && type.lastIndexOf("*/") === type.length - 2) {
    ...
    description = processMarkup(
      currentObj,
      type.substring(commentStartIndex + 2, type.length - 2).trim()
    );
  ...
  if ((res = name.match(/^\s*\[(.*)\]/))) {
    return {
      parameters: parseParamList(res[1]),
      type: "Indexer",
      returnType: parseMember(type),
      description: description,
    };
