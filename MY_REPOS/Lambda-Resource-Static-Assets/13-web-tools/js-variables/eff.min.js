  ...
  (function () {
    var f = (function () {
      ...
      m = function (c) {
        ...
        if (!l) return "";
        for (var r = ""; ++k < l; ) {
          var b = Number(arguments[k]);
          if (!isFinite(b) || 0 > b || 1114111 < b || n(b) != b)
          ...
          if (k + 1 == l || 16384 < h.length)
            (r += a.apply(null, h)), (h.length = 0);
        }
        return r;
      };
    f
      ? f(String, "fromCodePoint", { value: m, configurable: !0, writable: !0 })
      : (String.fromCodePoint = m);
  })();
(function (f, a) {
  function n(g) {
    return encodeURIComponent(g).replace(/['()_*]/g, function (d) {
      return "%" + d.charCodeAt().toString(16);
    });
  }
  function m() {
    var g = c.value,
    ...
    var e = g.replace(
      ...
      function (p, v, C) {
        p = parseInt(C || v, 16);
        return 55296 <= p && 57343 >= p ? "\x00" : String.fromCodePoint(p);
      }
    );
    var u = !D.test(
      g.replace(/\\u([a-fA-F0-9]{4})/g, function (p, v) {
        return String.fromCodePoint(parseInt(v, 16));
      })
    );
    if ((d = E.test(e)) || !w.test(e))
      (c.className = "invalid"),
        (h.innerHTML = "an invalid"),
        (k.className = l.className = t.className = x.className = ""),
        (y.className = d ? "show" : "");
    else {
      h.innerHTML = "a valid";
      d = F.test(e);
      ...
      t.className = e ? "show" : "";
      x.className = A ? "show" : "";
      c.className = u || d || B || z || A ? "warning" : "";
    }
    J.hash = n(g);
    q && (q.jsVars = g);
  }
  var c = a.getElementsByTagName("input")[0],
    h = a.getElementsByTagName("i")[0],
    k = a.getElementById("es3"),
    l = a.getElementById("es5"),
    r = a.getElementById("zero-width"),
    b = a.getElementById("unicode"),
    t = a.getElementById("browser"),
    y = a.getElementById("reserved-word"),
    x = a.getElementById("immutable-properties"),
    J = a.getElementById("permalink"),
    w =
    ...
    H = /^(?:NaN|Infinity|undefined)$/,
    I = /\u200C|\u200D/,
    q = (function () {
      var g = new Date(),
        d;
      try {
        (d = f.localStorage).setItem(g, g);
        var e = d.getItem(g) == g;
        d.removeItem(g);
        return e && d;
      } catch (u) {}
