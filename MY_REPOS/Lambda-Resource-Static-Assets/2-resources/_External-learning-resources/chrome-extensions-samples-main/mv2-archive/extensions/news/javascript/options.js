...
function setCountry() {
  ...
  if (!country || country == "noCountry") {
    ...
    function handleResponse() {
      ...
      $("countryList").value = newsUrl.substring(
        newsUrl.indexOf("&ned=") + 5,
        newsUrl.indexOf("&hl=")
      );
    }
  } else {
...
function showUserMessages(id, timeOut, message) {
  $(id).style.setProperty("transition", "opacity 0s ease-in");
  $(id).style.opacity = 1;
  $(id).innerText = chrome.i18n.getMessage(message);
  window.setTimeout(function () {
    $(id).style.setProperty("transition", "opacity" + timeOut + "s ease-in");
    $(id).style.opacity = 0;
  }, 1e3);
}
...
function setLocalizedTopicList() {
  ...
  $("custom_text").innerHTML =
    getI18nMsg("customText") +
    "<br/>" +
