...
(function () {
  ...
  function updateStatus(status) {
    ...
    var treeState = /open/i.exec(status)
      ? "open"
      : /throttled/i.exec(status)
      ? "throttled"
      : "closed";

    if (lastState && lastState != treeState) {
      prefs.getUseNotifications(function (useNotifications) {
        if (useNotifications) notifyStatusChange(treeState, status);
      });
    }
  ...
  function fetchTryJobResults(fullPatchset, builder, buildnumber, completed) {
    ...
    if (
      lastDefunctTryJob.hasOwnProperty(builder) &&
    ...
    buildbot.requestURL(
      ...
      function (errorStatus) {
        if (errorStatus == 404) {
          lastDefunctTryJob[builder] = Math.max(
            lastDefunctTryJob[builder] || 0,
            buildnumber
          );
        }
        completed();
  ...
  function fetchPatches(issue, updatedCallback) {
    ...
    issue.patchsets.forEach(function (patchset) {
      var patchURL =
        "https://codereview.chromium.org/api/" + issue.issue + "/" + patchset;

      buildbot.requestURL(patchURL, "json", function (patch) {
        if (!issue.full_patchsets) issue.full_patchsets = {};
      ...
      buildbot.requestURL(issueURL, "json", function (issue) {
        fetchPatches(issue, function (state) {
          ...
          if (activeIssues.getIssue(issue.issue)) {
            if (state == TRY_JOBS_COMPLETE) activeIssues.updateIssue(issue);
          } else {
