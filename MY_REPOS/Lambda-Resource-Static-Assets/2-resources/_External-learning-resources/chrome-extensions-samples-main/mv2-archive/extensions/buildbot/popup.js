...
(function () {
  ...
  function getClassForTryJobResult(result) {
    ...
    if (result === null) result = buildbot.RUNNING;
  ...
  function createTryJobAnchorTitle(tryJob, fullTryJob) {
    ...
    if (fullTryJob.results == buildbot.FAILURE && fullTryJob.text) {
      ...
      fullTryJob.steps.forEach(function (step) {
        if (step.results[0] == buildbot.FAILURE) text.push(step.results[1][0]);
      });
    ...
    if (stepText.length > 0) title += ": " + stepText.join("; ");
  ...
  function createPatchsetStatusElement(patchset) {
    ...
    tryJobs.forEach(function (tryJob) {
      ...
      var fullTryJob =
        patchset.full_try_job_results && patchset.full_try_job_results[key];
      ...
      tryJobAnchor.className =
        "issue-status-build " + getClassForTryJobResult(tryJob.result);
  ...
  function updateIssueDisplay(issue) {
    ...
    if (!row) row = createTryStatusRow(issue);
    ...
    var clAnchor =
      label.childNodes[0] || label.appendChild(document.createElement("a"));
    ...
    if (status.childElementCount < 1) status.appendChild(statusElement);
    else status.replaceChild(statusElement, status.firstChild);
  }
  ...
  function main() {
    ...
    buildbot.getActiveIssues().setEventCallback(function (request) {
      ...
      if (!chrome.extension.getViews({ type: "popup" })) return;

      switch (request.event) {
        case "issueUpdated":
        case "issueAdded":
          updateIssueDisplay(
            buildbot.getActiveIssues().getIssue(request.issue)
          );
          break;
    ...
    setInterval(function () {
      var botStatusElements = document.getElementsByClassName(
        "trunk-status-iframe"
      );
