...
var DrNicTest = {
  ...
  arrayfromargs: function (args) {
    ...
    for (i = 0; i < args.length; i++) myarray[i] = args[i];
  ...
  selectorMatch: function (expression, element) {
    ...
    var assertions = {
      ...
      attr: function (element, matches) {
        ...
        return (
          nodeValue &&
          operators[matches[2]](nodeValue, matches[5] || matches[6])
        );
      },
    };
...
DrNicTest.Template.prototype.evaluate = function (object) {
  ...
  return DrNicTest.gsub(this.template, this.pattern, function (match) {
    ...
    while (match != null) {
      var comp =
        match[1].indexOf("[]") === 0 ? match[2].gsub("\\\\]", "]") : match[1];
      ctx = ctx[comp];
...
DrNicTest.Unit.MessageTemplate.prototype.evaluate = function (params) {
  ...
  for (var i = 0; i < this.parts.length; i++) {
    ...
    var result =
      part == "?"
        ? DrNicTest.inspect(params.shift())
        : part.replace(/\\\?/, "?");
    results.push(result);
  }
  return results.join("");
...
DrNicTest.Unit.Assertions = {
  ...
  assertHashEqual: function (expected, actual, message) {
    message = this.buildMessage(
      message || "assertHashEqual",
      "expected <?>, actual: <?>",
      expected,
      actual
    );
    var expected_array = DrNicTest.flattenArray(
      DrNicTest.hashToSortedArray(expected)
    );
    var actual_array = DrNicTest.flattenArray(
      DrNicTest.hashToSortedArray(actual)
    );
    var block = function () {
      if (expected_array.length == actual_array.length) {
        for (var i = 0; i < expected_array.length; i++) {
  ...
  assertNotNullOrUndefined: function (obj, message) {
    message = this.buildMessage(
      message || "assertNotNullOrUndefined",
      "got <?>",
      obj
    );
    this.assertBlock(message, function () {
  ...
  assertInstanceOf: function (expected, actual, message) {
    message = this.buildMessage(
      ...
      "<?> was not an instance of the expected type",
      actual
    );
    this.assertBlock(message, function () {
  ...
  assertNotInstanceOf: function (expected, actual, message) {
    message = this.buildMessage(
      message || "assertNotInstanceOf",
      "<?> was an instance of the expected type",
      actual
    );
    this.assertBlock(message, function () {
      return !(actual instanceof expected);
  ...
  assertElementsMatch: function () {
    var pass = true,
      expressions = DrNicTest.arrayfromargs(arguments);
    var elements = expressions.shift();
    if (elements.length != expressions.length) {
      message = this.buildMessage(
        "assertElementsMatch",
        "size mismatch: ? elements, ? expressions (?).",
        elements.length,
        expressions.length,
        expressions
      );
...
DrNicTest.Unit.Runner = function (testcases) {
  ...
  options.testLog =
    "testLog" in argumentOptions ? argumentOptions.testLog : "testlog";
...
DrNicTest.Unit.Testcase.prototype.summary = function () {
  var msg =
    "#{assertions} assertions, #{failures} failures, #{errors} errors\n";
...
DrNicTest.Unit.Testcase.prototype.error = function (error, test) {
  ...
  this.messages.push(
    error.name + ": " + error.message + "(" + DrNicTest.inspect(error) + ")"
  );
};
