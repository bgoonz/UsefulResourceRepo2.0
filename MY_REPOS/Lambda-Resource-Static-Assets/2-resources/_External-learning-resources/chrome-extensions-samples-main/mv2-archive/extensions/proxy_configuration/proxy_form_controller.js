...
var ProxyFormController = function (id) {
  ...
  if (!this.form_) throw chrome.i18n.getMessage("errorIdNotFound", id);
...
ProxyFormController.prototype = {
  ...
  set singleProxy(data) {
    ...
    if (data) this.httpProxy = data;

    if (checkbox.checked)
      checkbox.parentNode.parentNode.classList.add("single");
    else checkbox.parentNode.parentNode.classList.remove("single");
  },
  ...
  handleRegularState_: function (c) {
    if (
      c.levelOfControl === ProxyFormController.LevelOfControl.AVAILABLE ||
  ...
  dispatchFormClick_: function (e) {
    ...
    if (t.nodeName === "INPUT" && t.getAttribute("type") === "submit") {
      ...
      while (
        t &&
        (t.nodeName !== "FIELDSET" || t.parentNode.nodeName !== "FORM")
      ) {
        t = t.parentNode;
      }
      if (t) {
  ...
  applyChanges_: function (e) {
    ...
    else this.config_.regular = this.generateProxyConfig_();
  ...
  toggleIncognitoMode_: function (e) {
    ...
    if (!this.isAllowedIncognitoAccess_) {
      var msg =
        "I'm sorry, Dave, I'm afraid I can't do that. Give me access " +
