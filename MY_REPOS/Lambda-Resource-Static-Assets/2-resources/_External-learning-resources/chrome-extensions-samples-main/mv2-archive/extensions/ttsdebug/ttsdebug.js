...
function isErrorEvent(evt) {
  return (
    evt.type == "error" || evt.type == "interrupted" || evt.type == "cancelled"
  );
}
...
function logSpeakCall(utterance, options, callback) {
  ...
  console.log(
    "Calling chrome.tts.speak('" +
      utterance +
      "', " +
      JSON.stringify(optionsCopy) +
      ")"
  );
  if (callback) chrome.tts.speak(utterance, options, callback);
  else chrome.tts.speak(utterance, options);
}
...
var tests = [
  {
    name: "Baseline",
    description:
      "Ensures that the speech engine sends both start and " +
      ...
      logSpeakCall("Alpha Bravo Charlie Delta Echo", {
        ...
        onEvent: function (evt) {
          ...
          if (isErrorEvent(evt)) {
            ...
            if (startTime == undefined) startTime = callTime;
            var endTime = new Date();
            var delta = (endTime - startTime) / 1000;
  ...
  },
  {
    ...
    description:
      "Interrupts partway through a long sentence and then " +
  ...
  },
  {
    name: "High pitch",
    description:
      "<b>Manual</b> test - verify that the pitch is " +
  ...
  },
  {
    ...
    description:
      "Checks to see if proper word and sentence callbacks " + "are received.",
    dependencies: ["Baseline"],
    trials: 1,
    run: function (testRunName, voiceName, callback) {
      var callTime = new Date();
  ...
  },
  {
    ...
    description:
      "Establishes a baseline time to speak a " +
  ...
  },
  {
    ...
    description:
      "Queue a sequence of three utterances, then before they " +
...
function registerTest(test) {
  ...
  function runTest(index, voiceName) {
    ...
    var testRunName =
      ...
      " of " +
      test.trials;
    console.log("*** Beginning " + testRunName + " with voice " + voiceName);
