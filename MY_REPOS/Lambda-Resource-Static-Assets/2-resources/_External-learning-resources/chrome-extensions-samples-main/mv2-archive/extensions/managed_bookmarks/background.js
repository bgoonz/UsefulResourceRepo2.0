...
CallbackChain.prototype.wrap = function (callback) {
  ...
  return function () {
    ...
    self._count--;
    if (self._count == 0) {
      for (var i = 0; i < self._callbacks.length; ++i) self._callbacks[i]();
    }
  };
};
...
Node.prototype.droppedFromParent = function () {
  ...
  if (this._children) chrome.bookmarks.removeTree(this._id);
  else chrome.bookmarks.remove(this._id);
};
  ...
  for (var i = 0; i < wantedChildren.length; ++i) {
    ...
    if (currentChild && currentChild.matches(wantedChild)) {
      ...
      chrome.bookmarks.create(
        ...
        callbackChain.wrap(
          function (wantedChild, newChild, createdNode) {
 ...
 */
Node.prototype.moveInModel = function (
  currentParentId,
  currentIndex,
  callback
) {
    ...
    for (var i = 0; i < currentOrder.length; ++i) {
      ...
      if (child && child.getIndex() != i) {
        ...
        child.moveInModel(
          node.parentId,
          node.index,
          self.reorderChildren.bind(self)
        );
        return;
      }
    }
...
var serializeNode = function (node) {
  ...
  if (node._url) result["url"] = node._url;
  else result["children"] = node._children.map(serializeNode);
  return result;
};
...
chrome.bookmarks.onMoved.addListener(function (id, info) {
  tree.load(function () {
    ...
    if (managedNode && !managedNode.isRoot()) {
      ...
      if (parentNode) parentNode.reorderChildren();
    }
  });
});
...
chrome.bookmarks.onChanged.addListener(function (id, info) {
  tree.load(function () {
    var managedNode = tree.getById(id);
    if (!managedNode || managedNode.isRoot()) return;
