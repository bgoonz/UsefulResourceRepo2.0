...
function search(query, callback) {
  ...
  if (/^re:/.test(query)) query = query.substring("re:".length);
  else if (/^file:/.test(query))
    query = 'file:"' + query.substring("file:".length) + '"';
  else query = '"' + query + '"';

  var url =
    "https://code.google.com/p/chromium/codesearch#search/&type=cs&q=" +
    query +
...
chrome.omnibox.onInputEntered.addListener(function (text) {
  ...
  if (/@\d+\b/.test(text)) {
    ...
    navigate(
      "https://code.google.com/p/chromium/codesearch#search/&type=cs" +
        "&q=" +
        text +
        "&exact_package=chromium&type=cs"
    );
