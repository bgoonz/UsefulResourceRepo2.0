Filer = function (filesystem, container, editor, isSyncable) {
  ...
  container.appendChild(
    createElement("div", { id: "filer-empty-label", innerText: "-- empty --" })
  );
  ...
  if (this.isSyncable) {
      ...
      chrome.syncFileSystem.onFileStatusChanged.addListener(
        function (detail) {
          if (detail.direction == "remote_to_local") {
            ...
            if (
              this.editor.getCurrentPath() == detail.fileEntry.fullPath &&
    ...
    if (chrome.syncFileSystem.onServiceStatusChanged) {
      chrome.syncFileSystem.onServiceStatusChanged.addListener(
        function (detail) {
          log(
            "Service state updated: " + detail.state + ": " + detail.description
          );
        }.bind(this)
      );
    }
...
Filer.prototype.didReadEntries = function (dir, reader, entries) {
  ...
  for (var i = 0; i < entries.length; ++i) {
    if (entries[i].isFile) {
      ...
      entries[i].file(
        this.addEntry.bind(this, node, entries[i]),
...
Filer.prototype.rename = function (oldName, newName) {
  this.filesystem.root.getFile(
    ...
    error.bind(null, "getFile:" + oldName)
  );
};
...
Filer.prototype.addEntry = function (parentNode, entry, file) {
  ...
  inputNode.addEventListener(
    ...
    function (ev) {
      if (ev.keyCode == 13) {
        ...
        if (oldName != newName) this.rename(oldName, newName);
      }
    }.bind(this)
  );
  ...
  a.addEventListener(
    "dblclick",
    function (ev) {
      if (this.resetRenameFocus) this.resetRenameFocus();
    ...
    function (ev) {
      ...
      this.filesystem.root.getFile(
        ...
        function (entry) {
          entry.remove(
            function () {
              parentNode.removeChild.bind(parentNode, li), this.reload();
            }.bind(this),
            error.bind(this, "remove:", entry)
          );
        }.bind(this)
      );
    }.bind(this)
...
Filer.prototype.showUsage = function () {
  ...
  webkitStorageInfo.queryUsageAndQuota(
    ...
    function (usage, quota) {
      $("#filer-usage").innerText = "Usage:" + this.formatSize(usage);
    }.bind(this)
  );
};
