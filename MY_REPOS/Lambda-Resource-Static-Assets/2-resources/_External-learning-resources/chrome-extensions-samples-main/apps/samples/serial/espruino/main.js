...
connection.onReadLine.addListener(function (line) {
  ...
  if (line.indexOf("TEMPERATURE=") == 0)
    document.querySelector("#get_temperature").innerHTML =
      "Temp = " + line.substr(12);
});
...
document.querySelector("#flash").addEventListener("click", function () {
  connection.send(
    "l=0; var interval = setInterval(function() { digitalWrite(LED2, l&1); if (++l>6) clearInterval(interval); }, 200);\n"
  );
});

// Get temperature
document
  .querySelector("#get_temperature")
  .addEventListener("click", function () {
    connection.send(
      "console.log('TEMPERATURE='+E.getTemperature().toFixed(1));\n"
    );
  });
