...
define(function (require, exports, module) {
  ...
  Pp.MaybeLeftParen = function MaybeLeftParen() {
    if (this.parenFreeMode) return this.match(LEFT_PAREN) ? LEFT_PAREN : END;
    ...
    switch (tt) {
      ...
      case FUNCTION:
        ...
        return this.FunctionDefinition(
          true,
          this.x.topLevel ? DECLARED_FORM : STATEMENT_FORM,
          comments
        );
          ...
          while ((tt = this.t.get()) !== RIGHT_CURLY) {
            switch (tt) {
              ...
              case CASE:
                ...
                else n2.caseLabel = this.Expression(COLON);
                break;
      ...
      case FOR:
        ...
        if (this.match(IDENTIFIER)) {
          if (this.t.token.value === "each") n.isEach = true;
          else this.t.unget();
        }
        if (!this.parenFreeMode) this.mustMatch(LEFT_PAREN);
        x2 = this.x.pushTarget(n).nest();
        ...
        if (n2 && this.match(IN)) {
          ...
          if (n.isEach) this.fail("Invalid for each..in loop");
          this.withContext(x3, function () {
            n.condition =
              this.peek(true) === SEMICOLON ? null : this.Expression();
            this.mustMatch(SEMICOLON);
            tt2 = this.peek(true);
            n.update = (
              this.parenFreeMode
        ...
        if (!this.parenFreeMode) this.mustMatch(RIGHT_PAREN);
        this.withContext(x2, function () {
      ...
      case TRY:
        ...
        while (this.match(CATCH)) {
          ...
          if (this.match(IF)) {
            if (!this.mozillaMode) this.fail("Illegal catch guard");
  ...
  Pp.ReturnOrYield = function ReturnOrYield() {
    ...
    if (tt === RETURN) {
      if (!this.x.inFunction) this.fail("Return not in function");
    } /* if (tt === YIELD) */ else {
      if (!this.x.inFunction) this.fail("Yield not in function");
  ...
  Pp.ImportPath = function ImportPath() {
    ...
    if (!this.match(DOT)) {
      if (n.type === IDENTIFIER) this.fail("cannot import local variable");
      return n;
    }
   ...
   *                    -> node
   */
  Pp.FunctionDefinition = function FunctionDefinition(
    requireName,
    functionForm,
    comments
  ) {
    ...
    if (this.match(MUL)) f.isExplicitGenerator = true;
    if (this.match(IDENTIFIER, false, true)) f.name = this.t.token.value;
  ...
  Pp.ModuleVariables = function ModuleVariables(n) {
    ...
    do {
      ...
      if (this.match(ASSIGN)) {
        ...
        else this.x.parentScript.modAssns.set(n1.value, n1);
      }
      n.push(n1);
    } while (this.match(COMMA));
  };
  ...
  Pp.LetBlock = function LetBlock(isStatement) {
    ...
    if (isStatement) n.block = this.Block();
    else n.expression = this.AssignExpression();
  ...
  Pp.DestructuringExpression = function DestructuringExpression(
    simpleNamesOnly
  ) {
  ...
  Pp.ComprehensionTail = function ComprehensionTail() {
    ...
    do {
      ...
      if (this.match(IDENTIFIER)) {
        ...
        if (this.mozillaMode && this.t.token.value === "each") n.isEach = true;
        else this.t.unget();
      }
      p = this.MaybeLeftParen();
      switch (this.t.get()) {
  ...
  Pp.AssignExpression = function AssignExpression() {
    ...
    if (this.match(YIELD, true)) return this.ReturnOrYield();
  ...
  Pp.EqualityExpression = function EqualityExpression() {
    ...
    while (
      this.match(EQ) ||
      this.match(NE) ||
      this.match(STRICT_EQ) ||
      this.match(STRICT_NE)
    ) {
      n2 = this.newNode();
      n2.push(n);
  ...
  Pp.UnaryExpression = function UnaryExpression() {
    ...
    switch ((tt = this.t.get(true))) {
      ...
      case PLUS:
      case MINUS:
        if (tt === PLUS) n = this.newNode({ type: UNARY_PLUS });
        else if (tt === MINUS) n = this.newNode({ type: UNARY_MINUS });
        else n = this.newNode();
      ...
      default:
        ...
        if (
          this.t.tokens[(this.t.tokenIndex + this.t.lookahead - 1) & 3]
            .lineno === this.t.lineno
        ) {
      ...
      case LEFT_CURLY:
        ...
        object_init: if (!this.match(RIGHT_CURLY)) {
          do {
            tt = this.t.get();
            if (
              (this.t.token.value === "get" || this.t.token.value === "set") &&
