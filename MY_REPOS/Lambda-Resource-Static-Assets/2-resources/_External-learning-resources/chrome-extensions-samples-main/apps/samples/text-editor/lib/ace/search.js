...
define(function (require, exports, module) {
  ...
  (function () {
    ...
    this.findAll = function (session) {
      ...
      var ignoreCursor =
        !options.start && options.wrap && options.scope == Search.ALL;
      if (ignoreCursor) options.start = { row: 0, column: 0 };
      ...
      if (ignoreCursor) options.start = null;
    ...
    this.$forwardMatchIterator = function (session) {
      ...
      return {
        forEach: function (callback) {
          self
            .$forwardLineIterator(session)
            .forEach(function (line, startIndex, row) {
              if (startIndex) {
    ...
    this.$forwardLineIterator = function (session) {
      ...
      var start =
        this.$options.start || range[searchSelection ? "start" : "end"];
    ...
    this.$backwardLineIterator = function (session) {
      ...
      var range = this.$options.range || session.getSelection().getRange();
      var start =
        this.$options.start || range[searchSelection ? "end" : "start"];
      ...
      return {
        forEach: function (callback) {
          ...
          while (!callback(line, startIndex, row)) {
            ...
            if (searchSelection) {
              if (row == firstRow) startIndex = firstColumn;
              else if (row == lastRow)
                line = line.substring(0, range.end.column);
            }

            if (inWrap && row == start.row) startIndex = start.column;
          }
        },
      };
    };
