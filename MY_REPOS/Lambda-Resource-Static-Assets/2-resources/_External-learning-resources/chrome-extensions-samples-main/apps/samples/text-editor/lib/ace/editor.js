  ...
  (function () {
    ...
    this.$updateHighlightActiveLine = function () {
      ...
      if (typeof this.$lastrow == "number")
        this.renderer.removeGutterDecoration(
          this.$lastrow,
      ...
      if (this.getHighlightActiveLine()) {
        ...
        if (
          ...
          session.$highlightLineMarker = session.addMarker(
            range,
            "ace_active_line",
            "background"
          );
    ...
    this.insert = function (text) {
      ...
      var shouldOutdent = mode.checkOutdent(
        lineState,
    ...
    this.onTextInput = function (text, pasted) {
      if (pasted) this._emit("paste", text);
    ...
    this.setHighlightSelectedWord = function (shouldHighlight) {
      ...
      if (shouldHighlight) this.session.getMode().highlightSelection(this);
      else this.session.getMode().clearSelectionHighlight(this);
    };
    ...
    this.setShowInvisibles = function (showInvisibles) {
      if (this.getShowInvisibles() == showInvisibles) return;
    ...
    this.selectPageDown = function () {
      var row =
        this.$getPageDownRow() + Math.floor(this.$getVisibleRowCount() / 2);

      this.scrollPageDown();

      var selection = this.getSelection();
      var leadScreenPos = this.session.documentToScreenPosition(
        selection.getSelectionLead()
      );
      var dest = this.session.screenToDocumentPosition(
        row,
        leadScreenPos.column
      );
    ...
    this.selectPageUp = function () {
      var visibleRows =
        this.renderer.getScrollTopRow() - this.renderer.getScrollBottomRow();
    ...
    this.findNext = function (options) {
      options = options || {};
      if (typeof options.backwards == "undefined") options.backwards = false;
      this.$search.set(options);
      this.$find();
    };
    ...
    this.findPrevious = function (options) {
      options = options || {};
      if (typeof options.backwards == "undefined") options.backwards = true;
      this.$search.set(options);
      this.$find();
    };
