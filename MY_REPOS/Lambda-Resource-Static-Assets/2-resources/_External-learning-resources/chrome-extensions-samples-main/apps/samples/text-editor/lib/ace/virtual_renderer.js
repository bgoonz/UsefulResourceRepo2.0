...
define(function (require, exports, module) {
  ...
  (function () {
    ...
    this.moveTextAreaToCursor = function (textarea) {
      ...
      textarea.style.top =
        bounds.top + pos.top - this.scrollTop + offset + "px";
    };
    ...
    this.$renderChanges = function (changes) {
      if (!changes || !this.session || !this.container.offsetWidth) return;
    ...
    this.$computeLayerConfig = function () {
      ...
      minHeight =
        ...
        session.getRowHeight(lineHeight, lastRow) +
        firstRowHeight;
    ...
    this.$calcSteps = function (fromValue, toValue) {
      ...
      var func = function (t, x_min, dx) {
        if ((t /= 0.5) < 1) return (dx / 2) * Math.pow(t, 3) + x_min;
    ...
    this.scrollToLine = function (line, center) {
      ...
      if (this.$animatedScroll && Math.abs(offset - this.scrollTop) < 10000) {
        ...
        this.$timer = setInterval(function () {
          ...
          if (!steps.length) clearInterval(_self.$timer);
        }, 10);
      } else {
    ...
    this.scrollToX = function (scrollLeft) {
      if (scrollLeft <= this.$padding) scrollLeft = 0;
    ...
    this._loadTheme = function (name, callback) {
      ...
      var filename =
        config.get("themePath") + "/theme-" + base + config.get("suffix");
