  ...
  (function () {
    this.onMouseDown = function (ev) {
      ...
      var onMouseSelectionEnd = function (e) {
        ...
        if (state == STATE_UNKNOWN) onStartSelect(pos);
        else if (state == STATE_DRAG) onMouseDragSelectionEnd(e);
      ...
      var onSelectionInterval = function () {
        ...
        if (state == STATE_DRAG) onDragSelectionInterval();
        else if (state == STATE_SELECT) onUpdateSelectionInterval();
      };
      ...
      var onUpdateSelectionInterval = function () {
        ...
        if (_self.$clickSelection) {
          if (_self.$clickSelection.contains(cursor.row, cursor.column)) {
            ...
            if (
              _self.$clickSelection.compare(cursor.row, cursor.column) == -1
            ) {
      ...
      var onDragSelectionInterval = function () {
        dragCursor = editor.renderer.screenToTextCoordinates(
          mousePageX,
          mousePageY
        );
    ...
    this.onScroll = function (ev) {
      ...
      if (
        editor.renderer.isScrollableBy(
          ev.wheelX * ev.speed,
          ev.wheelY * ev.speed
        )
      )
