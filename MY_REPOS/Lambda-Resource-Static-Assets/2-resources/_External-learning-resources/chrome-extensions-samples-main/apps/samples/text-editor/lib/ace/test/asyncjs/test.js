...
define(function (require, exports, module) {
  ...
  (function () {
    ...
    this.run = function () {
      return this.setupTest()
        .each(function (test, next) {
          if (test.setUpSuite) test.setUpSuite(next);
          else next();
        })
        .each(function (test, next) {
        ...
        .each(function (test, next) {
          if (test.tearDownSuite) test.tearDownSuite(next);
          else next();
        });
    };
    ...
    this.report = function () {
      return this.each(function (test, next) {
        ...
        if (!test.passed)
          if (test.err.stack) console.log(test.err.stack);
    ...
    this.setupTest = function () {
      return this.each(function (test, next) {
        ...
        if (test.setUp) var setUp = async.makeAsync(0, test.setUp, context);
        else setUp = empty;
  ...
  exports.testcase = function (testcase, suiteName, timeout) {
    var methods = [];
    for (var method in testcase) methods.push(method);
    ...
    if (testcase.tearDownSuite) {
      tests[tests.length - 1].tearDownSuite = async.makeAsync(
        0,
        testcase.tearDownSuite,
        testcase
      );
    }
