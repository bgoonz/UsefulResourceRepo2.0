...
function showStorageInfo(unit) {
  table =
    ...
    "</td>" +
    "<td>" +
    bytesToMegaBytes(unit.capacity) +
    "</td>" +
    "<td>" +
    bytesToMegaBytes(unit.availableCapacity) +
    "</td>" +
    "</tr>\n";
  return table;
}
...
function init() {
  ...
  (function getMemoryInfo() {
    systemInfo.memory.getInfo(function (memory) {
      var memoryInfo =
        "<b>Total Capacity:</b> " +
        bytesToMegaBytes(memory.capacity) +
        "MB" +
        "<br><b>Available Capacity: </b>" +
        bytesToMegaBytes(memory.availableCapacity) +
        "MB";
  ...
  (function getStorageInfo() {
    systemInfo.storage.getInfo(function (units) {
      ...
      if (units.length == 0) return showTable();
      units.forEach(function (unit, index) {
        systemInfo.storage.getAvailableCapacity(unit.id, function (info) {
          ...
          if (index == units.length - 1) showTable();
        });
      });
    });
