...
SlideDeck.prototype.focusOverview_ = function () {
  ...
  for (var i = 0, slide; (slide = this.slides[i]); i++) {
    slide.style.webkitTransform = overview
      ? "translateZ(-2500px) translate(" + (i - this.curSlide_) * 105 + "%, 0%)"
      : "";
  }
};
...
SlideDeck.prototype.loadConfig_ = function (config) {
  ...
  if (settings.title) {
    document.title =
      settings.title.replace(/<br\/?>/, " ") + " - Google IO 2012";
    document.querySelector("[data-config-title]").innerHTML = settings.title;
  }

  if (settings.subtitle) {
    document.querySelector("[data-config-subtitle]").innerHTML =
      settings.subtitle;
  }
