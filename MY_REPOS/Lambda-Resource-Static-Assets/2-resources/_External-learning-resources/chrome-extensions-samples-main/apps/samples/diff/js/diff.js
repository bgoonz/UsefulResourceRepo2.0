...
function keyboardShortcut(event) {
  if (event.which == 74) selectNextChunk();
  else if (event.which == 75) selectPrevChunk();
  else if (
    event.which == 76 &&
    selectedChunk > 0 &&
    selectedChunk <= totalChunks
  )
  ...
  else if (event.which == 69) expandAllMatches();
  else if (event.which == 67) collapseAllMatches();
}

function disableShortcuts() {
  return (
    !$("#modal-shield").hasClass("hidden") ||
...
function getText(fileNum) {
  ...
  for (var i = 0; i < lines.length; i++) {
    if (
      !isBlankLine($(lines[i]).parent(), null) &&
...
function computeDiff(file1, file2) {
  ...
  if (texts[0] != "") $(".file-diff.0").html(ds[0]);
  if (texts[1] != "") $(".file-diff.1").html(ds[1]);
  setLineTypes();
  setLineNums();
  if (texts[0] != "" && texts[1] != "") {
    setNumDiffs(true);
...
function createHtmlLines(diffs) {
      ...
      if (text[i] != "") {
        switch (op) {
          case DIFF_INSERT:
            ...
            if (len > 5 && text[i].slice(len - 6, len) == "&para;") line2 += 1;
          ...
          case DIFF_DELETE:
            var len = text[i].length;
            if (len > 5 && text[i].slice(len - 6, len) == "&para;") line1 += 1;
  ...
  html1 =
    html1.replace(/&para;/g, '</div><span class="hidden">&para;</span><div>') +
    "</div>";
  html2 =
    html2.replace(/&para;/g, '</div><span class="hidden">&para;</span><div>') +
    "</div>";
  return [html1, html2];
}
...
function setLineTypes() {
  $("#file0-container .file-diff > div").each(function () {
    ...
    for (var i = 0; i < spans.length; i++) {
      if (!$(spans[i]).hasClass("ins")) displayed.push(spans[i]);
    }
  ...
  $("#file1-container .file-diff > div").each(function () {
    ...
    var displayed = [];
    for (var i = 0; i < spans.length; i++) {
      if (!$(spans[i]).hasClass("del")) displayed.push(spans[i]);
    }
    if (displayed.length == 0) {
      $(this).addClass("blank");
    }
...
function insertArrowAndCheck(cont, numChunks) {
  ...
  $("#arrow-container").append(
    '<div class="holder hidden ' + "chunk-" + numChunks + '"></div>'
  );
  $("#check-container").append(
    '<div class="holder hidden ' + "chunk-" + numChunks + '"></div>'
  );
  $("#arrow-container").append(
    '<div class="arrow visible ' + "chunk-" + numChunks + '"></div>'
  );
  $("#arrow-container").append(
    '<div class="undo visible hidden ' + "chunk-" + numChunks + '"></div>'
  );
  $("#check-container").append(
    '<div class="check visible ' + "chunk-" + numChunks + '"></div>'
  );
  $("#check-container").append(
    '<div class="undo visible hidden ' + "chunk-" + numChunks + '"></div>'
  );
...
function setNumDiffs(set) {
  ...
  if (diffChunks == 1) $("#num-diffs").html(diffChunks + " conflict");
  else $("#num-diffs").html(diffChunks + " conflicts");
}
...
function numDiffChunks(set) {
  ...
  if (selectedChunk == 0) selectChunk(0);
  return totalChunks;
}
...
function selectChunk(chunkNum) {
  ...
  if (chunkNum > totalChunks) $("#next-chunk").addClass("disabled");
  if (chunkNum < 1) $("#prev-chunk").addClass("disabled");
  if (chunkNum >= 0 && chunkNum <= totalChunks + 1) {
    ...
    if (chunkNum == 0 || chunkNum == 1) $(".file-diff").scrollTop(0);
    else if (chunkNum == totalChunks + 1)
      $(".file-diff").scrollTop($(".file-diff").scrollHeight);
    else
      $(".file-diff").scrollTop(
        $(".file-diff").scrollTop() +
          $(".chunk-" + chunkNum).position().top -
          80
      );
  }
}
...
function createCollapsibleMatches() {
  ...
  if (numCollapsed > 0) $("#expand-all").removeClass("disabled");
}
...
function collapse(lines1, lines2, arrows, checks, numContMatches, i) {
  ...
  if (numContMatches > 10) {
    ...
    $(lines1[firstCol]).before(
      '<div class="collapsed-num ' +
        firstLine +
        '">' +
        '<div class="' +
        firstLine +
    ...
    $(arrows[firstCol]).before(
      '<div class="arrow collapsed-num ' + firstLine + '"></div>'
    );
    $(checks[firstCol]).before(
      '<div class="check collapsed-num ' + firstLine + '"></div>'
    );
...
function moveChunk(chunkNum) {
  ...
  if (numChunks == 1) $("#num-diffs").html(numChunks + " conflict");
  else $("#num-diffs").html(numChunks + " conflicts");
  var text = getText(1);
  saveFile(text, "file1.txt");
  var num = parseInt(chunkNum.slice(6));
  if (selectedChunk == num) selectNextChunk();
}
...
function expandSection($collapsedNum) {
  ...
  $("#collapse-all").removeClass("disabled");
  if (numCollapsed == 0) $("#expand-all").addClass("disabled");
}
...
function collapseSection($collapsedNum) {
  ...
  $("#expand-all").removeClass("disabled");
  if (numCollapsed == totalCollapsible) $("#collapse-all").addClass("disabled");
}
