...
Printest.printout = A.object.create.call(A.controller, {
  ...
  spellNumber_: function (number) {
    return number < 20
      ? this.spellUnits_(number)
      : number < 100
      ? this.spellTens_(number)
      : number < 1000
      ? this.spellHundreds_(number)
  ...
  spellTens_: function (number) {
    ...
    return number >= 90
      ? "ninety" + separator + suffix
      : number >= 80
      ? "eighty" + separator + suffix
      : number >= 70
      ? "seventy" + separator + suffix
      : number >= 60
      ? "sixty" + separator + suffix
      : number >= 50
      ? "fifty" + separator + suffix
      : number >= 40
      ? "forty" + separator + suffix
      : number >= 30
      ? "thirty" + separator + suffix
