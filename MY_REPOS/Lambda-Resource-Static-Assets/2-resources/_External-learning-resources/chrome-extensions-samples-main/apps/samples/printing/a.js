...
A.object = {
  ...
  makeListener_: function (listener) {
    ...
    return event
      ? this.callListeners.bind(this, listener)
      : listener.bind
      ? listener.bind(this)
      : listener;
  },
...
(A.console.console_ = (function () {
  ...
  (A.promise = A.object.create({
    ...
    fulfill: function (value) {
      if (!this.hasOwnProperty("value")) {
        this.value = value;
        if (this.derived_) this.derived_.forEach(this.propagateTo_, this);
      }
      return this;
    },
    ...
    propagateTo_: function (derived) {
      ...
      if (typeof propagated === "undefined") propagated = this.value;
...
A.controller = A.object.create({
  create: function (properties) {
    ...
    return controller.getDomWindow_().then(
      function (domWindow) {
  ...
  getFrameSize: function () {
    return {
      width: this.domWindow.outerWidth - this.domWindow.innerWidth,
  ...
  getOuterBounds: function (innerBounds) {
    ...
    return {
      left: inner.left - Math.round(frame.width / 2),
  ...
  getInnerBounds: function (outerBounds) {
    ...
    var frame = this.getFrameSize();
    return {
      left: outer.left + Math.round(frame.width / 2),
  ...
  queryElements: function (elementsOrSelector) {
    return Array.isArray(elementsOrSelector)
      ? elementsOrSelector
  ...
  getDomWindow_: function () {
    ...
    return domWindow
      ? A.promise.create().fulfill(domWindow)
      : this.createDomWindow_();
  },

  /** @private */
  createDomWindow_: function () {
    return this.createAppWindow_().then(
      function (appWindow) {
...
A.application = A.object.create({
  ...
  documentWasClosed: function () {
    ...
    if (index >= 0) Printest.application.documents.splice(index, 1);
  },
  ...
  doBoundsOverlapDocument: function (bounds) {
    var outer =
      (this.documents[0] && this.documents[0].getOuterBounds(bounds)) || {};
    return this.documents.some(function (document) {
      document = document.getOuterBounds();
      return (
        outer.left < document.left + document.width &&
