...
Printest.application = A.object.create.call(A.application, {
  ...
  getDefaultControlBounds_: function () {
    ...
    this.chromeSystemDisplayGetInfoStub_(
      function (displays) {
  ...
  chromeSystemDisplayGetInfoStub_: function (callback) {
    callback.call(this, [
      {
        isPrimary: true,
        workArea: { left: 0, top: 0, width: 1024, height: 768 },
      },
    ]);
  },
  ...
  createControls_: function (bounds) {
    return Printest.controls.create({ bounds: bounds }).then(
      function (controls) {
  ...
  createNewDocument_: function () {
    ...
    return Printest.document.create(properties).then(
      function (document) {
        document.addListener(
          document,
          "windowfocused",
          this.documentWasFocused
        );
  ...
  saveSettings_: function () {
    if (!this.settingsTimeout_) {
      this.settingsTimeout_ = window.setTimeout(
        function () {
  ...
  getNextDocumentBounds_: function () {
    ...
    for (i = this.documents.length; !found && i >= 0; i -= 1) {
      ...
      found =
        this.areBoundsInSameScreen(bounds, this.controls) &&
