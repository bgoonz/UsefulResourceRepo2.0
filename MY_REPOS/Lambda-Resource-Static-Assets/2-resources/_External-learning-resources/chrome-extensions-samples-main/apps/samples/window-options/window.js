...
function setIfANumber(dictionary, field, numberEdit) {
  ...
  if (!isNaN(number)) dictionary[field] = number;
}
...
function setNumberEdit(value, numberEdit, attr) {
  if (attr === "value" && value === null) $("#" + numberEdit)[0].value = "";
  else $("#" + numberEdit).attr(attr, value);
}
...
function createNewWindow() {
  ...
  optionsDictionary.hidden =
    $("input[name=newWindowHidden]:checked").val() === "hidden";
  ...
  if (optionsDictionary.frame === "chrome") {
    ...
    if ($("#newWindowInactiveColor").val()) {
      optionsDictionary.frame.inactiveColor = $(
        "#newWindowInactiveColor"
      ).val();
    }
  }
...
function initEditWindowTab() {
  ...
  $("#currentWindowOnTop")
    ...
    .change(function () {
      chrome.app.window
        .current()
        .setAlwaysOnTop($("#currentWindowOnTop").is(":checked"));
    });
