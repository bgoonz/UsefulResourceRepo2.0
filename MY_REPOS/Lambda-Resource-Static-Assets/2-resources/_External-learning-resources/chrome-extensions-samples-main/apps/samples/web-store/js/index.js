...
function updateVisibleButtons(input) {
  ...
  if (enabled) webStoreLink.classList.remove("disabled");
  else webStoreLink.classList.add("disabled");
}
...
function resetButton(button) {
  setTimeout(function () {
    ...
    if (button.classList.contains("publish")) button.textContent = "publish";
    else if (button.classList.contains("upload")) button.textContent = "upload";
  }, 2000);
}
    ...
    function scan(entries) {
      ...
      for (var i = 0; i < entries.length; i++) {
        if (entries[i].isDirectory) {
          // Skip .git folder.
          if (entries[i].name !== ".git") directories.push(entries[i]);
        } else {
...
function createProjectDiv(directoryEntry, directoryEntryId, callback) {
  getProjectManifest(directoryEntry, function (manifest) {
    getProjectIcon(directoryEntry, manifest, function (iconUrl) {
      ...
      chrome.fileSystem.getDisplayPath(directoryEntry, function (displayPath) {
...
getDirectoryEntryIds(function (directoryEntryIds) {
  ...
  insertProject(directoryEntryIds[i], function callback() {
    if (--i >= 0) insertProject(directoryEntryIds[i], callback);
  });
});
