...
var Terminal =
  ...
  function (containerId) {
    ...
    function formatColumns_(entries) {
      ...
      var height =
        entries.length == 1
          ? "height: " + entries.length * 30 + "px;"
          : entries.length <= 3
          ? "height: " + entries.length * 18 + "px;"
          : "";
    ...
    function toggle3DView_() {
      ...
      if (body.classList.contains("offscreen")) {
        ...
        var transEnd_ = function (e) {
          ...
          container_.removeEventListener(
            "webkitTransitionEnd",
            transEnd_,
            false
          );
    ...
    return {
      initFS: function (persistent, size) {
        if (!!!window.requestFileSystem) {
          output(
            "<div>Sorry! The FileSystem APIs are not available in your browser.</div>"
          );
          return;
        }
        ...
        window.requestFileSystem(
          ...
          function (filesystem) {
            ...
            cwd_.getDirectory(
              "testquotaforfsfolder",
              { create: true },
              function (dirEntry) {
                dirEntry.remove(function () {
                  // If successfully created, just delete it.
                  // noop.
                });
              },
              function (e) {
                if (e.code == FileError.QUOTA_EXCEEDED_ERR) {
                  output(
                    "ERROR: Write access to the FileSystem is unavailable.<br>"
                  );
                  output(
                    'Type "install" or run Chrome with the --unlimited-quota-for-files flag.'
                  );
                } else {
                  errorHandler_(e);
                }
