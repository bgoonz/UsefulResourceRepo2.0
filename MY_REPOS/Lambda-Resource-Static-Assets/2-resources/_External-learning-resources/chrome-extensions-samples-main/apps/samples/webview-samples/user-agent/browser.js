(function (config, bindings) {
  ...
  window.addEventListener("load", function (e) {
    ...
    if (
      typeof webViews[0].setZoom == "function" &&
      ...
      webViews.forEach(function (wv) {
        wv.addEventListener("findupdate", handleFindUpdate);
      });
  ...
  function doLayout() {
    ...
    webViewContainers.forEach(function (container) {
      ...
      var sadWebView = document.querySelector(
        '.sad-webview[data-name="' + name + '"]'
      );
      var webViewOverlay = document.querySelector(
        '.webview-overlay[data-name="' + name + '"]'
      );
  ...
  function closeFindBox() {
    ...
    findBox.style.left = "";
    findBox.style.opacity = "";
    findResultsLists.forEach(function (rl) {
      rl.innerText = "";
    });
    webViews.forEach(function (wv) {
      wv.stopFinding();
