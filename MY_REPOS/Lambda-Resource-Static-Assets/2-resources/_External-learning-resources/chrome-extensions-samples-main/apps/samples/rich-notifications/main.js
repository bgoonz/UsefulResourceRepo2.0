...
function doNotify(evt) {
  var path = chrome.runtime.getURL(
    asURLs[document.getElementById("img").options.selectedIndex]
  );
  var options = null;
  ...
  options.buttons = [];
  if (sBtn1.length) options.buttons.push({ title: sBtn1 });
  if (sBtn2.length) options.buttons.push({ title: sBtn2 });
...
function notificationBtnClick(notID, iBtn) {
  console.log(
    "The notification '" + notID + "' had button " + iBtn + " clicked"
  );
}
