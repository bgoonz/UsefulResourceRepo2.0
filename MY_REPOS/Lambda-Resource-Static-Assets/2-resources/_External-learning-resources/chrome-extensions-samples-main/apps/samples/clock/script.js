...
var base_url =
  "http://api.worldweatheronline.com/free/v1/tz.ashx?key=vfc3k7q22tjedr2rxse7xzke&format=json&q=";
...
$(document).ready(function () {
  ...
  setInterval(function () {
    ...
    else if (view == "world") $(".menu-item.new-world").removeClass("hidden");
  });
  ...
  $(".menu-item.world").click(function () {
    ...
    view = "world";
    if (sizeOf(clocks) == 0) $(".menu-item.new-world").click();
  ...
  $(".menu-item.alarm").click(function () {
    ...
    view = "alarm";
    if (sizeOf(alarms) == 0) $(".menu-item.new-alarm").click();
...
function addClock(city_class) {
  ...
  $("#container .world .new").before(
    '<div class="city-clock ' + city_class + '"></div>'
  );
    ...
    $.get(
      ...
      function (data) {
        if (data["data"]["error"]) {
          ...
          $(".new .error-message").html(
            "Could not find the time for " + city + ".<br>"
          );
...
function resize() {
  ...
  if (!$(".world .new").hasClass("hidden")) clocks_size += 1;
  ...
  if (!$(".alarm .new").hasClass("hidden")) alarms_size += 1;
