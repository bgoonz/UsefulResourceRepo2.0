...
var isAlwaysOnTopSupported =
  typeof chrome.app.window.current().setAlwaysOnTop !== "undefined";
...
function createNewWindow(optionsDictionary) {
  ...
  if ($("[value=ID1]").checked) optionsDictionary.id = "ID1";
  else if ($("[value=ID2]").checked) optionsDictionary.id = "ID2";
  else optionsDictionary.id = undefined;
  ...
  innerBounds.top =
    (innerBounds.top + newWindowOffset) % (screen.height - innerBounds.height);
  ...
  setIfANumber(
    optionsDictionary.innerBounds,
    "minWidth",
    parseInt($("#newWindowWidthMin").value)
  );
  setIfANumber(
    optionsDictionary.innerBounds,
    "maxWidth",
    parseInt($("#newWindowWidthMax").value)
  );
  setIfANumber(
    optionsDictionary.innerBounds,
    "minHeight",
    parseInt($("#newWindowHeightMin").value)
  );
  setIfANumber(
    optionsDictionary.innerBounds,
    "maxHeight",
    parseInt($("#newWindowHeightMax").value)
  );
...
$("#setbounds").onclick = function (e) {
  ...
  setIfANumber(
    bounds,
    "left",
    parseInt($("#moveWindowLeft").value) || currentInnerBounds.left
  );
  setIfANumber(
    bounds,
    "top",
    parseInt($("#moveWindowTop").value) || currentInnerBounds.top
  );
  setIfANumber(
    bounds,
    "width",
    parseInt($("#resizeWindowWidth").value) || currentInnerBounds.width
  );
  setIfANumber(
    bounds,
    "height",
    parseInt($("#resizeWindowHeight").value) || currentInnerBounds.height
  );
  setTimeout(function () {
...
function updateCurrentStateReadout() {
  ...
  $("#moveWindowLeft").placeholder =
    chrome.app.window.current().innerBounds.left;
  $("#moveWindowTop").placeholder = chrome.app.window.current().innerBounds.top;
  $("#resizeWindowWidth").placeholder =
    chrome.app.window.current().innerBounds.width;
  $("#resizeWindowHeight").placeholder =
    chrome.app.window.current().innerBounds.height;

  $("#newWindowWidthMin").placeholder =
    chrome.app.window.current().innerBounds.width;
  $("#newWindowWidthMax").placeholder =
    chrome.app.window.current().innerBounds.width;
  $("#newWindowHeightMin").placeholder =
    chrome.app.window.current().innerBounds.height;
  $("#newWindowHeightMax").placeholder =
    chrome.app.window.current().innerBounds.height;
}
...
chrome.app.window
  .current()
  .onBoundsChanged.addListener(updateCurrentStateReadout);
