...
function View(calcModel) {
  ...
  $(document).keydown(function (event) {
    ...
    else if (!shift && event.which in keyboard) clicked = keyboard[event.which];
    if (clicked != null) {
      var result = calcModel.HandleButtonClick(clicked);
      calc.buttonClicked(clicked, result);
    }
...
View.prototype.buttonClicked = function (clicked, result) {
  ...
  if (clicked == "AC") {
    ...
    if (this.lastDisplayElement) this.UpdateTotal(accumulator);
    operand = "";
    accumulator = "";
...
View.prototype.UpdateDisplayEquation = function (
  operator,
  operand,
  accumulator
) {
