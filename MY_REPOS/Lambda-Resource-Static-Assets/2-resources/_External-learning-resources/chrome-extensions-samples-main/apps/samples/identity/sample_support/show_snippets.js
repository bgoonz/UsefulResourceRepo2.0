...
window.snippetSupport = (function () {
  ...
  function extractFromManifest(content) {
    ...
    var note =
      "// these are only some of the keys relevant to this sample\n" +
  ...
  function addSnippet(snippetName, snippet) {
    ...
    var githubUrl =
      "https://github.com/GoogleChrome/chrome-app-samples/tree/master/" +
      "samples/" +
      sampleName +
      "/" +
      snippet.filename;
    ...
    var githubUrlHTML =
      '<a target="_blank" href="' + githubUrl + '">' + snippet.filename;
    if (snippet.startLine) {
