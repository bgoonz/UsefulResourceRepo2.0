CodeMirror.defineMode("plsql", function (config, parserConfig) {
  ...
  function tokenBase(stream, state) {
    ...
    // is it a operator?
      ...
      if (
        keywords &&
        keywords.propertyIsEnumerable(stream.current().toLowerCase())
      )
        return ret("keyword", "keyword");
      // is it one of the listed functions?
      if (
        functions &&
        functions.propertyIsEnumerable(stream.current().toLowerCase())
      )
      ...
      if (
        sqlplus &&
        sqlplus.propertyIsEnumerable(stream.current().toLowerCase())
      )
