    ...
    if (mode == "text/html") {
      ...
      if (ch == ">") {
        ...
        if (tok.className == "tag" && type != "selfcloseTag") {
          var tagName = state.htmlState
            ? state.htmlState.context.tagName
            : state.tagName; // htmlmixed : xml
          if (tagName.length > 0) {
        ...
        if (tok.className == "tag" && tok.string == "<") {
          var tagName = state.htmlState
            ? state.htmlState.context
              ? state.htmlState.context.tagName
              : ""
            : state.context.tagName; // htmlmixed : xml # extra htmlmized check is for '</' edge case
          if (tagName.length > 0) {
