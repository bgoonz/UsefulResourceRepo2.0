...
CodeMirror.defineMode("less", function (config) {
  ...
  function tokenBase(stream, state) {
    ...
    if (ch == "@") {
      ...
      // lesscss e.g.: .png will not be parsed as a class
      if (stream.eat("/")) {
      ...
      //removed . dot character original was [,.+>*\/]
      return ret(null, "select-op");
    } else if (/[;{}:\[\]()]/.test(ch)) {
      //added () char for lesscss original was [;{}:\[\]]
      if (ch == ":") {
      ...
      if (stream.current().match(/http|https/) != null) {
        ...
        if (
          (stream.eatSpace() && stream.peek().match(/[{<>.a-zA-Z]/) != null) ||
          stream.eol()
        )
      ...
      if (type == "hash" && context == "rule") style = "atom";
        ...
        //"tag"
          ...
          style =
            stream.current() == "when"
              ? "variable"
              : stream.string.match(/#/g) != undefined
              ? null
              : /[\s,|\s\)]/.test(stream.peek())
              ? "tag"
              : null;
        }
      }
