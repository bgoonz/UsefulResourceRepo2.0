...
var http = (function () {
  if (!chrome.sockets || !chrome.sockets.tcpServer) return {};
    ...
    chrome.sockets.tcp.onReceiveError.addListener(function (info) {
      ...
      if (pSocket) {
        if (pSocket.handlers) {
          ...
          pSocket.handlers.reject(
            new Error("chrome.sockets.tcp error " + info.resultCode)
          );
          delete pSocket.handlers;
        } else {
          // No pending read so put data on the queue.
  ...
  PSocket.prototype.read = function () {
    ...
    if (this.readQueue.length) {
      ...
      if (!info.resultCode) return Promise.resolve(info.data);
      else
  ...
  PSocket.prototype.write = function (data) {
    ...
    return new Promise(function (resolve, reject) {
      chrome.sockets.tcp.send(that.socketId, data, function (info) {
        if (info && info.resultCode >= 0) resolve(info.bytesSent);
        else
          reject(
            new Error("chrome sockets.tcp error " + (info && info.resultCode))
          );
      });
    });
  };
  ...
  var ary2utf8 = (function () {
    ...
    return function (ary) {
      ...
      while (cur < ary.length) {
        ...
        cur += pattern.bytes;
        var code = "";
        code += ("00000000" + (first & (255 ^ pattern.mask)).toString(2)).slice(
          -pattern.data_length
        );
        for (var i = 0, len = rest.length; i < len; i++) {
          code += (
            "00000000" + (rest[i] & parseInt("111111", 2)).toString(2)
          ).slice(-6);
        }
  ...
  var utf82ary = (function () {
    ...
    var code2utf8array = function (code) {
      ...
      for (var i = 0, len = pattern.bytes - 1; i < len; i++) {
        ary.unshift(
          parseInt("10" + ("000000" + code01.slice(-6)).slice(-6), 2)
        );
        code01 = code01.slice(0, -6);
      }
      ary.unshift(
        parseInt(
          pattern.header + ("00000000" + code01).slice(-pattern.data_length),
          2
        )
      );
      return ary;
  ...
  EventSource.prototype = {
    ...
    addEventListener: function (type, callback) {
      if (!this.listeners_[type]) this.listeners_[type] = [];
    ...
    removeEventListener: function (type, callback) {
      if (!this.listeners_[type]) return;
    ...
    dispatchEvent: function (type, var_args) {
      if (!this.listeners_[type]) return false;
  ...
  HttpServer.prototype = {
    ...
    onRequest_: function (request) {
      ...
      if (!this.dispatchEvent(type, request)) request.close();
      else if (keepAlive) this.readRequestFromSocket_(request.pSocket_);
    },
  };
  ...
  HttpRequest.prototype = {
    ...
    close: function () {
      ...
      if (this.headers["Connection"] != "keep-alive") pSocket.close();
    ...
    write: function (data) {
      if (this.responseHeaders_["Transfer-Encoding"] == "chunked") {
        ...
        var buffer = new ArrayBuffer(
          chunkLength.length + byteLength + newline.length
        );
        var bufferView = new Uint8Array(buffer);
        ...
        for (var i = 0; i < newline.length; i++)
          bufferView[chunkLength.length + byteLength + i] =
            newline.charCodeAt(i);
        data = buffer;
     ...
     */
      ...
      if (opt_data) this.write(opt_data);
      if (this.responseHeaders_["Transfer-Encoding"] == "chunked")
        this.write("");
      this.finished_ = true;
      this.checkFinished_();
    ...
    checkFinished_: function () {
      if (!this.finished_ || this.bytesRemaining > 0) return;
      this.close();
    },
  };
  ...
  WebSocketServer.prototype = {
    ...
    upgradeToWebSocket_: function (request) {
      ...
      if (this.dispatchEvent("request", new WebSocketRequest(request))) {
        if (request.pSocket_) request.reject();
        return true;
      }
  ...
  WebSocketRequest.prototype = {
    ...
    accept: function () {
      ...
      if (this.headers["Sec-WebSocket-Protocol"])
        responseHeader["Sec-WebSocket-Protocol"] =
  ...
  WebSocketServerSocket.prototype = {
    ...
    readFromSocket_: function () {
      ...
      var onDataRead = function (dataBuffer) {
        ...
        for (var i = 0; i < a.length; i++) data.push(a[i]);
        ...
        while (data.length) {
          ...
          if (length_code > 125) {
            ...
            if (data.length > 5) mask = data.slice(2, 6);
          }

          if (length_code > -1 && data.length >= data_start + length_code) {
            var decoded = data
              .slice(data_start, data_start + length_code)
              .map(function (byte, index) {
            ...
            if (fin && op > 0) {
              ...
              if (!t.onFrame_(op, decoded)) return;
            } else {
              // Fragmented message.
              ...
              if (fin) {
                ...
                if (!t.onFrame_(fragmentedOp, joinMessage)) return;
                fragmentedOp = 0;
                fragmentedMessages = [];
              }
            }
    ...
    sendFrame_: function (op, data) {
      ...
      var WebsocketFrameData = function (op, data) {
        ...
        if (ary.length > 65535) length += 10;
        else if (ary.length > 125) length += 4;
        else length += 2;
        var lengthBytes = 0;
        var buffer = new ArrayBuffer(length);
        var bv = new Uint8Array(buffer);
        bv[0] = 128 | (op & 15); // Fin and type text.
        bv[1] = ary.length > 65535 ? 127 : ary.length > 125 ? 126 : ary.length;
        if (ary.length > 65535) lengthBytes = 8;
        else if (ary.length > 125) lengthBytes = 2;
        var len = ary.length;
