...
const base_weather_url =
  "https://api.worldweatheronline.com/free/v1/weather.ashx?format=json&num_of_days=5&key=vfc3k7q22tjedr2rxse7xzke&q=";
const base_geolocation_url =
  "http://maps.googleapis.com/maps/api/geocode/json?sensor=true&language=EN&latlng=";
const base_searchterm_url =
  "http://maps.googleapis.com/maps/api/geocode/json?sensor=false&language=EN&address=";
// Samples:
...
function selectCity(city) {
  ...
  if (myScroll && myScroll.currPageX != index) myScroll.scrollToPage(index);
...
function getCurrentCity() {
  ...
  if (!city && cities.length() > 0) city = cities.asArray()[0];
  return city;
}
...
function showInputError(searchterm) {
  ...
  $("#new #error-message").text(
    "Could not find weather for '" + searchterm + "'"
  );
  ...
  $.get(
    ...
    function (data) {
      ...
      for (var i = 0; i < data.results.length; i++) {
        if (
          data.results[i].types.indexOf("locality") != -1 ||
          data.results[i].types.indexOf("administrative_area_level_1") != -1
        ) {
...
function updateCityDisplay(city, current_condition, forecast) {
  ...
  high_low_div.appendChild(
    document.createTextNode(tempMax + "\u00B0 / " + tempMin + "\u00B0")
  ); // &deg;
  ...
  */
  ...
  Array.prototype.forEach.call(
    document.querySelectorAll(".city-list .delete"),
    function (e, i) {
      e.onclick = function () {};
    }
  );
...
function setDots() {
  if (cities.length() <= num_dots_at_bottom) {
    ...
    if (first === 0) $("#dots #prev").removeClass("shown").addClass("disabled");
    else $("#dots #prev").addClass("shown").removeClass("disabled");

    if (first + num_dots_at_bottom >= c.length)
      $("#dots #next").removeClass("shown").addClass("disabled");
    else $("#dots #next").addClass("shown").removeClass("disabled");
  }
}
...
function dayDisplay(forecast_day, i) {
  ...
  var html =
    ...
    '"></div>' +
    '<div class="high">' +
    forecast_day["tempMax" + temp] +
    "&deg;</div>" +
    '<div class="low">' +
    forecast_day["tempMin" + temp] +
    "&deg;</div>" +
    "</div>";
  return html;
}
...
function initHandlers() {
  ...
  $("#settingsToggle").click(function () {
    if (currentlyOnSettingsPage()) hideSettings();
    else showSettings();
  });
  ...
  $(document).keyup(function (event) {
    if (currentlyOnSettingsPage()) return;
    if (event.which == 39)
      // right-arrow
      adjustnext(1);
  ...
  document.ontouchmove = function (e) {
    if (!currentlyOnSettingsPage()) e.preventDefault();
  };
...
function init() {
  $(document.body).addClass(
    window.cordova !== undefined ? "mobile" : "not-mobile"
  );

  chrome.storage.sync.get(function (items) {
    if (
      items.cities !== undefined &&
      items.cities.version === Cities.CurrentVersion
    ) {
      cities = items.cities;
