var main = (function () {
      ...
      function (chrcs) {
        ...
        chrcs.forEach(function (chrc) {
          if (chrc.uuid == HEART_RATE_MEASUREMENT_UUID) {
            console.log(
              "Setting Heart Rate Measurement Characteristic: " +
                chrc.instanceId
            );
          ...
          if (chrc.uuid == BODY_SENSOR_LOCATION_UUID) {
            console.log(
              "Setting Body Sensor Location Characteristic: " + chrc.instanceId
            );
          ...
          if (chrc.uuid == HEART_RATE_CONTROL_POINT_UUID) {
            console.log(
              "Setting Heart Rate Control Point Characteristic: " +
                chrc.instanceId
            );
  ...
  HeartRateSensor.prototype.updateHeartRateMeasurementValue = function () {
    ...
    UI.getInstance().setSensorContactStatus(
      (function () {
  ...
  HeartRateSensor.prototype.updateDiscoveryToggleState = function (
    discovering
  ) {
  ...
  HeartRateSensor.prototype.init = function () {
        ...
        devices.forEach(function (device) {
          ...
          chrome.bluetoothLowEnergy.getServices(
            ...
            function (services) {
              ...
              if (!self.deviceMap_.hasOwnProperty(device.address)) {
                console.log(
                  "Found device with Heart Rate service: " + device.address
                );
                self.deviceMap_[device.address] = device.name
                  ? device.name
                  : device.address;
    ...
    UI.getInstance().setDeviceSelectionHandler(function (selectedValue) {
      ...
      chrome.bluetoothLowEnergy.connect(selectedValue, function () {
        if (chrome.runtime.lastError) {
          console.log(
            'Failed to connect to HR device "' +
              selectedValue +
              '" ' +
              chrome.runtime.lastError.message
          );
    ...
    chrome.bluetoothLowEnergy.onServiceRemoved.addListener(function (service) {
      ...
      chrome.bluetooth.getDevice(service.deviceAddress, function (device) {
        ...
        if (
          !device.uuids ||
          device.uuids.indexOf(HEART_RATE_SERVICE_UUID) < 0
        ) {
