...
class TestParallel(TestNested):
    ...
    def test_init(self):
        ...
        m.to_C()
        self.assertEqual(
            ["C{0}1{0}a".format(State.separator), "C{0}2{0}a".format(State.separator)],
            m.state,
        )

    def test_enter(self):
        m = self.stuff.machine_cls(
            states=self.states, transitions=self.transitions, initial="A"
        )
        m.to_C()
        m.go()
        self.assertEqual(
            ["C{0}1{0}b".format(State.separator), "C{0}2{0}b".format(State.separator)],
            m.state,
        )
    ...
    def test_exit(self):
        ...
        model1 = Model()
        m = self.stuff.machine_cls(
            model1, states=self.states, transitions=self.transitions, initial="A"
        )
    ...
    def test_parent_transition(self):
        ...
        m.add_transition(
            "switch",
            "C{0}2{0}a".format(State.separator),
            "C{0}2{0}b".format(State.separator),
        )
        m.to_C()
        m.switch()
        self.assertEqual(
            ["C{0}1{0}a".format(State.separator), "C{0}2{0}b".format(State.separator)],
            m.state,
        )
    ...
    def test_multiple(self):
        ...
        self.assertEqual(
            [
                [
                    "B{0}1{0}a{0}z".format(State.separator),
                    "B{0}1{0}b{0}y".format(State.separator),
                ],
                "B{0}2{0}a".format(State.separator),
            ],
            m.state,
        )
    ...
    def test_parallel_initial(self):
        m = self.machine_cls(
            states=["A", "B", {"name": "C", "parallel": ["1", "2"]}], initial="C"
        )
        m = self.machine_cls(
            states=["A", "B", {"name": "C", "parallel": ["1", "2"]}],
            initial=["C_1", "C_2"],
        )
    ...
    def test_multiple_deeper(self):
        ...
        ref_state = [
            ...
            [
                ...
                [
                    ["P{0}2{0}b{0}x{0}1".format(sep), "P{0}2{0}b{0}x{0}2".format(sep)],
