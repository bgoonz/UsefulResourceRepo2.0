...
class TestRep(TestCase):
    ...
    def test_rep_partial_with_kwargs(self):
        ...
        self.assertTrue(pcheck())
        self.assertEqual(
            rep(pcheck, MarkupMachine.format_references), "check(result=True)"
        )
    ...
    def test_rep_partial_with_args_and_kwargs(self):
        ...
        self.assertTrue(pcheck())
        self.assertEqual(
            rep(pcheck, MarkupMachine.format_references),
...
class TestMarkupMachine(TestCase):
    ...
    def test_markup_model(self):
        ...
        self.assertEqual(len(m1.models), len(m2.models))
        self.assertTrue(
            model1.state == model2.state or model1.state.name == model2.state
        )
        self.assertEqual(sorted(m1.states.keys()), sorted(m2.states.keys()))
        self.assertEqual(sorted(m1.events.keys()), sorted(m2.events.keys()))
    ...
    def test_auto_transitions(self):
        ...
        self.assertEqual(
            len(m2.markup.get("transitions")), self.num_trans + self.num_auto
        )
        ...
        self.assertEqual(
            len(m1.markup.get("transitions")), self.num_trans + self.num_auto
        )
...
class TestMarkupHierarchicalMachine(TestMarkupMachine):
    ...
    def test_nested_definitions(self):
        ...
        machine = self.machine_cls(
            states=states, initial="A", auto_transitions=False, name="TestMachine"
        )
