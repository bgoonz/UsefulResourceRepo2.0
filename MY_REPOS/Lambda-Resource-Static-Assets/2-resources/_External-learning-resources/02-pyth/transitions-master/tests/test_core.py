...
class TestTransitions(TestCase):
    ...
    def test_init_machine_with_hella_arguments(self):
        states = [
            State("State1"),
            "State2",
            {"name": "State3", "on_enter": "hello_world"},
        ]
    ...
    def test_conditions(self):
        ...
        s.machine.add_transition(
            "advance", "C", "D", unless=["this_fails", "this_passes"]
        )
        s.advance()
        self.assertEqual(s.state, "B")
        s.advance()
    ...
    def test_uncallable_callbacks(self):
        s = self.stuff
        s.machine.add_transition(
            "advance", "A", "B", conditions=["property_that_fails", "is_false"]
        )
        # make sure parameters passed by trigger events can be handled
        s.machine.add_transition(
            "advance", "A", "C", before=["property_that_fails", "is_false"]
        )
    ...
    def test_conditions_with_partial(self):
        ...
        s.machine.add_transition(
            "advance", "C", "D", unless=[partial(check, False), partial(check, True)]
        )
        s.advance()
        self.assertEqual(s.state, "B")
        s.advance()
    ...
    def test_prepare(self):
        ...
        m.add_transition(
            "move", "C", "A", prepare="increase_level", conditions="this_fails"
        )
    ...
    def test_send_event_data_callbacks(self):
        ...
        m = Machine(
            model=s, states=states, initial="A", send_event=False, auto_transitions=True
        )
    ...
    def test_ignore_invalid_triggers(self):
        ...
        b_state = State("B")
        m1 = Machine(
            "self", states=[a_state, b_state], transitions=transitions, initial="B"
        )
        with self.assertRaises(MachineError):
            m1.a_to_b()
    ...
    def test_queued_remove(self):
        m = self.machine_cls(
            model=None, states=["A", "B", "C"], initial="A", queued=True
        )
    ...
    def test_state_and_transition_with_underscore(self):
        ...
        m.add_transition(
            "_on_exit_", "_C_", "_A_", prepare="increase_level", conditions="this_fails"
        )
    ...
    def test_callback_identification(self):
        ...
        m.add_transition(
            "on_exit_A", "C", "D", before="increase_level", conditions="this_fails"
        )
    ...
    def test_skip_override(self):
        ...
        m = self.stuff.machine_cls(
            model=model, states=["A", "B"], transitions=transitions, initial="A"
        )
        model.go()
    ...
    @skipIf(
        sys.version_info < (3,), "String-checking disabled on PY-2 because is different"
    )
    def test_repr(self):
        ...
        def check_prepare_repr(event_data):
            self.assertRegex(
                str(event_data), r"<EventData\('<State\('A'\)@\d+>', " r"None\)@\d+>"
            )
        ...
        m = Machine(
            ...
            before_state_change=check_before_repr,
            send_event=True,
            initial="A",
        )
        ...
        transitions = [
            {
                ...
                "source": "A",
                "dest": "B",
                "conditions": always_fails,
                "prepare": local_callback,
            },
            {
                "trigger": "go",
                "source": "A",
    ...
    def test_get_transitions(self):
        ...
        self.assertEqual(
            [(t.source, t.dest) for t in m.get_transitions(source="A", dest="D")],
            [("A", "D"), ("A", "D")],
        )
        self.assertEqual(
            sorted([(t.source, t.dest) for t in m.get_transitions(dest="D")]),
            [("A", "D"), ("A", "D"), ("B", "D"), ("C", "D")],
        )
        self.assertEqual(
            [
        ...
        self.assertEqual(
            sorted([(t.source, t.dest) for t in m.get_transitions(dest=m.states["D"])]),
            [("A", "D"), ("A", "D"), ("B", "D"), ("C", "D")],
        )
    ...
    def test_reflexive_transition(self):
        self.stuff.machine.add_transition(
            "reflex", ["A", "B"], "=", after="increase_level"
        )
    ...
    def test_dynamic_model_state_attribute(self):
        ...
        m = self.machine_cls(
            Model(), states=["A", "B"], initial="A", model_attribute="status"
        )
    ...
    def test_multiple_machines_per_model(self):
        ...
        instance = Model()
        machine_a = Machine(
            instance, states=["A", "B"], initial="A", model_attribute="car_state"
        )
        machine_a.add_transition("accelerate_car", "A", "B")
        machine_b = Machine(
            instance, states=["A", "B"], initial="B", model_attribute="driver_state"
        )
    ...
    def test_initial_not_registered(self):
        m1 = self.machine_cls(
            states=["A", "B"], initial=self.machine_cls.state_cls("C")
        )
