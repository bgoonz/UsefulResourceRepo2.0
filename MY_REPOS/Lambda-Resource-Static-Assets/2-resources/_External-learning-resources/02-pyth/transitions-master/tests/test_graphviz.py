...
class TestDiagrams(TestTransitions):
    ...
    def test_diagram(self):
        ...
        for e in edges:
            ...
            self.assertIsNotNone(
                getattr(m, re.match(r"\[label=([^\]]+)\]", e).group(1))
            )
    ...
    def test_multi_model_state(self):
        m1 = Stuff(
            machine_cls=None, extra_kwargs={"use_pygraphviz": self.use_pygraphviz}
        )
        m2 = Stuff(
            machine_cls=None, extra_kwargs={"use_pygraphviz": self.use_pygraphviz}
        )
    ...
    def test_to_method_filtering(self):
        m = self.machine_cls(
            states=["A", "B", "C"], initial="A", use_pygraphviz=self.use_pygraphviz
        )
    ...
    def test_loops(self):
        m = self.machine_cls(
            states=["A"], initial="A", use_pygraphviz=self.use_pygraphviz
        )
        ...
        try:
            ...
            self.assertRegexpMatches(
                dot_string, r'A\s+->\s*A\s+\[label="(fixed|reflexive)'
            )
    ...
    def test_label_attribute(self):
        ...
        m = CustomMachine(
            ...
            transitions=[
                {"trigger": "event", "source": "A", "dest": "B", "label": "LabelEvent"}
            ],
            initial="A",
            use_pygraphviz=self.use_pygraphviz,
        )
...
class TestDiagramsNested(TestDiagrams):
    ...
    def setUp(self):
        ...
        self.states = [
            ...
            {
                "name": "C",
                "children": [{"name": "1", "children": ["a", "b", "c"]}, "2", "3"],
            },
            "D",
        ]
        self.transitions = [
    ...
    def test_diagram(self):
        ...
        # Test that graph properties match the Machine
        self.assertEqual(
            set(m.states.keys()) - set(["C", "C%s1" % NestedState.separator]),
    ...
    def test_nested_notebook(self):
        ...
        transitions = [
            ...
            ["stop", "walking", "standing"],
            {
                "trigger": "drink",
                "source": "*",
                "dest": "caffeinated{0}dithering".format(
        ...
        machine = CustomStateMachine(
            states=states, transitions=transitions, **extra_args
        )
        ...
        if self.use_pygraphviz:
            ...
            dot_string = g1.source
        count = re.findall(
            '-> "?caffeinated{0}dithering"?'.format(machine.state_cls.separator),
            dot_string,
        )
...
class TestDiagramsLockedNested(TestDiagramsNested):
    def setUp(self):
        ...
        self.machine_cls = MachineFactory.get_predefined(
            graph=True, nested=True, locked=True
        )
