...
class TestAsync(TestTransitions):
    ...
    def setUp(self):
        ...
        self.machine = self.machine_cls(
            states=["A", "B", "C"], transitions=[["go", "A", "B"]], initial="A"
        )
    ...
    def test_async_condition(self):
        m = self.machine
        m.add_transition(
            "proceed", "A", "C", conditions=self.await_true, unless=self.await_false
        )
    ...
    def test_sync_conditions(self):
        ...
        m = self.machine
        m.add_transition(
            "proceed", "A", "C", conditions=self.synced_true, after=sync_process
        )
        asyncio.run(m.proceed())
        self.assertEqual(m.state, "C")
        self.assertTrue(mock.called)
    ...
    def test_multiple_models(self):
        ...
        m1.add_transition(
            trigger="check", source="C", dest="B", conditions=self.await_false
        )
        m1.add_transition(trigger="reset", source="C", dest="A")
        m2.add_transition(
            trigger="go", source="A", dest=None, conditions=m1.is_C, after=m1.reset
        )
    ...
    def test_async_callback_event_data(self):
        ...
        m.add_transition(
            ...
            "A",
            "B",
            conditions=[sync_condition, async_conditions],
    ...
    def test_queued(self):
        ...
        m = self.machine_cls(
            states=states, transitions=transitions, initial="A", queued=True
        )
        asyncio.run(m.walk(machine=m))
        ...
        async def run():
            transitions = [
                {"trigger": "mock", "source": ["A", "B"], "dest": "B", "after": mock},
                {
                    ...
                    "source": "A",
                    "dest": "B",
                    "before": partial(asyncio.sleep, 0.1),
                },
            ...
            await asyncio.gather(
                m1.delayed(),
                self.call_delayed(m1.check, 0.02),
                self.call_delayed(m2.mock, 0.04),
            )
    ...
    def test_queued_remove(self):
        ...
        def check_queue(expect, event_data):
            self.assertEqual(
                expect, len(event_data.machine._transition_queue_dict[event_data.model])
            )
        ...
        transitions = [
            {
                ...
                "source": "A",
                "dest": "B",
                "after": partial(asyncio.sleep, 0.1),
            },
        ...
        async def run():
            ...
            await asyncio.gather(
                m1.go(),
                m2.go(),
                self.call_delayed(m1.remove, 0.02),
                self.call_delayed(m2.go, 0.04),
            )
            _ = repr(
                m._transition_queue_dict
            )  # check whether _DictionaryMock returns a valid representation
            self.assertTrue(m1.is_B())
            ...
            await asyncio.gather(
                m1.go(),
                m2.go(),
                self.call_delayed(m1.remove_queue, 0.02),
                self.call_delayed(m2.go, 0.04),
            )
            self.assertTrue(m1.is_B())
            self.assertTrue(m2.is_C())
    ...
    def test_task_cleanup(self):
        ...
        self.assertEqual(
            0, len(m.async_tasks)
        )  # check whether other tests were already leaking tasks
...
@skipIf(
    asyncio is None or (pgv is None and gv is None),
    "AsyncGraphMachine requires asyncio and (py)gaphviz",
)
...
class AsyncHierarchicalGraphMachine(TestHierarchicalAsync):
    def setUp(self):
        ...
        self.machine_cls = MachineFactory.get_predefined(
            graph=True, asyncio=True, nested=True
        )
        self.machine = self.machine_cls(
            states=["A", "B", "C"], transitions=[["go", "A", "B"]], initial="A"
        )
