...
class TestReuse(TestCase):
    ...
    def test_blueprint_initial_false(self):
        ...
        parent = self.machine_cls(
            states=["a", "b", {"name": "c", "children": child, "initial": False}]
        )
        parent.to_c()
    ...
    def test_blueprint_remap(self):
        ...
        new_states = [
            ...
            "B",
            {
                ...
                "children": [
                    counter,
                    {"name": "X", "children": ["will", "be", "filtered", "out"]},
                ],
    ...
    def test_example_reuse(self):
        ...
        count_trans = [
            ...
            ["decrease", "2", "1"],
            {
                ...
                "source": "3",
                "dest": "done",
                "conditions": "this_passes",
            },
        ]

        counter = self.machine_cls(
            states=count_states, transitions=count_trans, initial="1"
        )
        ...
        states_remap = [
            "waiting",
            "collecting",
            {"name": "counting", "children": counter, "remap": {"done": "waiting"}},
        ]
        ...
        collector = self.stuff.machine_cls(
            states=states, transitions=transitions, initial="waiting"
        )
        ...
        collector = self.machine_cls(
            states=states_remap, transitions=transitions, initial="waiting"
        )
        collector.this_passes = self.stuff.this_passes
        collector.collect()  # collecting
        collector.count()  # let's see what we got
        ...
        transitions.append(
            [
                "increase",
                "counting%s1" % State.separator,
                "counting%s2" % State.separator,
            ]
        )
        transitions.append(
            [
    ...
    def test_reuse_prepare(self):
        ...
        m_model = Model()
        m = self.machine_cls(
            m_model, states=["A", "B", {"name": "NEST", "children": ms}]
        )
    ...
    def test_reuse_self_reference(self):
        ...
        class Nested(self.machine_cls):
            def __init__(self, parent):
                ...
                super(Nested, self).__init__(
                    states=states, transitions=transitions, initial="1"
                )
        ...
        class Top(self.machine_cls):
            ...
            def __init__(self):
                ...
                transitions = [
                    dict(
                        trigger="print_top", source="*", dest="=", after=self.print_msg
                    ),
                    dict(
                        trigger="to_nested", source="*", dest="B{0}1".format(separator)
                    ),
                ]
        ...
        self.assertIs(
            top_machine.nested.get_state("2").on_enter,
