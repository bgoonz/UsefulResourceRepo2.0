...
class TestEnumsAsStates(TestCase):
    ...
    def test_property_initial(self):
        ...
        m = self.machine_cls(
            states=self.States, initial=self.States.RED, transitions=transitions
        )
        m.switch_to_yellow()
        assert m.is_YELLOW()
        ...
        class Stuff(object):
            def __init__(self, machine_cls):
                ...
                self.machine = machine_cls(
                    states=States, initial=States.ONE, model=self
                )
    ...
    def test_enum_zero(self):
        ...
        transitions = [["foo", State.FOO, State.BAR], ["bar", State.BAR, State.FOO]]
    ...
    def test_get_transitions(self):
        ...
        self.assertEqual(
            1, len(m.get_transitions(source=self.States.RED, dest=self.States.YELLOW))
        )
        ...
        self.assertEqual(
            2, len(m.get_transitions(source=self.States.RED, dest=self.States.YELLOW))
        )
...
class TestNestedStateEnums(TestEnumsAsStates):
    ...
    def test_get_nested_transitions(self):
        ...
        m = self.machine_cls(
            states=Errors, initial=Errors.NONE.value.RED, auto_transitions=False
        )
        ...
        self.assertEqual(
            ...
            len(
                m.get_transitions(
                    source=self.States.RED, dest=self.States.RED, delegate=True
                )
            ),
        )
        self.assertEqual(
            1, len(m.get_transitions(source=self.States.RED, dest=self.States.GREEN))
        )
...
@skipIf(
    enum is None or (pgv is None and gv is None),
    "enum and (py)graphviz are not available",
)
