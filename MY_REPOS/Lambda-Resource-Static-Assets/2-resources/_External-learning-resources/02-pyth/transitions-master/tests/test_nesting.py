...
class TestNestedTransitions(TestTransitions):
    ...
    def test_nested_definitions(self):
        ...
        m = self.stuff.machine_cls(
            ...
            transitions=[
                ["go", "A", "B"],
                ["go", "B{0}2".format(separator), "B{0}1".format(separator)],
            ],
        )
    ...
    def test_enter_exit_nested_state(self):
        ...
        states = [
            ...
            "B",
            {
                ...
                "on_exit": callback,
                "children": [
                    {"name": "1", "on_enter": callback, "on_exit": callback},
                    "2",
                    "3",
                ],
            },
        ...
        transitions = [["go", "A", "C{0}1".format(State.separator)], ["go", "C", "D"]]
        m = self.stuff.machine_cls(states=states, transitions=transitions, initial="A")
        m.go()
    ...
    def test_ordered_transitions(self):
        ...
        # Via init argument
        m = self.stuff.machine_cls(
            "self", states, initial="first", ordered_transitions=True
        )
        m.next_state()
    ...
    def test_intial_state(self):
        ...
        states = [
            {"name": "A", "children": ["1", "2"], "initial": "2"},
            {
                ...
                "children": [
                    "1",
                    {"name": "2", "initial": "a", "children": ["a", "b"]},
                ],
            },
        ]
        transitions = [
            ["do", "A", "B"],
            ["do", "B{0}2".format(separator), "B{0}1".format(separator)],
        ]
        m = self.stuff.machine_cls(states=states, transitions=transitions, initial="A")
        ...
        m = self.stuff.machine_cls(
            states=states,
            transitions=transitions,
            initial="B{0}2{0}b".format(separator),
        )
    ...
    def test_get_triggers(self):
        ...
        transitions = [
            ...
            ["stop", "walking", "standing"],
            {
                "trigger": "drink",
                "source": "*",
                "dest": "caffeinated{0}dithering".format(seperator),
                "conditions": "is_hot",
                "unless": "is_too_hot",
            },
            [
                "walk",
                "caffeinated{0}dithering".format(seperator),
                "caffeinated{0}running".format(seperator),
            ],
            ["relax", "caffeinated", "standing"],
        ]

        machine = self.stuff.machine_cls(
            states=states, transitions=transitions, auto_transitions=False
        )
    ...
    def test_get_nested_transitions(self):
            ...
            "A",
            {
                ...
                "children": [
                    ...
                    "2",
                    {
                        "name": "3",
                        "children": ["a", "b"],
                        "transitions": [["inner", "a", "b"], ["inner", "b", "a"]],
                    },
                ],
                "transitions": [
                    ["mid", "1", "1"],
        ...
        self.assertEqual(
            3, len(machine.get_transitions(dest="B{0}1".format(seperator)))
        )
        self.assertEqual(
            2,
            len(
                machine.get_transitions(
                    source="B{0}2".format(seperator), dest="B{0}3".format(seperator)
                )
            ),
        )
        self.assertEqual(
            ...
            len(
                machine.get_transitions(
                    source="B{0}3{0}a".format(seperator),
        ...
        self.assertEqual(
            ...
            len(
                machine.get_transitions(
                    source=machine.states["B"].states["3"].states["b"]
                )
            ),
        )
        ...
        self.assertEqual(
            ...
            len(
                machine.get_transitions(
                    source=machine.states["B"].states["3"].states["b"], delegate=True
                )
            ),
        )
    ...
    def test_trigger_parent(self):
        ...
        machine = self.machine_cls(
            model,
            states=[{"name": "A", "children": ["1", "2"]}],
            transitions=[["go", "A", "A_2"], ["enter", "A", "A_1"]],
            initial="A",
        )
                    ...
                    "transitions": [
                        {
                            ...
                            "source": "1",
                            "dest": "2",
                            "conditions": self.stuff.this_fails,
                        }
                    ],
                },
                "B",
...
class TestSeparatorsBase(TestCase):
    ...
    def test_enter_exit_nested(self):
        ...
        s.machine.add_transition(
            "lower",
            ["C{0}1".format(separator), "C{0}3".format(separator)],
            "C{0}3{0}a".format(separator),
        )
        s.machine.add_transition(
            "rise", "C{0}3".format(separator), "C{0}1".format(separator)
        )
    ...
    def test_state_change_listeners(self):
        ...
        s.machine.add_transition("reverse", "C", "A")
        s.machine.add_transition(
            "lower", "C%s1" % State.separator, "C{0}3{0}a".format(State.separator)
        )
        s.machine.add_transition(
            "rise", "C%s3" % State.separator, "C%s1" % State.separator
        )
    ...
    def test_nested_auto_transitions(self):
        ...
        for state_name in s.machine.get_nested_state_names():
            ...
            self.assertEqual(
                len(s.machine.events[event_name].transitions), num_base_states
            )
    ...
    def test_example_two(self):
        ...
        machine = self.stuff.machine_cls(
            states=states, transitions=transitions, initial="A"
        )
