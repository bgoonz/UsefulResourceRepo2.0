...
class Graph(BaseGraph):
    ...
    def _add_edges(self, transitions, container):
        ...
        for transition in transitions:
            ...
            edge_labels[transition["source"]][dst].append(
    ...
    def generate(self, title=None, roi_state=None):
        ...
        if roi_state:
            transitions = [
                ...
                if t["source"] == roi_state
                or self.custom_styles["edge"][t["source"]][t["dest"]]
            ]
            state_names = [
                t
            ...
            state_names += [
                k for k, style in self.custom_styles["node"].items() if style
            ]
            ...
            if state.get("children", []):
                ...
                with container.subgraph(name=cluster_name, graph_attr=attr) as sub:
                    ...
                    self._add_nodes(
                        ...
                        prefix=prefix
                        + state["name"]
                        + self.machine.state_cls.separator,
                    )
            else:
        ...
        for transition in transitions:
            ...
            if edges_attr[src][dst]:
                ...
                attr[attr["label_pos"]] = " | ".join(
                    [
                        edges_attr[src][dst][attr["label_pos"]],
                        self._transition_label(transition),
                    ]
                )
            else:
