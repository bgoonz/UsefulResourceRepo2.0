    ...
    def _change_state(self, event_data):
        ...
        for state in _flatten(
            ...
            graph.set_node_style(
                self.dest if hasattr(state, "name") else state, "active"
            )
...
class GraphMachine(MarkupMachine):
    ...
    def __getstate__(self):
        ...
        return {
            k: v for k, v in self.__dict__.items() if k not in self._pickle_blacklist
        }
    ...
    def __setstate__(self, state):
        ...
        for model in self.models:
            try:
                ...
                _LOGGER.warning(
                    "Graph for model could not be initialized after pickling: %s", e
                )
...
class BaseGraph(object):
    ...
    def _transition_label(self, tran):
        ...
        if self.machine.show_conditions and any(
            prop in tran for prop in ["conditions", "unless"]
        ):
            ...
            while q:
                ...
                for transition in scope.get("transitions", []):
                    if prefix:
                        ...
                        t["source"] = self.machine.state_cls.separator.join(
                            prefix + [t["source"]]
                        )
                        if "dest" in t:  # don't do this for internal transitions
                            t["dest"] = self.machine.state_cls.separator.join(
                                prefix + [t["dest"]]
                            )
                    else:
                        t = transition
                ...
                for state in scope.get("children", []) + scope.get("states", []):
                    ...
                    if not isinstance(ini, list):
                        ...
                        t = dict(
                            trigger="",
                            source=self.machine.state_cls.separator.join(
                                prefix + [state["name"]]
                            )
                            + "_anchor",
