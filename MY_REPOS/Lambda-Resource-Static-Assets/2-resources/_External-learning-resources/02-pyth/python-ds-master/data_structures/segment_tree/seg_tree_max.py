...
class MaxSegTree:
    ...
    def _max_util(self, v, tl, tr, l, r):
        ...
        tm = (tl + tr) // 2
        return max(
            self._max_util(v * 2, tl, tm, l, min(r, tm)),
            self._max_util(v * 2 + 1, tm + 1, tr, max(l, tm + 1), r),
        )
    ...
    def _update_util(self, v, tl, tr, pos, new_val):
        if tl == tr:
            ...
            tm = (tl + tr) // 2
            if pos <= tm:
                self._update_util(v * 2, tl, tm, pos, new_val)
            else:
                self._update_util(v * 2 + 1, tm + 1, tr, pos, new_val)
            self._t[v] = max(self._t[v * 2], self._t[v * 2 + 1])
