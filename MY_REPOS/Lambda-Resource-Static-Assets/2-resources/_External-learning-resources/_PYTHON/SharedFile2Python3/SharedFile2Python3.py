...
def greengrass_internal_handler():
    try:
        ...
        if random.randint(RAND_MIN, RAND_MAX) == RAND_TARGET:
            ...
            logger.info(
                f"Writing random output string [{shared_output}] to shared output file [{SHARED_OUTPUT_FILE}]"
            )
        ...
        payload[
            "message"
        ] = f"Data found in shared input file [{SHARED_INPUT_FILE}], it was [{shared_input}]"
