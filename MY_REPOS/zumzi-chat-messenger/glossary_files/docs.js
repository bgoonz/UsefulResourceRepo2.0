...
var sidebarObj = document.getElementsByClassName("sidebar")[0]
  ...
  : document.getElementsByClassName("sidebar-home")[0];
...
function hr() {
  outputLetNav.push(
    '<div style="background-color: #FFFFFF; border: 1px solid #ccc; height: 1px; margin: 5px auto 5px auto; width: 95%;"> </div>'
  );
}
...
function walkTree(tree) {
  for (var j = 0; j < tree.length; j++) {
    ...
    if (tree[j].section) {
      ...
      outputLetNav.push(
        '<li><a  onclick="navClicked(' +
          totalTopics +
          ')" data-target="#item' +
          totalTopics +
          '" data-toggle="collapse" data-parent="#stacked-menu"'
      );
      if (sectionHasPath) {
      ...
      outputLetNav.push(
        ">" + tree[j].sectiontitle + '<span class="caret arrow"></span></a>'
      );
      ...
      outputLetNav.push(
        '<li><a  "test=p" href="' + jsvar_baseurl + tree[j].path + '"'
      );
...
function addSection(t) {
  var title = t.title;
  if (undefined != t.vtitle) title = t.vtitle;
...
function renderNav(docstoc) {
  for (i = 0; i < docstoc.horizontalnav.length; i++) {
    ...
    if (
      docstoc.horizontalnav[i].path == pageURL ||
      docstoc.horizontalnav[i].node == sectionToHighlight
    ) {
    ...
    outputHorzTabs.push(
      '><a href="' +
        jsvar_baseurl +
        docstoc.horizontalnav[i].path +
        '">' +
        docstoc.horizontalnav[i].title +
        "</a></li>\n"
    );
  }
  ...
  if (outputLetNav.length == 0) {
    ...
    for (var i = 0; i < glossary.length; i++) {
      ...
      if (tagToLookup)
        highlightGloss =
          glossary[i].term.toLowerCase() == tagToLookup.toLowerCase()
  ...
  document.getElementById("jsTOCHorizontal").innerHTML =
    outputHorzTabs.join("");
  ...
  if (document.location.pathname.indexOf("/documentation/glossary/") < 0) {
    ...
    if (heading == "title") {
      history.replaceState(
        {},
        "Top of page on " + document.location.pathname,
