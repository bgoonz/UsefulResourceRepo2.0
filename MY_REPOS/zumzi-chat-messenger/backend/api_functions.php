...
function get_uid_from_token($token)
{
    return GetValueFromDB(
        "select uid from tokens where token='$token' limit 1",
        "uid"
    );
}
...
{
    return GetValueFromDB(
        "select email from users, tokens where token='$token' and tokens.uid=users.uid limit 1",
        "email"
    );
}
...
{
    return GetRowFromDB(
        "select * from users, tokens where token='$token' and tokens.uid=users.uid limit 1"
    );
}
...
function login_callbackapi($user, &$result)
{
    ...
    $newuser = SetQueryDB(
        "insert into users set uid=$uid, address='$address', email='$email', name='$name', ts=$ts on duplicate key update uid=$uid, ts=$ts"
    );

    $user = GetRowFromDB(
        "select uid, name, address, ts from users where uid='$uid' limit 1"
    );
    SetQueryDB(
        "insert into tokens set uid='$uid', token='$token' on duplicate key update token='$token'"
    );
...
{
    return GetRowFromDB(
        "select name, uid, pin, spin, type from groupstable where gid='$gid' limit 1;"
    );
}
...
function setgroup_callbackapi($user, &$result)
{
    ...
    $spin = 0;
    if (0 == $gid) {
        $roomcount = GetValueFromDB(
            "select count(uid) c from groupstable where uid=$uid",
            "c"
        );
        if (0 && $roomcount >= 5) {
            $result["error"] = $error;
    ...
    $rv = SetQueryDB(
        "insert into groupstable set gid=$gid, uid=$uid, pin='$pin', spin='$spin', type='$type', name='$name', ts=$ts on duplicate key update type=$type, name='$name', ts=$ts"
    );
    SetQueryDB(
        "insert into members set gid=$gid, uid=$uid, pin='$pin', ts=unix_timestamp() on duplicate key update pin=values(pin), ts=unix_timestamp()"
    );
...
function rooms_callbackapi($user, &$result)
{
    ...
    GetRowsFromDB(
        "select g.uid, g.gid, name, if(m.uid=g.uid, g.pin, m.pin) pin, if(m.uid=g.uid, g.spin, '') spin from groupstable g, members m where m.uid=$uid and m.pin != '' and m.gid=g.gid order by m.ts desc",
        $result,
        null,
        $key = "rooms",
        $keycount = "count"
...
function webapi_getrecords(
    ...
    if (null == $result) {
        ...
        $user[$key] = []; //empty array so that we don't need to check class of data when json decoded
        return false;
    }
