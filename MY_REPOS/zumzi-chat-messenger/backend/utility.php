...
function check_empty_fields($req_fields)
{
    ...
    foreach ($req_fields as $key) {
        if (
            ...
            ($trimmedtext = trim($data_array[$key]) == "")
        ) {
            print "<br>Empty $key";
            return true;
        }
...
function GenerateSetQuery(
    ...
    foreach ($fields as $key) {
        ...
        //$setquery = $setquery.$prefix.$key."=".quote_smart(trim($data_array[$key]));
        $setquery =
            $setquery . $prefix . $key . "='" . trim($data_array[$key]) . "'";
        if ($i < $len) {
            $setquery = $setquery . ", ";
        }
    }
...
function GetTableDefaultValues($table, &$TableValue)
{
    ...
    ($result = mysql_query("SHOW COLUMNS FROM $table")) or
...
function GenerateDeleteQuery($fields, $data_array, $tablename, &$setquery)
{
    ...
    foreach ($fields as $key) {
        ...
        $setquery =
            ...
            $key .
            "='" .
            mysql_real_escape_string(trim($data_array[$key])) .
            "'";
        if ($i < $len) {
...
{
    foreach ($fields as $key) {
        $destarray[$key] = mysql_real_escape_string(
            trim(stripslashes(strip_tags($sourcearray[$key])))
        );
    }
}
