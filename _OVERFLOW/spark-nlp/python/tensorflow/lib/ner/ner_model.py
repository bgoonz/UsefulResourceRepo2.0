...
class NerModel:
    ...
    def __init__(
        self, session=None, dummy_tags=None, use_contrib=True, use_gpu_device=0
    ):
        ...
        with tf.compat.v1.device("/gpu:{}".format(self.use_gpu_device)):
            ...
            with tf.compat.v1.variable_scope("word_repr") as scope:
                # shape = (batch size)
                self.sentence_lengths = tf.compat.v1.placeholder(
                    tf.int32, shape=[None], name="sentence_lengths"
                )
        ...
        with tf.compat.v1.device("/gpu:{}".format(self.use_gpu_device)):
            ...
            with tf.compat.v1.variable_scope("char_repr_lstm") as scope:
                ...
                char_embeddings = tf.nn.embedding_lookup(
                    params=embeddings, ids=self.char_ids
                )
                ...
                char_embeddings_seq = tf.reshape(
                    char_embeddings, shape=[-1, s[-2], dim]
                )
                ...
                mask = tf.expand_dims(
                    tf.sequence_mask(word_lengths_seq, dtype=tf.float32), axis=-1
                )
    ...
    def add_pretrained_word_embeddings(self, dim=100):
        ...
        with tf.compat.v1.device("/gpu:{}".format(self.use_gpu_device)):
            with tf.compat.v1.variable_scope("word_repr") as scope:
                ...
                self.word_embeddings = tf.compat.v1.placeholder(
                    tf.float32, shape=[None, None, dim], name="word_embeddings"
                )

                if self.word_repr is not None:
                    self.word_repr = tf.concat(
                        [self.word_repr, self.word_embeddings], axis=-1
                    )
                else:
    ...
    def _create_lstm_layer(self, inputs, hidden_size, lengths):
        ...
        with tf.compat.v1.device("/gpu:{}".format(self.use_gpu_device)):
            ...
            if not self.use_contrib:
                ...
                mask = tf.expand_dims(
                    tf.sequence_mask(lengths, dtype=tf.float32), axis=-1
                )
                # shape = (batch x sentence, 2 x hidden)
                output = model(inputs, mask=mask)
            ...
            output_fw, _ = cell_fw(
                time_based, dtype=tf.float32, sequence_length=lengths
            )
            output_bw, _ = cell_bw(
                time_based, dtype=tf.float32, sequence_length=lengths
            )
    ...
    def _multiply_layer(self, source, result_size, activation=tf.nn.relu):
        ...
        with tf.compat.v1.device("/gpu:{}".format(self.use_gpu_device)):
            ...
            b = tf.compat.v1.get_variable(
                "b", shape=[result_size], dtype=tf.float32, use_resource=False
            )
    ...
    def add_context_repr(self, ntags, hidden_size=100, height=1, residual=True):
        assert (
            self._word_embeddings_added
            or self._char_cnn_added
            or self._char_bilstm_added,
            ...
            with tf.compat.v1.variable_scope("context_repr") as scope:
                for i in range(height):
                    with tf.compat.v1.variable_scope("lstm-{}".format(i)):
                        new_repr = self._create_lstm_layer(
                            context_repr, hidden_size, lengths=self.sentence_lengths
                        )
        ...
        with tf.device("/gpu:{}".format(self.use_gpu_device)):
            ...
            with tf.compat.v1.variable_scope("inference", reuse=None) as scope:
                ...
                if crf:
                    ...
                    self.prediction, _ = tf.contrib.crf.crf_decode(
                        self.scores, self.transition_params, self.sentence_lengths
                    )
        ...
        with tf.compat.v1.device("/gpu:{}".format(self.use_gpu_device)):
            ...
            with tf.compat.v1.variable_scope("training", reuse=None) as scope:
                ...
                if clip_gradient > 0:
                    ...
                    capped_gvs = [
                        (tf.clip_by_value(grad, -clip_gradient, clip_gradient), var)
                        for grad, var in gvs
                        if grad is not None
                    ]
                ...
                self.init_op = tf.compat.v1.variables_initializer(
    ...
    def get_word_lengths(batch, idx="char_ids"):
        ...
        return [
            NerModel.fill([len(chars) for chars in row[idx]], max_words, 0)
            for row in batch
        ]
    ...
    def get_word_embeddings(batch, idx="word_embeddings"):
        ...
        return [
            NerModel.fill(
                [word_embedding for word_embedding in sentence[idx]],
                max_words,
                [0] * embeddings_dim,
            )
            for sentence in batch
    ...
    def train(
        ...
        assert (
            self._training_added,
            "Add training layer by method add_training_op before running training",
        )
    ...
    def measure(self, dataset, batch_size=20, dropout=1.0):
        ...
        prec = num_correct_predicted / (num_predicted or 1.0)
        rec = num_correct_predicted / (num_correct or 1.0)
        ...
        for batch in exp_scores:
            for sentence in exp_scores:
                for i in range(len(sentence)):
                    ...
                    sentence[i] = [
                        p if threshold is None or p >= threshold else 0
