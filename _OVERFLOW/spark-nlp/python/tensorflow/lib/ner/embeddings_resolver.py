...
class TokenEmbeddings:
    ...
    def create_sentence(pieces, is_word_starts, embeddings):
        ...
        return [
            TokenEmbeddings(piece, is_start, vector)
            for (piece, is_start, vector) in zip(pieces, is_word_starts, embeddings)
        ]

    def __str__(self):
        return "TokenEmbeddings({}, {}, [{}])".format(
            self.piece, self.is_word_start, np.shape(self.vector)
        )
...
class EmbeddingsDbResolver:
    ...
    def __init__(
        self, dim, index_file="embeddings_index", lowercase=False, clear_if_exists=False
    ):
        full_index_file = EmbeddingsDbResolver.get_index_name(index_file, dim)
        ...
        self.add_vector("__oov__", [0.0] * dim)
...
class BertEmbeddingsResolver:
    def __init__(self, model_folder, max_length=256, lowercase=True):
        ...
        self.token_ids_op = tf.placeholder(
            tf.int32, shape=(None, max_length), name="token_ids"
        )
    ...
    def resolve_sentences(self, sentences):
        ...
        for sentence in sentences:
            ...
            to_input = np.pad(
                token_ids, [(0, self.max_length - len(token_ids))], mode="constant"
            )
        ...
        for i in range(len(sentences)):
            ...
            resolved = TokenEmbeddings.create_sentence(
                tokens, is_word_start, item_embeddings
            )
    ...
    def resolve_sentence(self, sentence):
        ...
        embeddings = self.session.run(
            self.embeddings_op, feed_dict={self.token_ids_op: to_input}
        )
