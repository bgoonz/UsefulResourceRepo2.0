...
class NerModelSaver:
    ...
    def save_models(self, folder):
        with tf.device("/gpu:0"):
            ...
            self.ner.session.run(
                "save/control_dependency", feed_dict={"save/Const:0": variables_file}
            )
            tf.train.write_graph(
                self.ner.session.graph, folder, "saved_model.pb", False
            )
    ...
    def save(self, export_dir):
        ...
        if self.embeddings_file:
            save_embeddings(
                self.embeddings_file, os.path.join(export_dir, "embeddings")
            )
