...
function Scheduler() {
  ...
  this.addLabel = function (name, priority) {
    var idx = 0;
    while (
      idx < scheduler.queues.length &&
      scheduler.labels[idx].priority <= priority
    ) {
      idx = idx + 1;
    }
  ...
  this.clearLabel = function (name) {
    ...
    while (
      idx < scheduler.queues.length &&
      scheduler.labels[idx].name != name
    ) {
      idx = idx + 1;
    }
  ...
  this.add = function (labelName, fn, self, args) {
    ...
    var idx = 0;
    while (
      idx < scheduler.labels.length &&
      scheduler.labels[idx].name != labelName
    ) {
      idx = idx + 1;
    }
    if (
