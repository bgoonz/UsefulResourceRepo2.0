...
$(document).ready(function () {
  ...
  if (Modernizr && !Modernizr.inlinesvg) return;

  // only execute this in the main window
  if (diagrams.isPopup) return;
...
diagrams.initPopup = function (id) {
  ...
  if (!jQuery.browser.msie) $("body").append(opener.$("#" + id).data("svg"));
...
diagrams.initHighlighting = function () {
  ...
  hover($("svg .edge.inheritance"), function (evt) {
    var toggleClass =
      evt.type == "mouseout" ? diagrams.removeClass : diagrams.addClass;
...
diagrams.toggle = function (container, dontAnimate) {
  ...
  if (div.is(":visible")) {
    ...
    diagrams.resize();
    if (dontAnimate) div.show();
    else div.slideDown(100);
...
diagrams.popup = function (diagram) {
  ...
  if (!diagrams.windows[id] || diagrams.windows[id].closed) {
    ...
    var svgIE = jQuery.browser.msie
      ? $("<div />").append(diagram.data("svg")).html()
      : "";
    var html =
      "" +
      ...
      "</title>\n" +
      '		<link href="' +
      $("#diagrams-css").attr("href") +
      '" media="screen" type="text/css" rel="stylesheet" />\n' +
      '		<script type="text/javascript" src="' +
      $("#jquery-js").attr("src") +
      '"></script>\n' +
      '		<script type="text/javascript" src="' +
      $("#diagrams-js").attr("src") +
      '"></script>\n' +
      '		<script type="text/javascript">\n' +
      "			diagrams.isPopup = true;\n" +
      "		</script>\n" +
      "	</head>\n" +
      "	<body onload=\"diagrams.initPopup('" +
      id +
      "');\">\n" +
    ...
    var parameters =
      ...
      ", top=" +
      top +
      ", scrollbars=yes, location=no, resizable=yes";
