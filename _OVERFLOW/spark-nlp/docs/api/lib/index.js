...
function setUrlFragmentFromFrameSrc() {
  try {
    ...
    if (!relativePath || frameLocation.pathname.indexOf("/") < 0) return;
...
(function (ns) {
  ...
  function createListItem(template) {
    ...
    if (
      ...
      inner += inner == "" ? '<div class="placeholder" />' : "</a>";
      inner += openLink(
        template,
        template["trait"] ? "trait" : template["type"] ? "type" : "class"
      );
    } else {
...
function configureTextFilter() {
  scheduler.add("init", function () {
    $("#textfilter").append(
      "<span class='pre'/><span class='input'><input id='index-input' type='text' accesskey='/'/></span><span class='post'/>"
    );
...
function textFilter() {
  ...
  if (
    typeof textFilter.lastQuery === "undefined" ||
    ...
    var searchLoop = function () {
      ...
      while (packages[index]) {
        ...
        if (matched.length > 0) {
          $("#tpl").append(
            Index.createPackageTree(pack, matched, focusFilterState)
          );
...
function configureFocusFilter() {
  scheduler.add("init", function () {
    ...
    if ($("#focusfilter").length == 0) {
      $("#filter").append(
        "<div id='focusfilter'>focused on <span class='focuscoll'></span> <a class='focusremove'><img class='icon' src='lib/remove.png'/></a></div>"
      );
...
function configureKindFilter() {
  scheduler.add("init", function () {
    kindFilterState = "all";
    $("#filter").append(
      "<div id='kindfilter'><a>display packages only</a></div>"
    );
