  ...
  $.layout = {
    ...
    borderWidth: function (el, side) {
      ...
      return $.css(el, b + "Style", true) === "none"
        ? 0
        : parseInt($.css(el, b + "Width", true), 10) || 0;
    },
...
,	trackMouse: function (evt) {
  ...
  $.layout.optionsMap = {
    ...
    layout: (
      "stateManagement,effects,zIndexes,errors," +
  ...
  $.layout.transformData = function (hash) {
    ...
    for (optKey in hash) {
      ...
      for (i = 0; i <= c; i++) {
        key = keys[i];
        if (i === c) branch[key] = val;
        else if (!branch[key]) branch[key] = {}; // create the subkey
  ...
  $.fn.layout = function (opts) {
    var // local aliases to global data
      ...
      _runCallbacks = function (evtName, pane, skipBoundEvents) {
        ...
        if (fn) {
          try {
            ...
            if ($.isFunction(fn)) {
              if (args.length) retVal = fn(args[1]);
              ...
              else retVal = fn(Instance, s, o, lName);
            }
          } catch (ex) {
            _log(
      ...
      setOuterSize = function (el, outerSize, autoHide) {
        ...
        // pane = east or west
        else setOuterWidth(el, outerSize, autoHide);
      },
      /**
	...
	*/
      ...
      onResizerLeave = function (evt, el) {
        ...
        else if (!state[pane].isResizing && $.fn.enableSelection)
          // 2nd call - by timer
      ...
      _initChildLayouts = function () {
        $.each(_c.allPanes, function (idx, pane) {
          if (options[pane].initChildLayout) createChildLayout(pane);
        });
      },
      /**
      ...
      createChildLayout = function (evt_or_pane, opts) {
        ...
        if ($P) {
          var $C = $Cs[pane],
            ...
            $Cont = o.containerSelector
              ? $P.find(o.containerSelector)
              : $C || $P,
          ...
          if (child) child.hasParentLayout = true; // set parent-flag in child
        }
      ...
      _initContainer = function () {
        ...
        try {
          ...
          if (fullPage) {
            ...
            // set required CSS for overflow and position
        ...
        $.each(_c.allPanes, function (i, pane) {
          ...
          if ($Ps[pane]) {
            if (state[pane].isVisible) {
              ...
              else resizeChildLayout(pane);
            }
            // init childLayout - even if pane is not visible
            if (o.initChildLayout && o.childOptions) createChildLayout(pane);
          }
        });
      },
      /**
      ...
      addPane = function (pane, force) {
        ...
        if (state.initialized) {
          ...
          if (s.isVisible) {
            ...
            // automatic if onresize called, otherwise call it specifically
            // resize child - IF inner-layout already exists (created before this layout)
            else resizeChildLayout(pane); // a previously existing childLayout
          }
          if (o.initChildLayout && o.childOptions) createChildLayout(pane);
        }
      },
      ...
      initContent = function (pane, resize) {
        ...
        if ($C && $C.length) {
          ...
          // SAVE original Pane CSS
          if (!$C.data("layoutCSS")) $C.data("layoutCSS", elCSS($C, "height"));
      ...
      initResizable = function (panes) {
        ...
        var resizePanes = function (evt, ui, pane, resizingDone, masks) {
          ...
          if (!resizingDone) {
            ...
            if (s.isSliding && masks)
              // RE-SHOW only 'object-masks' so objects won't show through sliding pane
      ...
      showMasks = function (panes, onlyForObjects) {
        ...
        $.each(a, function (i, p) {
          ...
          if (
            s.isVisible &&
            ((!onlyForObjects && o.maskContents) || o.maskObjects)
          ) {
      ...
      hideMasks = function () {
        ...
        if (!skip) $Ms.hide(); // hide ALL masks
      },
      ...
      removePane = function (evt_or_pane, remove, skipResize, destroyChild) {
        ...
        var o = options[pane],
          ...
          child =
            ...
            null,
          destroy =
            destroyChild !== undefined ? destroyChild : o.destroyChildLayout;
      ...
      setAsClosed = function (pane) {
        var $P = $Ps[pane],
          ...
          _sliding = "-sliding",
          _closed = "-closed";
        $R.css(side, sC[inset]) // move the resizer
      ...
      open = function (evt_or_pane, slide, noAnimation, noAlert) {
        ...
        $N.queue(function (queueNext) {
          ...
          // true=skipCallback/forceResize/noAnimation
          ...
          else if (o.slidable) bindStartSlidingEvent(pane, false); // UNBIND trigger events
      ...
      setAsOpen = function (pane, skipCallback) {
        var $P = $Ps[pane],
          ...
          _closed = "-closed",
          _sliding = "-sliding";
        $R.css(side, sC[inset] + getPaneSize(pane)) // move the resizer
        ...
        if (o.resizerDblClickToggle) $R.bind("dblclick", toggle);
      ...
      makePaneFit = function (pane, isOpening, skipCallback, force) {
        ...
        if (pane === "center") {
          ...
          if (s.size > s.maxSize)
            ...
            sizePane(pane, s.maxSize, skipCallback, force, true);
          // true = noAnimation
      ...
      resizeChildLayout = function (evt_or_pane) {
        ...
        if (L && L.destroyed) L = children[pane] = null; // clear child pointers
        // no child layout pointer is set - see if there is a child layout NOW
        if (!L)
          L = children[pane] = $P.data(d) || ($C ? $C.data(d) : 0) || null; // set/update child pointers
      ...
      enableSlidable = function (evt_or_pane) {
        ...
        if (state[pane].isClosed) bindStartSlidingEvent(pane, true);
      },
      /**
       * @param {string|Object}	evt_or_pane
       */
      ...
      swapPanes = function (evt_or_pane1, pane2) {
        ...
        function move(oPane, pane) {
          ...
          if (c.dir != _c[oldPane].dir) {
            ...
            $Rs[pane].css(
              side,
              sC[inset] + (state[pane].isVisible ? getPaneSize(pane) : 0)
            );
 ...
 */
  ...
  $.layout.state = {
    ...
    loadState: function (inst, stateData, animate) {
      ...
      if (inst.state.initialized) {
        ...
        $.each($.layout.config.borderPanes, function (idx, pane) {
          ...
          if (!vis) inst.sizePane(pane, s, false, false);
          if (h === true) inst.hide(pane, noAnimate);
          else if (c === false) inst.open(pane, false, noAnimate);
          else if (c === true) inst.close(pane, false, noAnimate);
          else if (h === false) inst.show(pane, false, noAnimate);
          // resize AFTER any other actions
          if (vis) inst.sizePane(pane, s, false, noAnimate); // animate resize if option passed
        });
      }
    },
    ...
    _create: function (inst) {
      ...
      if (oS.enabled) {
        if (oS.autoLoad)
          // update the options from the cookie
          inst.loadCookie();
        // don't modify options - just store cookie data in state.stateData
        else inst.state.stateData = inst.readCookie();
      }
    },
    ...
    _unload: function (inst) {
      ...
      if (oS.enabled) {
        if (oS.autoSave)
          // save a state-cookie automatically
          inst.saveCookie();
        // don't save a cookie, but do store state-data in state.stateData key
        else inst.state.stateData = inst.readState();
      }
    },
  };
  ...
  $.layout.browserZoom = {
    ...
    ratio: function () {
      ...
      if (s.deviceXDPI) return calc(s.deviceXDPI, s.systemXDPI);
