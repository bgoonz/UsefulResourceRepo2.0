...
def init():
    if os.path.exists("davisbase_columns.tbl"):
        ...
        davisbase_columns_cells = [
            ["davisbase_tables", "rowid", "INT", 1, "NO", "NO", "NO"],
        ...
        for i, cell in enumerate(davisbase_columns_cells):
            cell = table_create_cell(
                davisbase_columns_schema, cell, False, left_child_page=None, rowid=i + 1
            )
    ...
    if os.path.exists("davisbase_tables.tbl"):
        ...
        cells = [["davisbase_tables"], ["davisbase_columns"]]
        for i, cell in enumerate(cells):
            cell = table_create_cell(
                davisbase_tables_schema, cell, False, left_child_page=None, rowid=i + 1
            )
            try:
                page_insert_cell(file_name, 0, cell)
            except:
...
def initialize_indexes(column_dictionary):
    ...
    for col in column_names:
        if column_dictionary[table_name][col]["primary_key"] == "YES":
            ...
            initialize_file(
                index_name, False
            )  # create the empty ndx file for primary key
    return None
...
def catalog_add_table(column_dictionary):
    ...
    for col in column_names:
        values = [
            table_name,
            col.lower(),
            columns[col]["data_type"].upper(),
            columns[col]["ordinal_position"] + 1,
            columns[col]["is_nullable"].upper(),
            columns[col]["unique"].upper(),
            columns[col]["primary_key"].upper(),
        ]
...
def write_new_page(table_name, is_table, is_interior, rsibling_rchild, parent):
    ...
    with open(table_name + file_type, "ab") as f:
        ...
        newpage[2:16] = struct.pack(
            endian + "hhii2x", 0, PAGE_SIZE, rsibling_rchild, parent
        )
        f.write(newpage)
...
def int_to_fstring(key):
    ...
    int2packstring = {2: "h", 3: "i", 4: "q", 5: "f", 6: "d", 9: "i", 10: "Q", 11: "Q"}
...
def table_create_cell(
    schema, value_list, is_interior, left_child_page=None, rowid=None
):
    """
...
def index_create_cell(
    index_dtype, index_value, rowid_list, is_interior, left_child_page=None
):
    """
...
def update_array_values(
    page, first_array_loc_to_change, num_cells, shift_step, up=True
):
...
def get_cell_indices(page, cell_indx):
    cell_top_idx = struct.unpack(
        endian + "h", page[16 + 2 * cell_indx : 16 + 2 * (cell_indx + 1)]
    )[0]
    if cell_indx == 0:  # if cell is first on the page (bottom)
        cell_bot_idx = PAGE_SIZE
    else:
        cell_bot_idx = struct.unpack(
            endian + "h", page[16 + 2 * (cell_indx - 1) : 16 + 2 * (cell_indx)]
        )[0]
...
def page_delete_cell(file_name, page_num, cell_indx):
    ...
    if (cell_indx == num_cells - 1) & (cell_indx != 0):
        ...
        page[cell_top_loc:cell_bot_loc] = (
            b"\x00" * dis2replace
        ...
        page = shift_page_content(
            page, cell_content_area_start, cell_top_loc, dis2replace, up=False
        )
...
def page_update_cell(file_name, page_num, cell_indx, cell):
    ...
    if len(cell_2_update) == len(cell):
        ...
        page = shift_page_content(
            page, cell_content_area_start, cell_top_idx, dis2move, up=True
        )
        # since we just shifted every cell, every value in cell_array is off
        ...
        page = shift_page_content(
            page, cell_content_area_start, cell_top_idx, dis2move, up=False
        )
        # since we just shifted every cell, every value in cell_array is off
        page = update_array_values(page, cell_indx, num_cells, dis2move, up=True)
        # change cell content area start
...
def update_page_header(
    file_name, page_num, rsibling_rchild=None, is_interior=None, parent=None
):
...
def read_cells_in_page(file_bytes, page_num):
    ...
    while i <= num_cells - 1:
        if i == 0:
            ...
            cell_bot_loc = struct.unpack(
                endian + "h", page[16 + 2 * (i - 1) : 16 + 2 * (i)]
            )[0]
        cell_top_loc = struct.unpack(endian + "h", page[16 + 2 * i : 16 + 2 * (i + 1)])[
            0
        ]
    ...
    if is_interior:
        result["rightmost_child_page"] = parent_page = struct.unpack(
            endian + "i", page[6:10]
        )[0]
    else:
        result["right_sibling_page"] = parent_page = struct.unpack(
            endian + "i", page[6:10]
        )[0]
    result["cells"] = data
    if is_table:
    ...
    for page in data:
        if page["is_leaf"]:
            if page["right_sibling_page"] != -1:
                if (
                    data[page["right_sibling_page"]]["parent_page"]
                    == page["parent_page"]
                ):
                    data[page["right_sibling_page"]]["left_sibling_page"] = page[
                        "page_number"
                    ]
        else:
            for i, cell in enumerate(page["cells"]):
                ...
                if i != 0:
                    data[child_page]["left_sibling_page"] = page["cells"][i - 1][
                        "left_child_page"
                    ]
                if i + 1 != len(page["cells"]):
                    data[child_page]["right_sibling_page"] = page["cells"][i + 1][
                        "left_child_page"
                    ]
                else:
                    data[child_page]["right_sibling_page"] = page[
                        "rightmost_child_page"
                    ]
            data[page["rightmost_child_page"]]["left_sibling_page"] = page["cells"][-1][
                "left_child_page"
            ]
        ...
        for cell in page["cells"]:
            ...
            if col_table == table_name.lower():
                ...
                if col_name == "rowid" and not with_rowid:
                    continue
                all_cells.append(
                    (cell["data"][3], cell["data"][2])
                )  # list of [(ord_pos, dtype)]
...
def index_insert_cell_in_page(file_name, page_num, cell, cell_indx):
    ...
    # shift cell content down
    page = shift_page_content(
        page, cell_content_area_start, cell_bot_loc, dis2move, up=True
    )
    # since we just shifted every cell, every value in cell_array is off
    page = update_array_values(page, cell_indx, num_cells, dis2move, up=True)
    # change the cell_start area
    ...
    page[array_idx_top:array_idx_bot] = struct.pack(
        endian + "h", cell_bot_loc - dis2move
    )
...
def insert_into(command):
    ...
    for val in values:
        ...
        for filename in indexes:
            ...
            index_insert(
                table_name, index_colname, index_dtype, index_value, next_rowid
            )
...
def index_insert(table_name, column_name, index_dtype, index_value, rowid):
    ...
    cell = index_create_cell(
        index_dtype, index_value, [rowid], False, left_child_page=None
    )
...
def index_interior_split_page(
    file_name, split_page_num, cell2insert, new_rightmost_page, cell_index
):
    pages = read_all_pages_in_file(file_name)
    ...
    middle_cell = int(
        (num_cells + 1) // 2
    )  # have to add one since we havent actually added the cell
    ...
    if parent_num == -1:  # ROOT CONDITION #children will also be interior nodes
        rchild_num = write_new_page(
            table_name, is_table, is_interior, new_rightmost_page, split_page_num
        )
        lchild_num = write_new_page(
            table_name, is_table, is_interior, rightmost_child_page_left, split_page_num
        )

        for order_cell in insert_order[: middle_cell + 1]:  # Copy cells into left child
            update_page_header(
                file_name, order_cell["left_child_page"], parent=lchild_num
            )  # update child to point header to lchild
        page_insert_cell(file_name, lchild_num, cells2copy[:middle_cell])

        for order_cell in insert_order[
            middle_cell + 1 :
        ]:  # Copy cells into right child   #splitting the interior nodes,  the middle cell is not redundant in children
            update_page_header(
                file_name, order_cell["left_child_page"], parent=rchild_num
            )
        ...
        rsibling = write_new_page(
            table_name, is_table, is_interior, new_rightmost_page, parent_num
        )
        update_page_header(
            file_name, split_page_num, rsibling_rchild=rightmost_child_page_left
        )
        ...
        for order_cell in insert_order[: middle_cell + 1]:  # Copy cells into left child
            update_page_header(
                file_name, order_cell["left_child_page"], parent=split_page_num
            )  # update child to point header to lchild
        ...
        for order_cell in insert_order[
            ...
            update_page_header(
                file_name, order_cell["left_child_page"], parent=rsibling
            )
        ...
        if parent_page["available_bytes"] / PAGE_SIZE < 0.5:
            new_parent = index_interior_split_page(
                file_name, parent_num, middle_cell_binary, rsibling, parent_index
            )

        else:
            try:
                index_insert_cell_in_page(
                    file_name, parent_num, middle_cell_binary, parent_index
                )
            except:
                new_parent = index_interior_split_page(
                    file_name, parent_num, middle_cell_binary, rsibling, parent_index
...
def index_leaf_split_page(
    file_name, split_page_num, cell2insert, index_dtype, cell_index
):
    file_bytes = load_file(file_name)
    ...
    if parent_num == -1:  # IS ROOT ->create two children
        ...
        lchild_num = write_new_page(
            table_name, is_table, False, rchild_num, split_page_num
        )
        page_insert_cell(file_name, lchild_num, cells2copy[:middle_cell])
        page_insert_cell(file_name, rchild_num, cells2copy[middle_cell + 1 :])
        page_delete_cells_on_and_after(file_name, split_page_num, 0)
        ...
        page_insert_cell(file_name, split_page_num, middle_cell_binary)
        update_page_header(
            file_name, split_page_num, rsibling_rchild=rchild_num, is_interior=True
        )

    else:  # Non-root ->propagate upward
        right_sibling_page = values["right_sibling_page"]
        rsibling = write_new_page(
            table_name, is_table, is_interior, right_sibling_page, parent_num
        )
        ...
        page_insert_cell(file_name, rsibling, cells2copy[middle_cell + 1 :])
        middle_cell_binary = (
            struct.pack(endian + "i", split_page_num) + middle_cell_binary
        )
...
def page_insert_cell(file_name, page_num, cell):
    ...
    if type(cell) == list:
        cells = cell
        for cell in cells:
            assert len(cell) < page_available_bytes(
                file_bytes, page_num
            )  # CHECK IF PAGE FULL
            num_cells = struct.unpack(endian + "h", page[2:4])[0]
            bytes_from_top = 16 + (2 * num_cells)
            ...
            page[bytes_from_top : bytes_from_top + 2] = struct.pack(
                endian + "h", new_start_index
            )
...
def page_delete_cells_on_and_after(file_name, page_num, cell_indx):
    ...
    page[16 + 2 * cell_indx : 16 + 2 * num_cells] = (
        b"\x00" * 2 * (num_cells - cell_indx)
    )
    # update num of cells
...
def table_interior_split_page(
    file_name, split_page_num, cell2insert, new_rightmost_page
):
    pages = read_all_pages_in_file(file_name)
    values = pages[split_page_num]
    ...
    if parent_num == -1:  # ROOT CONDITION #children will also be interior nodes
        ...
        for i in range(middle_cell):
            ...
            update_page_header(
                file_name, cells[i]["left_child_page"], parent=lchild_num
            )
        update_page_header(
            file_name, rightmost_child_page_left, parent=lchild_num
        )  # update parent of rightmost
        # Copy cells into left child
        page_insert_cell(file_name, lchild_num, cells2copy)

        cells2copy = []
        for i in range(
            middle_cell + 1, num_cells
        ):  # update child to point header to rchild
            cells2copy.append(cells[i]["cell_binary"])
            update_page_header(
                file_name, cells[i]["left_child_page"], parent=rchild_num
            )
        ...
        rsibling = write_new_page(
            table_name, is_table, is_interior, rightmost_child_page_right, parent_num
        )

        cells2copy = []
        for i in range(
            middle_cell + 1, num_cells
        ):  # Copy cells into right child update child pointers
            cells2copy.append(cells[i]["cell_binary"])
        ...
        middle_cell_binary = table_create_cell(
            [], [], True, left_child_page=split_page_num, rowid=middle_rowid
        )
        update_page_header(
            file_name, split_page_num, rsibling_rchild=rightmost_child_page_left
        )
        ...
        try:
            ...
            new_parent = table_interior_split_page(
                file_name, parent_num, middle_cell_binary, rsibling
            )
...
def table_leaf_split_page(file_name, split_page_num, cell2insert):
    ...
    if parent_num == -1:  # IS ROOT ->create two children
        ...
        middle_cell_binary = table_create_cell(
            [], [], True, left_child_page=lchild_num, rowid=middle_rowid
        )
        page_delete_cells_on_and_after(file_name, split_page_num, 0)
        page_insert_cell(file_name, split_page_num, middle_cell_binary)
        update_page_header(
    ...
    if not is_table:
        for i, cell in enumerate(page["cells"]):
            if cell["index_value"] == value:  # add to list
                ...
                if page["is_leaf"] and i + 1 == len(
                    page["cells"]
                ):  # no more cells, move down and right or return final position
                    return page_num, len(page["cells"])
                if not page["is_leaf"] and i + 1 == len(page["cells"]):
                    return get_page_cell_indx(
                        pages, value, page["rightmost_child_page"]
                    )
                else:
                    continue
    else:
    ...
    for rowid in list_of_rowids:
        ...
        if is_interior:
            cell = table_create_cell(
                ...
                prev_row_values,
                is_interior,
                left_child_page=cell["left_child_page"],
                rowid=cell["rowid"],
            )
        else:
            cell = table_create_cell(
    ...
    for i, id in enumerate(page["rowids"]):
        if left:
            if id > id2fix:
                ...
                page["cells"][i]["cell_binary"] = update_cell_binary(
                    page["cells"][i]["cell_binary"], rowid=id2fix - 1
                )
            ...
            if id > id2fix:
                ...
                page["cells"][i - 1]["cell_binary"] = update_cell_binary(
                    page["cells"][i - 1]["cell_binary"], rowid=id2fix
                )
...
def try_borrowing(pages, borrower_page):
    ...
    if "left_sibling_page" in page:
        ...
        and pages[page["right_sibling_page"]]["parent_page"] == parent
    ):
        rsib = pages[page["right_sibling_page"]]
...
def merge_children(pages, page_num, child_page_num, left=True):
    ...
    if left:
        ...
        if not lsib["is_leaf"]:  # update parent of downstream
            ...
            middle_cell = table_create_cell(
                [], [], True, left_child_page=lsib["rightmost_child_page"], rowid=id2del
            )
        ...
        child_page["available_bytes"] = lsib["available_bytes"] - (
            PAGE_SIZE - child_page["available_bytes"]
        )
        ...
        if not rsib["is_leaf"]:  # update parent of downstream
            ...
            middle_cell = table_create_cell(
                [], [], True, left_child_page=rsib["rightmost_child_page"], rowid=id2del
            )
        ...
        child_page["available_bytes"] = rsib["available_bytes"] - (
            PAGE_SIZE - child_page["available_bytes"]
        )
        if ["right_sibling_page"] != -1 and "left_sibling_page" in child_page:
            pages[child_page["left_sibling_page"]][
                "right_sibling_page"
            ] = child_page_num
        ...
        if i + 1 == len(pages[page["parent_page"]]["cells"]):
            pages[page["parent_page"]]["rightmost_child_page"] = child_page[
                "page_number"
            ]
        else:
            cell["left_child_page"] = child_page["page_number"]
            cell["cell_binary"] = update_cell_binary(
                cell["cell_binary"], left_child=child_page["page_number"]
            )
...
def copy_page(file_name, pages, page_number, parent, i=None):
    ...
    if is_interior:
        if cells != sorted(cells, key=lambda x: x["left_child_page"]):
            for cell, scell in zip(
                cells, sorted(cells, key=lambda x: x["left_child_page"])
            ):
                cell["cell_binary"] = update_cell_binary(
                    cell["cell_binary"], left_child=scell["left_child_page"]
                )
    if page["num_cells"] > 1:
    ...
    if is_interior:
        children = [j["left_child_page"] for j in cells] + [
        ...
        if not page["is_leaf"]:
            ...
            for cell in page["cells"]:
                if cell["left_child_page"] >= page_number:
                    ...
                    cell["cell_binary"] = update_cell_binary(
                        cell["cell_binary"], left_child=cell["left_child_page"]
                    )
...
def parse_update(command):
    ...
    if re.match(query_match, command):
        ...
        where_clause = re.sub(
            "\s", "", re.split(";", re.sub("(?i)where", "", where_clause))[0]
        )
        ...
        set_col = [
            [
                i.split("=")[0],
                to_python(
                    schema_columns, schema, i.split("=")[0].lower(), i.split("=")[1]
                ),
            ]
            for i in set_col
        ]
            ...
            if node["is_leaf"]:
                for cell in node["cells"]:
                    ...
                    if index == 0:
                        ...
                        op2 = to_python(
                            column_list, schema, where_op.lower(), where_value
                        )
                        op1 = data[index - 1]
        ...
        if node["is_leaf"]:
            for cell in node["cells"]:
                ...
                if index == 0:
                    ...
                    op2 = to_python(
                        column_list[1:], schema, where_op.lower(), where_value
                    )
                    op1 = data[index - 1]
                if get_operator_fn(oper)(op1, op2):
                    matched_cells.append(cell)
...
def check_valid(file_name, pages=None, page_num=0, is_table=None):
    ...
    if page["parent_page"] != -1:
        ...
        parent_children = [
            i["left_child_page"] for i in pages[page["parent_page"]]["cells"]
        ]
