...
async def outTweet(tweet):
    ...
    timestamp = str(
        datetime.timedelta(seconds=int(tweet.find("span", "_timestamp")["data-time"]))
    ).rpartition(", ")[-1]
    ...
    replies = tweet.find("span", "ProfileTweet-action--reply u-hiddenVisually").find(
        "span"
    )["data-tweet-stat-count"]
    retweets = tweet.find("span", "ProfileTweet-action--retweet u-hiddenVisually").find(
        "span"
    )["data-tweet-stat-count"]
    likes = tweet.find("span", "ProfileTweet-action--favorite u-hiddenVisually").find(
        "span"
    )["data-tweet-stat-count"]
    """
    ...
    if arg.users:
        ...
        if arg.stats:
            output += " | {} replies {} retweets {} likes".format(
...
async def getUsername():
    ...
    async with aiohttp.ClientSession() as session:
        r = await fetch(
            session, "https://twitter.com/intent/user?user_id={0.userid}".format(arg)
        )
...
if __name__ == "__main__":
    ap = argparse.ArgumentParser(
        ...
        description="tweep.py - An Advanced Twitter Scraping Tool",
    )
    ...
    ap.add_argument(
        "--fruit", help="Display 'low-hanging-fruit' Tweets.", action="store_true"
    )
    ...
    ap.add_argument(
        "--users", help="Display users only (Use with -s).", action="store_true"
    )
