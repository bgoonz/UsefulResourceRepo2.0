...
def get_titles_from_cards(cards):
    ...
    for card in cards:
        ...
        variant7 = card.find(
            "h3",
            class_="graf graf--h3 graf--startsWithDoubleQuote graf-after--figure graf--trailing graf--title",
        )
...
def get_subtitles_from_cards(cards):
    ...
    for card in cards:
        ...
        variant2 = card.find(
            "h4", class_="graf graf--h4 graf-after--h3 graf--trailing graf--subtitle"
        )
        ...
        variant6 = card.find(
            "blockquote",
            class_="graf graf--pullquote graf-after--figure graf--trailing",
        )
        variant7 = card.find("p", class_="graf graf--p graf-after--figure")
        variant8 = card.find(
            "blockquote", class_="graf graf--blockquote graf-after--h3 graf--trailing"
        )
        variant9 = card.find(
            "p", class_="graf graf--p graf-after--figure graf--trailing"
        )
...
def get_image_from_cards(cards):
    ...
    for card in cards:
        img = card.find(
            "img", class_="progressiveMedia-image js-progressiveMedia-image"
        )
        if img is not None:
            images.append(1)
        else:
...
def scrape_tag(tag, yearstart, monthstart, yearstop, monthstop):
    ...
    while current_date <= end_date:
        ...
        year, month, day, tags = get_dates_and_tags(
            tag, current_date.year, current_date.month, current_date.day, cards
        )
