...
def Search_For_Checkin(key):
    res = Book.objects.filter(is_available=False).filter(book_loan__date_in__isnull=True).filter(Q(title__icontains=key)|Q(borrower__bname__icontains=key)|Q(isbn=key)|Q(borrower__card_id=key))
    if len(res)>1:
        return res[:100]
    else:
        return res
...
def New_Borrower(name=None, SSN=None, address=None, phone=None):
    ...
    new_borrower = Borrower(card_id = card_id, ssn=SSN, bname=name, address=address, phone=phone)
    new_borrower.save()
    return None
...
def Update_Fines():
    overdue = Book_Loan.objects.filter((Q(date_in__isnull=True)&Q(due_date__lt=datetime.now().date()))|(Q(date_in__isnull=False)&Q(due_date__gt=F('date_in'))))
    for l in overdue:
...
class Borrower(models.Model):
    card_id = models.CharField(max_length=20, primary_key=True, default=make_card_id)             #PK
...
class Fine(models.Model):
    ...
    fine_amt = models.DecimalField(max_digits=10, decimal_places=2, default=0.00, blank=True)
