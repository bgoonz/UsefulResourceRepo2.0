...
sched_lr = combine_scheds(pcts, cos_1cycle_anneal(lr / 10.0, lr, lr / 1e5))
...
url = (
    URLs.IMAGENETTE_160
    if size < 140
    else URLs.IMAGENETTE_320
    if size < 240
    else URLs.IMAGENETTE
)
path = untar_data(url)
...
xtra_cb = []
if sched_lr:
    xtra_cb.append(partial(ParamScheduler, "lr", sched_lr))
if sched_mom:
    xtra_cb.append(partial(ParamScheduler, "mom", sched_mom))
