...
BLOCK_REPEATS = [MODEL_LEVEL_0_BLOCKS, MODEL_LEVEL_1_BLOCKS, MODEL_LEVEL_2_BLOCKS, MODEL_LEVEL_3_BLOCKS]
...
def decode_image(img, train=True):
    ...
    img = tf.math.divide(tf.math.subtract(tf.dtypes.cast(img, tf.float32), DATA_MEAN), DATA_STD_DEV)
    if train:
...
def load_imagenet(imagenet_path=None):
    ...
    train_ds_cachefile = prepare_for_training(train_imgs, cache="F://Data/Images/ImageNet/tf-cachefiles/imagenet.tfcache")
    
    #TEST DATASET
    test_root = Path(imagenet_path) / "test"
