...
var fs = require("fs"),
  program /*:any*/ = (require("commander") /*:any*/);
program
...
if (program.list) {
  var l /*:Array<number>*/ = [];
  Object.keys(codepage).forEach(function (x) {
    if (parseInt(x, 10) == +x) l.push(+x);
  });
  Object.keys(codepage.utils.magic).forEach(function (x) {
    if (parseInt(x, 10) == +x && +x != 16969) l.push(+x);
  });
  l.sort(function (a, b) {
    return a - b;
  }).forEach(function (x) {
...
function concat(func) {
  ...
  var buf = [];
  writable._write = function (chunk, e, cb) {
    buf.push(chunk);
    cb();
  };
...
function process_text(text /*:Buffer*/) {
  ...
  var out /*:any*/ =
    to === 65001 ? dec.toString("utf8") : codepage.utils.encode(to, dec);

  /* if output file is specified */
  if (o) writefile(o, out, mybom);
  /* utf8 -> print to stdout */ else if (to === 65001) logit(out, mybom);
  /* stdout piped to process -> print */ else if (!process.stdout.isTTY)
    logit(out, mybom);
  /* forced */ else if (program.force) logit(out, mybom);
  /* input file specified -> write to file */ else if (f !== "-")
    writefile(f + "." + to, out, mybom);
  else {
    console.error(
      "codepage: use force (-F, --force) to print " + to + " codes"
    );
    process.exit(14);
  }
}
