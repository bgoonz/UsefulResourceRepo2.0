...
function die(msg /*:string*/, ec /*:?number*/) /*:void*/ {
  console.error(msg);
  process.exit(ec || 0);
}
...
for (var i = 0; i < args.length; ++i) {
  ...
  switch (m) {
    ...
    case "--seed":
    case "-S":
      seed = parseInt(m != arg ? arg.substr(m.length + 1) : args[++i], r) || 1;
      break;

    default:
      die("adler32: unrecognized option `" + arg + "'", 22);
  }
}

if (!process.stdin.isTTY) filename = filename || "-";
if (filename.length === 0)
  die("adler32: must specify a filename ('-' for stdin)", 1);

var adler32 = seed;
// $FlowIgnore -- Writable is callable but type sig disagrees
var writable = require("stream").Writable();
writable._write = function (chunk, e, cb) {
  adler32 = X.buf(chunk, adler32);
  cb();
};
writable._writev = function (chunks, cb) {
  chunks.forEach(function (c) {
    adler32 = X.buf(c.chunk, adler32);
  });
  cb();
};
