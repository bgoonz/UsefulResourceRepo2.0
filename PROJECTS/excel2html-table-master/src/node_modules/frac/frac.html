...
<ul>
  ...
  <li>
    <code>mixed</code> if true, return a mixed fraction (default); if false,
    improper
  </li>
</ul>
...
<p>
  In this version, the return value is
  <code>[quotient, numerator, denominator]</code>, where
  <code>quotient == 0</code> for improper fractions. The interpretation is
  <code>x ~ quotient + numerator / denominator</code> where
  <code>0 &lt;= numerator &lt; denominator</code> and
  <code>quotient &lt;= x</code> for negative <code>x</code>.
</p>
<div class="sourceCode" id="cb1">
  <pre
    class="sourceCode js&gt;frac.js"
  ><code class="sourceCode javascript"><a class="sourceLine" id="cb1-1" title="1"><span class="co">/* frac.js (C) 2013-2014 SheetJS -- http://sheetjs.com */</span></a>
<a class="sourceLine" id="cb1-2" title="2"><span class="kw">var</span> frac <span class="op">=</span> <span class="kw">function</span>(x<span class="op">,</span> D<span class="op">,</span> mixed) <span class="op">{</span></a></code></pre>
</div>
<p>
  ...
  <code>floor(x) / 1</code> and the upper bound is
  <code>(floor(x) + 1) / 1</code>. We keep track of the numerators and
  denominators separately:
</p>
<pre><code>  var n1 = Math.floor(x), d1 = 1;
  var n2 = n1+1, d2 = 1;</code></pre>
<p>
  If <code>x</code> is not integral, we bisect using mediants until a
  denominator exceeds our target:
</p>
...
<p>
  At this point, <code>d1 &gt; D</code> or <code>d2 &gt; D</code> (but not both
  – keep track of how <code>d1</code> and <code>d2</code> change). So we merely
...
<p>
  ...
  Note that this technique is inferior to the mediant method (at least,
  according to the desired goal of most accurately approximating the floating
  point number)
</p>
...
<p>
  Note that the variables are implicitly indexed at <code>k</code> (so
  <code>B</code> refers to <code>b_k</code>):
</p>
...
<p>
  <code>A</code> should be the floor of <code>B</code>. Originally the bit-or
  trick was used, but this is not correct for the range
  <code>B&gt;=2**32</code>.
</p>
...
<blockquote>
  <p>
    … for k = 0,1,…,K, where K is the first instance of k where either q_{k+1}
    &gt; Q or b_{k+1} is undefined (b_k = a_k).
  </p>
</blockquote>
...
<div class="sourceCode" id="cb19">
  <pre
    class="sourceCode js&gt;test.js"
  ><code class="sourceCode javascript"><a class="sourceLine" id="cb19-1" title="1"><span class="kw">var</span> fs <span class="op">=</span> <span class="at">require</span>(<span class="st">&#39;fs&#39;</span>)<span class="op">,</span> assert <span class="op">=</span> <span class="at">require</span>(<span class="st">&#39;assert&#39;</span>)<span class="op">;</span></a>
...
<a class="sourceLine" id="cb19-43" title="43"><span class="op">}</span>)<span class="op">;</span></a></code></pre>
</div>
...
<a class="sourceLine" id="cb21-19" title="19"><span class="fu">}</span></a></code></pre>
</div>
