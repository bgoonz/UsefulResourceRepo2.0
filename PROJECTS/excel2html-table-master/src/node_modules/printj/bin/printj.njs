...
function parse_arg(arg /*:string*/) /*:any*/ {
  var m = arg.substr(2),
    p /*:number*/ = 0;
  if (arg.charCodeAt(1) === 58)
    switch ((p = arg.charCodeAt(0))) {
      case /*n*/ 110:
        return parseInt(m, 10);
      case /*f*/ 102:
        return parseFloat(m);
      case /*b*/ 98:
        return !(m.toUpperCase() === "FALSE" || m === "0");
      case /*j*/ 106:
        return JSON.parse(m);
      case /*e*/ 101:
        return eval(m);
...
for (var i = 2; i < argv.length; ++i)
  switch (argv[i]) {
    ...
    case "--dump":
    case "-d":
      if (fmt.length === 0) fmt = argv[++i];
      process.exit(dump(fmt));
      break;
    default:
      if (n++ === 0) fmt = argv[i];
      else args.push(parse_arg(argv[i]));
  }
...
function dump(fmt /*:string*/) /*:number*/ {
  if (!fmt) {
    console.error("printj: missing format argument");
    return 1;
  }
  X._tokenize(fmt).forEach(function (x) {
    console.log(x);
  });
  return 0;
}
