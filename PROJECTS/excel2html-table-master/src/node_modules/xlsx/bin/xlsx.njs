...
if (target_sheet === "") {
  if (program.sheetIndex < (wb.SheetNames || []).length)
    target_sheet = wb.SheetNames[program.sheetIndex];
...
try {
  ...
  console.error(
    n + ": error parsing " + filename + " " + target_sheet + ": " + e
  );
  process.exit(4);
}
...
if (program.formulae) oo = X.utils.get_formulae(ws).join("\n");
  ...
  oo = JSON.stringify(X.utils.sheet_to_row_object_array(ws));
else if (program.rawJs)
  oo = JSON.stringify(X.utils.sheet_to_row_object_array(ws, { raw: true }));
else if (program.arrays)
  oo = JSON.stringify(
    X.utils.sheet_to_row_object_array(ws, { raw: true, header: 1 })
  );
