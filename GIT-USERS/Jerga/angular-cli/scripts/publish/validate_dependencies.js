...
function reportMissingDependencies(missingDeps) {
  if (missingDeps.length == 0) {
    ...
    console.log(
      chalk.yellow(`  ${missingDeps.length} missing from package.json:`)
    );
...
function reportExcessiveDependencies(overDeps) {
  if (overDeps.length == 0) {
    ...
    console.log(
      chalk.yellow(
        `  ${overDeps.length} excessive dependencies in package.json:`
      )
    );
...
for (const packageName of Object.keys(packages)) {
  ...
  const allSources = glob
    .sync(path.join(__dirname, "../../packages/", packageName, "**/*"))
  ...
  allSources.forEach(function (filePath) {
    ...
    listImportedModules(source).forEach(
      (modulePath) => (importMap[modulePath] = true)
    );
    listRequiredModules(source).forEach(
      (modulePath) => (importMap[modulePath] = true)
    );
  });
  ...
  const packageJson = JSON.parse(
    fs.readFileSync(packages[packageName].packageJson, "utf8")
  );
  const allDeps = []
  ...
  const overDeps = allDeps
    .filter((d) => dependencies.indexOf(d) == -1)
...
const missingRootDeps = overallDeps
  .filter((d) => allRootDeps.indexOf(d) == -1)
  .filter((d) => internalPackages.indexOf(d) == -1);
reportMissingDependencies(missingRootDeps);

const overRootDeps = allRootDeps
  .filter((d) => overallDeps.indexOf(d) == -1)
  .filter((x) => ANGULAR_PACKAGES.indexOf(x) == -1);
