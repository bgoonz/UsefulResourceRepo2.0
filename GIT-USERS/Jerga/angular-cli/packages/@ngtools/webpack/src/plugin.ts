...
export class AotPlugin implements Tapable {
  ...
  private _setupOptions(options: AotPluginOptions) {
    ...
    try {
      ...
      throw new Error(
        `An error happened while parsing ${this._tsConfigPath} JSON: ${err}.`
      );
    }
  ...
  apply(compiler: any) {
    ...
    compiler.plugin("context-module-factory", (cmf: any) => {
      cmf.plugin(
        "before-resolve",
        (request: any, callback: (err?: any, request?: any) => void) => {
          if (!request) {
            return callback();
          }
            ...
            .then(
              () => {
                ...
                result.resolveDependencies =
                  createResolveDependenciesFromContextMap((_: any, cb: any) =>
                    cb(null, this._lazyRoutes)
                  );
    ...
    this._donePromise = Promise.resolve()
      ...
      .then(() => {
        ...
        this._compilerHost.populateWebpackResolver(
          this._compiler.resolvers.normal
        );
      })
      .then(() => {
        ...
        Object.keys(allLazyRoutes).forEach((k) => {
          const lazyRoute = allLazyRoutes[k];
          k = k.split("#")[0];
          if (this.skipCodeGeneration) {
            this._lazyRoutes[k] = lazyRoute;
          } else {
            const lr = path.relative(
              this.basePath,
              lazyRoute.replace(/\.ts$/, ".ngfactory.ts")
            );
