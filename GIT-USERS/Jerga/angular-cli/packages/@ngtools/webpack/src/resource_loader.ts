...
export class WebpackResourceLoader implements ResourceLoader {
  ...
  private _compile(filePath: string, content: string): Promise<any> {
    ...
    return new Promise((resolve, reject) => {
      childCompiler.runAsChild(
        (err: Error, entries: any[], childCompilation: any) => {
          ...
          if (
            ...
            const outputName =
              this._parentCompilation.mainTemplate.applyPluginsWaterfall(
                "asset-path",
            ...
            this._parentCompilation.assets[outputName] =
  ...
  private _evaluate(fileName: string, source: string): Promise<string> {
    try {
      const vmContext = vm.createContext(
        Object.assign({ require: require }, global)
      );
  ...
  get(filePath: string): Promise<string> {
    return this._compile(filePath, readFileSync(filePath, "utf8")).then(
      (result: any) => this._evaluate(result.outputName, result.content)
    );
  }
}
