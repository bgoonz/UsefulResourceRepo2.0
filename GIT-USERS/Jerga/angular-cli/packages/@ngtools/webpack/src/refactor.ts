...
export class TypeScriptFileRefactor {
  ...
  getDiagnostics(): ts.Diagnostic[] {
    ...
    let diagnostics: ts.Diagnostic[] = this._program
      .getSyntacticDiagnostics(this._sourceFile)
  ...
  findAstNodes(
    ...
    if (max > 0) {
      for (const child of node.getChildren(this._sourceFile)) {
        this.findAstNodes(child, kind, recursive, max).forEach(
          (node: ts.Node) => {
            if (max > 0) {
              arr.push(node);
            }
