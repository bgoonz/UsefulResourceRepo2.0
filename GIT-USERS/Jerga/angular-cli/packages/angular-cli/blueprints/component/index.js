...
module.exports = {
  ...
  normalizeEntityName: function (entityName) {
    ...
    var prefix =
      this.options.prefix === "false" || this.options.prefix === ""
        ? ""
    ...
    if (this.selector.indexOf("-") === -1) {
      this._writeStatusToUI(
        chalk.yellow,
        "WARNING",
        "selectors should contain a dash"
      );
    }
  ...
  locals: function (options) {
    ...
    options.inlineStyle =
      options.inlineStyle !== undefined
        ? options.inlineStyle
        : this.project.ngConfigObj.get("defaults.inline.style");

    options.inlineTemplate =
      options.inlineTemplate !== undefined
        ? options.inlineTemplate
    ...
    options.viewEncapsulation =
      options.viewEncapsulation !== undefined
        ? options.viewEncapsulation
  ...
  files: function () {
    ...
    if (this.options && !this.options.spec) {
      fileList = fileList.filter(
        (p) => p.indexOf("__name__.component.spec.ts") < 0
      );
    }
  ...
  afterInstall: function (options) {
    ...
    const componentDir = path.relative(
      path.dirname(this.pathToModule),
      this.generatePath
    );
    const importPath = componentDir
    ...
    if (!options.skipImport) {
      returns.push(
        astUtils
          .addDeclarationToModule(this.pathToModule, className, importPath)
