...
module.exports = {
  ...
  files: function () {
    ...
    if (!this.options || !this.options.spec) {
      fileList = fileList.filter(
        (p) => p.indexOf("__name__.module.spec.ts") < 0
      );
    }
    if (this.options && !this.options.routing) {
      fileList = fileList.filter(
        (p) => p.indexOf("__name__-routing.module.ts") < 0
      );
    }
  ...
  fileMapTokens: function (options) {
    ...
    return {
      __path__: () => {
        this.generatePath =
          this.dynamicPath.dir + path.sep + options.dasherizedModuleName;
  ...
  afterInstall: function (options) {
    ...
    if (this.options && this.options.routing) {
      var componentPath = path.join(
        this.generatePath,
        this.dasherizedModuleName
      );
      options.entity.name = path.relative(
        this.dynamicPath.appRoot,
        componentPath
      );
      options.flat = true;
      options.route = false;
      ...
      options.prefix = null;
      options.spec = true;
      return Blueprint.load(path.join(__dirname, "../component")).install(
        options
      );
    }
  },
