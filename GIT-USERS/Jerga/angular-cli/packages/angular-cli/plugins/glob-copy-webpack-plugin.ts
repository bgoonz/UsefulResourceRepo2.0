...
export class GlobCopyWebpackPlugin {
  ...
  apply(compiler: any): void {
    ...
    patterns = patterns.map((pattern) =>
      fs.statSync(path.resolve(context, pattern)).isDirectory()
        ? (pattern += "/**/*")
        : pattern
    );
    ...
    compiler.plugin("emit", (compilation: any, cb: any) => {
      ...
      let addAsset = (relPath: string) =>
        compilation.assets[relPath]
          ? // don't re-add to assets
            Promise.resolve()
          : statPromise(path.resolve(context, relPath)).then(
              (stat: any) =>
                (compilation.assets[relPath] = {
                  size: () => stat.size,
      ...
      Promise.all(globs)
        ...
        .then((relPaths) =>
          relPaths.forEach((relPath: string) => addAsset(relPath))
        )
