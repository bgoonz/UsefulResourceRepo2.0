...
var Watcher = Task.extend({
  ...
  didChange: function (results) {
    ...
    this.ui.writeLine("");
    this.ui.writeLine(
      chalk.green("Build successful - " + Math.round(totalTime) + "ms.")
    );
  },
  ...
  buildOptions: function () {
    ...
    if (
      watcher &&
      ["polling", "watchman", "node", "events"].indexOf(watcher) === -1
    ) {
      throw new Error(
        "Unknown watcher type --watcher=[polling|watchman|node] but was: " +
          watcher
      );
    }
...
Watcher.detectWatcher = function (ui, _options) {
  ...
  if (options.watcher === "polling") {
    ...
    return exec("watchman version").then(
      function (output) {
        var version;
        try {
        ...
        if (semver.satisfies(version, ">= 3.0.0")) {
          ...
          ui.writeLine(
            "Invalid watchman found, version: [" +
              version +
              "] did not satisfy [>= 3.0.0], falling back to NodeWatcher."
          );
          ui.writeLine(watchmanInfo);
          options.watcher = "node";
        }
