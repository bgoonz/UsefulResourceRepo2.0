...
export class ModuleResolver {
   ...
   */
  applySortedChangePromise(
    changes: Change[],
    host: Host = NodeHost
  ): Promise<void> {
    return changes
      .sort(
        (currentChange, nextChange) => nextChange.order - currentChange.order
      )
      .reduce(
  ...
  resolveDependentFiles(): Promise<Change[]> {
    return dependentFilesUtils
      .getDependentFiles(this.oldFilePath, this.rootPath)
      .then((files: dependentFilesUtils.ModuleMap) => {
        ...
        let relavantFiles = Object.keys(files).filter((file) => {
          if (path.extname(path.basename(file, ".ts")) === ".spec") {
            return (
              path.basename(path.basename(file, ".ts"), ".spec") !==
              fileBaseName
            );
          } else {
            return true;
        ...
        relavantFiles.forEach((file) => {
          let tempChanges: ReplaceChange[] = files[file].map((specifier) => {
            let componentName = path.basename(this.oldFilePath, ".ts");
            let fileDir = path.dirname(file);
            let changeText = path.relative(
              fileDir,
              path.join(this.newFilePath, componentName)
            );
  ...
  resolveOwnImports(): Promise<Change[]> {
    return dependentFilesUtils
      .createTsSourceFile(this.oldFilePath)
      .then((tsFile: ts.SourceFile) =>
        dependentFilesUtils.getImportClauses(tsFile)
      )
