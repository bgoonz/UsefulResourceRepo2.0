...
export default function findParentModule(
  ...
  const sourceRoot = path.join(
    project.root,
    project.ngConfig.apps[0].root,
    "app"
  );

  // trim currentDir
  currentDir = currentDir.replace(
    path.join(project.ngConfig.apps[0].root, "app"),
    ""
  );
  ...
  while (pathToCheck.length >= sourceRoot.length) {
    ...
    if (files.length === 1) {
      ...
      throw new SilentError(
        `Multiple module files found: ${pathToCheck.replace(sourceRoot, "")}`
      );
    }
