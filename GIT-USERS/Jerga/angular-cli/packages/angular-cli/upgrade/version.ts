...
function _hasOldCliBuildFile() {
  return (
    existsSync(_findUp("angular-cli-build.js", process.cwd())) ||
...
export class Version {
  ...
  static assertAngularVersionIs2_3_1OrHigher(projectRoot: string) {
    const angularCorePath = path.join(
      projectRoot,
      "node_modules/@angular/core"
    );
    const pkgJson = existsSync(angularCorePath)
      ? JSON.parse(
          readFileSync(path.join(angularCorePath, "package.json"), "utf8")
        )
      : null;
    ...
    if (pkgJson && pkgJson["version"]) {
      ...
      if (v.isLocal()) {
        console.warn(
          yellow("Using a local version of angular. Proceeding with care...")
        );
      } else {
