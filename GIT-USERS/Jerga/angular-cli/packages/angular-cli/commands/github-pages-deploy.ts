...
const githubPagesDeployCommand = Command.extend({
  ...
  run: function (options: GithubPagesDeployOptions, rawArgs: string[]) {
    ...
    let branchErrMsg =
      " You might also need to return to the initial branch manually.";

    // declared here so that tests can stub exec
    const execPromise = <(cmd: string, options?: any) => Promise<string>>(
      denodeify(exec)
    );
     ...
     */
    const baseHref =
      options.baseHref || (options.userPage ? null : `/${projectName}/`);
    ...
    function checkForPendingChanges() {
      return execPromise("git status --porcelain").then((stdout: string) => {
        if (/\w+/m.test(stdout)) {
          let msg =
            "Uncommitted file changes found! Please commit all changes before deploying.";
    ...
    function saveStartingBranchName() {
      return execPromise("git rev-parse --abbrev-ref HEAD").then(
        (stdout: string) => (initialBranch = stdout.replace(/\s/g, ""))
      );
    }

    function createGitHubRepoIfNeeded() {
      return execPromise("git remote -v").then(function (stdout) {
        if (!/origin\s+(https:\/\/|git@)github\.com/m.test(stdout)) {
          return createGithubRepoTask.run(createGithubRepoOptions).then(() => {
    ...
    function checkoutGhPages() {
      return execPromise(`git checkout ${ghPagesBranch}`).catch(
        createGhPagesBranch
      );
    }
    ...
    function createGhPagesBranch() {
      return execPromise(`git checkout --orphan ${ghPagesBranch}`)
        ...
        .then(() =>
          execPromise(`git commit -m \"initial ${ghPagesBranch} commit\"`)
        );
    }

    function cleanGhPagesBranch() {
      return execPromise("git ls-files").then(function (stdout) {
        let files = "";
        stdout.split(/\n/).forEach(function (f) {
        ...
        return execPromise(`git rm -r ${files}`).catch(() => {
    ...
    function copyFiles() {
      return fsReadDir(outDir).then((files: string[]) =>
        Promise.all(
          files.map((file) => {
            if (file === ".gitignore") {
      ...
      return execPromise("git add .", execOptions)
        ...
        .catch(() => {
          ...
          return returnStartingBranch()
            ...
            .catch(() =>
              Promise.reject(new SilentError(msg.concat(branchErrMsg)))
            );
        });
    }
    ...
    function pushToGitRepo() {
      return execPromise(
        `git push origin ${ghPagesBranch}:${destinationBranch}`
      ).catch((err) => returnStartingBranch().catch(() => Promise.reject(err)));
    }

    function printProjectUrl() {
      return execPromise("git remote -v").then((stdout) => {
        let match = stdout.match(
          /origin\s+(?:https:\/\/|git@)github\.com(?:\:|\/)([^\/]+)/m
        );
        let userName = match[1].toLowerCase();
        let url = `https://${userName}.github.io/${
          options.userPage ? "" : baseHref + "/"
        }`;
    ...
    function failGracefully(error: Error) {
      if (
        error &&
        (/git clean/.test(error.message) ||
          /Permission denied/.test(error.message))
      ) {
        ui.writeLine(error.message);
        let msg =
          "There was a permissions error during git file operations, " +
