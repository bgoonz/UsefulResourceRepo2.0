...
describe("Change", () => {
  ...
  describe("ReplaceChange", () => {
    it("replaces the given text in the source code", () => {
      let sourceFile = path.join(sourcePath, "remove-replace-file.txt");
      let changeInstance = new ReplaceChange(
        sourceFile,
        7,
        "* as foo",
        "{ fooComponent }"
    ...
    it("fails for negative position", () => {
      let sourceFile = path.join(sourcePath, "remove-replace-file.txt");
      expect(
        () => new ReplaceChange(sourceFile, -6, "hello", " world!")
      ).toThrow();
    });
    it("fails for invalid replacement", () => {
      ...
      return changeInstance.apply(NodeHost).then(
      ...
      return changeInstance
        ...
        .then((contents) => {
          expect(contents).toEqual(
            'import { BarComponent, FooComponent } from "./baz"'
          );
        });
    });
