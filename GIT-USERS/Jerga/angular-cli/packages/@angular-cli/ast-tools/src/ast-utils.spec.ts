...
describe("ast-utils: findNodes", () => {
  ...
  it("finds no imports", () => {
    let editedFile = new RemoveChange(
      sourceFile,
      0,
...
describe("ast-utils: insertAfterLastOccurrence", () => {
  ...
  it("inserts after last import", () => {
    ...
    return editedFile
      ...
      .then((newContent) =>
        expect(newContent).toEqual(`import { foo, bar, baz } from 'fizz';`)
      );
  });
  ...
  it("inserts correctly if no imports", () => {
    ...
    return editedFile
      ...
      .then((newContent) => {
        ...
        let pos = findNodes(
          imports.sort((a, b) => a.pos - b.pos).pop(),
