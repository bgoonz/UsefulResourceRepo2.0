...
describe("Acceptance: ng generate directive", function () {
  ...
  it(
    path.sep + "my-dir from a child dir, gens under " + path.join("src", "app"),
    () => {
      fs.mkdirsSync(path.join(root, "tmp", "foo", "src", "app", "1"));
      return new Promise(function (resolve) {
        process.chdir("./src");
    ...
    return ng(["generate", "directive", ".." + path.sep + "my-dir"]).then(
      () => {
        throw new SilentError(
          `ng generate directive ..${path.sep}my-dir from root dir should fail.`
        );
      },
      (err) => {
        expect(err).to.equal(
          `Invalid path: "..${path.sep}my-dir" cannot be above the "src${path.sep}app" directory`
        );
      }
    );
  });
