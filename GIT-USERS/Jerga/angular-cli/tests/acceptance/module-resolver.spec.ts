...
describe("ModuleResolver", () => {
        ...
        "foo-baz": {
          qux: {
            quux: {
              ...
              fooqux: {
                "fooqux.component.ts":
                  'import * from                 "../foobar/foobar.component"',
              },
            },
          },
  ...
  describe("Rewrite imports", () => {
    ...
    let fooBarFile = path.join(
      rootPath,
      "foo-baz/qux/quux/foobar/foobar.component.ts"
    );
    let fooQuxFile = path.join(
      rootPath,
      "foo-baz/qux/quux/fooqux/fooqux.component.ts"
    );
    ...
    it("when there is no index.ts in oldPath", () => {
      ...
      return resolver
        ...
        .then((tsFileBaz: ts.SourceFile) => {
          ...
          let fooBarExpectedContent = path.normalize(
            "../foo-baz/qux/quux/foobar/foobar.component"
          );
    ...
    it("when there are multiple spaces between symbols and specifier", () => {
      ...
      return resolver
        ...
        .then((tsFileFooQux: ts.SourceFile) => {
          let contentsFooQux =
            dependentFilesUtils.getImportClauses(tsFileFooQux);
          let fooQuxExpectedContent = path.normalize(
            "../../../../foo/foobar.component"
          );
          expect(contentsFooQux[0].specifierText).to.equal(
            fooQuxExpectedContent
          );
        })
        .then(() => dependentFilesUtils.createTsSourceFile(bazFile))
        .then((tsFileBaz: ts.SourceFile) => {
        ...
        .then((tsFileFooBar: ts.SourceFile) => {
          let contentsFooBar =
            dependentFilesUtils.getImportClauses(tsFileFooBar);
          let fooExpectedContent = `.${path.sep}foo.component`;
          let fooQuxExpectedContent = path.normalize(
            "../foo-baz/qux/quux/fooqux.component"
          );
          expect(contentsFooBar[0].specifierText).to.equal(fooExpectedContent);
          expect(contentsFooBar[1].specifierText).to.equal(
            fooQuxExpectedContent
          );
        });
    });
