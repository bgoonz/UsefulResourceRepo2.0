      ...
      return ng(["generate", "service", ".." + path.sep + "my-svc"]).then(
        () => {
          throw new SilentError(
            `ng generate service ..${path.sep}my-svc from root dir should fail.`
          );
        },
        (err) => {
          expect(err).to.equal(
            `Invalid path: "..${path.sep}my-svc" cannot be above the "src${path.sep}app" directory`
          );
        }
      );
    }
