...
class GridTile extends Component {
  ...
  render() {
      ...
      titleBar = (
        <div key="titlebar" style={prepareStyles(styles.titleBar)}>
          <div style={prepareStyles(styles.titleWrap)}>
            ...
            {subtitle ? (
              <div style={prepareStyles(styles.subtitle)}>{subtitle}</div>
            ) : null}
          </div>
          {actionIcon ? (
            <div style={prepareStyles(styles.actionIcon)}>{actionIcon}</div>
          ) : null}
        </div>
      );
    }
      ...
      newChildren = React.Children.map(children, (child) => {
        if (child.type === "img") {
          return React.cloneElement(child, {
            ...
            ref: "img",
            style: prepareStyles(
              Object.assign({}, styles.childImg, child.props.style)
            ),
          });
        } else {
          return child;
    ...
    return React.isValidElement(containerElement)
      ? React.cloneElement(containerElement, containerProps, [
          newChildren,
          titleBar,
        ])
      : React.createElement(containerElement, containerProps, [
          newChildren,
          titleBar,
        ]);
  }
