...
describe("<AutoComplete />", () => {
  ...
  describe("filter", () => {
    it("search using fuzzy filter", () => {
      assert.strictEqual(
        AutoComplete.fuzzyFilter("ea", "Peach"),
        true,
        "should match Peach with ea"
      );
      assert.strictEqual(
        AutoComplete.fuzzyFilter("pah", "Peach"),
        true,
        "should match Peach with pah"
      );
      assert.strictEqual(
        AutoComplete.fuzzyFilter("peach", "Peach"),
        true,
        "should match Peach with peach"
      );

      assert.strictEqual(
        AutoComplete.fuzzyFilter("phc", "Peach"),
        false,
        "should not match Peach with phc"
      );
      assert.strictEqual(
        AutoComplete.fuzzyFilter("pp", "Peach"),
        false,
        "should not match Peach with pp"
      );
      assert.strictEqual(
        AutoComplete.fuzzyFilter("pb", "Peach"),
        false,
        "should not match Peach with pb"
      );
      ...
      search_result = AutoComplete.fuzzyFilter(
        "The a Boolean if wrong",
        test_string
      );
      assert.strictEqual(
        search_result,
        true,
        "should match pattern with spaces"
      );
      ...
      assert.strictEqual(
        search_result,
        true,
        "should match pattern with comma and period"
      );
      ...
      assert.strictEqual(
        search_result,
        false,
        "should not match pattern with letter is not contained in search text"
      );
      ...
      assert.strictEqual(
        search_result,
        false,
        "should not match pattern when can not find letters in order "
      );
    });
    ...
    it("should not pass filter property to children", () => {
      const wrapper = shallowWithContext(<AutoComplete dataSource={[]} />);
  ...
  describe("prop: popoverProps", () => {
    it("should pass popoverProps to Popover", () => {
      ...
      assert.strictEqual(
        popoverProps.zDepth,
        3,
        "should pass popoverProps to Popover"
      );
      assert.strictEqual(
        popoverProps.canAutoPosition,
        true,
        "should overrides the default"
      );
    });
  });
});
