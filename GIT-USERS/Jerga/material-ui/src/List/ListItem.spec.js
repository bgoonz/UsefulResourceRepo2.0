...
describe("<ListItem />", () => {
  ...
  it("should render an EnhancedButton", () => {
    const wrapper = shallowWithContext(<ListItem />);
  ...
  it("should display a list-item with text if primaryText is specified", () => {
    ...
    const wrapper = shallowWithContext(<ListItem primaryText={testText} />);
    const enhancedButton = wrapper.find(EnhancedButton);
  ...
  it("should display a list-item elment with a class if specified", () => {
    ...
    const wrapper = shallowWithContext(<ListItem className={testClass} />);
    const enhancedButton = wrapper.find(EnhancedButton);
    assert.strictEqual(enhancedButton.prop("className"), testClass);
  });

  it("should display a disabled list-item if specified.", () => {
    const wrapper = shallowWithContext(<ListItem disabled={true} />);
    assert.notOk(
      wrapper.find(EnhancedButton).length,
      "should not have an EnhancedButton"
    );
  });
  ...
  it("should display a disabled list-item with a class if specified.", () => {
    ...
    const wrapper = shallowWithContext(
      <ListItem className={testClass} disabled={true} />
    );
    ...
    assert.strictEqual(
      wrapper.find(`.${testClass}`).length,
      1,
      "should have a div with the test class"
    );
  });
  ...
  describe("prop: open", () => {
    it("should initially open nested list", () => {
      const wrapper = shallowWithContext(
        <ListItem initiallyOpen={true} nestedItems={[<ListItem key={1} />]} />
      );
    ...
    it("should toggle nested list", () => {
      const wrapper = shallowWithContext(
        <ListItem open={false} nestedItems={[<ListItem key={1} />]} />
      );
  ...
  describe("prop: hoverColor", () => {
    it("should use a background color on hover if hoverColor is specified", () => {
      ...
      const wrapper = shallowWithContext(<ListItem hoverColor={testColor} />);
      wrapper.find(EnhancedButton).simulate("mouseEnter");
      assert.strictEqual(
        wrapper.find(EnhancedButton).props().style.backgroundColor,
        testColor
      );
    });
  });
  ...
  describe("hover state", () => {
    it("should reset the hover state when disabled", () => {
      const wrapper = shallowWithContext(<ListItem primaryText="foo" />);

      wrapper.find(EnhancedButton).simulate("mouseEnter");
      assert.strictEqual(
        wrapper.state().hovered,
        true,
