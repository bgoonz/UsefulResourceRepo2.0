...
describe("<Avatar />", () => {
  ...
  it("renders children by default", () => {
    const wrapper = shallowWithContext(<Avatar>{testChildren}</Avatar>);
  ...
  it("only renders an image when the src prop is set", () => {
    ...
    assert.notOk(
      !wrapper.contains(testChildren),
      "should not contain the children"
    );
    assert.ok(wrapper.is("img"), "should be an image");
    assert.ok(
      wrapper.is({ src: "face.jpg" }),
      "should have the src passed into props"
    );
