...
describe("<AppBar />", () => {
  ...
  it("renders children by default", () => {
    ...
    const wrapper = shallowWithContext(<AppBar>{testChildren}</AppBar>);

    assert.ok(wrapper.contains(testChildren), "should contain the children");
  });

  it("renders className", () => {
    const wrapper = shallowWithContext(<AppBar className="testClassName" />);
  ...
  it("renders iconClassNameLeft", () => {
    ...
    assert.strictEqual(
      wrapper.find(IconButton).get(0).props.iconClassName,
      iconClassName,
  ...
  it("renders iconClassNameRight", () => {
    ...
    assert.strictEqual(
      wrapper.find(IconButton).get(1).props.iconClassName,
      iconClassName,
  ...
  describe("iconElementLeft", () => {
    it("renders the node", () => {
      ...
      assert.strictEqual(
        ...
        1,
        "should contain iconElementLeft"
      );
    });
  ...
  it("renders iconElementRight", () => {
    ...
    assert.strictEqual(
      wrapper.find(".icon").length,
      1,
      "should contain iconElementRight"
    );
  });
  ...
  describe("onLeftIconButtonTouchTap", () => {
    it("should trigger the onTouchTap", () => {
      ...
      wrapper.find(IconButton).simulate("touchTap");
      assert.strictEqual(
        onLeftIconButtonTouchTap.callCount,
        1,
  ...
  describe("onRightIconButtonTouchTap", () => {
    it("should trigger the onTouchTap", () => {
      ...
      const wrapper = shallowWithContext(
        <AppBar
          onRightIconButtonTouchTap={handleRightIconButtonTouchTap}
          iconClassNameRight="foo"
        />
      );

      wrapper.find(IconButton).at(1).simulate("touchTap");
      assert.strictEqual(
        handleRightIconButtonTouchTap.callCount,
        1,
  ...
  it("renders AppBar and overwrite styles", () => {
    ...
    const wrapper = shallowWithContext(<AppBar title="Title" style={style} />);
  ...
  it("renders title", () => {
    const wrapper = shallowWithContext(<AppBar title="Title" />);
  ...
  it("renders title and overwrite title styles", () => {
    ...
    assert.strictEqual(wrapper.find("h1").length, 1, "should have title");
    assert.strictEqual(
      wrapper.find("h1").get(0).props.style.backgroundColor,
  ...
  it("renders zDepth to paper component", () => {
    const wrapper = shallowWithContext(<AppBar title="Title" zDepth={2} />);

    assert.strictEqual(
      wrapper.find("Paper").get(0).props.zDepth,
      2,
      "should have zDepth to 2"
    );
  });

  it("menuElementLeft's style should be iconButtonStyle", () => {
    const wrapper = shallowWithContext(<AppBar />);

    const menuElementLeft = wrapper.find(IconButton).get(0);
    const style = menuElementLeft.props.style;
    const iconButtonStyle = getStyles(
      wrapper.props(),
      wrapper.context()
    ).iconButtonStyle;
  ...
  it("if pass iconElementLeft and iconStyleLeft={marginTop}, change the marginTop/muiPrepared only", () => {
    ...
    const expectedStyle = Object.assign({}, iconButtonStyle, {
      marginTop: 99,
      muiPrepared: true,
    });
    assert.deepEqual(
