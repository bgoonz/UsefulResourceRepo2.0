...
describe("<Paper />", () => {
  ...
  it("renders children by default", () => {
    const wrapper = shallowWithContext(<Paper>{testChildren}</Paper>);
  ...
  it("renders children and have borderRadius by default", () => {
    ...
    assert.ok(wrapper.contains(testChildren), "should contain the children");
    assert.strictEqual(
      wrapper.prop("style").borderRadius,
      "2px",
  ...
  it("renders children and overwrite styles", () => {
    ...
    assert.strictEqual(
      ...
      "red",
      "should have red backgroundColor"
    );
    assert.strictEqual(
      wrapper.prop("style").borderRadius,
      "70px",
      "should have borderRadius"
    );
  });
  ...
  it("renders children and disable css transitions", () => {
    ...
    assert.ok(wrapper.contains(testChildren), "should contain the children");
    assert.isNotOk(
      wrapper.prop("style").transition,
      "should not have css transitions"
    );
  });
  ...
  it("renders children and change zDepth", () => {
    ...
    assert.ok(wrapper.contains(testChildren), "should contain the children");
    assert.strictEqual(
      wrapper.prop("style").boxShadow,
      muiTheme.paper.zDepthShadows[zDepth - 1],
