...
describe("<Chip />", () => {
  ...
  describe("state", () => {
    const wrapper = themedShallow(<Chip onTouchTap={() => {}}>Label</Chip>);
  ...
  describe("rendering", () => {
    it("renders an EnhancedButton", () => {
      const wrapper = themedShallow(<Chip>Label</Chip>);
      assert.ok(wrapper.is("EnhancedButton"));
    });

    it("renders children", () => {
      const wrapper = themedShallow(<Chip>Hello world</Chip>);
    ...
    it("renders a delete icon after the label when onRequestDelete is provided", () => {
      const wrapper = themedShallow(<Chip onRequestDelete={spy()}>Label</Chip>);
  ...
  describe("callbacks", () => {
    ...
    it("bubbles callbacks used internally", () => {
      ...
      events.forEach((event) => {
        handlers[event] = spy();
        props[`on${event.charAt(0).toUpperCase() + event.slice(1)}`] =
          handlers[event];
      });

      const wrapper = themedShallow(<Chip {...props}>Step One</Chip>);

      events.forEach((event) => {
        wrapper.simulate(event, { stopPropagation() {} });
        assert.ok(
          handlers[event].calledOnce,
          `should trigger the ${event} callback`
        );
      });
    });
  });
