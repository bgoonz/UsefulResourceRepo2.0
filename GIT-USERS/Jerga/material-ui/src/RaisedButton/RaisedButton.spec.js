...
describe("<RaisedButton />", () => {
  ...
  it("renders an enhanced button inside paper", () => {
    const wrapper = shallowWithContext(<RaisedButton>Button</RaisedButton>);
  ...
  it("renders a hover overlay of equal height to the button", () => {
    const wrappers = [
      () => mountWithContext(<RaisedButton>Hello World</RaisedButton>),
      () =>
        mountWithContext(
          <RaisedButton backgroundColor="#a4c639" icon={<ActionAndroid />} />
        ),
    ];
    ...
    wrappers.forEach((createWrapper) => {
      ...
      const button = ReactDOM.findDOMNode(
        TestUtils.findRenderedDOMComponentWithTag(wrapper.instance(), "button")
      );
  ...
  it("inherits fontSize from theme", () => {
    const wrapper = shallowWithContext(<RaisedButton label="test" />);
  ...
  it("if an svg icon is provided, renders the icon with the correct color", () => {
    ...
    const wrapper = shallowWithContext(<RaisedButton icon={icon} />);

    const svgIcon = wrapper.find("svg");
    assert.strictEqual(svgIcon.length, 1, "should have an svg icon");
    assert.strictEqual(
      svgIcon.node.props.color,
      "red",
      "should have color set as the prop"
    );
  });
  ...
  describe("validateLabel", () => {
    ...
    it("should throw when using wrong label", () => {
      assert.strictEqual(
        validateLabel({}, "label", "RaisedButton").message,
  ...
  describe("hover state", () => {
    it("should reset the hover state when disabled", () => {
      const wrapper = shallowWithContext(<RaisedButton label="foo" />);
