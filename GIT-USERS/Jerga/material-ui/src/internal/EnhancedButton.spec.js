...
describe("<EnhancedButton />", () => {
  ...
  it("renders a button", () => {
    const wrapper = shallowWithContext(<EnhancedButton>Button</EnhancedButton>);
  ...
  it("renders a disabled button when disabled={true} which blocks onTouchTap from firing", () => {
    ...
    assert.ok(wrapper.text(), "Button", "should say Button");
    assert.ok(
      wrapper.is("button[disabled]"),
      "should be a disabled button element"
    );
  ...
  it("renders a dummy link button when disabled={true} which blocks onTouchTap from firing", () => {
    const wrapper = shallowWithContext(
      <EnhancedButton disabled={true} href="http://google.com">
        Button
      </EnhancedButton>
    );
    assert.ok(wrapper.text(), "Button", "should say Button");
  ...
  it("overrides default styles", () => {
    const wrapper = shallowWithContext(<EnhancedButton>Button</EnhancedButton>);
    assert.strictEqual(
      wrapper.node.props.style.background,
      "none",
      "should be none"
    );
    wrapper.setProps({
  ...
  it('should not have "background: none" style when passed a backgroundColor', () => {
    ...
    assert.strictEqual(
      ...
      "blue",
      "backgroundColor should be blue"
    );
  ...
  it("should set the button type", () => {
    ...
    assert.ok(wrapper.text(), "Button", "should say Button");
    assert.ok(
      wrapper.is('button[type="submit"]'),
      "should have the type attribute"
    );
    wrapper.setProps({ type: "reset" });
    assert.ok(
      wrapper.is('button[type="reset"]'),
      "should have the type attribute"
    );
  });
  ...
  it("has no TouchRipple when disableTouchRipple={true}", () => {
    ...
    assert.strictEqual(
      wrapper.find("TouchRipple").length,
      0,
      "should not have a TouchRipple"
    );
  });

  it("have a FocusRipple when keyboard focused (tracked internally) and control it using props", () => {
    const wrapper = shallowWithContext(
      <EnhancedButton focusRippleColor="red" focusRippleOpacity={0.8}>
        Button
      </EnhancedButton>
    );

    assert.strictEqual(
      wrapper.find("FocusRipple").length,
      0,
      "should not have a FocusRipple"
    );
  ...
  it("should have no ripples when button is disabled", () => {
    const wrapper = shallowWithContext(
      <EnhancedButton keyboardFocused={true} disabled={true}>
        Button
      </EnhancedButton>
    );
    assert.strictEqual(
  ...
  it("should fire the touchtap handler if keyboard focused and the enter or space keys are hit", () => {
    ...
    assert.strictEqual(
      eventStack.length,
      2,
      "should not work when button is disabled"
    );
  });
});
