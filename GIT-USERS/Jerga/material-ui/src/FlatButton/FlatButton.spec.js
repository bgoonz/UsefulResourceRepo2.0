...
describe("<FlatButton />", () => {
  ...
  it("renders an enhanced button", () => {
    const wrapper = shallowWithContext(<FlatButton>Button</FlatButton>);
    assert.ok(wrapper.is("EnhancedButton"));
  });

  it("renders children", () => {
    const wrapper = shallowWithContext(<FlatButton>{testChildren}</FlatButton>);
  ...
  it("renders a label with an icon before", () => {
    const wrapper = shallowWithContext(
      <FlatButton icon={<span className="test-icon" />} label="Hello" />
    );
  ...
  it("colors the icon with the passed color in prop", () => {
    ...
    assert.strictEqual(
      icon.prop("color"),
      color,
      "icon should have same color as that of color prop"
    );
  });

  it("colors the button the secondary theme color", () => {
    const wrapper = shallowWithContext(
      <FlatButton secondary={true} icon={<span className="test-icon" />}>
        Button
      </FlatButton>
    );
    assert.ok(wrapper.is("EnhancedButton"));
  ...
  it("overrides the ripple color via props", () => {
    ...
    assert.strictEqual(
      wrapper.node.props.focusRippleColor,
      "yellow",
      "should be yellow"
    );
    assert.strictEqual(
      wrapper.node.props.touchRippleColor,
      "yellow",
      "should be yellow"
    );
  });
  ...
  describe("validateLabel", () => {
    ...
    it("should throw when using wrong label", () => {
      assert.strictEqual(
        validateLabel({}, "label", "FlatButton").message,
  ...
  describe("hover state", () => {
    it("should reset the hover state when disabled", () => {
      const wrapper = shallowWithContext(<FlatButton label="foo" />);
