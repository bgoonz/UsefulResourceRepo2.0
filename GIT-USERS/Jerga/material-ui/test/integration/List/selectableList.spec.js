...
describe("makeSelectable", () => {
  ...
  const testChildren = [
    <ListItem
      ...
      nestedItems={[<ListItem value={2} primaryText="Grace Ng" />]}
    />,
    <ListItem key={3} value={3} primaryText="Kerem Suer" />,
  ];
  ...
  it("should display the children", () => {
    ...
    const wrapper = shallowWithContext(
      <SelectableList>{testChildren}</SelectableList>
    );
  ...
  it("should select the right item", () => {
    ...
    const render = TestUtils.renderIntoDocument(
      <SelectableList value={2}>{testChildren}</SelectableList>
    );

    const nodeTree = TestUtils.scryRenderedDOMComponentsWithTag(render, "div");
    assert.equal(
      nodeTree[0].firstChild.lastChild.querySelector("span").style
        .backgroundColor,
      "rgba(0, 0, 0, 0.2)",
