...
describe("Vertical Stepper", () => {
  ...
  const wrapper = mountWithContext(<VerticalLinearStepper />);
  ...
  describe("initial rendering", () => {
    ...
    describe("steps", () => {
      ...
      it("should have an active 1st step", () => {
        assert.strictEqual(
          wrapper.find({ "data-test-content": 0 }).length,
          1,
          "step 1 content should be visible"
        );
        assert.strictEqual(
          wrapper.find({ "data-test-content": 1 }).length,
          0,
          "step 2 content should not be visible"
        );
        assert.strictEqual(
          wrapper.find({ "data-test-content": 2 }).length,
          0,
