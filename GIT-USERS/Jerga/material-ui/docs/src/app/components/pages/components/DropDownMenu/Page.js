...
const descriptions = {
  simple:
    "`DropDownMenu` is implemented as a controlled component, with the current selection set through the " +
    "`value` property.",
  openImmediate:
    "With `openImmediately` property set, the menu will open on mount.",
  long:
    "With the `maxHeight` property set, the menu will be scrollable if the number of items causes the height " +
    "to exceed this limit.",
  label:
    "With a `label` applied to each `MenuItem`, `DropDownMenu` displays a complementary description " +
