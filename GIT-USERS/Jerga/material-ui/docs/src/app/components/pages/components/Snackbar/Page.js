...
const descriptions = {
  simple:
    "`Snackbar` is a controlled component, and is displayed when `open` is `true`. Click away from the " +
    "Snackbar to close it, or wait for `autoHideDuration` to expire.",
  action:
    "A single `action` can be added to the Snackbar, and triggers `onActionTouchTap`. Edit the textfield to " +
    "change `autoHideDuration`",
  consecutive:
    "Changing `message` causes the Snackbar to animate - it isn't necessary to close and reopen the " +
