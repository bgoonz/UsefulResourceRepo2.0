...
def extract_adjoined_candidates(
    sentence_list, stoplist, min_keywords, max_keywords, min_freq
):
    adjoined_candidates = []
    for s in sentence_list:
        ...
        adjoined_candidates += adjoined_candidates_from_sentence(
            s, stoplist, min_keywords, max_keywords
        )
    ...
    for num_keywords in range(min_keywords, max_keywords + 1):
        ...
        for i in range(0, len(sl) - num_keywords):
            ...
            if sl[i] not in stoplist:
                ...
                if (
                    contains_stopword
                    and candidate.split()[-1] not in stoplist
...
if test and __name__ == "__main__":
    ...
    phraseList = generate_candidate_keywords(
        sentenceList, stopwordpattern, load_stop_words(stoppath)
    )
