...
def _get_raw_docstring(py_object):
  ...
  """
    ...
    if (
        ...
        or tf_inspect.ismethod(py_object)
  ...
  """
    ...
    """
        ...
        for name, obj in visitor.index.items():
            has_page = (
                tf_inspect.isclass(obj)
                or tf_inspect.ismodule(obj)
                or is_free_function(obj, name, visitor.index)
            )
    ...
    """
        ...
        if master_name not in self._all_names:
            raise TFDocsError('Cannot make link to "%s": Not in index.' % master_name)
    ...
    def _one_ref(self, match, relative_path_to_root):
        ...
        # Error!
        raise TFDocsError('Did not understand "%s"' % match.group(0), "BROKEN_LINK")
    ...
    def _cc_link(self, string, link_text, relative_path_to_root):
        ...
        return '<a href="{}"><code>{}</code></a>'.format(cc_relative_path, link_text)
...
_DocstringInfo = collections.namedtuple(
    "_DocstringInfo", ["brief", "docstring", "function_details", "compatibility"]
)
                    ...
                    if match:
                        ...
                        for internal_name, public_name in six.iteritems(internal_names):
                            if match.group(0).startswith(internal_name):
                                lookup_text = (
                                    public_name + default_text[len(internal_name) :]
                                )
                                break
...
_MethodInfo = collections.namedtuple(
    "_MethodInfo", ["short_name", "full_name", "obj", "doc", "signature", "decorators"]
)
  ...
  """
    ...
    def get_metadata_html(self):
        ...
        for item in itertools.chain(
            self.classes, self.properties, self.methods, self.other_members
        ):
    ...
    """
        ...
        for short_name in parser_config.tree[self.full_name]:
            ...
            child_doc = _parse_md_docstring(
                child, relative_path, parser_config.reference_resolver
            )
            ...
            if isinstance(child, property):
                ...
                tf_inspect.ismethod(child)
                or tf_inspect.isfunction(child)
                ...
                if issubclass(py_class, ProtoMessage):
                    if short_name.endswith("_FIELD_NUMBER") or short_name in [
                        "__slots__",
                        "DESCRIPTOR",
                    ]:
                        continue
...
class _ModulePageInfo(object):
    ...
    def _add_other_member(self, short_name, full_name, obj, doc):
        self._other_members.append(_OtherMemberInfo(short_name, full_name, obj, doc))
    ...
    """
        ...
        for name in member_names:
            ...
            member_doc = _parse_md_docstring(
                member, relative_path, parser_config.reference_resolver
            )
...
def docs_for_object(full_name, py_object, parser_config):
  ...
  """
    ...
    if (
        ...
        raise RuntimeError(
            "Cannot make docs for object %s: %r" % (full_name, py_object)
        )
    ...
    page_info.set_doc(
        _parse_md_docstring(py_object, relative_path, parser_config.reference_resolver)
    )
