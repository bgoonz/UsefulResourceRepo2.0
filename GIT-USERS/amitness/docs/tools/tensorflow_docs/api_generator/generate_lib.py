  ...
  """
    ...
    for full_name, py_object in six.iteritems(parser_config.index):
        ...
        if not (
            ...
            or parser.is_free_function(py_object, full_name, parser_config.index)
        ):
            continue

        sitepath = os.path.join(
            "api_docs/python", parser.documentation_path(full_name)[:-3]
        )
        ...
        for dup in duplicates:
            from_path = os.path.join(
                site_path, "api_docs/python", dup.replace(".", "/")
            )
            to_path = os.path.join(
                site_path, "api_docs/python", full_name.replace(".", "/")
            )
            redirects.append((os.path.join("/", from_path), os.path.join("/", to_path)))
...
def extract(
    ...
    api_visitor = public_api.PublicAPIVisitor(
        visitor, base_dir, private_map, do_not_descend_map
    )
  ...
  """
    ...
    for dirpath, _, filenames in os.walk(src_dir):
        ...
        for base_name in filenames:
            ...
            content = reference_resolver.replace_references(
                content, relative_path_to_root
            )
