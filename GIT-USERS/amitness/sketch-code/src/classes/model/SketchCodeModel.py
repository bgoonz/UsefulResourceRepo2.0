...
class SketchCodeModel:
    def __init__(
        self, model_output_path, model_json_file=None, model_weights_file=None
    ):
    ...
    def save_model(self):
        ...
        with open(
            "{}/model_json.json".format(self.model_output_path), "w"
        ) as json_file:
    ...
    def create_model(self):
        ...
        language_model = Embedding(
            vocab_size, 50, input_length=MAX_LENGTH, mask_zero=True
        )(language_input)
    ...
    def construct_callbacks(self, validation_path):
        checkpoint_filepath = (
            "{}/".format(self.model_output_path)
            + "weights-epoch-{epoch:04d}--val_loss-{val_loss:.4f}--loss-{loss:.4f}.h5"
        )
        csv_logger = CSVLogger(
            "{}/training_val_losses.csv".format(self.model_output_path)
        )
