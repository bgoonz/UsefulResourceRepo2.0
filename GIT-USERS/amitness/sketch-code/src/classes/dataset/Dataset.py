...
class Dataset:
    ...
    def preprocess_data(self, training_path, validation_path, augment_training_data):
        ...
        train_img_preprocessor.build_image_dataset(
            training_path, augment_data=augment_training_data
        )
    ...
    def create_generator(cls, data_input_path, max_sequences):
        ...
        data_gen = Dataset.data_generator(
            text_features, img_features, max_sequences, tokenizer, vocab_size
        )
        return data_gen, steps_per_epoch

    @classmethod
    def data_generator(
        cls, text_features, img_features, max_sequences, tokenizer, vocab_size
    ):
        while 1:
    ...
    @classmethod
    def process_data_for_generator(
        cls, texts, features, max_sequences, tokenizer, vocab_size
    ):
    ...
    def copy_files_to_folders(self, sample_ids, output_folder):
        ...
        for sample_id in sample_ids:
            ...
            if os.path.exists(sample_id_png_path) and os.path.exists(
                sample_id_gui_path
            ):
                ...
                copied_count += 1
        print(
            "Moved {} files from {} to {}".format(
                copied_count, self.data_input_folder, output_folder
            )
        )
    ...
    def populate_sample_ids(self):
        ...
        for f in os.listdir(full_path):
            if f.find(".gui") != -1:
                ...
                if os.path.isfile(
                    "{}/{}.png".format(self.data_input_folder, file_name)
                ):
    ...
    def get_all_id_sets(self, validation_split, sample_ids):
        ...
        print(
            "Splitting datasets, training samples: {}, validation samples: {}".format(
