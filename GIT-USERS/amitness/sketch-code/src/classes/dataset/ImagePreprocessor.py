...
class ImagePreprocessor:
    ...
    def build_image_dataset(self, data_input_folder, augment_data=True):
        ...
        print(
            "Converting images from {} into arrays, augmentation: {}".format(
        ...
        if augment_data == 1:
            self.augment_and_save_images(
                resized_img_arrays, sample_ids, data_input_folder
            )
        else:
            self.save_resized_img_arrays(
                resized_img_arrays, sample_ids, data_input_folder
            )
    ...
    def augment_and_save_images(
        self, resized_img_arrays, sample_ids, data_input_folder
    ):
        ...
        for i in range(len(resized_img_arrays)):
            ...
            count += 1
        print(
            "Saved down {} augmented images to folder {}".format(
    ...
    def resize_img(self, png_file_path):
        ...
        img_adapted = cv2.adaptiveThreshold(
            img_grey, 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY, 101, 9
        )
