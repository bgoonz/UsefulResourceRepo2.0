...
class Sampler:
    ...
    def convert_batch_of_images(
        self, output_folder, pngs_path, get_corpus_bleu, original_guis_filepath, style
    ):
            ...
            if filename.find(".png") != -1:
                ...
                try:
                    self.convert_single_image(
                        ...
                        get_sentence_bleu=0,
                        original_gui_filepath=png_path,
                        style=style,
                    )
                    generated_count += 1
                except:
        ...
        if (get_corpus_bleu == 1) and (original_guis_filepath is not None):
            print(
                "BLEU score: {}".format(
                    Evaluator.get_corpus_bleu(original_guis_filepath, output_folder)
                )
            )
            ...
            print(
                "BLEU score: {}".format(
                    Evaluator.get_sentence_bleu(
                        original_gui_filepath, gui_output_filepath
                    )
                )
            )
    ...
    def generate_gui(self, png_path, print_generated_output, sample_id, output_folder):
        ...
        gui_output_filepath = self.write_gui_to_disk(
            generated_gui, sample_id, output_folder
        )
