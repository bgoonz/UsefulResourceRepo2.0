...
class ShiftReduceDemo(object):
    ...
    def _init_fonts(self, root):
        ...
        self._boldfont = tkinter.font.Font(
    ...
    def help(self, *e):
        ...
        try:
            ...
            ShowText(self._top, "Help: Chart Parser Demo", (__doc__).strip(), width=75)
    ...
    def _animate_reduce_frame(self, frame, widgets, dy):
        if frame > 0:
            ...
            if not isinstance(tok, parse.Tree):
                raise ValueError()
            label = TextWidget(
                self._canvas, str(tok.node), color="#006060", font=self._boldfont
            )
...
def demo():
    ...
    (S, VP, NP, PP, P, N, Name, V, Det) = [
        cfg.Nonterminal(s) for s in nonterminals.split()
    ]
