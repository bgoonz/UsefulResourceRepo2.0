...
class CanvasPlotFrame(PlotFrameI):
    ...
    def _plot(self, *args):
        ...
        self._canvas.create_line(neginf, yzero, posinf, yzero, fill="gray", width=2)
        self._canvas.create_line(xzero, neginf, xzero, posinf, fill="gray", width=2)
    ...
    def bind(self, *args):
        self._canvas.bind(*args)

    def unbind(self, *args):
        self._canvas.unbind(*args)
...
class BLTPlotFrame(PlotFrameI):
    def __init__(self, root, vals, rng):
        ...
        # Create the plot.
        self._graph.line_create("plot", xdata=tuple(rng), ydata=tuple(vals), symbol="")
    ...
    def _set_scrollbars(self):
        ...
        self._yscroll.set(
            1 - (j2 - jmin) / (jmax - jmin), 1 - (j1 - jmin) / (jmax - jmin)
        )
    ...
    def _xview(self, *command):
        ...
        self.zoom(
            imin + f * (imax - imin), j1, imin + f * (imax - imin) + (i2 - i1), j2
        )
        self._set_scrollbars()
    ...
    def _yview(self, *command):
        ...
        self.zoom(
            i1, jmin + f * (jmax - jmin), i2, jmin + f * (jmax - jmin) + (j2 - j1)
        )
        self._set_scrollbars()
    ...
    def bind(self, *args):
        self._graph.bind(*args)

    def unbind(self, *args):
        self._graph.unbind(*args)
...
class Plot(object):
    ...
    def __init__(self, vals, rng=None, **kwargs):
        ...
        if type(vals) in (FunctionType, BuiltinFunctionType, MethodType):
            if rng is None:
    ...
    def _init_menubar(self, parent):
        ...
        zoommenu.add_command(
            label="View 100%", command=self._zoom_all, accelerator="Ctrl-a"
        )
    ...
    def help(self, *e):
        ...
        try:
            ShowText(self._root, "Help: Plot Tool", doc, width=75, font="fixed")
        except:
    ...
    def _zoom_in_drag(self, event):
        self._plot.adjust_zoom_marker(self._press_x, self._press_y, event.x, event.y)

    def _zoom_in_buttonrelease(self, event):
        self._plot.delete_zoom_marker()
        self._plot.unbind("<Motion>", self._bind_id)
        if (time.time() - self._press_time > 0.1) and abs(
            self._press_x - event.x
        ) + abs(self._press_y - event.y) > 5:
...
if __name__ == "__main__":
    ...
    Plot(
        lambda x: abs(x ** 2 - sin(20 * x ** 3)) + 0.1,
