...
class ChartMatrixView(object):
    ...
    def _init_matrix(self, root):
        ...
        self._canvas = tkinter.Canvas(cframe, width=200, height=200, background="white")
...
class ChartResultsView(object):
    def __init__(self, parent, chart, grammar, toplevel=True):
        ...
        if toplevel:
            ...
            tkinter.Button(buttons, text="Quit", command=self.destroy).pack(
                side="right"
            )
            tkinter.Button(buttons, text="Print All", command=self.print_all).pack(
                side="left"
            )
            tkinter.Button(
                buttons, text="Print Selection", command=self.print_selection
            ).pack(side="left")
...
class ChartComparer(object):
    ...
    def _init_chartviews(self, root):
        ...
        # The equals sign
        tkinter.Label(frame, text="=", width=3, background="#c0c0c0", font=eqfont).pack(
    ...
    def _init_buttons(self, root):
        ...
        tkinter.Button(buttons, text="Intersection", command=self._intersection).pack(
            side="left"
        )
        tkinter.Button(buttons, text="Union", command=self._union).pack(side="left")
        tkinter.Button(buttons, text="Difference", command=self._difference).pack(
            side="left"
        )
        tkinter.Frame(buttons, width=20).pack(side="left")
        tkinter.Button(buttons, text="Swap Charts", command=self._swapcharts).pack(
            side="left"
        )

        tkinter.Button(buttons, text="Detatch Output", command=self._detatch_out).pack(
            side="right"
        )
...
class ChartView(object):
    ...
    def _init_fonts(self, root):
        self._boldfont = tkinter.font.Font(
            family="helvetica", weight="bold", size=self._fontsize
        )
        self._font = tkinter.font.Font(family="helvetica", size=self._fontsize)
...
class ChartDemo(object):
    ...
    def _init_rulelabel(self, parent):
        ...
        self._rulelabel1 = tkinter.Label(parent, text=ruletxt, font=self._boldfont)
        self._rulelabel2 = tkinter.Label(
            parent, width=40, relief="groove", anchor="w", font=self._boldfont
        )
        self._rulelabel1.pack(side="left")
        self._rulelabel2.pack(side="left")
        step = tkinter.Checkbutton(parent, variable=self._step, text="Step")
    ...
    def _init_menubar(self):
        ...
        rulemenu.add_command(
            label="Earley Algorithm",
            underline=0,
            command=self.bottom_up_strategy,
            accelerator="e",
        )
        rulemenu.add_separator()
        rulemenu.add_command(label="Bottom Up Init Rule", command=self.bottom_up_init)
        ...
        rulemenu.add_command(label="Top Down Expand Rule", command=self.top_down_expand)
        rulemenu.add_command(label="Top Down Match Rule", command=self.top_down_match)
    ...
    def help(self, *e):
        ...
        try:
            ...
            ShowText(self._root, "Help: Chart Parser Demo", (__doc__).strip(), width=75)
    ...
    def load_chart(self, *args):
        ...
        try:
            ...
            raise
            tkinter.messagebox.showerror(
                "Error Loading Chart", "Unable to open file: %r" % filename
            )
    ...
    def save_chart(self, *args):
        ...
        try:
            ...
            raise
            tkinter.messagebox.showerror(
                "Error Saving Chart", "Unable to open file: %r" % filename
            )
    ...
    def save_grammar(self, *args):
        ...
        try:
            ...
            tkinter.messagebox.showerror(
                "Error Saving Grammar", "Unable to open file: %r" % filename
            )
