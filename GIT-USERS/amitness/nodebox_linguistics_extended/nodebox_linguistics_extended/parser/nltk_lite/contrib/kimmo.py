...
class KimmoGUI:
    def __init__(self, grammar, text, title="Kimmo Interface v1.78"):
        ...
        try:
            ...
            Tkinter.Entry(
                ...
                foreground="black",
                width=30,
                textvariable=self.wordIn,
            ).pack(side="left")

            lexiconFrame = Tkinter.Frame(level1)
            Tkinter.Label(lexiconFrame, text="Lexicon & Alternations").pack(
                side="top", fill="x"
            )
            self.lexicon = ScrolledText(
                lexiconFrame,
                background="white",
                foreground="black",
                width=50,
            ...
            Tkinter.Label(rulesFrame, text="Rules/Subsets").pack(side="top", fill="x")
            self.rules = ScrolledText(
                rulesFrame,
                background="white",
                foreground="black",
                width=60,
            ...
            Tkinter.Label(midbetweenFrame, text="Results           ").pack(side="right")
            ...
            self.loadMenu.add_command(
                label="Load Lexicon",
                underline=0,
                command=lambda filetype=".lex", targetWindow=self.lexicon, tf="l": self.loadTypetoTarget(
                    filetype, targetWindow, tf
                ),
            )
            self.loadMenu.add_command(
                label="Load Rules",
                underline=0,
                command=lambda filetype=".rul", targetWindow=self.rules, tf="r": self.loadTypetoTarget(
                    filetype, targetWindow, tf
                ),
            )
            ...
            self.saveMenu.add_command(
                label="Save Lexicon",
                underline=0,
                command=lambda filename=self.lexfilename, sourceWindow=self.lexicon: self.writeToFilefromWindow(
                    filename, sourceWindow, "w", 0, "l"
                ),
            )
            self.saveMenu.add_command(
                label="Save Rules",
                underline=0,
                command=lambda filename=self.rulfilename, sourceWindow=self.rules: self.writeToFilefromWindow(
                    filename, sourceWindow, "w", 0, "r"
                ),
            )
            self.saveMenu.add_command(
                label="Save Results",
                underline=0,
                command=lambda filename=".results", sourceWindow=self.results: self.writeToFilefromWindow(
                    filename, sourceWindow, "w", 0
                ),
            )
            self.saveMenu.add_command(
            ...
            for x in currentconfigfiles:
                ...
                self.configsMenu.add_command(
                    label=newname,
                    underline=0,
                    command=lambda newname=x: self.configLoader(newname),
                )  # Callback(self.configLoader,newname))
            ...
            self.tracingbtn = Tkinter.Button(
                ctlbuttons,
                text="Tracing",
                background="#fff0f0",
                foreground="black",
                command=lambda: self.create_destroyDebugTracing(),
            ).pack(side="right")
            ...
            self.graphMenu.add_command(
                label="Graph Lexicon",
                underline=0,
                command=lambda which="l": self.graph(which),
            )
            self.graphMenu.add_command(
                label="Graph FSA Rules",
                underline=0,
                command=lambda which="r": self.graph(which),
            )
            # self.loadMenu.add_command(label='Load Lexicon', underline=0,command = lambda filetype='.lex', targetWindow = self.lexicon : loadTypetoTarget(self, filetype, targetWindow))
            self.graphMenuButton["menu"] = self.graphMenu

            self.helpbtn = Tkinter.Button(
                ctlbuttons,
                text="Help",
                background="#f0fff0",
                foreground="black",
                command=self.kimmoHelp,
            ).pack(side="right")
    ...
    def init_menubar(self):
        ...
        filemenu.add_command(
            label="Save Rules", underline=0, command=self.save, accelerator="Ctrl-s"
        )
        self._root.bind("<Control-s>", self.save)
        filemenu.add_command(
            label="Load Rules", underline=0, command=self.load, accelerator="Ctrl-o"
        )
        self._root.bind("<Control-o>", self.load)
        filemenu.add_command(
            label="Clear Rules", underline=0, command=self.clear, accelerator="Ctrl-r"
        )
    ...
    def writeToFilefromWindow(self, filename, windowName, mode, auto, wt=None):
        ...
        if not (auto and windowName and filename):
            ...
            ftypes = [
                ...
                ("Rule file", ".rul"),
                ("Lexicon file", ".lex"),
                ("Alternations file", ".alt"),
                ("All files", "*"),
            ]
            filename = asksaveasfilename(
                filetypes=ftypes, defaultextension="", initialfile=filename
            )
    ...
    def batch(self, *args):
        ...
        if filename:
            ...
            self.results.insert(
                1.0,
                "----- "
                + time.strftime("%a, %d %b %Y %I:%M %p", time.gmtime())
                + " -----\n",
            )
    ...
    def graph(self, which):
        ...
        if which == "l":
            ...
            dotstring = dotformat(
                self.kimmoinstance.fsasNodes[string.atoi(ld.result[0])]["nodes"]
            )
            graphtitle = (
                "FSA " + self.kimmoinstance.fsasNodes[string.atoi(ld.result[0])]["name"]
            )
    ...
    def initKimmo(self, *args):
        ...
        if (
            (not self.kimmoinstance)
            or (self.rulemd5 != tmprmd5)
            or (self.lexmd5 != tmplmd5)
        ):
            self.guiError("Creating new Kimmo instance")
            self.kimmoinstance = KimmoControl(
                self.lexicon.get(1.0, Tkinter.END),
                self.rules.get(1.0, Tkinter.END),
                "",
                "",
                self.debug,
            )
    ...
    def kimmoHelp(self, *args):
        ...
        while 1:
            ...
            matchLenStr = (
                matchIdxFields[0]
                + "."
                + str(string.atoi(matchIdxFields[1], 10) + matchLen.get())
            )

            print((matchIdx, matchLenStr))
            help.tag_add(tagId, matchIdx, matchLenStr)
            help.tag_configure(
                tagId, background="aquamarine", foreground="blue", underline=True
            )
            tagId += 1
...
def _classeq(instance1, instance2):
    ...
    """
    return (
        type(instance1) == types.InstanceType
        and type(instance2) == types.InstanceType
        and instance1.__class__ == instance2.__class__
    )
...
class KimmoControl:
            ...
            # assume that a : contained in the line that is not a last char means it is an alternation.
                ...
                for x in range(len(moreitems)):
                    ...
                    if (moreitems[x][0] == '"') and (rangestart < 0):
                        rangestart = x
                    elif (moreitems[x][len(moreitems[x]) - 1] == '"') and (
                        rangestart > -1
                    ):
                        rangeend = x
                ...
                for x in range(len(moreitems)):
                    if x > 2:
                        ...
                        moreitems[x] = ""
                    elif (moreitems[x][0] == '"') and (
                        moreitems[x][len(moreitems[x]) - 1] == '"'
                    ):
    ...
    def processAlternations(self, text):
        ...
        if len(altgroup) > 0:
            if len(kimmoAlts) > 0:
                self.kalternations.append(KimmoAlternation(altgroup, kimmoAlts))
                self.validateAlternations.append(altgroup)
                for x in kimmoAlts:
                    self.validateAlternations.append(x)
                self.lexicalNodes.append(tmpnode)
    ...
    def processRules(self, text):
        ...
        for line in text.split("\n"):
            ...
            # also boundary would need to be defined before ...
                ...
                # using the boundary char, set the final boundary & also add to the any set.
                    ...
                    self.kdefaults = [
                        KimmoDefaults(string.join(items[1 : len(items)], " "))
                    ]
                    ...
                    self.krules.append(
                        KimmoArrowRule(
                            items[1], string.join(items[2 : len(items)], " ")
                        )
                    ...
                    # i.e. not text
                    if ((":" in items[0]) or ("." in items[0])) and (
                        not ct.match(items[0])
                    ):
                        ...
                        tmpnode[
                            "features"
                        ] = 'shape=box, fillcolor="lavender blush", style=filled'
                        if finalstate and (items[0] == 1):
                            tmpnode[
                                "features"
                            ] = 'shape=circle, color="paleturquoise2", style=filled'
                        elif items[0] == 1:
                            tmpnode[
                                "features"
                            ] = 'color="paleturquoise2", style=filled, shape=box'
                        elif finalstate:
                            tmpnode[
                                "features"
                            ] = 'shape=circle,fillcolor="honeydew2", style=filled'
                        tmpnode["edges"] = []
                        tmpnode["edgenames"] = []
                        # add as strings
                        ...
                        if items[0] == rulerowcnt:
                            self.krules.append(
                                KimmoFSARule(
                                    str(rulerowcnt) + ":" + rulegroup,
                                    string.join(ruleTran, " "),
                                    kimmoRule,
                                )
                            )
        ...
        if anyset[0] and anyset[1]:
            self.ksubsets.append(
                KimmoSubset(anyset[0], string.join(anyset[1 : len(anyset)], " "))
            )
...
class KimmoRule:
    def pairs(self):
        raise RuntimeError("unimplemented: KimmoRule.pairs()")
...
class KimmoArrowRule:
    ...
    def leftFSA(self):
        return self._left_fsa

    def rightFSA(self):
        return self._right_fsa
    ...
    def _parse_singleton(self, tokens, i):
        ...
        if t == "(":
            ...
            if t != ")":
                raise ValueError("missing final parenthesis, instead found " + t)
            j = j + 1
        elif t == "[":
...
class KimmoFSARule:
    def __init__(self, name, pair_description, state_descriptions):
        ...
        for (index, is_final, next_state_array) in state_descriptions:
            ...
            if len(next_state_array) != len(self.pairs()):
                raise ValueError(
                    "transition array of wrong size "
                    + str(len(next_state_array))
                    + " "
                    + str(len(self.pairs()))
                )
    ...
    def start(self):
        return self._state_descriptions[0][0]

    def is_state(self, index):
        return self.transitions.has_key(index)
    ...
    def sorted_pairs(self, subsets):
        ...
        sorted_with_index.sort(
            lambda x, y: self.mycompare(x[2], y[2], x[3], y[3])
        )  # lambda x, y: x[2] - y[2])
    ...
    def right_advance(self, current_states, input, output, subsets):
        ...
        for index in current_states:
            ...
            for i, pair, size1, size2 in self.sorted_pairs(
                subsets
...
class KimmoWord:
    ...
    def next_alternation(self):
        return self._next_alternation
...
class KimmoLexicon:
    ...
    def trie(self):
        return self._trie  # tree is ([KimmoWord], [ (char, sub-trie), ... ])
...
class KimmoAlternation:
    ...
    def __repr__(self):
        return (
            "<KimmoAlternation " + self.name() + ": " + str(self.lexicon_names()) + ">"
        )
...
class KimmoRuleSet:
    ...
    def _debug_print_input_and_output(
        self,
        position,
        rule_states,
        morphological_state,
        ...
        if self.debug:
            ...
            if False:  # morphological_state:
                print(
                    "    possible input chars = %s"
                    % invert.possible_next_characters(morphological_state)
                )
    ...
    def _generate(
        ...
        if position >= len(input_tokens):  # and (not morphological_state)
            ...
            if morphological_state:
                ...
                if ("0" in possible_next_input_chars) or (
                    "#" in possible_next_input_chars
                ):
                    ...
                    for next_morphological_state, new_words in invert.advance(
                        morphological_state, boundary
                    ):
                ...
                for (start, rule, fsa_states, required_truth_value) in rule_states:
                    ...
                    if required_truth_value != truth_value:
                        if self.debug:
                            print(
                                "    BLOCKED by rule {%d %s %s}"
                                % (start, rule, required_truth_value)
                            )
                            print(fsa_states)
                        break
                    else:
            ...
            if (
                morphological_state
            ):  # recognizer; get the next possible surface chars that can result in
                # the next char
                possible_next_input_chars = invert.possible_next_characters(
                    morphological_state
                )
            ...
            for pair_input, pair_output in self._pair_alphabet:
                ...
                if not (
                    compare_token == self.null()
                    or compare_token == input_tokens[position]
                ):
                    continue
                ...
                for s in rule_states:
                    ...
                    (next_fsa_state_set, contains_halt_state) = rule.right_advance(
                        fsa_state_set, pair_input, pair_output, self.subsets()
                    )
                ...
                for rule in self.rules():
                    ...
                    if rule.rightFSA():
                        if self.debug:
                            print(
                                "    adding rule {%d %s %s}"
                                % (position, rule, required_truth_value)
                            )
                        next_rule_states.append(
                            (
                                position,
                ...
                if fail == None:
                    ...
                    if morphological_state and pair_input != self.null():
                        for next_morphological_state, new_words in invert.advance(
                            morphological_state, pair_input
                        ):
                            # print 'ENTERING LEXICON '
                            for o in self._generate(
                                input_tokens,
                                new_position,
                                next_rule_states,
                                next_morphological_state,
                                new_input,
                                new_output,
                                new_result_str,
                                result_words + new_words,
    ...
    def _initial_rule_states(self):
        return [
            (0, rule, [rule.start()], True)
            for rule in self.rules()
            if isinstance(rule, KimmoFSARule)
        ]

    def generate(self, input_tokens):
        """Generator: yields output strings"""
        for o, w in self._generate(
            input_tokens, 0, self._initial_rule_states(), None, [], [], "", None
        ):
            yield o
