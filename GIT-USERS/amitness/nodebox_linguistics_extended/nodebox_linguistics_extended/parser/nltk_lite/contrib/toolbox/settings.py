    ...
    def parse(self, encoding=None):
        ...
        for proc in self._tree.findall("intprclst/intprc"):
            ...
            if trilook:
                print("  -- trilook --")
                print(
                    ("    DB TYPE:       [%s]" % self.__parse_value(trilook, "dbtyp"))
                )
                print(
                    ("    MKR OUTPUT:    [%s]" % self.__parse_value(trilook, "mkrOut"))
                )
            ...
            if tripref:
                print("  -- tripref --")
                print(
                    ("    DB TYPE:       [%s]" % self.__parse_value(tripref, "dbtyp"))
                )
                print(
                    ("    MKR OUTPUT:    [%s]" % self.__parse_value(tripref, "mkrOut"))
                )
                try:
                    for d in tripref.findall("drflst/drf"):
                        print(
                            ("    DB:            [%s]" % self.__parse_value(d, "File"))
                        )
                except:
                    pass
                try:
            ...
            if triroot:
                print("  -- triroot --")
                print(
                    ("    DB TYPE:       [%s]" % self.__parse_value(triroot, "dbtyp"))
                )
                print(
                    ("    MKR OUTPUT:    [%s]" % self.__parse_value(triroot, "mkrOut"))
                )
                try:
