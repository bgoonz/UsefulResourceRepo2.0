...
class UpdateRunner(DocTestRunner):
    ...
    def run(self, test, compileflags=None, out=None, clear_globs=True):
        ...
        for example in test.examples:
            ...
            if example.want:
                assert (
                    example.want.split("\n")[0] == old_lines[lineno][example.indent :]
    ...
    def _header(self, test, example):
        if test.filename is None:
            tag = "On line #%s of %s" % (example.lineno + 1, test.name)
            ...
            tag = "On line #%s of %s (%s)" % (lineno, test.filename, test.name)
...
class Debugger:
    ...
    def _check_output(self, example):
        ...
        if not self.checker.check_output(want, got, optionflags):
            self.runner.report_failure(self.save_stdout.write, self.test, example, got)
            return False
        else:
            return True
    ...
    def _check_exception(self, example):
        ...
        if not self.checker.check_output(want_exc_msg, got_exc_msg, optionflags):
    ...
    def _script_from_examples(self, s):
        ...
        for piece in DocTestParser().parse(s):
            if isinstance(piece, Example):
                ...
                output += [self._comment_line(l) for l in piece.split("\n")[:-1]]
    ...
    def debug(self, test, pm=False):
        ...
        try:
            if pm is False:
                debugger.run("execfile(%r)" % srcfilename, test.globs, test.globs)
            else:
                try:
                    ...
                    try:
                        exec(
                            compile(
                                open(srcfilename, "rb").read(), srcfilename, "exec"
                            ),
                            test.globs,
                        )
                    except bdb.BdbQuit:
    ...
    if os.path.exists(name):
        ...
        if ext[-3:] != ".py" and ext[-4:-1] != ".py":
            ...
            if testname is not None:
                raise ValueError("test names can't be specified " "for text files")
...
def find_module_from_filename(filename):
    ...
    if (
        os.path.exists(os.path.join(basedir, "__init__.py"))
        or os.path.exists(os.path.join(basedir, "__init__.pyc"))
        or os.path.exists(os.path.join(basedir, "__init__.pyw"))
    ):
        package = []
...
def run(names, optionflags, verbosity):
    ...
    for name in names:
        try:
            ...
            print(
                ("%s: Error processing %s -- %s" % (sys.argv[0], name, e)),
                file=sys.stderr,
            )
...
CHECK_OPT = Option(
    ...
    const="check",
    default="check",
    help="Verify the output of the doctest examples in the " "given files.",
)
...
ELLIPSIS_OPT = Option(
    ...
    const=1,
    default=0,
    help='Allow "..." to be used for ellipsis in the ' "expected output.",
)
NORMWS_OPT = Option(
...
def main():
    ...
    optparser = OptionParser(
        ...
        version="Edloper's Doctest Driver, " "version %s" % __version__,
    )
    ...
    reporting_group.add_options(
        [VERBOSE_OPT, QUIET_OPT, UDIFF_OPT, CDIFF_OPT, NDIFF_OPT]
    )
