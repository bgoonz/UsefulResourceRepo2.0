...
class Category(FeatureStructure, cfg.Nonterminal):
    ...
    def feature_names(self):
        ...
        """
        return filter(
            lambda x: not (x in self._required and self[x] is None),
            self._features.keys(),
        )
    ...
    def _destructively_unify(self, other, bindings, trace=False, depth=0):
        FeatureStructure._destructively_unify(
            self, other, bindings, trace=trace, ci_str_cmp=True, depth=depth
        )
    ...
    def parse_rules(cls, s):
        ...
        while position < len(s):
            rhs = []
            while (
                position < len(s) and _PARSE_RE["disjunct"].match(s, position) is None
            ):
                try:
