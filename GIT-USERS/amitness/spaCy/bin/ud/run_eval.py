...
ALL_LANGUAGES = (
    "ar, ca, da, de, el, en, es, fa, fi, fr, ga, he, hi, hr, hu, id, "
            ...
            for txt_path in treebank_dir.iterdir():
                if txt_path.name.endswith("-ud-" + corpus + ".txt"):
                    ...
                    if file_lang in languages:
                        gold_path = treebank_dir / txt_path.name.replace(
                            ".txt", ".conllu"
                        )
...
def run_single_eval(
    ...
    for score_name in eval_headers:
        ...
        print_string_1.extend(
            ["%.2f" % score.precision, "%.2f" % score.recall, "%.2f" % score.f1]
        )
        print_string_1.append(
            "-" if score.aligned_accuracy is None else "%.2f" % score.aligned_accuracy
        )
        print_string_1.append(
            "-" if score.undersegmented is None else "%.4f" % score.under_perc
        )
        print_string_1.append(
            "-" if score.oversegmented is None else "%.4f" % score.over_perc
        )
        ...
        if score_name in print_freq_tasks:
            print_header_1.extend(
                [
                    score_name + "_word_under_ex",
                    score_name + "_shape_under_ex",
                    score_name + "_word_over_ex",
                    score_name + "_shape_over_ex",
                ]
            )

            d_under_words = get_freq_tuples(score.undersegmented, PRINT_TOTAL)
            d_under_shapes = get_freq_tuples(
                [word_shape(x) for x in score.undersegmented], PRINT_TOTAL
            )
            d_over_words = get_freq_tuples(score.oversegmented, PRINT_TOTAL)
            d_over_shapes = get_freq_tuples(
                [word_shape(x) for x in score.oversegmented], PRINT_TOTAL
            )

            # saving to CSV with ; seperator so blinding ; in the example output
            print_string_1.append(
                str({k: v for k, v in d_under_words if v > PRINT_FREQ}).replace(
                    ";", "*SEMICOLON*"
                )
            )
            print_string_1.append(
                str({k: v for k, v in d_under_shapes if v > PRINT_FREQ}).replace(
                    ";", "*SEMICOLON*"
                )
            )
            print_string_1.append(
                str({k: v for k, v in d_over_words if v > PRINT_FREQ}).replace(
                    ";", "*SEMICOLON*"
                )
            )
            print_string_1.append(
                str({k: v for k, v in d_over_shapes if v > PRINT_FREQ}).replace(
                    ";", "*SEMICOLON*"
                )
            )
            ...
            try:
                ...
                for nlp, nlp_loading_time, nlp_name in models[tb_lang]:
                    try:
                        ...
                        tmp_output_path = text_path.parent / str(
                            "tmp_" + nlp_name + ".conllu"
                        )
                        run_single_eval(
                            nlp,
                            nlp_loading_time,
...
@plac.annotations(
    ...
    exclude_multi=(
        "Set flag to exclude the multi-language model as default baseline",
        "flag",
        "m",
        bool,
    ),
    hide_freq=(
        "Set flag to avoid printing out more detailed high-freq tokenization errors",
        "flag",
        "f",
        bool,
    ),
    ...
    best_per_language=(
        "Set flag to only keep the largest treebank for each language",
        "flag",
        "b",
        bool,
    ),
...
def main(
    ...
    check_parse=False,
    langs=ALL_LANGUAGES,
    exclude_trained_models=False,
    exclude_multi=False,
    hide_freq=False,
    corpus="train",
    best_per_language=False,
):
    """"
