...
clf1 = Pipeline(
    [("vect", tfidfVectorizer), ("clf", MultinomialNB(alpha=0.01, fit_prior=True))]
)
...
clf3 = Pipeline([("vect", tfidfVectorizer), ("clf", BernoulliNB(alpha=0.01))])

# SVC Linear Kernel
clf4 = Pipeline([("vect", tfidfVectorizer), ("clf", SVC(kernel="linear"))])
# SVC RBF Kernel
clf5 = Pipeline([("vect", tfidfVectorizer), ("clf", SVC(kernel="rbf"))])
# SVC Poly Kernel
clf6 = Pipeline([("vect", tfidfVectorizer), ("clf", SVC(kernel="poly"))])
...
def trainAndEvaluate(clf, xTrain, xTest, yTrain, yTest):
    ...
    print(
        "Predicted Target Name ", trainNews.target_names[clf.predict([xTest[600]])[0]]
    )
    ...
    print(
        "Predicted Target Name ", trainNews.target_names[clf.predict([xTest[1100]])[0]]
    )
