...
flags.DEFINE_string(
    "additional_data", None, "Should we provide additional input data? maybe."
)
...
flags.DEFINE_bool(
    "predict_test", False, "Whether to run the model in inference mode on the test set."
)
...
def main(_):
    ...
    if tf.gfile.Exists(FLAGS.output_dir):
        ...
        for split in ["val", "test"]:
            if tf.gfile.Exists(
                os.path.join(FLAGS.output_dir, f"{split}-probs.npy")
            ) and getattr(FLAGS, f"predict_{split}"):
    ...
    with tf.gfile.Open(FLAGS.input_data, "r") as f:
        for l in f:
            ...
            examples[item["split"]].append(
                {"info": item, "ids": context_ids, "label": item["label"]}
            )
    ...
    if FLAGS.max_training_examples > 0:
        ...
        new_examples = []
        print(
            "Unique machine examples: {} -> {}".format(
        ...
        print(
            "Length of examples: {} -> {}".format(
                len(examples["train"]), len(new_examples)
            ),
            flush=True,
        )
    ...
    if FLAGS.do_train:
        num_train_steps = int(
            (len(examples["train"]) / FLAGS.batch_size) * FLAGS.num_train_epochs
        )
    ...
    for split in splits_to_predict:
        ...
        for i, res in enumerate(
            estimator.predict(input_fn=val_input_fn, yield_single_examples=True)
        ):
        ...
        labels = np.array(
            [LABEL_INV_MAP[x["label"]] for x in examples[split][:num_actual_examples]]
        )
        print(
            "{} ACCURACY IS {:.3f}".format(split, np.mean(labels == preds)), flush=True
        )
