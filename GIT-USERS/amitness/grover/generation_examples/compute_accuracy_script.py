...
def score(probs, full_info):
    ...
    machine_v_human = {
        k: v.set_index("ind30k", drop=True) for k, v in groups["gen"].groupby("labels")
    }
    machine_vs_human_joined = machine_v_human["machine"].join(
        machine_v_human["human"], rsuffix="_humanpair"
    )
    machine_vs_human_joined["is_right"] = (
        machine_vs_human_joined["machine"]
        > machine_vs_human_joined["machine_humanpair"]
    )
    ...
    combined_acc = np.mean(
        combined_scores[["machine", "human"]].idxmax(1) == combined_scores["labels"]
    )
