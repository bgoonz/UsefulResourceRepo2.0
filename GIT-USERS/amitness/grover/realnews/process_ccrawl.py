...
with open(
    os.path.join(os.path.dirname(__file__), "domain_to_allowed_subdomains.json"), "r"
) as f:
    ALLOWED_SUBDOMAINS = json.load(f)

# FOR HANNAH
PROPAGANDA_SUBDOMAINS = {
    "wnd.com": True,
    "infowars.com": True,
    "breitbart.com": True,
    "dailycaller.com": True,
    "yournewswire.com": True,
    "prageru.com": True,
    "newsmax.com": True,
    "twitchy.com": True,
    "dailywire.com": True,
    ...
    "nationalreport.net": True,
    "naturalnews.com": True,
    "prntly.com": True,
    "worldnewsdailyreport.com": True,
    "libertywriters.com": True,
    "globalresearch.ca": True,
}
...
class Article(object):
    ...
    def __init__(self, html):
        ...
        if self.dummy_article.publish_date and not isinstance(
            self.dummy_article.publish_date, str
        ):
            try:
...
def parse_record(record, propaganda=False):
    ...
    url_was_ok = _url_seems_ok(
        record.rec_headers["WARC-Target-URI"],
        domain_to_allowed_subdomains=PROPAGANDA_SUBDOMAINS
        if propaganda
        else ALLOWED_SUBDOMAINS,
    )
    if not url_was_ok:
        return
...
parser.add_argument("-bucket_name", type=str, help="out path")
parser.add_argument(
    "-propaganda",
    action="store_true",
