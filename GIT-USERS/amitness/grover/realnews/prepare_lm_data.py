...
class S3TFRecordWriter(object):
    def __init__(self, fn):
        ...
        if fn.startswith("s3://"):
            ...
            self.bucket_name, self.file_name = self.fn.split("s3://", 1)[1].split(
                "/", 1
            )
    ...
    with open(args.input_fn, "r") as f:
        for l_no, l in enumerate(f):
            if l_no % args.num_folds == args.fold:
                ...
                article["input_ids"] = tokenize_for_grover_training(
                    encoder,
                    article,
                    desired_size=final_desired_size,
...
def _stream_from_buffer(
    buffer, current_desired_size, pad_token=0, add_articles_to_end=False
):
...
def buffered_and_sliding_window_article_iterator(
    encoder, current_desired_size, final_desired_size=1025
):
    ...
    for article in article_iterator(encoder, final_desired_size=final_desired_size):
        ...
        if article["split"] == "val" or amount_to_pad <= 0:
            for sub_index, sub_article in enumerate(
                sliding_window(
                    article,
                    max_seq_length=current_desired_size,
        ...
        if len(buffer) % 100 == 0:
            yield from _stream_from_buffer(
                buffer,
...
with S3TFRecordWriter(train_file) as train_writer, S3TFRecordWriter(
    ...
    for article in buffered_and_sliding_window_article_iterator(
        ...
        if article["inst_index"] % 1000 == 0:
            print(
                "{} articles, {} written".format(article["inst_index"], total_written),
                flush=True,
            )
