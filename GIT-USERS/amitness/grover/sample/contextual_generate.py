...
with open(args.metadata_fn, "r") as f:
    articles = [
        json.loads(l) for i, l in enumerate(f) if i % args.num_folds == args.fold
    ]

tf_config = tf.ConfigProto(allow_soft_placement=True)

with tf.Session(config=tf_config, graph=tf.Graph()) as sess, open(
    args.out_fn, "w"
) as f_out:
    ...
    for i, article in enumerate(tqdm(articles)):
        ...
        for chunk_i in range(num_chunks):
            ...
            for t_i, p_i in zip(tokens_out, probs_out):
                ...
                # better add that!
                gens_raw.append(
                    t_i[extraction["start_ind"] : extraction["end_ind"] + 1].tolist()
                )

                assert extraction["start_ind"] == len(context_formatted)
                gen_probs.append(
                    p_i[: extraction["end_ind"] - len(context_formatted) + 1].tolist()
                )
