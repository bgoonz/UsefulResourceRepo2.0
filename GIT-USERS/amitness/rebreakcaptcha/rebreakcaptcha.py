...
class rebreakcaptcha(object):
    ...
    def get_recaptcha_challenge(self):
        while 1:
            ...
            if not self.is_exists_by_xpath(
    ...
    def get_audio_challenge(self, iframes):
        ...
        if not self.is_exists_by_xpath('//button[@title="Get an audio challenge"]'):
            print(
                "[{0}] No element of audio challenge!!".format(self.current_iteration)
            )
            return False
    ...
    def speech_to_text(self, audio_source):
        ...
        try:
            ...
            print(
                "[{0}] Google Speech Recognition: ".format(self.current_iteration)
                + audio_output
            )
        except sr.UnknownValueError:
            print(
                "[{0}] Google Speech Recognition could not understand audio".format(
                    self.current_iteration
                )
            )
    ...
    def solve_audio_challenge(self):
        ...
        ) and not self.is_exists_by_xpath('//div[@class="rc-text-challenge"]'):
            print(
                "[{0}] No element in audio challenge download link!!".format(
                    self.current_iteration
                )
            )
            return False
        ...
        while self.is_exists_by_xpath('//div[@class="rc-text-challenge"]'):
            print(
                "[{0}] Got a text challenge! Reloading!".format(self.current_iteration)
            )
            self.driver.find_element_by_id("recaptcha-reload-button").click()
            time.sleep(random.uniform(MIN_RAND, MAX_RAND))

        # Get the audio challenge URI from the download link
        download_object = self.driver.find_element_by_xpath(
            '//a[@class="rc-audiochallenge-download-link"]'
        )
    ...
    def solve(self, current_iteration):
        ...
        while self.is_exists_by_xpath(
            '//div[@class="rc-audiochallenge-error-message"]'
        ) and self.is_exists_by_xpath(
            '//div[contains(text(), "Multiple correct solutions required")]'
        ):
            print(
                "[{0}] Need to solve more. Let's do this!".format(
                    self.current_iteration
                )
            )
