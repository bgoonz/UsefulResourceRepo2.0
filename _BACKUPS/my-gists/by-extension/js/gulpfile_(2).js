...
gulp.task("jshint", function () {
  return gulp
    .src([srcScripts, "./gulpfile.js", "./src/helpers/**.*.js"])
    .pipe(plumber())
...
gulp.task("sprites", function () {
  var sprites = gulp
    .src(["./src/images/sprites/*.png", "!./src/images/sprites/*@2x.png"])
    .pipe(plumber())
    .pipe(
      spritesmith({
        imgName: "sprite.png",
  ...
  sprites.img.pipe(plumber()).pipe(gulp.dest(destSpritesImages));

  sprites2x.img.pipe(plumber()).pipe(gulp.dest(destSpritesImages));

  sprites.css.pipe(plumber()).pipe(gulp.dest(destSpritesStyles));

  sprites2x.css.pipe(plumber()).pipe(gulp.dest(destSpritesStyles));
});
...
gulp.task("static", function () {
  gulp.src(srcStatic).pipe(gulp.dest(destPublic));
});

gulp.task("build", ["content", "scripts", "styles", "static"]);
