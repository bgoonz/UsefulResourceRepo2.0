var ie_png = {
  ...
  vmlOpacity: function (el) {
    if (el.currentStyle.filter.search("lpha") != -1) {
      ...
      trans =
        parseInt(
          trans.substring(trans.lastIndexOf("=") + 1, trans.lastIndexOf(")")),
          10
        ) / 100;
      el.vml.color.shape.style.filter =
        el.currentStyle.filter; /* complete guesswork */
  ...
  handlePseudoHover: function (el) {
    setTimeout(function () {
      /* wouldn't work as intended without setTimeout */ ie_png.applyVML(el);
    }, 1);
  },
  ...
  fix: function (selector) {
    ...
    for (var i = 0; i < selectors.length; i++) {
      this.styleSheet.addRule(
        selectors[i],
        "behavior:expression(ie_png.fixPng(this))"
    ...
    if (elStyle.backgroundImage != "none" || el.isImg) {
      ...
      if (!lib.imgSize[el.vmlBg]) {
        ...
        img.runtimeStyle.cssText =
          "behavior:none; position:absolute; left:-10000px; top:-10000px; border:none;"; /* make sure to set behavior to none to prevent accidental matching of the helper elements! */
  ...
  vmlOffsets: function (el) {
      ...
      switch (position) {
        ...
        default:
          if (position.search("%") != -1) {
            bg[axis] = parseInt(position) * 0.01;
          } else {
            fraction = false;
          }
      }
    ...
    if (bgR != "repeat") {
      ...
      el.vml.image.shape.style.clip =
        "rect(" + dC.T + "px " + dC.R + "px " + dC.B + "px " + dC.L + "px)";
    }
  },
  ...
  fixPng: function (el) {
    ...
    if (el.nodeName == "IMG") {
      ...
      el.currentStyle.backgroundImage.toLowerCase().search(".png") == -1
    ) {
      return;
    }
    var lib = ie_png;
    ...
    el.vml.image.shape.fillcolor =
      "none"; /* Don't show blank white shapeangle when waiting for image to load. */
