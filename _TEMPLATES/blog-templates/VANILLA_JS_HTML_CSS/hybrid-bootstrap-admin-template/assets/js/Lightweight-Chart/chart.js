...
var thychart = {
  donut: function (node) {
    ...
    $chart.on("show-donut-chart", function () {
      $title.find("p").text(0);
      $({ countNum: $title.find("p").text() }).animate(
        { countNum: (val / 360) * 100 },
        {
          duration: 500,
          easing: "linear",
          step: function () {
    ...
    var chart = {
      nodes: {
        ...
        mask: function () {
          return $temp
            .clone()
            .addClass(chart.values.selector)
            .attr(
              "style",
      ...
      for (i = 0; i < data[0].length; i++) {
        ...
        $.each(v[i], function (index, val) {
          ...
          if (!barWidth) {
            ...
            li.find("span").attr(
              "style",
              "height:" + percent + "%;" + "width:" + barWidth + "px"
            );
          }
          ul.append(li);
        });
  ...
  line: function (node) {
    var setAngle = function (cord, area, node) {
      var hypotenuse = Math.round(
        Math.sqrt(Math.pow(area.width, 2) + Math.pow(area.height, 2))
      );
    ...
    var setPosition = function (data) {
      ...
      var totalWidth = parseInt(
        $("ul").find(data).attr("data-width").replace("-", "")
      );
      var totalHeight = parseInt(
        $("ul").find(data).attr("data-height").replace("-", "")
      );
      ...
      if (prevNode.length === 0) {
        ...
        $("ul")
          .find(data)
          .attr("data-total-height", totalY - totalHeight);
        $("ul")
          .find(data)
          .css("bottom", totalY + "px");

        $("ul").find(data).attr("data-y", totalY);
        $("ul").find(data).attr("data-x", 0);
      } else {
        var currentWidth = parseInt(
          prevNode.attr("data-total-width").replace("-", "")
        );
        totalWidth =
          parseInt(prevNode.attr("data-total-width").replace("-", "")) +
          parseInt(data.attr("data-width").replace("-", ""));

        $("ul").find(data).attr("data-total-width", totalWidth);
        $("ul")
          .find(data)
          .css("left", currentWidth + "px");

        var currentHeight = parseInt(
          prevNode.attr("data-total-height").replace("-", "")
        );
        totalHeight =
          parseInt(prevNode.attr("data-total-height")) +
          parseInt(data.attr("data-height"));

        $("ul").find(data).attr("data-total-height", totalHeight);
        $("ul")
          .find(data)
          .css("bottom", currentHeight + "px");
    ...
    var drawSVG = function () {
        ...
        for (i = 0; i < data.length; i++) {
          ...
          for (k = 0; k < keys.length; k++) {
            obj[k] = {
              x: keys[k],
              y:
                $chart.parent().find(".grid hr:last-child").attr("data-y") -
                data[i][k],
            };
          }
          output.push(obj);
      ...
      function addPoint(x, y, isFirst) {
        ...
        $chart
          .parent()
          .find("path")
          .attr("d", $chart.parent().find("path").attr("d") + "" + new_point);
        counter++;
        if (counter < Object.keys(points).length) {
          setTimeout(
            addPoint(points[counter].x, points[counter].y, false),
            200
          ); // Add a new point after 200 milliseconds
        }
        if (counter == Object.keys(points).length) {
    ...
    for (var i = 0; i < data[0].length; i++) {
      ...
      if (i % 2 === 0) {
        ...
        var line = $("<hr/>")
          .css({ bottom: i * gridSpace })
          .attr("data-y", i * gridSpace);
