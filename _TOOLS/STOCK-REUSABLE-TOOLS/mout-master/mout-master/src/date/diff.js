define([
    ...
    function getYearsDiff(start, end) {
        let elapsedYears = getElapsedYears(start, end);
    ...
    function getFractionalMonth(start, end) {
        ...
        if (startDay !== endDay) {
            let startTotalDays = totalDaysInMonth(start);
            let endTotalDays = totalDaysInMonth(end);
            let totalDays;
            let daysElapsed;

            if (startDay > endDay) {
                // eg: Jan 29 - Feb 27 (29 days elapsed but not a full month)
                let baseDay = startTotalDays - startDay;
    ...
    function getFractionalYear(start, end, elapsedYears) {
        let base = elapsedYears
            ? new Date(end.getFullYear(), start.getMonth(), start.getDate())
            : start;
        let elapsedDays = diff(base, end, "day");
