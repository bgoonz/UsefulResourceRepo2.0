...
class SystemStats(object):
    ...
    def uptime_str(self):
        ...
        up = self.uptime()
        st.append(
            "Up for %s days, %s hours and %s minutes"
    ...
    def disk_usage_str(self):
        ...
        if self._verbose_du:
            for mount in sorted(disks):
                ...
                for datum in ("partition", "size", "used", "available", "capacity"):
            ...
            for mount in sorted(disks):
                disk_info = disks[mount]
                st.append(
                    "%s\tsize=%s used=%s available=%s capacity=%s"
                    % (
                        mount,
                        disk_info["size"],
                        disk_info["used"],
    ...
    def memory_usage_str(self):
        ...
        for mem_type in ("total", "available", "used", "free", "swap_total", "swapped"):
    ...
    def disk_usage(self):
        ...
        proc = subprocess.Popen(
            ("df", "-P"), stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )
        ...
        for l in df[1:]:
            ...
            if self._show_bytes:
                ...
                info["available"] = "%s (%s)" % (avail, size_str(avail))
            else:
    ...
    def logical_cpu_count(self):
        ...
        if platform.system() == "Linux":
            ...
            ncpu = subprocess.check_output(["sysctl", "-a", "hw.ncpu"]).decode("utf-8")
    ...
    def _linux_mem(self):
        ...
        if os.path.exists("/proc/meminfo"):
            try:
                ...
                if self._show_bytes:
                    mem_free = "%d (%s)" % (mem_free, size_str(mem_free))
                    mem_total = "%d (%s)" % (mem_total, size_str(mem_total))
                    mem_avail = "%d (%s)" % (mem_avail, size_str(mem_avail))
                    mem_used = "%d (%s)" % (mem_used, size_str(mem_used))
                    swapped = "%d (%s)" % (swapped, size_str(swapped))
                    swap_total = "%d (%s)" % (swap_total, size_str(swap_total))
                else:
    ...
    def _freebsd_mem(self):
        ...
        try:
            sysctl = {}
            out = (
                subprocess.check_output(("/sbin/sysctl", "-a")).decode("utf-8").strip()
            )
            for l in out.split("\n"):
                if ":" not in l:
                    continue
