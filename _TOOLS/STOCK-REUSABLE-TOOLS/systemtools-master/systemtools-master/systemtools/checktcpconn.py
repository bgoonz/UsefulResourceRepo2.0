...
def main():
    ...
    ap.add_argument("host", help="IP address or DNS name of host to connect to.")
    ap.add_argument("port", type=int, help="Port to connect to host on.")
    ap.add_argument(
        "--poll",
        action="store_true",
        help="Try repeatedly to make a TCP connection until " "timeout.",
    )
    ap.add_argument(
        "--wait",
        metavar="seconds",
    ...
    if args.poll:
        if not tcp_chk.poll(args.timeout):
            print(
                "timed out waiting to connect:", tcp_chk.last_error(), file=sys.stdout
            )
            return 1
    else:
        if not tcp_chk.try_connect(args.timeout):
            print("unable to connect:", tcp_chk.last_error(), file=sys.stdout)
            return 1
