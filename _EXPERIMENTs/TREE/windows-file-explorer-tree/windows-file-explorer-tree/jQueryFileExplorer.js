(function ($) {
  ...
  FolderNode.prototype.Expand = function () {
    $this = this;
    $this.element
      .children(".foldertree-expander")
      .removeClass("foldertree-expander-collapsed")
      .addClass("waiting");
    this.GetChildren(function (error) {
      if (!error) {
        ...
        $this.element.append(
          "<div class='foldertree-node foldertree-error'>Failed to retrieve folder content</div>"
        );
      }
      $this.element
        .children(".foldertree-expander")
        .removeClass("waiting")
        .addClass("foldertree-expander-expanded");
      if (!$this.icon) {
        $this.element
          .children(".foldertree-icon.foldertree-icon-collapsed")
          .removeClass("foldertree-icon-collapsed")
          .addClass("foldertree-icon-expanded");
      }
  ...
  FolderNode.prototype.Collapse = function () {
    ...
    this.element
      .children(".foldertree-expander")
      .addClass("foldertree-expander-collapsed")
      .removeClass("foldertree-expander-expanded");
    if (!this.icon) {
      this.element
        .children(".foldertree-icon.foldertree-icon-expanded")
        .removeClass("foldertree-icon-expanded")
        .addClass("foldertree-icon-collapsed");
    }
  ...
  FolderNode.prototype.ShowCrumbs = function (
    crumbContainer,
    contentContainer
  ) {
    ...
    for (var i = 0; i < crumbs.length - 1; i++) {
      $("<span>")
        .addClass("folder-crumb")
        .text(crumbs[i].label)
        .data("node", crumbs[i])
        .on("click", function (e) {
          $(e.target)
            .data("node")
            .ShowContent(contentContainer, crumbContainer);
        })
        .appendTo($(crumbContainer));
      $(crumbContainer).append($("<span>").addClass("folder-crumb-separator"));
    }
    $(crumbContainer).append(
      $("<span>").addClass("folder-crumb").text(this.label)
    );
    $(crumbContainer).scrollLeft($(crumbContainer)[0].scrollWidth);
  };
  FolderNode.prototype.ShowContent = function (
    contentContainer,
    crumbContainer,
    message
  ) {
    ...
    this.GetChildren(function (error) {
      ...
      if (error) {
        $("<span>")
          .addClass("foldertree-error")
          .text("Failed to get folder content.")
          .appendTo(contentElem);
        return;
      }
      ...
      $.each(children, function (index, child) {
        ...
        if (child.subitems) {
          $.each(child.subitems, function (index, text) {
            var subitem = $("<span>")
              .addClass("foldercontent-subitem")
              .addClass("foldercontent-subitem-" + index)
              .text(text);
            item.append(subitem);
          });
        }
  ...
  FileExplorer = function (options) {
    ...
    container
      .append(crumbs)
      .append(explorer.append(foldertree).append(foldercontent));
