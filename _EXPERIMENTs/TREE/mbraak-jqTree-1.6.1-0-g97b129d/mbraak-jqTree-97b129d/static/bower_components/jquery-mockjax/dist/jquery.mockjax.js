...
(function (root, factory) {
    ...
    function isMockDataEqual(mock, live) {
        logger.debug(mock, [
            "Checking mock data against request data",
            mock,
            live,
        ]);
        var identical = true;
    ...
    function getMockForRequest(handler, requestSettings) {
        ...
        if ($.isFunction(handler.url.test)) {
            ...
            if (!!namespace) {
                ...
                var pattern = handler.url.source
                    .replace(/^(\^+)/, "")
                    .replace(/^/, "^(" + namespace + ")?/?");
            ...
            if (
                (effectiveUrl !== requestSettings.url && star === -1) ||
                !new RegExp(
                    effectiveUrl
                        .replace(/[-[\]{}()+?.,\\^$|#\s]/g, "\\$&")
                        .replace(/\*/g, ".+")
        ...
        if (mockHandler.proxy) {
            ...
            _ajax({
                ...
                complete: function (xhr) {
                    ...
                    mockHandler.responseXML = mockHandler.responseText =
                        xhr.responseText;
    ...
    function xhr(mockHandler, requestSettings, origSettings, origHandler) {
        ...
        return {
            ...
            getAllResponseHeaders: function () {
                ...
                // since jQuery 1.9 responseText type has to match contentType
                if (mockHandler.contentType) {
                    mockHandler.headers["content-type"] =
                        mockHandler.contentType;
                }
    ...
    function jsonpSuccess(requestSettings, callbackContext, mockHandler) {
        ...
        if (requestSettings.global) {
            (requestSettings.context
                ? $(requestSettings.context)
                : $.event
            ).trigger("ajaxSuccess", [{}, requestSettings]);
        }
    }
    ...
    function handleAjax(url, origSettings) {
        ...
        for (var k = 0; k < mockHandlers.length; k++) {
            ...
            if ($.isFunction(mockHandler.onAfterComplete)) {
                origSettings.complete = overrideCallback(
                    "Complete",
                    mockHandler
                );
            }
