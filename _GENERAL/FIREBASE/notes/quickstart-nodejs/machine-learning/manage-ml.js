...
require("yargs")
  ...
  .usage("$0 <cmd> [args]")
  .command(
    "new <name> [-f <model_file>] [-a <automl_ref>] [-t tags]]",
    ...
    (yargs) => {
      ...
      yargs.option("automl", {
        ...
        demandOption: false,
        describe:
          "AutoML model reference (e.g. projects/12345678/" +
  ...
  .command(
    "update <model_id> [-m model_file] [-n name] [-t new_tags] [-d remove_tags]",
    ...
    (argv) => {
      (async () => {
        ...
        const removeTags =
          argv.remove_tags == null ? null : argv.remove_tags.split(",");
        await updateModel(
          argv.model_id.toString(),
          argv.model_file,
          argv.name,
          newTags,
          removeTags
