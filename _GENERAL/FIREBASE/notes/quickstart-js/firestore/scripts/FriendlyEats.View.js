...
FriendlyEats.prototype.viewList = function (filters, filter_description) {
  ...
  var renderResults = function (doc) {
    ...
    var existingRestaurantCardEl = mainEl.querySelector(
      "#" + that.ID_CONSTANT + doc.id
    );
    var el =
      existingRestaurantCardEl || that.renderTemplate("restaurant-card", data);
...
FriendlyEats.prototype.initFilterDialog = function () {
  ...
  this.replaceElement(
    ...
    that.renderTemplate("item-list", {
      items: ["Any"].concat(that.data.categories),
    })
  );
  ...
  this.replaceElement(
    ...
    that.renderTemplate("item-list", {
      items: ["Any"].concat(that.data.cities),
    })
  );
