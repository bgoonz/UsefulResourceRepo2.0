...
function onEnrollVerifyCode(e) {
  ...
  if (firebase.auth().currentUser && phoneVerificationId) {
    ...
    var displayName =
      document.getElementById("enroll-display-name").value || undefined;
...
function onSignInSendCode(e) {
  ...
  if (isCaptchaOK() && mfaResolver) {
    ...
    var index = parseInt(
      node.options[node.selectedIndex].getAttribute("data-val"),
      10
    );
...
function onUnenroll(e) {
  ...
  if (firebase.auth().currentUser) {
    ...
    if (info) {
      firebase
        .auth()
        .currentUser.multiFactor.unenroll(info)
        .then(function () {
...
function initApp() {
  ...
  document
    ...
    .addEventListener("click", passwordSignIn);
  document
    .getElementById("quickstart-sign-up")
    .addEventListener("click", passwordSignUp);
  document
    .getElementById("quickstart-sign-out")
    .addEventListener("click", signOut);
  document
    .getElementById("quickstart-verify-email")
    .addEventListener("click", sendEmailVerification);
  document
    .getElementById("quickstart-password-reset")
    .addEventListener("click", sendPasswordReset);
  document
    .getElementById("quickstart-enroll")
    .addEventListener("click", onEnrollClick);

  document
    .getElementById("phone-number")
    .addEventListener("keyup", updateEnrollSendCodeButtonUI);
  document
    .getElementById("phone-number")
    .addEventListener("change", updateEnrollSendCodeButtonUI);

  document
    .getElementById("enroll-send-code-form")
    .addEventListener("submit", onEnrollSendCode);
  document
    .getElementById("enroll-cancel-send-code-button")
    .addEventListener("click", onCancel);

  document
    .getElementById("enroll-verification-code-form")
    .addEventListener("submit", onEnrollVerifyCode);
  document
    .getElementById("enroll-cancel-verify-code-button")
    .addEventListener("click", onCancel);

  document
    .getElementById("sign-in-send-code-form")
    .addEventListener("submit", onSignInSendCode);
  document
    .getElementById("sign-in-cancel-send-code-button")
    .addEventListener("click", onCancel);
  document.getElementById("mfa-hints").addEventListener("change", onSelect);

  document
    .getElementById("sign-in-verification-code-form")
    .addEventListener("submit", onSignInVerifyCode);
  document
    .getElementById("sign-in-cancel-verify-code-button")
    .addEventListener("click", onCancel);
}
