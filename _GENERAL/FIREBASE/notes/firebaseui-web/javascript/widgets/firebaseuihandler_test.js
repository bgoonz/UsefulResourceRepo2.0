...
function assertProgressBarVisible(container) {
  const elements = dom.getElementsByClass(
...
function assertIdpButtonLabels(button, expectedShortLabel, expectedLongLabel) {
  ...
  assertEquals(expectedLongLabel, dom.getTextContent(idpTextLong[0]));
  assertEquals(expectedShortLabel, dom.getTextContent(idpTextShort[0]));
}
...
function assertSelectTenantPageVisible(container) {
  const element = dom.getElementByClass(
    "firebaseui-id-page-select-tenant",
    container
  );
  assertNotNull(element);
}
...
function assertErrorPageVisible(container, message, recoverable) {
  const element = dom.getElementByClass(
    "firebaseui-id-page-recoverable-error",
    container
  );
  assertNotNull(element);
...
testSuite({
  setUp() {
    ...
    firebase.initializeApp = (options, name) => {
      ...
      if (firebase.instances_[key]) {
        ...
        firebase.instances_[key] = new FakeAppClient(options, name);
      }
    ...
    testStubs.replace(FakeAppClient.prototype, "delete", function () {
      // Already deleted.
    ...
    for (let key in idp.AuthProviders) {
      ...
      if (key != "twitter.com" && key != "password") {
        firebase["auth"][idp.AuthProviders[key]].prototype.addScope = function (
          scope
        ) {
      ...
      if (key != "password") {
        ...
        firebase["auth"][idp.AuthProviders[key]].prototype.setCustomParameters =
      ...
      if (key == "password") {
        ...
        firebase.auth.EmailAuthProvider.credentialWithLink = (email, link) => ({
          email: email,
          link: link,
          providerId: "password",
  ...
  testSelectTenant_optionFirst() {
    ...
    const selectPromise = handler.selectTenant(projectConfig, [
      "tenant1",
      "tenant2",
    ]);
    ...
    return selectPromise.then((selectedTenantInfo) => {
      ...
      assertEquals(
        "tenant1",
        signInUiShownCallback.getLastCall().getArgument(0)
      );
      ...
      for (let i = 0; i < buttons.length; i++) {
        assertEquals(
          selectedTenantInfo.providerIds[i],
  ...
  testSelectTenant_optionFirst_invalidApiKey() {
    ...
    return selectPromise.then(fail, (error) => {
      let errorMessage = strings
        .errorCIAP({ code: "invalid-configuration" })
        .toString();
  ...
  testSelectTenant_identifierFirst_noMatchingTenant() {
    ...
    let errorMessage = strings
      .errorCIAP({ code: "no-matching-tenant-for-email" })
      .toString();
  ...
  testSelectTenant_identifierFirst_hdNotConfigured() {
    ...
    configs["API_KEY"]["displayMode"] = "identifierFirst";
    handler = new FirebaseUiHandler(container, configs);
    const selectPromise = handler.selectTenant(projectConfig, ["tenant2"]);
  ...
  testGetAuth_noTopLevelProjectConfigError() {
    ...
    handler = new FirebaseUiHandler(container, configWithoutTopLevelProject);
  ...
  testStartSignIn() {
    ...
    internalAuth
      ...
      .then(() => {
        externalAuth.assertUpdateCurrentUser([internalAuth.currentUser], () => {
  ...
  testStartSignIn_selectedTenantInfo_idp_popup() {
    ...
    internalAuth.assertSignInWithPopup([expectedProvider], {
      user: internalAuth.currentUser,
      credential: cred,
      operationType: "signIn",
      additionalUserInfo: {
  ...
  testStartSignIn_selectedTenantInfo_idp_immediateRedirect_success() {
    ...
    configs["API_KEY"]["tenants"]["tenant1"][
  ...
  testStartSignIn_selectedTenantInfo_idp_immediateRedirect_redirectStatus() {
    ...
    // Should immediately redirect to IdP without button click.
    internalAuth.assertGetRedirectResult([], function () {
  ...
  testStartSignIn_selectedTenantInfo_idp_immediateRedirect_error() {
    ...
    // Should immediately redirect to IdP without button click.
    internalAuth.assertSignInWithRedirect([expectedProvider], null, () => {
    ...
    return internalAuth.process().then(() => {
      ...
      assertInfoBarMessage(
        strings.error({ code: networkError.code }).toString(),
        container
      );
  ...
  testStartSignIn_pendingRedirect_error() {
    ...
    assertEquals("tenant1", externalAuth.tenantId);
    assertEquals("tenant1", internalAuth.tenantId);
    internalAuth.assertGetRedirectResult([], null, function () {
      // signInUiShown callback should not be triggered.
      assertEquals(0, signInUiShownCallback.getCallCount());
      // Spinner progress bar should be visible.
      assertProgressBarVisible(container);
  ...
  testHandleError() {
    ...
    let errorMessage = strings.errorCIAP({ code: code }).toString();
    ...
    errorMessage = strings.errorCIAP({ code: code }).toString();
  ...
  testHandleError_retry() {
    ...
    const errorMessage = strings.errorCIAP({ code: code }).toString();
    handler = new FirebaseUiHandler(container, configs);
    auth1 = handler.getAuth("API_KEY", "tenant1");
    handler.startSignIn(auth1);
  ...
  testProcessUser_reject() {
    ...
    const userCallback = recordFunction((user) =>
      GoogPromise.reject(expectedError)
    );
    // Set the beforeSignInSuccess callback in UI config.
    configs["API_KEY"]["callbacks"] = {
      beforeSignInSuccess: userCallback,
