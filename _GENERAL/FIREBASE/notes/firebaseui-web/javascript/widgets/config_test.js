...
testSuite({
  ...
  testGetQueryParameterForSignInSuccessUrl() {
    ...
    assertEquals("continue", config.getQueryParameterForSignInSuccessUrl());
  },

  testGetRequiredWidgetUrl() {
    assertThrows(() => {
      config.getRequiredWidgetUrl();
    });
    ...
    widgetUrl = config.getRequiredWidgetUrl(Config.WidgetMode.SIGN_IN);
  ...
  testGetSignInFlow() {
    ...
    assertEquals("redirect", config.getSignInFlow());
    // Update sign-in flow parameter to popup flow.
    config.update("signInFlow", "popup");
    // Confirm value changed.
    assertEquals("popup", config.getSignInFlow());
  ...
  testGetWidgetUrl_notSpecified() {
    ...
    widgetUrl = config.getWidgetUrl(Config.WidgetMode.SIGN_IN);
  ...
  testGetWidgetUrl_notSpecified_withQueryAndFragment() {
    ...
    assertEquals(util.getCurrentUrl(), widgetUrl);
    // Only the mode query param should be overwritten.
    widgetUrl = config.getWidgetUrl(Config.WidgetMode.SIGN_IN);
    assertEquals(
      "http://www.example.com/path/?mode2=bar&mode=signIn#a=1",
      widgetUrl
    );
    ...
    widgetUrl = config.getWidgetUrl(Config.WidgetMode.SIGN_IN);
    assertEquals(
      "http://www.example.com/path/?mode=foo&mode2=signIn#a=1",
      widgetUrl
    );
  },
  ...
  testGetWidgetUrl_specified() {
    ...
    let widgetUrl = config.getWidgetUrl();
    assertEquals("http://localhost/callback", widgetUrl);
    widgetUrl = config.getWidgetUrl(Config.WidgetMode.CALLBACK);
  ...
  testGetSignInSuccessUrl() {
    ...
    assertEquals("http://localhost/home", config.getSignInSuccessUrl());
  },
  ...
  testGetRecaptchaParameters() {
    ...
    config.update("signInOptions", [
      "github.com",
      { provider: "google.com" },
      { provider: "phone", recaptchaParameters: [1, true] },
      "password",
    ]);
    assertNull(config.getRecaptchaParameters());
    ...
    config.update("signInOptions", [
      "github.com",
      { provider: "google.com" },
      { provider: "phone", recaptchaParameters: {} },
      "password",
    ]);
    ...
    config.update("signInOptions", [
      "github.com",
      { provider: "google.com" },
      { provider: "phone", recaptchaParameters: mixed },
      "password",
    ]);
  ...
  testGetEmailProviderHelperLink() {
    ...
    stub.replace(util, "isCordovaInAppBrowserInstalled", () => true);
    helpLinkCallback();
  ...
  testIsAccountSelectionPromptEnabled_noAdditionalParameters() {
    config.update("signInOptions", ["google.com"]);
    assertFalse(config.isAccountSelectionPromptEnabled());
  },

  testIsAccountSelectionPromptEnabled_noOAuthProvider() {
    config.update("signInOptions", ["phone", "password"]);
    assertFalse(config.isAccountSelectionPromptEnabled());
  },
  ...
  testGetGoogleYoloClientId_availableAndEnabled() {
    ...
    config.update("credentialHelper", Config.CredentialHelper.GOOGLE_YOLO);
  ...
  testGetPhoneAuthSelectedCountries_whitelist_overlap() {
    ...
    const actualKeys = googArray.map(countries, (country) => country.e164_key);
    assertSameElements(
      ["44-GG-0", "44-IM-0", "44-JE-0", "44-GB-0"],
      actualKeys
    );
  },
  ...
  testUpdateConfig_phoneSignInOption_error() {
    ...
    assertEquals("BlacklistedCountries must be an array.", error.message);
  ...
  testGetProviderAdditionalScopes_multipleIdp() {
    ...
    assertArrayEquals([], config.getProviderAdditionalScopes("facebook.com"));
    assertArrayEquals([], config.getProviderAdditionalScopes("twitter.com"));
  },
  ...
  testSetConfig_nonDefined() {
    ...
    assertArrayEquals(
      ['Invalid config: "test1"', 'Invalid config: "test2"'],
      errorLogMessages
    );
  },
  ...
  testGetCallbacks() {
    ...
    assertEquals(signInSuccessCallback, config.getSignInSuccessCallback());
    assertEquals(
    ...
    assertEquals(signInFailureCallback, config.getSignInFailureCallback());
  },
  ...
  testGetCredentialHelper_httpOrHttps() {
    ...
    stub.replace(util, "isHttpOrHttps", () => true);
    // Default is none.
  ...
  testGetCredentialHelper_nonHttpOrHttps() {
    ...
    stub.replace(util, "isHttpOrHttps", () => false);
    // Default is none.
    assertEquals("none", config.getCredentialHelper());
