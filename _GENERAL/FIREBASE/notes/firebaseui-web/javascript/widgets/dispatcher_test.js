...
function assertHandlerInvoked(handlerName, var_args) {
  const handler = firebaseui.auth.widget.handlers_[handlerName];
  // Assert expected handler called.
  assertEquals(1, handler.getCallCount());
  // Assert expected parameters used for handler.
  for (let i = 0; i < arguments.length - 1; i++) {
    assertEquals(arguments[i + 1], handler.getLastCall().getArgument(i));
  }
}
...
function setModeAndUrlParams(mode, opt_params) {
  ...
  stub.replace(util, "getCurrentUrl", () => {
    let currentUrl = "https://www.example.com/";
    if (mode) {
      currentUrl += "?mode=" + encodeURIComponent(mode);
      for (let name in params) {
        currentUrl +=
          `&${name}=` + encodeURIComponent(asserts.assertString(params[name]));
      }
    }
    return currentUrl;
  });
...
testSuite({
  setUp() {
    ...
    app.updateConfig("queryParameterForSignInSuccessUrl", "signInSuccessUrl");
    // Assume widget already rendered and Auth UI global reference set.
    stub.replace(AuthUI, "getAuthUi", () => app);
  },
  ...
  testGetMode() {
    ...
    assertEquals(Config.WidgetMode.CALLBACK, dispatcher.getMode(app, url));
  },
  ...
  testGetRedirectUrl() {
    ...
    assertEquals(null, dispatcher.getRedirectUrl(app, url));
    // Set current page URL to include a redirect URL.
    url =
      "http://localhost/callback?mode=signIn&signInSuccessUrl=" +
      encodeURIComponent(redirectUrl);
    // Check that the redirect URL is successfully retrieved.
    assertEquals(redirectUrl, dispatcher.getRedirectUrl(app, url));
    ...
    app.updateConfig("queryParameterForSignInSuccessUrl", "continue");
    // Confirm that previous redirect URL no longer valid.
    assertEquals(null, dispatcher.getRedirectUrl(app, url));
    // Update the query parameter for redirect URL.
    url =
      "http://localhost/callback?mode=signIn&continue=" +
      encodeURIComponent(redirectUrl);
  ...
  testDispatchOperation_select() {
    ...
    assertHandlerInvoked(HandlerName.PROVIDER_SIGN_IN, app, element);
  },
  ...
  testDispatchOperation_selectWithRedirectUrl() {
    ...
    assertEquals(redirectUrl, storage.getRedirectUrl(app.getAppId()));
    // Provider sign-in should be invoked.
    assertHandlerInvoked(HandlerName.PROVIDER_SIGN_IN, app, element);
  },
  ...
  testDispatchOperation_callbackWithRedirectUrl() {
    ...
    assertHandlerInvoked(HandlerName.CALLBACK, app, element);
  },
  ...
  testDispatchOperation_callback_canSkipNascarScreen() {
    ...
    assertHandlerInvoked(HandlerName.FEDERATED_REDIRECT, app, element);
  },
  ...
  testDispatchOperation_revokeChangeEmail() {
    const element = dom.createElement("div");
    setModeAndUrlParams(Config.WidgetMode.RECOVER_EMAIL, {
      oobCode: "ACTION_CODE",
    });
    dispatcher.dispatchOperation(app, element);
    assertHandlerInvoked(
  ...
  testDispatchOperation_verifyEmail() {
    const element = dom.createElement("div");
    setModeAndUrlParams(Config.WidgetMode.VERIFY_EMAIL, {
      oobCode: "ACTION_CODE",
    });
    dispatcher.dispatchOperation(app, element);
    assertHandlerInvoked(
  ...
  testDispatchOperation_emailLinkSignIn() {
    const element = dom.createElement("div");
    setModeAndUrlParams(Config.WidgetMode.SIGN_IN, {
      oobCode: "ACTION_CODE",
      lang: "en",
    });
    dispatcher.dispatchOperation(app, element);
  ...
  testDispatchOperation_verifyAndChangeEmail() {
    const element = dom.createElement("div");
    setModeAndUrlParams(Config.WidgetMode.VERIFY_AND_CHANGE_EMAIL, {
      oobCode: "ACTION_CODE",
    });
    dispatcher.dispatchOperation(app, element);
    assertHandlerInvoked(
  ...
  testDispatchOperation_revertSecondFactorAddition() {
    const element = dom.createElement("div");
    setModeAndUrlParams(Config.WidgetMode.REVERT_SECOND_FACTOR_ADDITION, {
      oobCode: "ACTION_CODE",
    });
    dispatcher.dispatchOperation(app, element);
    assertHandlerInvoked(
  ...
  testDispatchOperation_resetPassword() {
    const element = dom.createElement("div");
    setModeAndUrlParams(Config.WidgetMode.RESET_PASSWORD, {
      oobCode: "ACTION_CODE",
    });
    dispatcher.dispatchOperation(app, element);
    assertHandlerInvoked(
