...
testSuite({
  ...
  testGetPrivacyPolicyUrl() {
    ...
    assertEquals(
      privacyPolicyUrlCallback,
      uiHandlerConfig.getPrivacyPolicyUrl()
    );
  ...
  testGetProvidersForTenant() {
    ...
    configObject["tenants"]["*"] = {
      ...
      buttonColor: "#37D2AC",
      iconUrl: "<icon-url-of-sign-in-button>",
      signInOptions: ["microsoft.com", "google.com"],
    };
    assertArrayEquals(
  ...
  testGetProvidersForTenant_emailMatch() {
    ...
    let providers = uiHandlerConfig.getProvidersForTenant(
      "tenantId1",
      "user@acme.com"
    );
    ...
    configObject["tenants"]["tenantId1"]["signInOptions"][0]["hd"] =
      /@acme\.com$/;
    providers = uiHandlerConfig.getProvidersForTenant(
      "tenantId1",
      "user@acme.com"
    );
    assertArrayEquals(["microsoft.com"], providers);
    ...
    providers = uiHandlerConfig.getProvidersForTenant("_", "user@example.com");
  ...
  testGetSignInConfigForTenant_eligibleProviders() {
    ...
    const tenantSignInConfig = uiHandlerConfig.getSignInConfigForTenant(
      "tenantId1",
      ["microsoft.com"]
    );
  ...
  testGetSignInConfigForTenant_topLevelProject() {
    ...
    const tenantSignInConfig = uiHandlerConfig.getSignInConfigForTenant("_");
    // All sign-in related properties in the original config object should be
    // copied over to the returned sign-in config object.
    UiHandlerConfig.SUPPORTED_SIGN_IN_CONFIG_KEYS.forEach((key) => {
  ...
  testGetSelectionButtonConfigForTenant_defaultConfig() {
    ...
    const tenantButtonConfig =
      uiHandlerConfig.getSelectionButtonConfigForTenant("arbitrary_tenant_id");
    assertObjectEquals(
      {
