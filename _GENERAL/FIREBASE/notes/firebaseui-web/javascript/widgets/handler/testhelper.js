...
var federatedIdToken =
  "HEADER.eyJhdWQiOiAiY2xpZW50X2lkIiwgImVtYWlsIjogInVzZ" +
...
var passwordIdToken =
  "HEADER.eyJhdWQiOiAiY2xpZW50X2lkIiwgImVtYWlsIjogInVzZX" +
...
var passwordAccount = new firebaseui.auth.Account(
  "user@example.com",
  "Password User"
);
...
var operationNotSupportedError = {
  ...
  message:
    "This operation is not supported in the environment this " +
...
var googleYoloIdTokenCredential = {
  credential:
    "HEADER." +
    btoa(JSON.stringify({ email: federatedAccount.getEmail() })) +
    ".SIGNATURE",
...
function waitForPageChange() {
  return new goog.Promise(function (resolve, reject) {
    goog.events.listenOnce(pageEventDispatcher, "pageEnter", function (event) {
      resolve();
    });
    goog.events.listenOnce(pageEventDispatcher, "pageExit", function (event) {
      resolve();
    });
  });
}
...
function simulateCordovaEnvironment() {
  testStubs.replace(firebaseui.auth.util, "getScheme", function () {
    return "file:";
  });
}
...
function generateSignInLink(
  ...
  var url =
    "https://www.example.com/signIn?mode=" +
...
function clickHelpLink() {
  const helpLink = goog.dom.getElementByClass(
    "firebaseui-id-unauthorized-user-help-link",
    container
  );
...
function getEmailInfoContainer() {
  return goog.dom.getElementByClass(
    "firebaseui-id-email-info-container",
    container
  );
}
...
function getPasswordInfoContainer() {
  return goog.dom.getElementByClass(
    "firebaseui-id-password-info-container",
    container
  );
}
...
function getLinkedAccountsContainer() {
  return goog.dom.getElementByClass(
    "firebaseui-id-linked-accounts-container",
    container
  );
}
...
function getResetPasswordLinkElement() {
  return goog.dom.getElementByClass(
    "firebaseui-id-reset-password-link",
    container
  );
}
...
function assertAdminEmail(adminEmail) {
  const element = goog.dom.getElementByClass(
    "firebaseui-id-unauthorized-user-admin-email",
    container
  );
  assertNotNull(element);
  if (adminEmail) {
...
function assertRevertSecondFactorAdditionSuccessPage() {
  assertPage_(
    container,
    "firebaseui-id-page-revert-second-factor-addition-success"
  );
}
...
function assertRevertSecondFactorAdditionFailurePage() {
  assertPage_(
    container,
    "firebaseui-id-page-revert-second-factor-addition-failure"
  );
}
