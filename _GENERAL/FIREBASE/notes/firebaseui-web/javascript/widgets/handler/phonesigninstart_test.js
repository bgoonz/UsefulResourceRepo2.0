...
function createMockConfirmationResult(operationType, isNewUser) {
  return {
    confirm: function (code) {
      ...
      app
        .getExternalAuth()
        .setUser({ phone: "+11234567890", displayName: "John Smith" });
...
function testHandlePhoneSignInStart_visible() {
  ...
  return recaptchaVerifierInstance
    ...
    .then(function () {
      ...
      var mockConfirmationResult = createMockConfirmationResult(
        "signIn",
        false
      );
    ...
    .then(function () {
      ...
      assertResendCountdown(
        "0:" + firebaseui.auth.widget.handler.RESEND_DELAY_SECONDS
      );
...
function testHandlePhoneSignInStart_anonymousUpgrade_signInError() {
  ...
  return recaptchaVerifierInstance
    ...
    .then(function () {
      ...
      assertEquals(
        5,
        goog.global["grecaptcha"].reset.getLastCall().getArgument(0)
      );
      // No dialog shown.
      assertNoDialog();
