...
firebaseui.auth.widget.handler.handleEmailLinkSignInCallback = function (
  ...
  var checkActionCodeAndGetUser = function () {
    ...
    if ((anonymousUid && isNewDevice) || (isNewDevice && forceSameDevice)) {
      ...
      anonymousUserPromise = app
        .getUpgradeableEmailLinkUser(link)
        .then(function (user) {
    ...
    return anonymousUserPromise
      .then(function (user) {
        ...
        return !!opt_skipCodeCheck
          ? null
          : app.getAuth().checkActionCode(oobCode);
      })
      .then(function () {
...
firebaseui.auth.widget.handler.completeEmailLinkSignIn_ = function (
  ...
  var timer = null;
  var p = signInPromise.then(
    function (authResult) {
      // Clear credentials and email on success.
      firebaseui.auth.storage.removeEncryptedPendingCredential(app.getAppId());
