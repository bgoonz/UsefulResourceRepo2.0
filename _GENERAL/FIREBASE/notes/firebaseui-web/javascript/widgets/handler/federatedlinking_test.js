...
function testHandleFederatedLinking_githubLoginHint() {
  ...
  // github.com provider.
  var expectedProvider = getExpectedProviderWithCustomParameters("github.com", {
    login: federatedAccount.getEmail(),
  });
  setPendingEmailCredentials();
  firebaseui.auth.widget.handler.handleFederatedLinking(
    app,
...
function testHandleFederatedLinking_popup_upgradeAnonymous() {
  ...
  testAuth.assertSignInWithPopup([expectedProvider], function () {
    // User should be signed in.
    testAuth.setUser({
      email: federatedAccount.getEmail(),
      displayName: federatedAccount.getDisplayName(),
  ...
  return testAuth
    ...
    .then(function () {
      ...
      // instance user.
      testAuth.currentUser.assertLinkWithCredential([credential], {
        user: testAuth.currentUser,
        credential: credential,
        operationType: "link",
        additionalUserInfo: {
