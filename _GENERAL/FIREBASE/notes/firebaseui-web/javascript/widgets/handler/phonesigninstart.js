...
firebaseui.auth.widget.handler.handlePhoneSignInStart = function (
  ...
  var defaultNationalNumber = isPhoneProviderOnly
    ? app.getConfig().getPhoneAuthDefaultNationalNumber()
    : null;
  var countryId =
    (opt_phoneNumberValue && opt_phoneNumberValue.countryId) ||
    (defaultCountry && defaultCountry.e164_key) ||
    null;
  var nationalNumber =
    (opt_phoneNumberValue && opt_phoneNumberValue.nationalNumber) ||
  ...
  recaptchaParameters["callback"] = function (recaptchaToken) {
    ...
    if (component.getRecaptchaErrorElement()) {
      firebaseui.auth.ui.element.hide(component.getRecaptchaErrorElement());
    }
 ...
 * @private
 */
firebaseui.auth.widget.handler.onPhoneSignInStartSubmit_ = function (
  app,
  component,
  recaptchaVerifier,
  opt_isKeyCode
  ...
  if (!firebaseui.auth.widget.handler.recaptchaToken_) {
    ...
    if (firebaseui.auth.widget.handler.enableVisibleRecaptcha_) {
      ...
      firebaseui.auth.ui.element.show(
        ...
        firebaseui.auth.soy2.strings.errorMissingRecaptchaResponse().toString()
      );
    } else if (
      !firebaseui.auth.widget.handler.enableVisibleRecaptcha_ &&
      !!opt_isKeyCode
          ...
          switch (error["code"]) {
            case "auth/too-many-requests":
              errorMessage = firebaseui.auth.soy2.strings
                .errorTooManyRequestsPhoneNumber()
                .toString();
              break;
