 ...
 * @package
 */
firebaseui.auth.widget.handler.common.normalizeError = function (error) {
  if (
    ...
    error["message"] &&
    error["message"].indexOf("HTTP Cloud Function returned an error:") !== -1
  ) {
 ...
 * @package
 */
firebaseui.auth.widget.handler.common.setLoggedInWithAuthResult = function (
  app,
  component,
  authResult,
  opt_alreadySignedIn
 ...
 * @private
 */
firebaseui.auth.widget.handler.common.setUserLoggedInExternal_ = function (
  app,
  component,
  user,
  credential
  ...
  var redirectUrl =
    firebaseui.auth.storage.getRedirectUrl(app.getAppId()) || undefined;
  ...
  if (firebaseui.auth.util.hasOpener()) {
    ...
    if (
      !callback ||
      callback(/** @type {!firebase.User} */ (user), credential, redirectUrl)
    ) {
...
firebaseui.auth.widget.handler.common.setUserLoggedInExternalWithAuthResult_ =
  function (app, component, authResult) {
    ...
    var signInSuccessWithAuthResultCallback = app
      .getConfig()
 ...
 * @private
 */
firebaseui.auth.widget.handler.common.getSignedInRedirectUrl_ = function (
  app,
  opt_redirectUrl
) {
  ...
  if (!redirectUrl) {
    throw new Error(
      "No redirect URL has been found. You must either specify " +
...
firebaseui.auth.widget.handler.common.getErrorMessage = function (error) {
  ...
  var message = firebaseui.auth.soy2.strings
...
firebaseui.auth.widget.handler.common.federatedSignIn = function (
    ...
    switch (normalizedError["code"]) {
      ...
      case "auth/user-cancelled":
        ...
        component.showInfoBar(
          firebaseui.auth.widget.handler.common.getErrorMessage(normalizedError)
        );
        break;
      default:
  ...
  if (isRedirectMode) {
    ...
    app.registerPending(
      app.startSignInWithPopup(provider).then(function (result) {
        // Pass result in promise to callback handler.
        component.dispose();
        firebaseui.auth.widget.handler.handle(
          firebaseui.auth.widget.HandlerName.CALLBACK,
 ...
 * @package
 */
firebaseui.auth.widget.handler.common.handleGoogleYoloCredential = function (
  app,
  component,
  credential
) {
  ...
  if (
    ...
    if (
      ...
      return signInWithCredential(
        firebase.auth.GoogleAuthProvider.credential(credential.credential)
      );
    }
  } else if (credential) {
 ...
 * @package
 */
firebaseui.auth.widget.handler.common.verifyPassword = function (
  app,
  component
) {
 ...
 */
firebaseui.auth.widget.handler.common.handleStartEmailFirstFlow = function (
  app,
  component,
  email,
  opt_infoBarMessage
...
firebaseui.auth.widget.handler.common.handleSignInFetchSignInMethodsForEmail =
  function (
    ...
    if (
      ...
      (app.getConfig().isEmailPasswordSignInAllowed() ||
 ...
 */
firebaseui.auth.widget.handler.common.handleSignInWithEmail = function (
  app,
  container,
  opt_email
) {
