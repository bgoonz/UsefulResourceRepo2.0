...
function setUp() {
  ...
  goog.storage.mechanism.mechanismfactory
    .createHTML5LocalStorage(firebaseui.auth.storage.NAMESPACE_)
    .clear();
  goog.storage.mechanism.mechanismfactory
    .createHTML5SessionStorage(firebaseui.auth.storage.NAMESPACE_)
    .clear();
  // Mock credential.
...
function initializeCookieStorageMock(maxAge, path, domain, secure) {
  ...
  stubs.replace(goog.net.cookies, "get", function (key) {
...
function testGetSetRemoveRedirectUrl_withAppId() {
  ...
  assertEquals(
    "http://www.example.com",
    firebaseui.auth.storage.getRedirectUrl(appId)
  );
  assertEquals(
...
function testGetSetRemoveEmailForSignIn_withAppId() {
  ...
  assertEquals(email1, firebaseui.auth.storage.getEmailForSignIn(key1, appId));
  assertNull(firebaseui.auth.storage.getEmailForSignIn(key2, appId));
  assertEquals(email2, firebaseui.auth.storage.getEmailForSignIn(key2, appId2));
