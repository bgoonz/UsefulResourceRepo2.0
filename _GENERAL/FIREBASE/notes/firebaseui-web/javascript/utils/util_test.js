...
function testGetElement() {
  ...
  try {
    firebaseui.auth.util.getElement(
      "#notFound",
...
function testIsHttpOrHttps() {
  // HTTP scheme.
  stubs.replace(firebaseui.auth.util, "getScheme", function () {
    return "http:";
  });
...
function testGoTo_unsafeUrl() {
  ...
  assertThrows(function () {
    firebaseui.auth.util.goTo("javascript:doEvilStuff()", mockWin);
  });
...
function testOpenerGoTo_unsafeUrl() {
  ...
  // called.
  assertThrows(function () {
    firebaseui.auth.util.openerGoTo("javascript:doEvilStuff()", mockWin);
  });
  // location.assign should not be called due to assertion error.
...
function testSanitizeUrl_safeUrl() {
  assertEquals(
    "https://www.example.com:80/path/?a=1#b=2",
    firebaseui.auth.util.sanitizeUrl("https://www.example.com:80/path/?a=1#b=2")
  );
}
...
function testGenerateRandomAlphaNumericString() {
  ...
  for (var i = 0; i < 10; i++) {
    assertEquals(
      i,
      firebaseui.auth.util.generateRandomAlphaNumericString(i).length
    );
  }
}
...
function testFilterProperties_emptySourceObject() {
  ...
  assertObjectEquals({}, firebaseui.auth.util.filterProperties({}, keys));
}
