...
firebaseui.auth.util.isHttpOrHttps = function () {
  return (
    firebaseui.auth.util.getScheme() === "http:" ||
 ...
 *     new window.
 */
firebaseui.auth.util.open = function (
  url,
  windowName,
  opt_width,
  opt_height,
 ...
 * @param {number=} opt_height height of the popup
 */
firebaseui.auth.util.popup = function (
  opt_url,
  opt_name,
  opt_width,
  opt_height
...
firebaseui.auth.util.getElement = function (element, opt_notFoundDesc) {
  element = goog.dom.isElement(element)
    ? element
    : document.querySelector(String(element));
  if (element == null) {
 ...
 */
firebaseui.auth.util.filterProperties = function (
  source,
  keys,
  initialValue = {}
) {
