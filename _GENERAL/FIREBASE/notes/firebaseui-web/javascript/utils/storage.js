...
goog.scope(function () {
  ...
  storage.isAvailable = function () {
    return (
      new goog.storage.mechanism.HTML5LocalStorage().isAvailable() &&
  ...
  storage.get_ = function (key, opt_id) {
    return key.storage.get(storage.getKeyName_(key, opt_id));
  };
  ...
  storage.remove_ = function (key, opt_id) {
    key.storage.remove(storage.getKeyName_(key, opt_id));
  };
  ...
  storage.set_ = function (key, value, opt_id) {
    key.storage.set(storage.getKeyName_(key, opt_id), value);
  };
   ...
   *     associates storage values with specific apps.
   */
  storage.setPendingEmailCredential = function (
  ...
  storage.getEmailForSignIn = function (encryptionKey, opt_id) {
    var encryptedEmailObject = storage.get_(
