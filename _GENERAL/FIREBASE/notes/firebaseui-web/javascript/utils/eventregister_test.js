...
function testEventDispatcher() {
  ...
  goog.events.listen(eventDispatcher, myEvent, function () {
    triggered++;
  });
  // Trigger event.
  firebaseui.auth.EventRegister.dispatchEvent(root, myEvent);
  ...
  // Trigger myEvent2.
  firebaseui.auth.EventRegister.dispatchEvent(root, myEvent2);
...
function testEventDispatcher_sameEvent_duplicateElement() {
  ...
  goog.events.listen(eventDispatcher2, myEvent, function () {
    triggered2++;
  });
  ...
  assertArrayEquals([eventDispatcher2], firebaseui.auth.EventRegister.map_[0]);
  // Dispatch myEvent on root.
  firebaseui.auth.EventRegister.dispatchEvent(root, myEvent);
...
function testEventDispatcher_sameEvent_differentElements() {
  ...
  assertArrayEquals([eventDispatcher], firebaseui.auth.EventRegister.map_[0]);
  assertArrayEquals([eventDispatcher2], firebaseui.auth.EventRegister.map_[1]);
  // Only root's dispatcher triggered.
  assertEquals(1, triggered);
  assertEquals(0, triggered2);
  // Dispatch myEvent on root2.
  firebaseui.auth.EventRegister.dispatchEvent(root2, myEvent);
...
function testEventDispatcher_multipleEvents_sameElement() {
  ...
  goog.events.listen(eventDispatcher, myEvent2, function () {
    triggered2++;
  });
...
function testEventDispatcher_multipleEvents_multipleElements() {
  ...
  goog.events.listen(eventDispatcher2, myEvent2, function () {
    triggered2++;
  });
  // Dispatch myEvent on root.
  firebaseui.auth.EventRegister.dispatchEvent(root, myEvent);
  // Dispatch myEvent2 on root2.
  firebaseui.auth.EventRegister.dispatchEvent(root2, myEvent2);
