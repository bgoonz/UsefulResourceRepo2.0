 ...
 */
firebaseui.auth.idp.getFirstFederatedSignInMethod = function (
  signInMethods,
  enabledProviders
) {
...
firebaseui.auth.idp.getAuthProvider = function (providerId) {
  if (
    firebaseui.auth.idp.AuthProviders[providerId] &&
  ...
  if (
    ...
    if (credentialObject["secret"] && credentialObject["accessToken"]) {
      return firebase.auth[
        firebaseui.auth.idp.AuthProviders[providerId]
      ].credential(credentialObject["accessToken"], credentialObject["secret"]);
    } else if (providerId == firebase.auth.GoogleAuthProvider.PROVIDER_ID) {
      return firebase.auth[
        firebaseui.auth.idp.AuthProviders[providerId]
      ].credential(
        credentialObject["idToken"],
