...
firebaseui.auth.data.country.getCountriesByIso2 = function (code) {
  ...
  for (var i = 0; i < firebaseui.auth.data.country.COUNTRY_LIST.length; i++) {
    if (
      firebaseui.auth.data.country.COUNTRY_LIST[i].iso2_cc === normalizedCode
    ) {
...
firebaseui.auth.data.country.getCountriesByE164OrIsoCode = function (code) {
  ...
  if (code.length > 0 && code.charAt(0) == "+") {
    ...
    countries = firebaseui.auth.data.country.getCountriesByIso2(code);
  }
  return countries;
};
 ...
 * @param {string} locale
 */
firebaseui.auth.data.country.sortCountryListForLocale = function (
  list,
  locale
) {
