...
goog.scope(function () {
  ...
  element.phoneNumber.initPhoneNumberElement = function (
    ...
    var countrySelectorElement =
      element.phoneNumber.getCountrySelectorElement.call(this);
    var errorElement =
   ...
   * @private
   */
  element.phoneNumber.selectDefaultCountry_ = function (
    lookupTree,
    opt_countryId
  ) {
    var availableCountries = lookupTree.getCountries();
    ...
    if (
      opt_countryId &&
      element.phoneNumber.isCountryAvailable_(opt_countryId, availableCountries)
    ) {
      ...
      if (!predefinedDefaultCountryAvailable) {
        ...
        // country.
        countryId =
          availableCountries.length > 0 ? availableCountries[0].e164_key : null;
      } else {
   ...
   * @private
   */
  element.phoneNumber.isCountryAvailable_ = function (
    countryId,
    availableCountries
  ) {
  ...
  element.phoneNumber.createListBoxItemList_ = function (availableCountries) {
    return goog.array.map(availableCountries, function (country) {
      return {
        id: country.e164_key,
        iconClass:
  ...
  element.phoneNumber.handleCountrySelectorButtonClick_ = function (
    ...
    var self = this;
    firebaseui.auth.ui.element.listBoxDialog.showListBoxDialog.call(
      this,
      element.phoneNumber.createListBoxItemList_(lookupTree.getCountries()),
      function (id) {
        element.phoneNumber.selectCountry.call(self, id, lookupTree, true);
   ...
   * @this {goog.ui.Component}
   */
  element.phoneNumber.selectCountry = function (
    id,
    lookupTree,
    ...
    var flagElement =
      element.phoneNumber.getCountrySelectorFlagElement_.call(this);
    if (oldCountry) {
