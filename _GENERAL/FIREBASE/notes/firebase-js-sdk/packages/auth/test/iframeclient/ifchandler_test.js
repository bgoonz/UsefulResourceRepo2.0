...
function simulateAuthService(appName, languageCode, frameworks) {
  stubs.replace(firebase, 'app', function (name) {
...
function testOAuthUrlBuilder() {
  ...
  var partialUrl =
    'https://example.firebaseapp.com/__/auth/handler?apiKey=A' +
    ...
    '&scopes=' +
    encodeURIComponent('profile,scope1,scope2');
  ...
  assertEquals(
    partialUrl +
      '&redirectUrl=' +
      encodeURIComponent(redirectUrl2) +
      '&eventId=EVENT_ID1',
  ...
  assertEquals(
    partialUrl +
      ...
      encodeURIComponent('com.example.android') +
      '&sessionId=SESSION_ID1',
  ...
  // Delete eventId.
  assertEquals(partialUrl + '&v=3.4.1', builder.setEventId(null).toString());
  // Delete version.
  assertEquals(partialUrl, builder.setVersion(null).toString());
  // Set eid parameter.
  builder.setEndpointId('p');
  assertEquals(partialUrl + '&eid=p', builder.toString());
  // Modify eid parameter.
  assertEquals(partialUrl + '&eid=s', builder.setEndpointId('s').toString());
  // Remove eid parameter.
  assertEquals(partialUrl, builder.setEndpointId(null).toString());
...
function testOAuthUrlBuilder_localization() {
  ...
  expectedUrl =
    ...
    'm&customParameters=' +
    encodeURIComponent(
      fireauth.util.stringifyJSON(expectedCustomParamsWithLang)
    ) +
    '&scopes=' +
    encodeURIComponent('profile,scope1,scope2');
  assertEquals(expectedUrl, builder.toString());
  ...
  expectedUrl =
    ...
    'm&customParameters=' +
    encodeURIComponent(
      fireauth.util.stringifyJSON(expectedCustomParamsWithoutLang)
    ) +
    '&scopes=' +
    encodeURIComponent('profile,scope1,scope2');
  assertEquals(expectedUrl, builder.toString());
...
function testOAuthUrlBuilder_withEmulatorConfig() {
  ...
  var url =
    'http://emulator.host:1234/emulator/auth/handler?' +
    'apiKey=API_KEY&appName=APP_NAME&authType=signInWithPopup&' +
...
function testIfcHandler() {
  ...
  var handler1 = goog.testing.recordFunction(function () {
    return true;
  });
  var handler2 = goog.testing.recordFunction(function () {
    return true;
  });
  var handler3 = goog.testing.recordFunction(function () {
    return false;
  });
  ...
  assertEquals(
    0,
    fireauth.iframeclient.IframeWrapper.prototype.onReady.getCallCount()
  );
  ifcHandler.initialize().then(function () {
    ...
    assertObjectEquals(authEvent, handler1.getLastCall().getArgument(0));
    assertEquals(1, handler2.getCallCount());
    assertObjectEquals(authEvent, handler2.getLastCall().getArgument(0));
    assertEquals(1, handler3.getCallCount());
    assertObjectEquals(authEvent, handler3.getLastCall().getArgument(0));
    // Remove handler2.
...
function testIfcHandler_shouldNotBeInitializedEarly() {
  ...
  stubs.replace(fireauth.util, 'iframeCanSyncWebStorage', function () {
    return true;
  });
...
function testIfcHandler_initializeAndWait_success() {
  ...
  stubs.replace(fireauth.util, 'goTo', goog.testing.recordFunction());
  ...
  rpcHandlerConstructor(apiKey, endpointConfig, expectedClientVersion).$returns(
    rpcHandler
  );
...
function testIfcHandler_processPopup_notAlreadyRedirected_success() {
  ...
  ifcHandler
    ...
    .then(function () {
      ...
      assertEquals(popupWin, fireauth.util.goTo.getLastCall().getArgument(1));
      asyncTestCase.signal();
    });
}
...
function testIfcHandler_processRedirect_tenantId_success() {
  ...
  // Should succeed and redirect.
  ifcHandler
    .processRedirect('linkViaRedirect', provider, '1234', tenantId)
    .then(function () {
      /** @suppress {missingRequire} */
      assertEquals(
        expectedUrl,
...
function testGetAuthIframeUrl_frameworks() {
  ...
  assertEquals(
    'https://subdomain.firebaseapp.com/__/auth/iframe?apiKey=apiKey1&appNa' +
      ...
      endpointId +
      '&fw=' +
      encodeURIComponent(expectedFrameworks.join(',')),
    fireauth.iframeclient.IfcHandler.getAuthIframeUrl(
      authDomain,
      apiKey,
      appName,
...
function testGetOAuthHelperWidgetUrl_redirectUrlEventIdVersionAndNoScopes() {
  ...
  var version = '3.0.0-rc.1';
  var endpointId = 's';
  var expectedWidgetUrl =
    'https://subdomain.firebaseapp.com/__/auth/handler' +
    '?apiKey=' +
    encodeURIComponent(apiKey) +
    '&appName=' +
    encodeURIComponent(appName) +
    '&authType=' +
    encodeURIComponent(authType) +
    '&providerId=' +
...
function testGetOAuthHelperWidgetUrl_injections() {
  ...
  var eventId = '12345678';
  var expectedWidgetUrl =
    'https://subdomain.firebaseapp.com%2F%3FredirectUr' +
...
function testGetOAuthHelperWidgetUrl_redirectUrlEventIdAndScopes() {
  ...
  var expectedWidgetUrl =
    ...
    encodeURIComponent(providerId) +
    '&scopes=' +
    encodeURIComponent('scope1,scope2,scope3') +
    '&redirectUrl=' +
    encodeURIComponent(redirectUrl) +
    '&eventId=' +
    encodeURIComponent(eventId);
...
function testGetOAuthHelperWidgetUrl_frameworksAndLanguageCode() {
  ...
  var expectedWidgetUrl =
    ...
    encodeURIComponent(providerId) +
    '&customParameters=' +
    encodeURIComponent(fireauth.util.stringifyJSON({ 'locale': 'fr' })) +
    '&scopes=' +
    encodeURIComponent('scope1,scope2,scope3') +
    '&fw=' +
    encodeURIComponent(frameworks.join(','));
  assertEquals(
    expectedWidgetUrl,
    fireauth.iframeclient.IfcHandler.getOAuthHelperWidgetUrl(
      authDomain,
...
function testGetOAuthHelperWidgetUrl_withEmulator() {
  ...
  var expectedWidgetUrl =
    'http://emulator.test.domain:1234/' +
