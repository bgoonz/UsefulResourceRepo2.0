 ...
 */
fireauth.common.testHelper.assertUserCredentialResponse = function (
  expectedUser,
  expectedCred,
  ...
  if (!expectedCred) {
    assertTrue(
      response['credential'] === null || response['credential'] === undefined
    );
  } else {
 ...
 */
fireauth.common.testHelper.assertUserStorage = function (
  appId,
  persistence,
  expectedUser,
  manager
...
fireauth.common.testHelper.assertIdTokenResult = function (
  ...
  assertEquals(
    fireauth.util.utcTimestampToDateString(expirationTime * 1000),
    idTokenResult['expirationTime']
  );
  assertEquals(
    fireauth.util.utcTimestampToDateString(authTime * 1000),
    idTokenResult['authTime']
  );
  assertEquals(
    fireauth.util.utcTimestampToDateString(issuedAtTime * 1000),
 ...
 */
fireauth.common.testHelper.installMockStorages = function (
  stub,
  mockLocalStorage,
  mockSessionStorage
) {
 ...
 */
fireauth.common.testHelper.createMockJwt = function (payload, expirationTime) {
  ...
  const basePayload = {
    ...
    'iat': now,
    'exp':
      typeof expirationTime === 'undefined' ? now + 3600 : expirationTime / 1000
  };
  // Extend base payload.
  Object.assign(basePayload, payload || {});
  const encodedPayload = goog.crypt.base64.encodeString(
