 ...
 */
function assertRpcHandler(expectedData, xhrResponse, error) {
  stubs.replace(
...
function testStsTokenManager_getToken_withClockSkew_clientBehind() {
  ...
  token.getToken().then(response => {
    ...
    assertObjectEquals(
      { 'accessToken': newJwt, 'refreshToken': 'refreshToken2' },
      response
    );
    asyncTestCase.signal();
  });
...
function testStsTokenManager_getToken_tokenAlmostExpired() {
  ...
  const almostExpiredJwt = fireauth.common.testHelper.createMockJwt(
    null,
    expirationTime
  );
  const newJwt = fireauth.common.testHelper.createMockJwt();
...
function testStsTokenManager_getToken_exchangeRefreshToken() {
  ...
  const newJwt = fireauth.common.testHelper.createMockJwt(null, unexpiredTime);
  token.setRefreshToken('refreshToken');
  // Expired access token.
  token.setAccessToken(expiredJwt);
  token.setExpiresAt(expirationTime);
...
function testFromPlainObject() {
  ...
  assertObjectEquals(
    token,
    fireauth.StsTokenManager.fromPlainObject(rpcHandler, {
      'apiKey': 'apiKey',
      'refreshToken': 'refreshToken',
      'accessToken': jwt,
      'expirationTime': expirationTime
