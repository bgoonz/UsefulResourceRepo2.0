...
var iOS7iPhoneUA =
  ...
  'fari/9537.53';
var androidUA =
  'Mozilla/5.0 (Linux; U; Android 4.0.3; ko-kr; LG-L160L Buil' +
  'd/IML74K) AppleWebkit/534.30 (KHTML, like Gecko) Version/4.0 Mobile Sa' +
  'fari/534.30';
var blackberryUA =
  'Mozilla/5.0 (BlackBerry; U; BlackBerry 9900; en) AppleW' +
  'ebKit/534.11+ (KHTML, like Gecko) Version/7.1.0.346 Mobile Safari/534.' +
  '11+';
var webOSUA =
  'Mozilla/5.0 (webOS/1.3; U; en-US) AppleWebKit/525.27.1 (KHTM' +
...
var chriosUA =
  'Mozilla/5.0 (iPhone; U; CPU iPhone OS 5_1_1 like Mac OS X; ' +
  'en) AppleWebKit/534.46.0 (KHTML, like Gecko) CriOS/19.0.1084.60 Mobile' +
  '/9B206 Safari/7534.48.3';
var iOS9iPhoneUA =
  'Mozilla/5.0 (iPhone; CPU iPhone OS 9_2 like Mac OS X) A' +
...
function testSanitizeRequestUri() {
  ...
  assertEquals(
    ...
    fireauth.util.sanitizeRequestUri('http://localhost/path/#abc')
  );
  assertEquals(
...
function testGoTo() {
  ...
  assertEquals(
    'http://www.google.com/some?complicated=path',
...
function testGetKeyDiff() {
  var a = { 'key1': 'a', 'key2': 'b' };
  var b = { 'key2': 'b', 'key3': 'c' };
  assertArrayEquals(['key1', 'key3'], fireauth.util.getKeyDiff(a, b));
  var c = {
    'key1': { 'c': 3, 'd': 4 },
    'key2': [5, 6],
    'key3': { 'a': 1, 'b': 2 }
  ...
  assertArrayEquals(['key3'], fireauth.util.getKeyDiff(c, d));
  var e = {
    'key1': { 'c': 3, 'd': 4 },
    'key2': [5, 6],
    'key3': { 'a': 1, 'b': 2 }
  ...
  assertArrayEquals(['key2', 'key3'], fireauth.util.getKeyDiff(e, f));
  ...
  assertArrayEquals([], fireauth.util.getKeyDiff(k, l));
  var m = { 'key1': [{ 'a': [{ 'c': 1 }], 'b': 2 }] };
  var n = { 'key1': [{ 'a': [{ 'c': 1 }], 'b': 2 }] };
  assertArrayEquals([], fireauth.util.getKeyDiff(m, n));
  var o = {
    'key1': [
      { 'a': 1, 'b': 2 },
      { 'c': 3, 'd': 4 }
  ...
  assertArrayEquals(['key1'], fireauth.util.getKeyDiff(o, p));
}
...
function testIsAuthorizedDomain() {
  ...
  assertTrue(
    fireauth.util.isAuthorizedDomain(
      ['other.com', 'domain.com'],
      'http://www.domain.com/abc?a=1#b=2'
    )
  );
  assertFalse(
    fireauth.util.isAuthorizedDomain(
  ...
  assertTrue(
    fireauth.util.isAuthorizedDomain(
      ['127.0.0.1'],
      'http://127.0.0.1:8080/?redirect=132.0.0.1'
    )
  );
  assertFalse(
    fireauth.util.isAuthorizedDomain(
  ...
  assertFalse(
    fireauth.util.isAuthorizedDomain(
      ['127.0.0.1'],
      'http://127.0.0.1.appdomain.com/?redirect=127.0.0.1'
    )
  );
  assertFalse(
    fireauth.util.isAuthorizedDomain(
      ['127.0.0.1'],
      'http://a127.0.0.1/?redirect=127.0.0.1'
    )
  );
  // Other schemes.
  assertFalse(
    fireauth.util.isAuthorizedDomain(['domain.com'], 'file://www.domain.com')
  );
  assertFalse(
    fireauth.util.isAuthorizedDomain(['domain.com'], 'other://www.domain.com')
  );
}

function testMatchDomain_chromeExtensionPattern() {
  assertFalse(
    fireauth.util.matchDomain(
      'chrome-extension://abcdefghijklmnopqrstuvwxyz123456',
      'abcdefghijklmnopqrstuvwxyz123456',
      'http'
    )
...
function testMatchDomain_ipAddressPattern() {
  ...
  assertFalse(
    fireauth.util.matchDomain('127.0.0.1', '127.0.0.1', 'chrome-extension')
  );
}
...
function testMatchDomain_caseInsensitiveMatch() {
  ...
  assertTrue(
    fireauth.util.matchDomain('doMAin.com', 'abC.domain.COM', 'https')
  );
  assertFalse(
    fireauth.util.matchDomain(
      'doMAin.com',
...
function testMatchDomain_domainMismatch() {
  ...
  assertFalse(
    fireauth.util.matchDomain('domain.com', 'abc.domain.com.lb', 'http')
  );
  assertFalse(
    fireauth.util.matchDomain('domain2.com', 'abc.domain.com', 'http')
  );
}
...
function testMatchDomain_subdomainComparison() {
  ...
  assertFalse(
    fireauth.util.matchDomain('other.domain.com', 'abc.domain.com', 'http')
  );
  assertTrue(
    fireauth.util.matchDomain('domain.com', 'abc.ef.gh.domain.com', 'http')
  );
  assertTrue(
    fireauth.util.matchDomain('domain.com', 'abc.ef.gh.domain.com', 'https')
  );
  assertFalse(
    fireauth.util.matchDomain(
      'domain.com',
      'abc.ef.gh.domain.com',
      'chrome-extension'
    )
  );
}
...
function testMatchDomain_dotsInPatternEscaped() {
  ...
  assertFalse(
    fireauth.util.matchDomain('domain.com', 'abc.domainacom', 'http')
  );
  assertFalse(fireauth.util.matchDomain('abc.def.com', 'abczdefzcom', 'http'));
}
...
function testGetEnvironment_reactNative() {
  ...
  assertEquals(fireauth.util.Env.REACT_NATIVE, fireauth.util.getEnvironment());
}
...
function testIsFetchSupported() {
  ...
  assertTrue(
    fireauth.util.isFetchSupported({
      'fetch': function () {},
      'Request': function () {},
      'Headers': function () {}
    })
...
function testGetBrowserName_other() {
  var ua =
    'Mozilla/5.0 (iPhone; CPU iPhone OS 8_2 like Mac OS X) AppleWebKi' +
...
function testGetFrameworkIds() {
  ...
  assertArrayEquals(
    ['FirebaseUI-web'],
    fireauth.util.getFrameworkIds(['FirebaseUI-web'])
  );
  assertArrayEquals(
...
function testGetClientVersion_frameworkVersion_single() {
  ...
  assertEquals('Chrome/JsCore/3.0.0/FirebaseUI-web', clientVersion);
}
...
function testGetClientVersion_frameworkVersion_multiple() {
  ...
  assertEquals(
    'Chrome/JsCore/3.0.0/FirebaseCore-web,FirebaseUI-web',
    clientVersion
  );
}
...
function testParseJSON_syntaxError() {
  assertThrows(function () {
    fireauth.util.parseJSON('{"a":2');
  });
  assertThrows(function () {
    fireauth.util.parseJSON('b:"hello"}');
  });
}
...
function testCheckIfCordova_incorrectFileEnvironment() {
  return installAndRunTest('checkIfCordova_incorrectFileEnv', function () {
    ...
    return fireauth.util
      .checkIfCordova(null, 10)
      .then(function () {
...
function testCheckIfCordova_deviceReadyTimeout() {
  return installAndRunTest('checkIfCordova_deviceReadyTimeout', function () {
    ...
    return fireauth.util
      ...
      .thenCatch(function (error) {
        assertEquals('Cordova framework is not ready.', error.message);
      });
  });
}
...
function testRemoveEntriesWithKeys() {
  ...
  assertObjectEquals({}, fireauth.util.removeEntriesWithKeys({}, []));

  // Remove keys from an empty object.
  assertObjectEquals({}, fireauth.util.removeEntriesWithKeys({}, ['a', 'b']));
...
function testCopyWithoutNullsOrUndefined() {
  ...
  assertObjectEquals(obj2, fireauth.util.copyWithoutNullsOrUndefined(obj2));
  // Empty object.
  assertObjectEquals({}, fireauth.util.copyWithoutNullsOrUndefined({}));
...
function testIsMobileDevice() {
  // Mobile devices.
  assertTrue(fireauth.util.isMobileDevice(chriosUA, fireauth.util.Env.BROWSER));
  assertTrue(
  ...
  assertFalse(fireauth.util.isMobileDevice(null, fireauth.util.Env.NODE));
  ...
  assertTrue(fireauth.util.isMobileDevice(chriosUA, fireauth.util.Env.WORKER));
  assertFalse(fireauth.util.isMobileDevice(chromeUA, fireauth.util.Env.WORKER));
}
...
function testDelay_desktopBrowser() {
  var delay = new fireauth.util.Delay(
    10,
    50,
    chromeUA,
    fireauth.util.Env.BROWSER
...
function testDelay_reactNative() {
  ...
  var delay = new fireauth.util.Delay(
    ...
    50,
    null,
    fireauth.util.Env.REACT_NATIVE
  );
  assertEquals(50, delay.get());
}
...
function testOnAppVisible_initiallyVisible() {
  return installAndRunTest('onAppVisible_initiallyVisible', function () {
    ...
    stubs.replace(fireauth.util, 'isAppVisible', function () {
      return true;
    });
...
function testPersistsStorageWithIndexedDB() {
  ...
  stubs.replace(fireauth.util, 'isAuthHandlerOrIframe', function () {
    return false;
  });
  stubs.replace(fireauth.util, 'isLocalStorageNotSynchronized', function () {
    return true;
  });
  stubs.replace(fireauth.util, 'isIndexedDBAvailable', function () {
    return true;
  });
