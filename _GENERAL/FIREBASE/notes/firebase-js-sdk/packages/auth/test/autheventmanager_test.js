...
function setOAuthSignInHandlerEnvironment(isCordova) {
  ...
  stubs.replace(fireauth.util, 'checkIfCordova', function () {
    if (isCordova) {
  ...
  if (isCordova) {
    ...
    stubs.replace(fireauth.util, 'getUserAgentString', function () {
      return androidUA;
    });
...
function testInstantiateOAuthSignInHandler_cordovaHandler() {
  ...
  var cordovaHandler = mockControl.createStrictMock(fireauth.CordovaHandler);
...
function testAuthEventManager_getRedirectResult_noRedirect() {
  ...
  setOAuthSignInHandlerEnvironment(false);
  asyncTestCase.waitForSignals(1);
  stubs.replace(fireauth.AuthEventManager.prototype, 'initialize', function () {
...
function testAuthEventManager_subscribeAndUnsubscribe() {
  ...
  assertEquals(
    0,
    fireauth.PopupAuthEventProcessor.prototype.processAuthEvent.getCallCount()
  );
...
function testAuthEventManager_testEventToProcessor() {
  ...
  var handler = {
    'canHandleAuthEvent': function (type, eventId) {
      return true;
    },
    'resolvePendingPopupEvent': goog.testing.recordFunction(),
    'getAuthEventHandlerFinisher': goog.testing.recordFunction()
  ...
  assertEquals(
    1,
    fireauth.RedirectAuthEventProcessor.prototype.processAuthEvent.getCallCount()
  );
  assertArrayEquals(
...
function testProcessPopup_success() {
  ...
  manager
    .processPopup(popupWin, 'linkViaPopup', provider, '1234')
    .then(function () {
...
function testProcessPopup_success_tenantId() {
  ...
  manager
    .processPopup(popupWin, 'linkViaPopup', provider, '1234', false, tenantId)
    .then(function () {
      assertTrue(popupRedirected);
      // This should resolve now as it is already initialized.
      asyncTestCase.signal();
...
function testReset_ifcHandler() {
  ...
  stubs.replace(
    OAuthSignInHandler.prototype,
    'initializeAndWait',
    goog.testing.recordFunction(OAuthSignInHandler.prototype.initializeAndWait)
  );
  stubs.replace(
    OAuthSignInHandler.prototype,
    'addAuthEventListener',
  ...
  var initializedInstance1 = OAuthSignInHandler.prototype.initializeAndWait
    .getLastCall()
    .getThis();
  manager.reset();
  ...
  var initializedInstance2 = OAuthSignInHandler.prototype.initializeAndWait
    .getLastCall()
    .getThis();
...
function testInitialize_errorLoadingOAuthHandler() {
  ...
  stubs.replace(OAuthSignInHandler.prototype, 'initializeAndWait', function () {
    if (willFail) {
...
function testProcessPopup_errorLoadingIframe() {
  ...
  stubs.replace(fireauth.util, 'goTo', function (url, win) {
    if (willFail) {
      ...
      fail(
        'OAuth handler loading failure should not lead to a popup ' +
          'redirect.'
      );
    } else {
    ...
    .thenCatch(function (error) {
      ...
      manager
        ...
        .then(function () {
          ...
          assertEquals(
            1,
            fireauth.AuthEventManager.prototype.reset.getCallCount()
          );
          assertTrue(popupRedirected);
          // This should resolve now as it is already initialized.
          asyncTestCase.signal();
...
function testProcessPopup_alreadyRedirected() {
  ...
  manager
    .processPopup(popupWin, 'linkViaPopup', provider, '1234', true)
    .then(function () {
...
function testProcessPopup_error_invalidOrigin() {
  ...
  var expectedError = new fireauth.InvalidOriginError(
...
function testProcessPopup_error_blockedPopup() {
  ...
  manager
    .processPopup(null, 'linkViaPopup', provider, '1234')
    .thenCatch(function (error) {
      assertErrorEquals(
...
function testProcessRedirect_success_ifchandler() {
  ...
  stubs.replace(fireauth.util, 'goTo', function (url) {
    assertEquals(expectedUrl, url);
...
function testAuthEventManager_nonCordovaIosOrAndroidFileEnvironment() {
  ...
  manager
    .processRedirect('linkViaRedirect', provider, '1234')
    .thenCatch(function (error) {
      assertErrorEquals(expectedError, error);
      asyncTestCase.signal();
    });
  ...
  manager
    .startPopupTimeout(handler, 'linkViaPopup', popupWin, '1234')
    .then(function () {
...
function testProcessRedirect_success_cordovahandler_tenantId() {
  ...
  manager
    .processRedirect('linkViaRedirect', provider, '1234', expectedTenantId)
    .then(function () {
      // Pending redirect should be cleared on redirect back to app.
      return pendingRedirectManager.getPendingStatus();
    })
...
function testGetRedirectResult_success_cordovahandler() {
  ...
  savePartialEventManager
    .setAuthEvent(storageKey, partialEvent)
    .then(function () {
      var manager = fireauth.AuthEventManager.getManager(
        authDomain1,
        apiKey1,
...
function testProcessAuthEvent_invalidAuthEvent() {
  ...
  manager
    .getRedirectAuthEventProcessor()
    .processAuthEvent(expectedAuthEvent, handler)
    .thenCatch(function (error) {
      assertErrorEquals(
...
function testProcessAuthEvent_finisher_successfulPopupAuthEvent_noPostBody() {
  ...
  manager.popupAuthEventProcessor_
    .processAuthEvent(expectedAuthEvent, handler)
    .then(function () {
      ...
      assertNull(handler.resolvePendingPopupEvent.getLastCall().getArgument(2));
      assertEquals(
        '1234',
        handler.resolvePendingPopupEvent.getLastCall().getArgument(3)
      );
...
function testProcessAuthEvent_finisher_successfulRedirect_noPostBody() {
  ...
  manager.redirectAuthEventProcessor_
    .processAuthEvent(expectedAuthEvent, handler)
    .then(function () {
...
function testProcessAuthEvent_noHandler() {
  ...
  handler.getAuthEventHandlerFinisher = function () {
    return null;
  };
  asyncTestCase.waitForSignals(1);
  var expectedError = new fireauth.AuthError(
    fireauth.authenum.Error.INVALID_AUTH_EVENT
  );
  var expectedAuthEvent = new fireauth.AuthEvent(
    'linkViaRedirect',
    '1234',
...
function testGetRedirect_noHandlerCanProcessEvent() {
  ...
  handler.canHandleAuthEvent = function () {
    return false;
  };
  manager.initialize();
  manager.subscribe(handler);
