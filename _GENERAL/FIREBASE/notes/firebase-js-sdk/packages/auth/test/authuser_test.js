...
function setUp() {
  ...
  idTokenGmail.jwt = fireauth.common.testHelper.createMockJwt(
    idTokenGmail.data
  );
  ...
  idTokenCustomClaims.jwt = fireauth.common.testHelper.createMockJwt(
  ...
  stubs.replace(fireauth.util, 'isIframe', function () {
    return false;
  });
  ...
  accountInfo2 = {
    ...
    'photoURL':
      'http://abs.twimg.com/sticky/default_profile_images/defaul' +
...
function testUser() {
  ...
  assertObjectEquals(
    new fireauth.UserMetadata(createdAt, lastLoginAt),
    user.metadata
  );
...
function testUser_copyUser() {
  ...
  var oAuthSignInHandlerInstance = mockControl.createStrictMock(
  ...
  oAuthSignInHandlerInstance
    ...
    .$does(function (actualMode, actualProvider, actualEventId) {
      assertEquals(fireauth.AuthEvent.Type.LINK_VIA_REDIRECT, actualMode);
...
function testUser_copyUser_expiredToken() {
  ...
  tokenResponse['idToken'] = fireauth.common.testHelper.createMockJwt(
    null,
    now - 5
  );
  stubs.replace(
...
function testUser_rpcHandlerEndpoints() {
  ...
  stubs.replace(fireauth.constants, 'getEndpointConfig', function (opt_id) {
...
function testExtractLinkedAccounts() {
  var resp = {
    ...
    'providerUserInfo': [
      ...
      },
      {
        ...
        'displayName': 'John Doe',
        'photoUrl':
          'http://abs.twimg.com/sticky/default_profile_images/defa' +
  ...
  user = new fireauth.AuthUser(config1, tokenResponse, accountInfo);
  assertObjectEquals(expectedProviders, user.extractLinkedAccounts_(resp));
}
...
function testUser_getIdTokenResult_expiredToken_reauth() {
  ...
  user.getIdTokenResult().thenCatch(function (error) {
    ...
    user.getIdTokenResult().thenCatch(function (error) {
      ...
      // This should resolve.
      user.reauthenticateWithCredential(credential).then(function (result) {
        ...
        user.getIdTokenResult().then(function (idTokenResult) {
          ...
          assertEquals(
            validTokenResponse['refreshToken'],
            user['refreshToken']
          );
          fireauth.common.testHelper.assertIdTokenResult(
            idTokenResult,
...
function testUser_getIdToken_expiredToken_reauthWithPopupAfterInvalidation() {
  ...
  oAuthSignInHandlerInstance
    .addAuthEventListener(ignoreArgument)
...
function testUpdatePhoneNumber_success() {
  ...
  phoneAuthCredential
    .linkToIdToken(ignoreArgument, tokenResponse['idToken'])
    .$once()
  ...
  getAccountInfoByIdToken(newJwt)
    .$returns(goog.Promise.resolve(getAccountInfoResponse))
    .$once();
  ...
  user = new fireauth.AuthUser(config1, tokenResponse, accountInfo);
  user.updatePhoneNumber(phoneAuthCredential).then(function () {
...
function testUpdateProfile_success() {
  ...
  stubs.replace(fireauth.AuthUser.prototype, 'reload', function () {
    return goog.Promise.resolve();
  });
  ...
  stubs.replace(user, 'updateTokensIfPresent', goog.testing.recordFunction());
...
function testUpdateProfile_success_withPasswordProvider() {
  ...
  user
    ...
    .then(function () {
      ...
      assertObjectEquals(user['providerData'][0], {
        'uid': 'UID1',
        'displayName': null,
        'photoURL': null,
      ...
      assertObjectEquals(user['providerData'][1], {
        'uid': 'UID2',
        'displayName': expectedDisplayName,
        'photoURL': expectedPhotoUrl,
...
function testReauthenticateAndRetrieveDataWithCredential_success() {
  ...
  stubs.replace(fireauth.deprecation, 'log', goog.testing.recordFunction());
  ...
  user
    .reauthenticateAndRetrieveDataWithCredential(expectedGoogleCredential)
...
function testReauthenticateWithCredential_userMismatch() {
  ...
  assertNoTokenEvents(user);
  assertNoUserInvalidatedEvents(user);
  user.reauthenticateWithCredential(credential).thenCatch(function (error) {
    fireauth.common.testHelper.assertErrorEquals(expectedError, error);
    asyncTestCase.signal();
  });
  asyncTestCase.waitForSignals(1);
...
function testLinkAndRetrieveDataWithCredential_success() {
  ...
  user
    .linkAndRetrieveDataWithCredential(expectedGoogleCredential)
    .then(function (result) {
      // Confirm expected response returned.
      assertEquals(expectedResponse, result);
      asyncTestCase.signal();
...
function testLinkWithCredential_emailAndPassword() {
  ...
  assertNoUserInvalidatedEvents(user);
  var credential = fireauth.EmailAuthProvider.credential(email, password);
  user
...
function testLinkWithCredential_federatedIdP() {
  ...
  stubs.replace(
    ...
    function (request) {
      ...
      getAccountInfoResponse['users'][0]['providerUserInfo'].push(
        getAccountInfoResponseGoogleProviderData
      );
  ...
  assertNoUserInvalidatedEvents(user);
  user
    .linkWithCredential(expectedGoogleCredential)
    .then(function (result) {
      // Expected result returned.
      fireauth.common.testHelper.assertUserCredentialResponse(
        // Expected current user returned.
...
function testLinkWithCredential_alreadyLinked() {
  ...
  user.linkWithCredential(credential).thenCatch(function (actualError) {
    fireauth.common.testHelper.assertErrorEquals(error, actualError);
    asyncTestCase.signal();
  });
  asyncTestCase.waitForSignals(1);
...
function testUnlink_success() {
  ...
  var userWithoutProvider2 = new fireauth.AuthUser(
    config1,
    tokenResponse,
    accountInfoWithPhone
  );
  ...
  user.unlink('providerId2').then(function (passedUser) {
    // Update user in Auth state.
    assertObjectEquals(
      userWithoutProvider2.toPlainObject(),
      user.toPlainObject()
    );
...
function testUnlink_alreadyDeleted() {
  ...
  stubs.replace(
    ...
    function (idToken, providersToDelete) {
      fail(
        'deleteLinkedAccounts RPC should not be called when the linked ' +
  ...
  user.unlink('providerId2').thenCatch(function (error) {
    fireauth.common.testHelper.assertErrorEquals(
...
function testUnlink_failure() {
  ...
  user.unlink('providerId1').thenCatch(function (actualError) {
    fireauth.common.testHelper.assertErrorEquals(error, actualError);
    asyncTestCase.signal();
  });
  asyncTestCase.waitForSignals(1);
...
function testUnlink_phone() {
  ...
  stubs.replace(
    ...
    function (idToken, providersToDelete) {
      ...
      var response = {
        'email': 'user@default.com',
        'providerUserInfo': [{ 'providerId': 'providerId1' }]
      };
      return goog.Promise.resolve(response);
    }
  );
  ...
  var userWithoutPhone = new fireauth.AuthUser(
    config1,
    tokenResponse,
    accountInfo
  );
  ...
  user
    .unlink(fireauth.PhoneAuthProvider['PROVIDER_ID'])
    .then(function (passedUser) {
      // Update user in Auth state.
      assertObjectEquals(
        userWithoutPhone.toPlainObject(),
        user.toPlainObject()
      );
...
function testDelete_error() {
  ...
  stubs.replace(user, 'destroy', function () {
...
function testDestroy() {
  ...
  assertEquals(
    user,
    fireauth.AuthEventManager.prototype.subscribe.getLastCall().getArgument(0)
  );
  assertEquals(
    0,
    fireauth.AuthEventManager.prototype.unsubscribe.getCallCount()
  );
  // Destroy user.
  user.destroy();
  ...
  assertEquals(
    user,
    fireauth.AuthEventManager.prototype.unsubscribe.getLastCall().getArgument(0)
  );
  assertNull(user['refreshToken']);
  return user.reload().then(fail, function (error) {
    fireauth.common.testHelper.assertErrorEquals(
      new fireauth.AuthError(fireauth.authenum.Error.MODULE_DESTROYED),
      error
    );
...
function testUser_initializeFromIdTokenResponse() {
  ...
  stubs.replace(
    ...
    function (data) {
      return new goog.Promise(function (resolve, reject) {
        ...
        assertArrayEquals(
          frameworks,
          fireauth.AuthUser.prototype.setFramework.getLastCall().getArgument(0)
        );
        assertEquals(jwt, data);
        resolve(response);
      });
  ...
  fireauth.AuthUser.initializeFromIdTokenResponse(
    ...
    assertEquals(1, fireauth.AuthUser.prototype.setFramework.getCallCount());
    // Confirm frameworks set on created user.
    assertArrayEquals(frameworks, createdUser.getFramework());
    assertObjectEquals(
      expectedUser.toPlainObject(),
      createdUser.toPlainObject()
    );
    ...
    assertEquals('refreshToken', createdUser.stsTokenManager_.refreshToken_);
    asyncTestCase.signal();
  });
}
...
function testUser_authEventManager_noAuthDomain() {
  ...
  fireauth.AuthEventManager.ENABLED = true;
  var expectedError = new fireauth.AuthError(
    fireauth.authenum.Error.MISSING_AUTH_DOMAIN
  );
...
function testUser_finishPopupAndRedirectLink_success_withoutPostBody() {
  ...
  user1
    .finishPopupAndRedirectLink('REQUEST_URI', 'SESSION_ID', null, null)
...
function testUser_finishPopupAndRedirectLink_success_withPostBody() {
  ...
  // Finish popup and redirect linking.
  user1
    .finishPopupAndRedirectLink('REQUEST_URI', 'SESSION_ID', null, 'POST_BODY')
    .then(function (response) {
      fireauth.common.testHelper.assertUserCredentialResponse(
        user1,
        null,
...
function testUser_finishPopupAndRedirectReauth_success_withoutPostBody() {
  ...
  user1
    .finishPopupAndRedirectReauth('REQUEST_URI', 'SESSION_ID', null, null)
    .then(function (response) {
      fireauth.common.testHelper.assertUserCredentialResponse(
        user1,
        expectedCred,
...
function testUser_finishPopupAndRedirectLink_noCredential() {
  ...
  user1
    ...
    .then(function (response) {
      ...
      // It should have updated the tokens.
      assertEquals(newJwt, user1.getStsTokenManager().accessToken_.toString());
      assertEquals('newRefreshToken', user1.getStsTokenManager().refreshToken_);
      asyncTestCase.signal();
    });
}
...
function testUser_linkWithRedirect_success_unloadsOnRedirect() {
  ...
  user1.linkWithRedirect(expectedProvider).then(function () {
    fail(
      'LinkWithRedirect should remain pending in environment where ' +
...
function testUser_reauthenticateWithRedirect_success_unloadsOnRedirect() {
  ...
  oAuthSignInHandlerInstance
    ...
    .$does(function (actualMode, actualProvider, actualEventId) {
      assertEquals(fireauth.AuthEvent.Type.REAUTH_VIA_REDIRECT, actualMode);
      assertEquals(expectedProvider, actualProvider);
      assertEquals(expectedEventId, actualEventId);
      // Redirect event ID should be saved.
      assertEquals(expectedEventId, user1.getRedirectEventId());
  ...
  user1.reauthenticateWithRedirect(expectedProvider).then(function () {
    fail(
      'ReauthenticateWithRedirect should remain pending in environment ' +
...
function testUser_linkWithRedirect_alreadyLinked() {
  ...
  var user = new fireauth.AuthUser(config1, tokenResponse, accountInfo);
  user
    .linkWithRedirect(new fireauth.GoogleAuthProvider())
    .thenCatch(function (actualError) {
      fireauth.common.testHelper.assertErrorEquals(
        new fireauth.AuthError(fireauth.authenum.Error.PROVIDER_ALREADY_LINKED),
        actualError
      );
      asyncTestCase.signal();
    });
...
function testUser_linkWithPopup_unsupportedEnvironment() {
  ...
  stubs.replace(fireauth.util, 'isPopupRedirectSupported', function () {
    return false;
  });
  fireauth.AuthEventManager.ENABLED = true;
  // Expected error.
  ...
  var user = new fireauth.AuthUser(config1, tokenResponse, accountInfo);
  user
    .linkWithPopup(new fireauth.GoogleAuthProvider())
    .thenCatch(function (error) {
      fireauth.common.testHelper.assertErrorEquals(expectedError, error);
      asyncTestCase.signal();
    });
...
function testUser_reauthenticateWithPopup_unsupportedEnvironment() {
  ...
  var user = new fireauth.AuthUser(config1, tokenResponse, accountInfo);
  user
    .reauthenticateWithPopup(new fireauth.GoogleAuthProvider())
    .thenCatch(function (error) {
      fireauth.common.testHelper.assertErrorEquals(expectedError, error);
      asyncTestCase.signal();
    });
...
function testUser_reauthenticateWithRedirect_unsupportedEnvironment() {
  ...
  var user = new fireauth.AuthUser(config1, tokenResponse, accountInfo);
  user
    .reauthenticateWithRedirect(new fireauth.GoogleAuthProvider())
    .thenCatch(function (error) {
      fireauth.common.testHelper.assertErrorEquals(expectedError, error);
      asyncTestCase.signal();
    });
...
function testUser_linkWithPopup_success_cannotRunInBackground_emulatorConfig() {
  ...
  // Mock OAuth sign in handler.
  const oAuthSignInHandlerInstance = mockControl.createStrictMock(
    fireauth.OAuthSignInHandler
  );
  mockControl.createConstructorMock(fireauth, 'OAuthSignInHandler');
...
function testUser_reauthenticateWithPopup_success_cannotRunInBkg_tenantId() {
  ...
  // reauthenticateWithPopup should succeed with the expected popup result.
  tenantUser
    .reauthenticateWithPopup(expectedProvider)
...
function testUser_reauthWithPopup_webStorageUnsupported_cantRunInBackground() {
  ...
  user1.reauthenticateWithPopup(expectedProvider).thenCatch(function (error) {
    // Popup should be closed.
    assertTrue(isClosed);
    fireauth.common.testHelper.assertErrorEquals(expectedError, error);
    // Popup user should never be saved in storage.
...
function testUser_linkWithPopup_timeout() {
  ...
  var expectedError = new fireauth.AuthError(
    fireauth.authenum.Error.EXPIRED_POPUP_REQUEST
  );
  // The expected popup result.
  var expectedPopupResult = {
    'user': user1,
...
function testUser_returnFromReauthenticateWithRedirect_success_multipleUsers() {
  ...
  pendingRedirectManager.setPendingStatus().then(function () {
    ...
    authEventManager.getRedirectResult().then(function (response) {
      assertEquals(
        1,
        fireauth.AuthUser.prototype.finishPopupAndRedirectReauth.getCallCount()
      );
      fireauth.common.testHelper.assertUserCredentialResponse(
        user1,
        expectedCred,
...
function testUser_returnFromReauthenticateWithRedirect_invalidUser() {
  ...
  stubs.replace(
    ...
    function (requestUri, sessionId, tenantId, postBody) {
      fail(
        'finishPopupAndRedirectReauth should not call due to UID ' + 'mismatch!'
      );
    }
  );
  var config = {
...
function testUser_returnFromReauthenticateWithRedirect_error() {
  ...
  stubs.replace(
    ...
    function (requestUri, sessionId, tenantId, postBody) {
      fail(
        'finishPopupAndRedirectReauth should not call due to event ' + 'error!'
      );
    }
  );
  var config = {
...
function testUser_sessionInvalidation_linkWithPopup_tokenExpired() {
  ...
  stubs.replace(fireauth.util, 'popup', function (url, name, width, height) {
    ...
    if (popupCalls > 1) {
      fail(
        'The second call to linkWithPopup should fail without openin' +
          'g the popup!'
      );
    }
    return expectedPopup;
...
function testUser_sessionInvalidation_sendEmailVerification_tokenExpired() {
  ...
  simulateSessionInvalidation('sendEmailVerification', [], invalidationError);
}
...
function testUser_proactiveRefresh_startAndStop() {
  ...
  proactiveRefreshConstructor(
    ...
    .$does(function (
      ...
      assertEquals(0, fireauth.AuthUser.prototype.getIdToken.getCallCount());
      // Run operation.
      operation();
      // getIdToken(true) should be called underneath.
      assertEquals(1, fireauth.AuthUser.prototype.getIdToken.getCallCount());
      assertTrue(
        fireauth.AuthUser.prototype.getIdToken.getLastCall().getArgument(0)
      );
      ...
      assertFalse(
        retryPolicy(
          new fireauth.AuthError(fireauth.authenum.Error.USER_DISABLED)
        )
      );
...
function testUser_proactiveRefresh_externalTokenRefresh() {
  ...
  proactiveRefreshConstructor(
    ...
    fireauth.TokenRefreshTime.RETRIAL_MAX_WAIT,
    false
  ).$returns(proactiveRefreshInstance);
  proactiveRefreshInstance.isRunning().$returns(false);
  ...
  user.dispatchEvent(fireauth.UserEventType.TOKEN_CHANGED);
...
function testLinkWithPhoneNumber_success() {
  ...
  var phoneAuthProviderInstance = mockControl.createStrictMock(
  ...
  phoneAuthProviderConstructor(auth)
    .$returns(phoneAuthProviderInstance)
    .$once();
  ...
  // ID.
  phoneAuthProviderInstance
    .verifyPhoneNumber(expectedPhoneNumber, appVerifier)
    .$returns(goog.Promise.resolve(expectedVerificationId))
    .$once();
  ...
  var user = new fireauth.AuthUser(config1, tokenResponse, accountInfo);
  user
    .linkWithPhoneNumber(expectedPhoneNumber, appVerifier)
    .then(function (confirmationResult) {
      ...
      assertEquals(
        1,
        fireauth.AuthUser.prototype.linkWithCredential.getCallCount()
      );
      // Confirm linkWithCredential is bound to current user.
      assertEquals(
        user,
        fireauth.AuthUser.prototype.linkWithCredential.getLastCall().getThis()
      );
      asyncTestCase.signal();
    });
}
...
function testReauthenticateWithPhoneNumber_success() {
  ...
  var user = new fireauth.AuthUser(config1, tokenResponse, accountInfo);
  user
    .reauthenticateWithPhoneNumber(expectedPhoneNumber, appVerifier)
    .then(function (confirmationResult) {
      // Confirmation result returned should contain expected verification ID.
      assertEquals(
        expectedVerificationId,
      ...
      assertEquals(
        1,
        fireauth.AuthUser.prototype.reauthenticateWithCredential.getCallCount()
      );
...
function testReauthenticateWithPhoneNumber_success_skipInvalidation() {
  ...
  getToken(true)
    .$does(function (opt_forceRefresh) {
      return goog.Promise.reject(expectedError);
    })
    .$once();
  // Provider instance should be initialized with the expected Auth instance
  ...
  user
    ...
    .thenCatch(function (error) {
      ...
      // error.
      return user.reauthenticateWithPhoneNumber(
        expectedPhoneNumber,
        appVerifier
      );
    })
...
function testUser_customLocaleChanges() {
  ...
  assertEquals(
    1,
    fireauth.RpcHandler.prototype.updateCustomLocaleHeader.getCallCount()
  );
  assertEquals(
    'de',
...
function testUser_frameworkLoggingChanges() {
  ...
  assertEquals(
    1,
    fireauth.RpcHandler.prototype.updateClientVersion.getCallCount()
  );
  assertEquals(
    getVersion(['v1']),
...
function testReauthenticateWithCredential_multiFactor_success() {
  ...
  var mockCredential = mockControl.createStrictMock(fireauth.AuthCredential);
  ...
  mockCredential
    .matchIdTokenWithUid(ignoreArgument, ignoreArgument)
    .$once()
  ...
  mockAssertion
    .process(ignoreArgument, ignoreArgument)
    .$once()
  ...
  return user
    .reauthenticateAndRetrieveDataWithCredential(mockCredential)
...
function testUser_getIdTokenResult_expiredToken_reauth_multiFactor() {
  ...
  return user
    ...
    .then(fail, function (error) {
      // Refresh token nullified.
      assertEquals(
        nonMultiFactorTokenResponse['refreshToken'],
        user['refreshToken']
      );
      // Confirm expected error.
      fireauth.common.testHelper.assertErrorEquals(expectedError, error);
    ...
    .then(function (result) {
      ...
      // Set via reauthentication.
      assertEquals(
        multiFactorTokenResponse['refreshToken'],
        user['refreshToken']
      );
      // Auth token change triggered.
      assertEquals(1, authChangeCounter);
