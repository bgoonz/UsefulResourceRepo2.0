...
function testCurrentUser() {
  ...
  auth1 = app1.auth();
  var user = new fireauth.AuthUser(config3, expectedTokenResponse, accountInfo);
...
function testUseEmulator() {
  ...
  assertEquals(
    0,
    fireauth.AuthSettings.prototype.setAppVerificationDisabledForTesting.getCallCount()
  );
  ...
  if (goog.global.document) {
    ...
    fireauth.util.onDomReady().then(() => {
      const el = goog.global.document.querySelector(
  ...
  currentUserStorageManager.setCurrentUser(user).then(function () {
    ...
    var unsubscribe = auth1.onIdTokenChanged(function (currentUser) {
      ...
      auth1['currentUser'].getIdToken().then(function (token) {
        ...
        // Second trigger.
        assertEquals(1, app1AuthTokenListener.getCallCount());
        assertEquals(
          'accessToken2',
          app1AuthTokenListener.getLastCall().getArgument(0)
        );
    ...
    auth2.onIdTokenChanged(function (currentUser) {
      ...
      assertEquals(1, app2AuthTokenListener.getCallCount());
      assertNull(app2AuthTokenListener.getLastCall().getArgument(0));
...
function testAuth_onAuthStateChanged() {
  ...
  var user2 = new fireauth.AuthUser(config3, expectedTokenResponse2, {
    'uid': '5678'
  });
  var observer1 = mockControl.createFunctionMock('observer1');
  var observer2 = mockControl.createFunctionMock('observer2');
...
function testFetchSignInMethodsForEmail() {
  ...
  stubs.replace(
    fireauth.RpcHandler.prototype,
    'fetchSignInMethodsForIdentifier',
    function (data) {
...
function testAuth_delete() {
  ...
  auth1.onIdTokenChanged(function (user) {
    ...
    assertEquals(
      1,
      fireauth.storage.UserManager.prototype.removeCurrentUserChangeListener.getCallCount()
    );
...
function testSendSignInLinkToEmail_success() {
  ...
  assertAuthTokenListenerCalledOnce(auth1);
  auth1
    .sendSignInLinkToEmail(expectedEmail, actionCodeSettings)
    .then(function () {
      asyncTestCase.signal();
    });
  asyncTestCase.waitForSignals(1);
...
function testSendSignInLinkToEmail_emptyContinueUrl_error() {
  ...
  assertAuthTokenListenerCalledOnce(auth1);
  auth1
    .sendSignInLinkToEmail(expectedEmail, settings)
    .then(function () {
      fail('sendSignInLinkToEmail should not resolve!');
    })
    .thenCatch(function (error) {
...
function testSendPasswordResetEmail_error() {
  ...
  assertAuthTokenListenerCalledOnce(auth1);
  auth1
    .sendPasswordResetEmail(expectedEmail)
    .then(function () {
...
function testSendPasswordResetEmail_actionCodeSettings_success() {
  ...
  assertAuthTokenListenerCalledOnce(auth1);
  auth1
    .sendPasswordResetEmail(expectedEmail, actionCodeSettings)
    .then(function () {
      asyncTestCase.signal();
    });
  asyncTestCase.waitForSignals(1);
...
function testSendPasswordResetEmail_actionCodeSettings_error() {
  ...
  assertAuthTokenListenerCalledOnce(auth1);
  auth1
    .sendPasswordResetEmail(expectedEmail, settings)
    .then(function () {
      fail('sendPasswordResetEmail should not resolve!');
    })
    .thenCatch(function (error) {
...
function testVerifyPasswordResetCode_success() {
  ...
  auth1 = app1.auth();
  assertAuthTokenListenerCalledOnce(auth1);
  auth1.verifyPasswordResetCode(expectedCode).then(function (email) {
    assertEquals(expectedEmail, email);
    asyncTestCase.signal();
  });
}
...
function testConfirmPasswordReset_success() {
  ...
  assertAuthTokenListenerCalledOnce(auth1);
  auth1
    .confirmPasswordReset(expectedCode, expectedNewPassword)
    .then(function () {
      asyncTestCase.signal();
    });
  asyncTestCase.waitForSignals(1);
...
function testCheckActionCode_success() {
  ...
  var expectedActionCodeInfo = new fireauth.ActionCodeInfo(
  ...
  auth1 = app1.auth();
  assertAuthTokenListenerCalledOnce(auth1);
  auth1.checkActionCode(expectedCode).then(function (info) {
...
function testApplyActionCode_error() {
  ...
  app1 = firebase.initializeApp(config1, appId1);
  auth1 = app1.auth();
  auth1.applyActionCode(expectedCode).then(
    function () {
      fail('confirmPasswordReset should not resolve!');
    },
    function (error) {
...
function testAuth_authEventManager() {
  ...
  auth1.onIdTokenChanged(function (user) {
    ...
    assertEquals(auth1, expectedManager.subscribe.getLastCall().getArgument(0));
    ...
    assertEquals(
      auth1,
      expectedManager.unsubscribe.getLastCall().getArgument(0)
    );
    asyncTestCase.signal();
  });
}
...
function testAuth_signout() {
  ...
  var counter = 0;
  stubs.replace(fireauth.StsTokenManager.prototype, 'getToken', function () {
  ...
  currentUserStorageManager.setCurrentUser(user1).then(function () {
    ...
    auth1.addAuthTokenListener(function (token) {
      ...
      if (tokenChangeCalls == 1) {
        ...
        assertArrayEquals(
          ['firebaseui', 'angularfire'],
          auth1['currentUser'].getFramework()
        );
...
function testAuth_initState_signedInStatus_withEmulator() {
  ...
  currentUserStorageManager.setCurrentUser(user1).then(function () {
    ...
    auth1.onAuthStateChanged(function (user) {
      ...
      assertEquals(
        auth1['currentUser'].getRpcHandler(),
        fireauth.RpcHandler.prototype.updateEmulatorConfig
          .getLastCall()
          .getThis()
      );
...
function testAuth_initState_signedInStatus_withRedirectUser() {
  ...
  stubs.replace(
    fireauth.AuthUser.prototype,
    'enablePopupRedirect',
    goog.testing.recordFunction(fireauth.AuthUser.prototype.enablePopupRedirect)
  );
  asyncTestCase.waitForSignals(2);
  // Logged in user to be detected in initState.
  var user1 = new fireauth.AuthUser(
  ...
  redirectUserStorageManager
    ...
    .then(function () {
      ...
      auth1.addAuthTokenListener(function (token) {
        ...
        assertEquals(
          2,
          fireauth.AuthUser.prototype.enablePopupRedirect.getCallCount()
        );
        // Redirect user.
        var redirectUser = fireauth.AuthUser.prototype.enablePopupRedirect
          .getLastCall()
          .getThis();
...
function testAuth_updateCurrentUser_nullUserError() {
  var expectedError = new fireauth.AuthError(fireauth.authenum.Error.NULL_USER);
  asyncTestCase.waitForSignals(1);
  app1 = firebase.initializeApp(config3, appId1);
  auth1 = app1.auth();
...
function testAuth_signInWithCustomToken_success() {
  ...
  auth1.signInWithCustomToken(expectedCustomToken).then(function (result) {
...
function testAuth_signInWithEmailLink_success() {
  ...
  var expectedIdToken =
    'HEAD.ew0KICAiaXNzIjogImh0dHBzOi8vc2VjdXJldG9rZW4uZ2' +
  ...
  auth1
    .signInWithEmailLink(expectedEmail, expectedLink)
...
function testAuth_signInWithEmailLink_success_tenantId() {
  ...
  const expectedLink =
    'https://www.example.com?mode=signIn&oobCode=code' +
    '&apiKey=API_KEY&tenantId=TENANT_ID';
  const expectedOobCode = 'code';

  app1 = firebase.initializeApp(config3, appId1);
  auth1 = app1.auth();
  const emailLinkSignIn = mockControl.createMethodMock(
    auth1.getRpcHandler(),
    'emailLinkSignIn'
  );
  emailLinkSignIn(expectedEmail, expectedOobCode)
    .$once()
  ...
  // Sign in with email and link.
  return auth1.signInWithEmailLink(expectedEmail, expectedLink).then(result => {
    fireauth.common.testHelper.assertUserCredentialResponse(
      auth1.currentUser,
      null,
...
function testAuth_signInWithEmailLink_error_tenantIdMismatch() {
  ...
  // Auth instance.
  const expectedError = new fireauth.AuthError(
    fireauth.authenum.Error.TENANT_ID_MISMATCH
  );
  fireauth.AuthEventManager.ENABLED = true;
  const expectedEmail = 'user@example.com';
  ...
  // Sign in with email and link.
  auth1.signInWithEmailLink(expectedEmail, expectedLink).thenCatch(error => {
    fireauth.common.testHelper.assertErrorEquals(expectedError, error);
    asyncTestCase.signal();
  });
}
...
function testAuth_signInWithEmailAndPassword_success() {
  ...
  // Sign in with email and password.
  auth1
    .signInWithEmailAndPassword(expectedEmail, expectedPass)
    .then(function (result) {
      fireauth.common.testHelper.assertUserCredentialResponse(
        expectedResult['user'],
        expectedResult['credential'],
...
function testAuth_createUserWithEmailAndPassword_success() {
  ...
  auth1 = app1.auth();
  auth1
    .createUserWithEmailAndPassword(expectedEmail, expectedPass)
    .then(function (result) {
      fireauth.common.testHelper.assertUserCredentialResponse(
        expectedResult['user'],
        expectedResult['credential'],
...
function testAuth_signInAndRetrieveDataWithCredential_success() {
  ...
  auth1
    .signInAndRetrieveDataWithCredential(expectedGoogleCredential)
    .then(function (userCredential) {
...
function testAuth_signInWithCredential_success() {
      ...
      assertObjectEquals(
        {
          ...
          'postBody':
            'id_token=googleIdToken&access_token=googleAccessToke' +
            'n&providerId=' +
  ...
  auth1.signInWithCredential(expectedGoogleCredential).then(function (result) {
...
function testAuth_signInWithCredential_emailPassCredential() {
  ...
  auth1.signInWithCredential(cred).then(function (result) {
    // Confirm fireauth.Auth.prototype.signInWithIdTokenResponse is called
    // underneath. This will save the new user in storage and trigger Auth
    // token changes.
    assertEquals(
...
function testAuth_signInWithCredential_error() {
  ...
  // the expected error.
  auth1
    .signInWithCredential(expectedGoogleCredential)
    .thenCatch(function (error) {
      fireauth.common.testHelper.assertErrorEquals(expectedError, error);
      asyncTestCase.signal();
    });
...
function testAuth_finishPopupAndRedirectSignIn_success_withoutPostBody() {
  ...
  auth1
    .finishPopupAndRedirectSignIn('REQUEST_URI', 'SESSION_ID', null, null)
...
function testAuth_finishPopupAndRedirectSignIn_success_withPostBody() {
  ...
  stubs.replace(
    ...
    function (options, idTokenResponse) {
      assertObjectEquals(config3, options);
      assertObjectEquals(expectedSamlTokenResponseWithIdPData, idTokenResponse);
      asyncTestCase.signal();
      return goog.Promise.resolve(expectedUser);
    }
  );
...
function testAuth_finishPopupAndRedirectSignIn_noCredential() {
  ...
  app1 = firebase.initializeApp(config3, appId1);
  auth1 = app1.auth();
  var expectedUser = new fireauth.AuthUser(config3, expectedResponse, {
    'uid': 'USER_ID',
    'email': 'user@example.com'
  });
...
function testAuth_signInWithPopup_emailCredentialError() {
  ...
  var credential = fireauth.GoogleAuthProvider.credential(null, 'ACCESS_TOKEN');
...
function testAuth_signInWithPopup_blockedPopup() {
  ...
  var expectedEventId = '1234';
  var expectedError = new fireauth.AuthError(
    fireauth.authenum.Error.POPUP_BLOCKED
  );
  // Replace random number generator.
  stubs.replace(fireauth.util, 'generateRandomString', function () {
    return '87654321';
...
function testAuth_returnFromSignInWithRedirect_success_tenantId() {
  ...
  stubs.replace(
    ...
    function (requestUri, sessionId, tenantId, postBody) {
      ...
      asyncTestCase.signal();
      return currentUserStorageManager
...
function testAuth_signInWithRedirect_success_unloadsOnRedirect() {
    ...
    function (authDomain, apiKey, appName) {
      return {
        ...
        'processRedirect': function (
          ...
          assertEquals(
            1,
            fireauth.storage.UserManager.prototype.savePersistenceForRedirect.getCallCount()
          );
          asyncTestCase.signal();
          return goog.Promise.resolve();
        }
  ...
  auth1.signInWithRedirect(expectedProvider).then(function () {
    fail(
      'SignInWithRedirect should remain pending in environment where ' +
...
function testAuth_returnFromSignInWithRedirect_timeout() {
  ...
  var expectedError = new fireauth.AuthError(fireauth.authenum.Error.TIMEOUT);
  clock = new goog.testing.MockClock(true);
  // Stub instantiateOAuthSignInHandler.
  stubs.replace(
    fireauth.AuthEventManager,
...
function testAuth_proactiveTokenRefresh_multipleUsers() {
  ...
  app1.container
    ...
    .getImmediate()
    .addAuthTokenListener(subscriber);
  // Simulate user1 signed in.
  auth1
    .signInWithIdTokenResponse(expectedTokenResponse)
    .then(function () {
      // Current user should be set to user1.
      assertEquals(user1, auth1['currentUser']);
      ...
      assertEquals(
        1,
        fireauth.AuthUser.prototype.startProactiveRefresh.getCallCount()
      );
      assertEquals(
        user1,
      ...
      assertEquals(
        0,
        fireauth.AuthUser.prototype.stopProactiveRefresh.getCallCount()
      );
    ...
    .then(function () {
      ...
      assertEquals(
        user1,
        fireauth.AuthUser.prototype.stopProactiveRefresh.getLastCall().getThis()
      );
      // Sign out the user2.
      return auth1.signOut();
    })
...
function testAuth_signInWithPhoneNumber_success() {
  ...
  auth1
    .signInWithPhoneNumber(expectedPhoneNumber, appVerifier)
    .then(function (confirmationResult) {
      ...
      assertEquals(
        auth1,
        fireauth.Auth.prototype.signInWithCredential.getLastCall().getThis()
      );
      asyncTestCase.signal();
    });
}
...
function testAuth_setPersistence_noExistingAuthState() {
  ...
  app1 = firebase.initializeApp(config3, appId1);
  auth1 = app1.auth();
  var user1 = new fireauth.AuthUser(config3, expectedTokenResponse, {
    'uid': '1234'
  });
  var user2 = new fireauth.AuthUser(config3, expectedTokenResponse2, {
    'uid': '5678'
...
function testAuth_temporaryPersistence_externalChange() {
  ...
  var user3 = new fireauth.AuthUser(config3, expectedTokenResponse3, {
    'uid': '9012'
  });
  var storageEvent = new goog.testing.events.Event(
    goog.events.EventType.STORAGE,
    ...
    if (calls == 1) {
      ...
      fireauth.common.testHelper
        ...
        .then(function () {
          ...
          mockLocalStorage.set(storageKey, user2.toPlainObject());
          mockLocalStorage.fireBrowserEvent(storageEvent);
        });
    } else if (calls == 2) {
...
function testAuth_storedPersistence_returnFromRedirect() {
  ...
  var currentUserStorageManager = new fireauth.storage.UserManager(
    config3['apiKey'] + ':' + appId1
  );
...
function testSignInWithCredential_multiFactor_success() {
  ...
  stubs.restore(fireauth.AuthUser.prototype, 'reload');
  ...
  mockCredential
    .getIdTokenProvider(ignoreArgument)
    .$once()
  ...
  authStateListener(ignoreArgument)
    .$once()
    .$does(function (user) {
      assertEquals(auth1.currentUser, user);
    });
  idTokenListener(null).$once();
  idTokenListener(ignoreArgument)
    .$once()
    .$does(function (user) {
      assertEquals(auth1.currentUser, user);
    });
  ...
  var unsubscribe = auth1.onAuthStateChanged(function (currentUser) {
    ...
    auth1
      .signInAndRetrieveDataWithCredential(mockCredential)
...
function testSignInWithCredential_multiFactor_assertionError() {
  ...
  getAccountInfoByIdToken(ignoreArgument).$times(0);
