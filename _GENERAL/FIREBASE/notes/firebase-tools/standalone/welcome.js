...
(async () => {
  ...
  console.log("");
  console.log(`${chalk.green("+")} You can now use the 'firebase' or 'npm' commands!`);
  console.log(`${chalk.blue("~")} For more help see https://firebase.google.com/docs/cli/`);
  console.log("");
...
async function CheckFirebaseTools() {
  console.log(`${chalk.cyan("~")} Let's make sure your Firebase CLI is ready...`);
  firebase_bin = await GetFirebaseToolsBins();
  const isSetup = !!firebase_bin;

  if (isSetup) {
    console.log(`${chalk.green("+")} Looks like ${chalk.green("your CLI is set up")}!`);
  } else {
    console.log(`${chalk.cyan("~")} Looks like ${chalk.yellow("your CLI needs to be set up")}.\n`);
    ...
    return new Promise((resolve) => {
      ...
      const install = spawn(exe_split[0], [...exe_split.slice(1), "--tool:force-setup"], {});
...
async function GetFirebaseToolsBins() {
  return new Promise((resolve) => {
    const exe_split = firebase_exe.split(" ");
    const checkSpawn = spawn(exe_split[0], [...exe_split.slice(1), "--tool:setup-check"]);
    let checkSpawnData = "";
...
async function CheckIsLoggedIn() {
  ...
  return new Promise((resolve) => {
    const finish = SimpleSpinner(`${chalk.cyan("~")} Checking your Firebase credentials`);
    ...
    listSpawn.on("close", () => {
      const isLoggedIn = JSON.parse(listSpawnData.toString()).status === "success";

      if (!isLoggedIn) {
        finish(
          "🚫",
          `${chalk.cyan("~")} Looks like you're not authenticated. ${chalk.yellow(
            "Let's log in"
          )}!\n`
        );
      } else {
