...
async function createRuntimeBinaries() {
  ...
  const runtimeBins = {
    ...
    npm: `"${unsafeNodePath}" "${FindTool("npm/bin/npm-cli")[0]}" ${npmArgs.join(" ")} "$@"`,
...
async function SetupFirebaseTools() {
  ...
  if (!flags["ignore-embedded-cache"]) {
    ...
    debug(shell.cp("-R", path.join(__dirname, "vendor/*"), nodeModulesPath).toString());
  } else {
    /*
...
function uninstallLegacyFirepit() {
   ...
   */
  const isLegacyFirepit = !shell.ls(path.join(homePath, ".cache", "firebase", "cli")).code;
  ...
  const firepitFirebaseToolsPackage = JSON.parse(shell.cat(firepitFirebaseToolsPackagePath));
  try {
    installedFirebaseToolsPackage = JSON.parse(shell.cat(installedFirebaseToolsPackagePath));
  } catch (err) {
    debug("No existing firebase-tools install found.");
  }

  debug(
    `Installed ft@${installedFirebaseToolsPackage.version || "none"} and packaged ft@${
      firepitFirebaseToolsPackage.version
    }`
  );

  const isLegacyFirebaseTools =
    installedFirebaseToolsPackage.version !== firepitFirebaseToolsPackage.version;
