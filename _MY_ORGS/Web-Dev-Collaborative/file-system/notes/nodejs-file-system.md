# File system | Node.js v15.8.0 Documentation
...
* `options` [<Object>](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Object) | [<string>](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Data_structures#String_type)
    ...
    - `signal` [<AbortSignal>](chrome-extension://cjedbglnccaioiolemnfhjncicchinao/globals.html#globals_class_abortsignal) allows aborting an in-progress readFile
...
### Performance Considerations[#](#fs_performance_considerations)
...
`fs.rm(path[, options], callback)` [#](#fs_fs_rm_path_options_callback)
----------------------------------------------------------------------
...
`fs.rmSync(path[, options])` [#](#fs_fs_rmsync_path_options)
-----------------------------------------------------------
...
*   `options` [<Object>](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Object) | [<string>](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Data_structures#String_type)
    ...
    - `signal` [<AbortSignal>](chrome-extension://cjedbglnccaioiolemnfhjncicchinao/globals.html#globals_class_abortsignal) allows aborting an in-progress writeFile
...
It is possible to use an [<AbortSignal>](chrome-extension://cjedbglnccaioiolemnfhjncicchinao/globals.html#globals_class_abortsignal) to cancel an `fs.writeFile()` . Cancelation is "best effort", and some amount of data is likely still to be written.
...
#### Event: `'close'` [#](#fs_event_close_3)

Added in: v15.4.0

The `'close'` event is emitted when the `FileHandle` and any of its underlying resources (a file descriptor, for example) have been closed.
...
### `fsPromises.rm(path[, options])` [#](#fs_fspromises_rm_path_options)
...
It is unsafe to use `fsPromises.writeFile()` multiple times on the same file without waiting for the `Promise` to be fulfilled (or rejected).
...
It is possible to use an [<AbortSignal>](chrome-extension://cjedbglnccaioiolemnfhjncicchinao/globals.html#globals_class_abortsignal) to cancel an `fsPromises.writeFile()` . Cancelation is "best effort", and some amount of data is likely still to be written.
...
[Source](https://nodejs.org/api/fs.html#fs_class_fs_stats)
