...
const runSpecs = () => {
  ...
  const seederModuleName = seederFilename.slice(0, seederFilename.lastIndexOf('.'));
    ...
    describe('`up` property function should call the passed `queryInterface.bulkInsert()` method with', () => {
      ...
      describe('`records` argument', () => {
        ...
        describe('each array element', () => {
          const hasProperty = (propertyName) => queryInterface
            .insertRecords.filter((r) => Object.prototype.hasOwnProperty.call(r, propertyName));
          ...
          expectedProperties.forEach((property) => {
            it(`should have a property named \`${property.name}\``, () => {
              expect(hasProperty(property.name).length)
                .to.equal(queryInterface.insertRecords.length);
            });

            it(`with the type \`${getTypeName(property.type)}\``, () => {
              expect(hasPropertyWithType(property.name, property.type).length)
                .to.equal(queryInterface.insertRecords.length);
            });
          });
        });
