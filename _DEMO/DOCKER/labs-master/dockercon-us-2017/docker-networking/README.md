...
Execute the following command from **node0-a** to create a new service called _myservice_ on the _overnet_ network with two tasks/replicas.
...
Execute the `docker service rm myservice` command on **node0-a** to remove the service called _myservice_.
