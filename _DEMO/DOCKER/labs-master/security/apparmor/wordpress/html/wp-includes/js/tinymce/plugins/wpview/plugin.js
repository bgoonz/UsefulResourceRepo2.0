...
tinymce.PluginManager.add("wpview", function (editor) {
  ...
  function deselect() {
    ...
    if (selected) {
      ...
      dom.unbind(
        selected,
        "beforedeactivate focusin focusout click mouseup",
        _stop
      );
  ...
  function emptyViewNodes(rootNode) {
    $("div[data-wpview-text], p[data-wpview-marker]", rootNode).each(function (
      i,
      node
    ) {
      node.innerHTML = ".";
  ...
  editor.on("keydown", function (event) {
    ...
    if (selected) {
      ...
      if (
        (cursorAfter && key === VK.UP) ||
        (cursorBefore && key === VK.BACKSPACE)
      ) {
        if (view.previousSibling) {
          if (getView(view.previousSibling)) {
        ...
        (cursorAfter && key === VK.LEFT) ||
        (cursorBefore && key === VK.RIGHT)
      ) {
        select(view);
        event.preventDefault();
  ...
  editor.on("NodeChange", function (event) {
    ...
    if (focus && view) {
      if (
        (className === "wpview-selection-before" ||
          className === "wpview-selection-after") &&
        ...
        !getParent(event.element, "wpview-clipboard") &&
        !setViewCursorTries
      ) {
        deselect();
        setViewCursorTries++;
  ...
  editor.on("ExecCommand", function () {
    ...
    if (execCommandView) {
      node =
        execCommandView[execCommandBefore ? "previousSibling" : "nextSibling"];
  ...
  editor.on("ResolveName", function (event) {
    if (editor.dom.hasClass(event.target, "wpview-wrap")) {
      event.name =
        editor.dom.getAttrib(event.target, "data-wpview-type") || "wpview";
      event.stopPropagation();
