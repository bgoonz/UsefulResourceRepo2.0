...
if (typeof SWFUpload === "function") {
  ...
  SWFUpload.speed.fileQueueErrorHandler = function (file, errorCode, message) {
    if (
      typeof this.speedSettings.user_file_queue_error_handler === "function"
    ) {
      file = SWFUpload.speed.extendFile(file);
  ...
  SWFUpload.speed.uploadErrorHandler = function (file, errorCode, message) {
    ...
    if (typeof this.speedSettings.user_upload_error_handler === "function") {
      return this.speedSettings.user_upload_error_handler.call(
        this,
        file,
        errorCode,
        message
  ...
  SWFUpload.speed.uploadProgressHandler = function (
    file,
    bytesComplete,
    bytesTotal
  ) {
  ...
  SWFUpload.prototype.updateTracking = function (file, bytesUploaded) {
    ...
    if (!tracking.startTime) {
      ...
      tracking.averageSpeed =
        (tracking.bytesUploaded * 8) / ((now - tracking.startTime) / 1000);

      // Calculate moving average
      tracking.movingAverageHistory.push(tracking.currentSpeed);
      if (
        tracking.movingAverageHistory.length >
        this.settings.moving_average_history_size
      ) {
        tracking.movingAverageHistory.shift();
      }

      tracking.movingAverageSpeed = SWFUpload.speed.calculateMovingAverage(
        tracking.movingAverageHistory
      );

      // Update times
      tracking.timeRemaining =
        ((file.size - tracking.bytesUploaded) * 8) /
  ...
  SWFUpload.speed.formatUnits = function (
    baseNumber,
    unitDivisors,
    unitLabels,
    singleFractional
    ...
    if (singleFractional) {
      unit = baseNumber;
      unitLabel =
        unitLabels.length >= unitDivisors.length
  ...
  SWFUpload.speed.formatBPS = function (baseNumber) {
    ...
    return SWFUpload.speed.formatUnits(
      baseNumber,
      bpsUnits,
      bpsUnitLabels,
      true
