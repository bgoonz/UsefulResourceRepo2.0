  ...
  wpList = {
    ...
    nonce: function (e, s) {
      ...
      return (
        s.nonce ||
        url._ajax_nonce ||
        $("#" + s.element + ' input[name="_ajax_nonce"]').val() ||
        url._wpnonce ||
        $("#" + s.element + ' input[name="_wpnonce"]').val() ||
        0
      );
    },
    ...
    parseData: function (e, t) {
      ...
      try {
        ...
        if (wpListsData) d = wpListsData[0].split(":");
      } catch (r) {}
    ...
    ajaxAdd: function (e, s) {
      ...
      s.data = $.param(
        $.extend(
          { _ajax_nonce: s.nonce, action: s.action },
          wpAjax.unserialize(data[4] || "")
        )
      );
      formData = $.isFunction(es.fieldSerialize)
        ? es.fieldSerialize()
        : es.serialize();
    ...
    ajaxDel: function (e, s) {
      ...
      s = s || {};
      var list = this,
        data = wpList.parseData(e, "delete"),
        element,
        res,
        rres;
    ...
    ajaxDim: function (e, s) {
      ...
      if (data[4]) s.dimAddColor = "#" + data[4];
      else s.dimAddColor = s.dimAddColor || "#FFFF33";

      if (data[5]) s.dimDelColor = "#" + data[5];
      ...
      s.success = function (r) {
        ...
        if (!res || res.errors) {
          element
            ...
            [isClass ? "removeClass" : "addClass"](s.dimClass)
            .show()
            .queue(function () {
              list.wpList.recolor();
              $(this).dequeue();
    ...
    add: function (e, s) {
      ...
      if ("none" != s.addColor) {
        ...
        e.css("backgroundColor", s.addColor).animate(
          { backgroundColor: color },
          {
            complete: function () {
              $(this).css("backgroundColor", "");
    ...
    clear: function (e) {
      ...
      e.find(":input").each(function () {
        if ($(this).parents(".form-no-clear").size()) return;
    ...
    this.each(function () {
      ...
      this.wpList = {
        settings: $.extend(
          {},
          wpList.settings,
          { what: wpList.parseData(this, "list")[1] || "" },
          settings
        ),
      };
