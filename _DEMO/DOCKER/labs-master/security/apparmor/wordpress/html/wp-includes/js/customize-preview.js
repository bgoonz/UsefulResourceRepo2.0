  ...
  $(function () {
    ...
    api.preview.bind("settings", function (values) {
      $.each(values, function (id, value) {
        if (api.has(id)) api(id).set(value);
        else api.create(id, value);
      });
    });
    ...
    api.preview.bind("setting", function (args) {
      ...
      if ((value = api(args.shift()))) value.set.apply(value, args);
    });
    ...
    /* Custom Backgrounds */
    bg = $.map(
      ["color", "image", "position_x", "repeat", "attachment"],
      function (prop) {
    ...
    api.when
      .apply(api, bg)
      .done(function (color, image, position_x, repeat, attachment) {
