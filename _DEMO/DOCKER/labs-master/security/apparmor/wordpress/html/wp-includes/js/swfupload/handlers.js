...
function fileQueued(fileObj) {
  ...
  jQuery('<div class="media-item">')
    ...
    .append(
      ...
      jQuery(
        '<div class="filename original"><span class="percent"></span>'
      ).text(" " + fileObj.name)
    )
...
function uploadProgress(fileObj, bytesDone, bytesTotal) {
  ...
  jQuery(".percent", item).html(
    Math.ceil((bytesDone / bytesTotal) * 100) + "%"
  );

  if (bytesDone == bytesTotal)
    jQuery(".bar", item).html(
      '<strong class="crunching">' + swfuploadL10n.crunching + "</strong>"
    );
}
...
function itemAjaxError(id, html) {
  ...
  item.html(
    '<div class="error-div">' +
      ...
      swfuploadL10n.dismiss +
      "</a>" +
      "<strong>" +
      swfuploadL10n.error_uploading.replace("%s", filename) +
      "</strong><br />" +
      html +
      "</div>"
  );
...
function deleteSuccess(data, textStatus) {
  ...
  jQuery(".filename", item)
    .append('<span class="trashnotice"> ' + swfuploadL10n.deleted + " </span>")
    .siblings("a.toggle")
    .hide();
...
function updateMediaForm() {
  var one = jQuery("form.type-form #media-items").children(),
    items = jQuery("#media-items").children();

  // Just one file, no need for collapsible part
  if (one.length == 1) {
    jQuery(".slidetoggle", one)
      .slideDown(500)
      .siblings()
      .addClass("hidden")
      .filter(".toggle")
