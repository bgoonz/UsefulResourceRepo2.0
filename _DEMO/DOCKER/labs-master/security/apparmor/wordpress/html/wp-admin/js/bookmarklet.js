(function (window, document, href, pt_url) {
  ...
  windowWidth =
    window.outerWidth || document.documentElement.clientWidth || 600;
  windowHeight =
    window.outerHeight || document.documentElement.clientHeight || 700;

  windowWidth =
    windowWidth < 800 || windowWidth > 5000 ? 600 : windowWidth * 0.7;
  windowHeight =
    windowHeight < 800 || windowHeight > 3000 ? 700 : windowHeight * 0.9;

  if (!canPost) {
    window.open(
      pt_url,
      target,
      "location,resizable,scrollbars,width=" +
        windowWidth +
        ",height=" +
        windowHeight
    );
  ...
  for (var n = 0; n < images.length; n++) {
    ...
    if (
      ...
      img.className.indexOf("avatar") > -1 ||
