...
(function ($, window) {
  var PressThis = function () {
    ...
    function textEditorResize(reset) {
      ...
      reset =
        reset === "reset" ||
        (textLength && textLength > textEditor.value.length);
      ...
      if (reset) {
        ...
        textEditor.style.height =
          Math.max(textEditor.scrollHeight, textEditorMinHeight) + "px";
      ...
      $.ajax({
        ...
        .done(function (response) {
          if (!response.success) {
            ...
            if (
              window.opener &&
              (settings.redirInParent || response.data.force)
            ) {
              try {
      ...
      $.post(window.ajaxurl, data, function (response) {
        if (!response.success) {
          ...
          $.each(response.data, function (i, newCat) {
            var $node = $("<li>").append(
              $(
                '<div class="category selected" tabindex="0" role="checkbox" aria-checked="true">'
              )
            ...
            if (newCat.parent) {
              if (!$ul || !$ul.length) {
                $parent = $wrap
                  .find('div[data-term-id="' + newCat.parent + '"]')
                  .parent();
    ...
    function addImg(src, displaySrc, i) {
      ...
      $element
        .attr("data-wp-src", src)
        .css("background-image", "url(" + displaySrc + ")")
        .find("span")
        .text(__("suggestedImgAlt").replace("%d", i + 1));
    ...
    function renderDetectedMedia() {
      ...
      $mediaThumbWrap = $(
        '<li class="suggested-media-thumbnail" tabindex="0"><span class="screen-reader-text"></span></li>'
      );
      ...
      if (data._embeds) {
        $.each(data._embeds, function (i, src) {
          ...
          $element
            .attr("data-wp-src", src)
            .find("span")
            .text(__("suggestedEmbedAlt").replace("%d", i + 1));
      ...
      if (data._images) {
        $.each(data._images, function (i, src) {
          ...
          if (src.indexOf("files.wordpress.com/") > -1) {
            displaySrc =
              displaySrc.replace(/\?.*$/, "") + "?w=" + smallestWidth;
          } else if (src.indexOf("gravatar.com/") > -1) {
            displaySrc =
              displaySrc.replace(/\?.*$/, "") + "?s=" + smallestWidth;
          } else {
            displaySrc = src;
          }
    ...
    function monitorOptionsModal() {
      ...
      $postOption.on("click", function () {
        ...
        $postOptions.addClass(isOffScreen).one(transitionEndEvent, function () {
          $(this).addClass(isHidden);
        });

        $targetSettingModal
          .removeClass(offscreenHidden)
    ...
    function monitorCatList() {
      $(".categories-select").on(
        "click.press-this keydown.press-this",
        function (event) {
    ...
    function monitor() {
      ...
      $document
        .on("tinymce-editor-init", function (event, ed) {
          editor = ed;
        ...
        .on(
          "click.press-this keypress.press-this",
        ...
        .on("click.press-this", function (event) {
          if (
            !$(event.target).closest("button").hasClass("split-button-toggle")
          ) {
            splitButtonClose();
          }
        });
      ...
      $(".options-panel, .options-panel-back").on(
        "focusout.press-this",
        function () {
          setTimeout(function () {
      ...
      $window
        .on("beforeunload.press-this", function () {
      ...
      $("button.add-cat-toggle").on("click.press-this", function () {
        ...
        $(
          ".setting-modal .add-category, .categories-search-wrapper"
      ...
      $textEditor.on(
        "focus.press-this input.press-this propertychange.press-this",
        textEditorResize
      );
