...
(function (exports, $) {
  ...
  focus = function (params) {
    ...
    focus = function () {
      ...
      if (
        ...
        focusContainer = construct.container.find(
  ...
  Container = api.Class.extend({
    ...
    initialize: function (id, options) {
      ...
      options.params = _.defaults(options.params || {}, container.defaults);
    ...
    onChangeActive: function (active, args) {
      ...
      if (!$.contains(document, construct.container[0])) {
        ...
        construct.container
          .stop(true, true)
          .slideDown(duration, args.completeCallback);
      } else {
        if (construct.expanded()) {
          construct.collapse({
            ...
            completeCallback: function () {
              construct.container
                .stop(true, true)
                .slideUp(duration, args.completeCallback);
            },
          });
        } else {
          construct.container
  ...
  api.Section = Container.extend({
    ...
    onChangeExpanded: function (expanded, args) {
      var section = this,
        ...
        sectionTitle = section.container
      ...
      if (expanded && !section.container.hasClass("open")) {
        if (args.unchanged) {
          ...
          expand = function () {
            ...
            $(window).on(
              "resize.customizer-section",
              _.debounce(resizeContentHeight, 100)
            );
    ...
    _recalculateTopMargin: function () {
      ...
      if (0 < offset) {
        content.css(
          "margin-top",
          parseInt(content.css("margin-top"), 10) - offset
        );
      }
    },
  });
  ...
  api.ThemesSection = api.Section.extend({
    ...
    attachEvents: function () {
      ...
      section.container
        .find(".change-theme, .customize-theme")
        .on("click keydown", function (event) {
          if (api.utils.isKeydownButNotEnterEvent(event)) {
            return;
          }
      ...
      var renderScreenshots = _.throttle(
        _.bind(section.renderScreenshots, this),
        100
      );
    ...
    onChangeExpanded: function (expanded, args) {
      ...
      if (expanded) {
        ...
        content.show(0, function () {
          ...
          content.css(
            "margin-top",
            $("#customize-header-actions").height() - position - scroll
          );
          section.addClass("current-panel");
          overlay.addClass("in-themes-panel");
          container.scrollTop(0);
          _.delay(panel.renderScreenshots, 10); // Wait for the controls
          panel.$customizeSidebar.on(
            "scroll.customize-themes-section",
            _.throttle(panel.renderScreenshots, 300)
          );
          if (args.completeCallback) {
            args.completeCallback();
          }
  ...
  api.Panel = Container.extend({
    ...
    onChangeExpanded: function (expanded, args) {
      ...
      var position,
        ...
        siblings = container.find(".open"),
        topPanel = overlay.find(
          "#customize-theme-controls > ul > .accordion-section > .accordion-section-title"
        ),
    ...
    _recalculateTopMargin: function () {
      ...
      content.css(
        "margin-top",
        parseInt(content.css("margin-top"), 10) -
          (content.offset().top - headerActionsHeight)
      );
    },
    ...
    renderContent: function () {
      ...
      if (template && panel.container) {
        panel.container
          .find(".accordion-sub-container")
          .html(template(panel.params));
      }
    },
  });
  ...
  api.Control = api.Class.extend({
    ...
    embed: function () {
      ...
      inject = function (sectionId) {
        var parentContainer;
        if (!sectionId) {
          // @todo allow a control to be embedded without a section, for instance a control embedded in the frontend
          return;
        }
        ...
        if (section.expanded()) {
          ...
          section.expanded.bind(function (expanded) {
            if (expanded && !control.isRendered) {
              api.Control.prototype.renderContent.apply(
                control,
                renderContentArgs
              );
              control.isRendered = true;
  ...
  api.PreviewFrame = api.Messenger.extend({
    ...
    run: function (deferred) {
      ...
      this.request.done(function (response) {
        ...
        response =
          response.slice(0, index) + response.slice(index + signature.length);
  ...
  api.Previewer = api.Messenger.extend({
    ...
    initialize: function (params, options) {
      ...
      this.add("previewUrl", params.previewUrl).setter(function (to) {
        ...
        if (/\/wp-admin(\/|$)/.test(to.replace(/[#?].*$/, ""))) return null;
  ...
  $(function () {
    ...
    _.each(["panel", "section", "control"], function (type) {
      var instance,
        id = api.settings.autofocus[type];
      if (id && api[type](id)) {
    ...
    $(".customize-controls-preview-toggle").on(
      "click keydown",
      function (event) {
        if (api.utils.isKeydownButNotEnterEvent(event)) {
          return;
    ...
    api.control("display_header_text", function (control) {
      ...
      control.element.bind(function (to) {
        if (!to) last = api("header_textcolor").get();
