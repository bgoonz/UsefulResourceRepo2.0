...
(function ($, wp, pagenow) {
  ...
  wp.updates.updatePlugin = function (plugin, slug) {
    ...
    if ("plugins" === pagenow || "plugins-network" === pagenow) {
      ...
      $message.attr(
        "aria-label",
        wp.updates.l10n.updatingLabel.replace("%s", name)
      );
  ...
  wp.updates.updateSuccess = function (response) {
    ...
    if ("plugins" === pagenow || "plugins-network" === pagenow) {
      ...
      $updateMessage.attr(
        "aria-label",
        wp.updates.l10n.updatedLabel.replace("%s", name)
      );
    }
  ...
  wp.updates.updateError = function (response) {
    ...
    if ("plugins" === pagenow || "plugins-network" === pagenow) {
      $message = $('[data-slug="' + response.slug + '"]')
        .next()
        .find(".update-message");
      ...
      $button
        .attr(
          "aria-label",
          wp.updates.l10n.updateFailedLabel.replace("%s", name)
        )
        .html(wp.updates.l10n.updateFailedShort)
  ...
  wp.updates.requestForCredentialsModalCancel = function () {
    ...
    if (
      wp.updates.updateLock === false &&
      wp.updates.updateQueue.length === 0
    ) {
      return;
    }
  ...
  $(document).ready(function () {
     ...
     */
    wp.updates.shouldRequestFilesystemCredentials =
      $("#request-filesystem-credentials-dialog").length <= 0 ? false : true;
    ...
    $(
      '#request-filesystem-credentials-dialog [data-js-action="close"], .notification-dialog-background'
    ).on("click", function () {
      wp.updates.requestForCredentialsModalCancel();
    });
    ...
    $("#plugin_update_from_iframe").on("click", function (e) {
      ...
      if (
        $.support.postMessage === false ||
        target === null ||
        window.parent.location.pathname.indexOf("update-core.php") !== -1
      )
        return;
      ...
      case "updatePlugin":
        ...
        if ("plugins" === pagenow || "plugins-network" === pagenow) {
          ...
          $("#" + message.slug)
            ...
            .focus();
          // trigger the update
          $('.plugin-update-tr[data-slug="' + message.slug + '"]')
            .find(".update-link")
            .trigger("click");
        } else if ("plugin-install" === pagenow) {
          $(".plugin-card-" + message.slug)
            .find(".column-name a")
            .focus();
          $(".plugin-card-" + message.slug)
            .find('[data-slug="' + message.slug + '"]')
            .trigger("click");
        }
        break;
    }
