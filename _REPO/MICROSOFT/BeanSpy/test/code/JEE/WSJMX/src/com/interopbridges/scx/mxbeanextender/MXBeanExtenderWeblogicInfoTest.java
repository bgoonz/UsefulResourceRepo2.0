...
import com.interopbridges.scx.mbeans.DummyWeblogicMBean;
 ...
 * specifically aimed at the Weblogic portion of the jeeserverinfo.
 * These tests test the loading of the JMX store
 * and the retrieval of the JEE server information
 * @author Geoff Erasmus
 */
public class MXBeanExtenderWeblogicInfoTest 
{
    /**
     * <p>
     ...
     * </p>
     */
    private static String mbeanName = "com.bea:Type=ServerRuntime";
    
    /**
     * <p>
     * The varsion string for the registered mbean 
     * </p>
     */
    private static String weblogicVer = "WebLogic Server 10.3.2.0 Tue Oct 20 12:16:15 PDT 2009 1267925";
    ...
    public void Setup() throws Exception 
    {
        ...
        JmxStores.addStoreToJmxStores(new WeblogicRuntimeJMXAbstraction(_MBeanstore)); 
         ...
         */
        DynamicMBean dummymbean = new DummyWeblogicMBean();
        try
        {
            /* 
            ...
            assertTrue(JmxStores.getListOfJmxStoreAbstractions().get(0).
                    getMBeanInfo(new ObjectName(mbeanName)).
                        getDescription().compareTo("Dummy Weblogic MBean")==0); 
            ...
            al.add(new Attribute("WeblogicVersion",weblogicVer));
            _MBeanstore.setAttributes(new ObjectName(mbeanName), al);
            
            /* 
             * Verify that the values were actually set 
             */
            String[] s = {"WeblogicVersion"};
            al = _MBeanstore.getAttributes(new ObjectName(mbeanName), s);
            assertTrue("WeblogicVersion", al.asList().get(0).getValue().toString().compareTo(weblogicVer)==0);
        }
        catch(Exception e)
        {
    ...
    public void testDoGet_AppServerInfo_JEEServerInfo() 
    {
        try 
        {
            ...
            String[] s = (String []) SAXParser.XPathQuery(xml,"/Info/JEEServer/Properties/AppServerName");
            Assert.assertTrue(
                    "Incorrect response to /Info/JEEServer/Properties/AppServerVersion does not exist.\n "+
                    "The XML element (/Info/JEEServer/Properties/AppServerVersion) is empty",s[0].compareTo(weblogicVer)==0);
        } 
        catch (Exception e) 
        {
