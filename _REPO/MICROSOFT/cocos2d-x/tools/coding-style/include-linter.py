...
def lint_one(header, ctx):
    ...
    linted = re.sub(
        '#\s*(include|import)\s*"(.*)"', lambda m: fix(m, cwd, ctx, fixed), content
    )
    if content != linted:
        ctx.error_files += 1
        if ctx.fix:
...
def main():
    ...
    parser.add_argument(
        "-f", "--fix", action="store_true", help="fixe the headers while linting"
    )
    parser.add_argument(
        "root",
        nargs="?",
        default=default_root,
        help="path to cocos2d-x source root directory",
    )
