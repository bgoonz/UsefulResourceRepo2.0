...
class RetrievalWidget(object):
    ...
    def update(self):
        ...
        im_paths = [str(distances_obj[i][0]) for i in range(len(distances_obj))]
        distances = [float(distances_obj[i][1]) for i in range(len(distances_obj))]
        ...
        for i in range(len(self.w_imgs)):
            ...
            if i < len(im_paths):
                ...
                w_button.tooltip = (
                    f"Rank {i} ({distance:.2f}), image index: {img_index}"
                )
    ...
    def _create_ui(self):
        ...
        for r in range(self.rows):
            hbox = widgets.HBox(
                children=[w_img_buttons[r * self.cols + c] for c in range(self.cols)]
            )
