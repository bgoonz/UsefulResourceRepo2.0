...
def instance2xml_base(anno, download_images):
    ...
    if "coco_url" not in anno:
        if "url" in anno:
            ...
            raise Exception(
                "Annotation has to contain a 'url' or 'coco_url' field to download the image."
            )
...
def parse_instance(content, outdir, download_images=False):
    ...
    if download_images:
        ...
        for index, obj in enumerate(content["images"]):
            ...
            im_local_filename = (
                os.path.splitext(os.path.basename(obj["file_name"]))[0] + ".jpg"
            )
    ...
    merged_info_list = list(
        map(
            cytoolz.merge,
            cytoolz.join("id", content["images"], "image_id", content["annotations"]),
        )
    )
    ...
    for instance in merged_info_list:
        assert (
            "category_id" in instance
        ), f"WARNING: annotation error: image {instance['file_name']} has a rectangle without a 'category_id' field."
...
def parse_keypoints(content, outdir):
    keypoints = dict(
        zip(
            range(1, len(content["categories"][0]["keypoints"]) + 1),
    ...
    for name, groups in cytoolz.groupby("file_name", merged_info_list).items():
        ...
        for group in groups:
            anno_tree = keypoints2xml_object(
                group, anno_tree, keypoints, bbox_type="xyxy"
            )
