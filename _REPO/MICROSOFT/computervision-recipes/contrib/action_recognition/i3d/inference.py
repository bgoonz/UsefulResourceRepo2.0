...
def predict_over_video(video_frame_list, window_width=9, stride=1):
    ...
    model = load_model(
        modality="RGB", state_dict_file="pretrained_chkpt/rgb_hmdb_split1.pt"
    )
    ...
    with torch.no_grad():
        ...
        for i in range(stride + window_width - 1, len(video_frame_list), stride):
            window_frame_list = [
                video_frame_list[j] for j in range(i - window_width, i)
            ]
            ...
            print(
                (
                    "Window:{0} Class pred:{1} Class proba:{2}".format(
                        window_count,
