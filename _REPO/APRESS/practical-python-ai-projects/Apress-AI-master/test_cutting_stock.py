...
def main():
    ...
    if sys.argv[1] == "data":
        tableutils.printmat(
            tableutils.wrapmat(
                C, [str(i) for i in range(n)], ["Order", "Nb rolls", "% Width"]
            )
        )
    elif sys.argv[1] == "run":
        start = time.clock()
        ...
        if rc != 0:
            ...
            R = [
                [
                    str(rolls[i][0]),
                    tableutils.set2string(tableutils.flatten(rolls[i][1:])),
                ]
                for i in range(nb)
            ]
            tableutils.printmat(
        ...
        R = [
            [str(rolls[i][0]), tableutils.set2string(tableutils.flatten(rolls[i][1:]))]
            for i in range(nb)
        ]
        tableutils.printmat(
            tableutils.wrapmat(
                ...
                [
                    "rolls",
                    "Waste " + str(sum(rolls[i][0] for i in range(nb))),
                    "Pattern ",
                ],
            ),
            True,
