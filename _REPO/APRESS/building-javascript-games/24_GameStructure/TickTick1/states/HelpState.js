...
function HelpState(layer) {
  ...
  // the background
  var background = new powerupjs.SpriteGameObject(
    sprites.background_help,
    ID.layer_background
  );
  this.add(background);

  // add a back button
  this.backButton = new powerupjs.Button(
    sprites.button_back,
    ID.layer_overlays
  );
  this.backButton.position = new powerupjs.Vector2(
    this.backButton.screenCenterX,
    750
  );
