...
function LevelFinishedState() {
  ...
  this.overlay = new powerupjs.SpriteGameObject(
    sprites.level_finished,
    ID.layer_overlays
  );
...
LevelFinishedState.prototype = Object.create(
...
LevelFinishedState.prototype.handleInput = function (delta) {
  if (powerupjs.Touch.isTouchDevice) {
    if (!powerupjs.Touch.containsTouchPress(this.overlay.boundingBox)) return;
  } else if (!powerupjs.Mouse.left.pressed) return;
