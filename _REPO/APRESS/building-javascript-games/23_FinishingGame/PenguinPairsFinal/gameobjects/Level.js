...
function Level(levelIndex) {
  ...
  this.hintButton = new powerupjs.Button(
    sprites.button_hint,
    ID.layer_overlays
  );
  ...
  this.retryButton = new powerupjs.Button(
    sprites.button_retry,
    ID.layer_overlays
  );
  ...
  this.quitButton = new powerupjs.Button(
    sprites.button_quit,
    ID.layer_overlays
  );
  ...
  playingField.position = new powerupjs.Vector2(
    (powerupjs.Game.size.x - width * 73) / 2,
    100
  );
  this.add(playingField);

  var hint = new powerupjs.SpriteGameObject(
    sprites.arrow_hint,
    ID.layer_objects_2
  );
  hint.sheetIndex = levelData.hint_arrow_direction;
  hint.position = new powerupjs.Vector2(
    levelData.hint_arrow_x * 73,
    ...
    for (var col = 0; col < width; col++) {
      ...
      switch (levelData.tiles[row][col]) {
        ...
        case "X":
          ...
          if (
            levelData.tiles[row][col] ===
            levelData.tiles[row][col].toUpperCase()
          )
        ...
        case "@":
          ...
          t.sheetIndex = row + (col % 2);
          tileField.addAt(t, col, row);
          var s = new powerupjs.SpriteGameObject(
            sprites.shark,
            ID.layer_objects_1
          );
  ...
  var helpField = new powerupjs.SpriteGameObject(
    sprites.help,
    ID.layer_overlays
  );
  ...
  helpLabel.position = new powerupjs.Vector2(
    helpLabel.screenCenterX,
    780 + (helpField.height - helpLabel.height) / 2
  );
  this.add(helpLabel);
}
...
Level.prototype.handleInput = function (delta) {
  ...
  if (this.hintButton.pressed) this.hintVisible.startVisible();
  else if (this.retryButton.pressed) this.reset();
...
Level.prototype.findAnimalAtPosition = function (pos) {
  for (var i = 0, j = this.animals.length; i < j; ++i)
    if (
      this.animals[i].currentBlock.equals(pos) &&
      this.animals[i].velocity.isZero
    )
...
Level.prototype.findSharkAtPosition = function (pos) {
  ...
  var sharkPos = new powerupjs.Vector2(
    pos.x * tileField.cellWidth,
    pos.y * tileField.cellHeight
  );
  for (var i = 0, l = this.sharks.length; i < l; ++i)
    if (this.sharks[i].position.equals(sharkPos)) return this.sharks[i];
  return null;
};
