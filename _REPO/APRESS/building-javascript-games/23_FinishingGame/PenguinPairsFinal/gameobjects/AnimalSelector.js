...
AnimalSelector.prototype.handleInput = function (delta) {
  ...
  if (this._arrowdown.pressed) animalVelocity.y = 1;
  else if (this._arrowup.pressed) animalVelocity.y = -1;
  else if (this._arrowleft.pressed) animalVelocity.x = -1;
  else if (this._arrowright.pressed) animalVelocity.x = 1;
  animalVelocity.multiplyWith(300);
  if (
    powerupjs.Mouse.left.pressed ||
    powerupjs.Touch.containsTouchPress(powerupjs.Game.screenRect)
  )
    this.visible = false;

  if (this.selectedAnimal === null || animalVelocity.isZero) return;
