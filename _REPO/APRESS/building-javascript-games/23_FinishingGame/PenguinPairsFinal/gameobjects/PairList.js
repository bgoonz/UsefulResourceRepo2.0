...
function PairList(nrPairs, layer, id) {
  ...
  this._pairs = [];
  for (var i = 0; i < nrPairs; i++) this._pairs.push(7);
...
Object.defineProperty(PairList.prototype, "completed", {
  get: function () {
    for (var i = 0, l = this._pairs.length; i < l; i++)
      if (this._pairs[i] === 7) return false;
    return true;
  },
});
...
PairList.prototype.addPair = function (index) {
  var i = 0;
  while (i < this._pairs.length && this._pairs[i] !== 7) i++;
  if (i < this._pairs.length) this._pairs[i] = index;
};

PairList.prototype.reset = function () {
  for (var i = 0, l = this._pairs.length; i < l; i++) this._pairs[i] = 7;
};
