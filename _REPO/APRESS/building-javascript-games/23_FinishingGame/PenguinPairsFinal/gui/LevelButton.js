...
function LevelButton(levelIndex, layer, id) {
  ...
  this._levelSolved = new powerupjs.SpriteGameObject(
    sprites.level_solved,
    ID.layer_overlays
  );
  this._levelUnsolved = new powerupjs.SpriteGameObject(
    sprites.level_unsolved,
    ID.layer_overlays
  );
  ...
  this._levelLocked = new powerupjs.SpriteGameObject(
    sprites.level_locked,
    ID.layer_overlays_2
  );
  ...
  textLabel.position = new powerupjs.Vector2(
    this._levelSolved.width - textLabel.width,
    this._levelSolved.height - textLabel.height + 50
  ).divideBy(2);
...
LevelButton.prototype.handleInput = function (delta) {
  if (window.LEVELS[this.levelIndex].locked) return;
  if (powerupjs.Touch.isTouchDevice)
    this.pressed =
      this.visible &&
      powerupjs.Touch.containsTouchPress(this._levelLocked.boundingBox);
  else
    this.pressed =
      this.visible &&
