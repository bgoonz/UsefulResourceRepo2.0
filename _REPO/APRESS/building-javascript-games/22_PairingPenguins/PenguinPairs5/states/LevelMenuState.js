...
function LevelMenuState(layer) {
  ...
  this.background = new SpriteGameObject(
    sprites.background_levelselect,
    ID.layer_background
  );
...
LevelMenuState.prototype.getSelectedLevel = function () {
  for (var i = 0, j = this.levelButtons.length; i < j; i += 1) {
    if (this.levelButtons[i].pressed) return this.levelButtons[i].levelIndex;
  }
  return -1;
};
...
LevelMenuState.prototype.handleInput = function (delta) {
  ...
  } else if (this.back.pressed) GameStateManager.switchTo(ID.game_state_title);
};
