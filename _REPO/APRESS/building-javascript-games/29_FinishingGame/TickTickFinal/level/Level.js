...
function Level(levelIndex, id) {
  ...
  this._quitButton = new powerupjs.Button(
    sprites.button_quit,
    ID.layer_overlays
  );

  this._quitButton.position = new powerupjs.Vector2(
    powerupjs.Game.size.x - this._quitButton.width - 10,
    10
  );
  ...
  var background_main = new powerupjs.SpriteGameObject(
    sprites.background_sky,
    ID.layer_background_1
  );
  background_main.position = new powerupjs.Vector2(
    0,
    powerupjs.Game.size.y - background_main.height
  );
  backgrounds.add(background_main);

  for (var i = 0; i < 5; i++) {
    var sprid = "mountain_" + Math.ceil(Math.random() * 2);
    var mountain = new powerupjs.SpriteGameObject(
      sprites[sprid],
      ID.layer_background_2
    );
    mountain.position = new powerupjs.Vector2(
      Math.random() * powerupjs.Game.size.x - mountain.width / 2,
  ...
  // timer
  var timerBackground = new powerupjs.SpriteGameObject(
    sprites.timer,
    ID.layer_overlays
  );
...
Object.defineProperty(Level.prototype, "completed", {
  get: function () {
    ...
    if (!exit.collidesWith(player)) return false;
    for (var i = 0, l = this._waterdrops.length; i < l; ++i) {
      if (this._waterdrops.at(i).visible) return false;
    }
    return true;
  },
});
...
Level.prototype.loadTiles = function () {
  ...
  this.add(hintField);
  var hintFrame = new powerupjs.SpriteGameObject(
    sprites.frame_hint,
    ID.layer_overlays_1
  );
  ...
  var hintTimer = new VisibilityTimer(
    hintField,
    ID.layer_overlays,
    ID.hint_timer
  );
  ...
  if (powerupjs.Touch.isTouchDevice) {
    var walkLeftButton = new powerupjs.Button(
      sprites.buttons_player,
      ID.layer_overlays,
      ID.button_walkleft
    );
    walkLeftButton.position = new powerupjs.Vector2(10, 500);
    this.add(walkLeftButton);
    var walkRightButton = new powerupjs.Button(
      sprites.buttons_player,
      ID.layer_overlays,
      ID.button_walkright
    );
    walkRightButton.position = new powerupjs.Vector2(
      walkRightButton.width + 20,
      500
    );
    walkRightButton.sheetIndex = 1;
    this.add(walkRightButton);
    var jumpButton = new powerupjs.Button(
      sprites.buttons_player,
      ID.layer_overlays,
      ID.button_jump
    );
    jumpButton.position = new powerupjs.Vector2(
      powerupjs.Game.size.x - jumpButton.width - 10,
      500
    );
  ...
  var tiles = new TileField(
    levelData.tiles.length,
    levelData.tiles[0].length,
    1,
    ID.tiles
  );
  this.add(tiles);
...
Level.prototype.loadTile = function (tileType, x, y) {
  switch (tileType) {
    ...
    case "+":
      return this.loadBasicTile(
        sprites.platform_hot,
        TileType.platform,
        true,
        false
      );
...
Level.prototype.loadStartTile = function (x, y) {
  ...
  var startPosition = new powerupjs.Vector2(
    (x + 0.5) * tiles.cellWidth,
...
Level.prototype.loadEndTile = function (x, y) {
  var tiles = this.find(ID.tiles);
  var exit = new powerupjs.SpriteGameObject(
    sprites.goal,
    ID.layer_objects,
    ID.exit
  );
  exit.position = new powerupjs.Vector2(
    x * tiles.cellWidth,
    (y + 1) * tiles.cellHeight
  );
...
Level.prototype.loadWaterTile = function (x, y) {
  ...
  w.position = new powerupjs.Vector2(
    (x + 0.5) * tiles.cellWidth,
...
Level.prototype.handleInput = function (delta) {
  ...
  if (!this._quitButton.pressed) return;
  this.reset();
...
Level.prototype.update = function (delta) {
  ...
  // check if we died
  if (!player.alive) timer.running = false;

  // check if we ran out of time
  if (timer.gameOver) player.explode();
