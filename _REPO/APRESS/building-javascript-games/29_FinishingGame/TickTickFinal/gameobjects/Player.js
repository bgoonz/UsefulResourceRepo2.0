...
Player.prototype.handleInput = function (delta) {
  ...
  } else if (this.onTheGround) this.velocity.x = 0;
  ...
  if (powerupjs.Touch.isTouchDevice) {
    ...
    if (walkLeftButton.down) this.velocity.x = -walkingSpeed;
    else if (walkRightButton.down) this.velocity.x = walkingSpeed;
  } else {
  ...
  if (this.velocity.x != 0) this.mirror = this.velocity.x < 0;

  if (powerupjs.Touch.isTouchDevice) {
    var jumpButton = this.root.find(ID.button_jump);
    if (jumpButton.pressed && this.onTheGround) this.jump();
  } else {
...
Player.prototype.update = function (delta) {
  ...
  if (this.onTheGround)
    if (this.velocity.x == 0) this.playAnimation("idle");
    else this.playAnimation("run");
  else if (this.velocity.y < 0) this.playAnimation("jump");

  var timer = this.root.find(ID.timer);
  if (this.walkingOnHot) timer.multiplier = 2;
  else if (this.walkingOnIce) timer.multiplier = 0.5;
  else timer.multiplier = 1;

  var tiles = this.root.find(ID.tiles);
  if (this.boundingBox.top >= tiles.rows * tiles.cellHeight) this.die(true);
};
...
Player.prototype.doPhysics = function () {
  if (!this.exploded) this.velocity.y += 55;
  if (this.alive) this.handleCollisions();
};
  ...
  for (var y = y_floor - 2; y <= y_floor + 1; ++y)
    for (var x = x_floor - 1; x <= x_floor + 1; ++x) {
      ...
      if (Math.abs(depth.x) < Math.abs(depth.y)) {
        if (tileType === TileType.normal) this.position.x += depth.x;
        continue;
      }
      if (
        this._previousYPosition <= tileBounds.top &&
