...
function Clouds(layer, id) {
  ...
  for (var i = 0; i < 3; i++) {
    var cloud = new powerupjs.SpriteGameObject(
      sprites["cloud_" + Math.ceil(Math.random() * 5)]
    );
    cloud.position = new powerupjs.Vector2(
      Math.random() * powerupjs.Game.size.x - cloud.width / 2,
...
Clouds.prototype.update = function (delta) {
  ...
  for (var i = 0, l = this.length; i < l; ++i) {
    ...
    if (
      ...
      else cloud.position = new powerupjs.Vector2(-cloud.width, cloudHeight);
      this.add(cloud);
      return;
    }
  }
