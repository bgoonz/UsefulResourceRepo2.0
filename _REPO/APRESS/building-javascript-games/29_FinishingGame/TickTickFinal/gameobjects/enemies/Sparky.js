...
Sparky.prototype.update = function (delta) {
  ...
  if (this.idleTime <= 0) {
    ...
    if (this.velocity.y != 0) {
      ...
      if (this.yoffset <= 0) this.velocity.y = 0;
      else if (this.yoffset >= 120) this.reset();
    } else if (this.animationEnded()) this.velocity.y = -60;
  } else {
    this.playAnimation("idle");
    this.idleTime -= delta;
    if (this.idleTime <= 0) this.velocity.y = 300;
  }
...
Sparky.prototype.checkPlayerCollision = function () {
  ...
  if (this.idleTime <= 0 && this.collidesWith(player)) player.die(false);
};
