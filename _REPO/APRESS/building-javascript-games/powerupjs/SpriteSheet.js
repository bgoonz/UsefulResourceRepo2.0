...
var powerupjs = (function (powerupjs) {
  function SpriteSheet(imageName, createCollisionMask) {
    ...
    if (colRow.length === 2) this._sheetRows = colRow[1];
  }
  ...
  SpriteSheet.prototype.getAlpha = function (x, y, sheetIndex, mirror) {
    if (this._collisionMask === null) return 255;

    var columnIndex = sheetIndex % this._sheetColumns;
    var rowIndex =
      Math.floor(sheetIndex / this._sheetColumns) % this._sheetRows;
    var textureX = columnIndex * this.width + x;
    if (mirror) textureX = (columnIndex + 1) * this.width - x - 1;
    var textureY = rowIndex * this.height + y;
    var arrayIndex = Math.floor(textureY * this._image.width + textureX);
    if (arrayIndex < 0 || arrayIndex >= this._collisionMask.length) return 0;
    else return this._collisionMask[arrayIndex];
  };
  ...
  SpriteSheet.prototype.draw = function (position, origin, sheetIndex, mirror) {
    ...
    var rowIndex =
      Math.floor(sheetIndex / this._sheetColumns) % this._sheetRows;
    var imagePart = new powerupjs.Rectangle(
      columnIndex * this.width,
      rowIndex * this.height,
      this.width,
      this.height
    );
    powerupjs.Canvas2D.drawImage(
      this._image,
      position,
      0,
      1,
