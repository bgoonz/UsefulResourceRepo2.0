...
JewelGrid.prototype.handleInput = function (delta) {
  if (Touch.isTouchDevice) this.handleInputTouch(delta);
  else this.handleInputMouse(delta);
};
...
JewelGrid.prototype.handleInputMouse = function (delta) {
  if (Mouse.left.down && !this.dragging) {
    var rect = new Rectangle(
      ...
      this.columns * this.cellHeight,
      this.rows * this.cellWidth
    );
    if (Mouse.containsMouseDown(rect)) {
      this.dragging = true;
      this._dragRow = Math.floor(
        (Mouse.position.y - this.worldPosition.y) / this.cellHeight
      );
  ...
  if (this.dragging) {
    ...
    if (
      firstObj.position.x < -this.cellWidth / 2 &&
    ...
    if (
      lastObj.position.x >
        (this.columns - 1) * this.cellWidth + this.cellWidth / 2 &&
      newpos - this._draggingLastX > 0
    )
...
JewelGrid.prototype.handleInputTouch = function (delta) {
  ...
  if (Touch.isTouching && !this.dragging) {
    if (Touch.containsTouch(rect)) {
      ...
      this.dragging = true;
      this._dragRow = Math.floor(
        (pos.y - this.worldPosition.y) / this.cellHeight
      );
...
JewelGrid.prototype.isValidCombination = function (a, b, c) {
  ...
  for (var i = 0; i < 3; i += 1) {
    if (
      (Math.floor(curra / divider) +
        Math.floor(currb / divider) +
        Math.floor(currc / divider)) %
        3 !==
      0
    )
      return false;
...
JewelGrid.prototype.shiftRowRight = function (selectedRow) {
  ...
  lastObj.position = new Vector2(
    -this.cellWidth + positionOffset,
...
JewelGrid.prototype.shiftRowLeft = function (selectedRow) {
  ...
  for (var x = 0; x < this._columns - 1; x += 1)
    this._gameObjects[selectedRow * this._columns + x] =
      this._gameObjects[selectedRow * this._columns + x + 1];
  this._gameObjects[selectedRow * this._columns + (this._columns - 1)] =
    firstObj;
  firstObj.position = new Vector2(
    this._columns * this.cellWidth + positionOffset,
    selectedRow * this.cellHeight
  );
};
