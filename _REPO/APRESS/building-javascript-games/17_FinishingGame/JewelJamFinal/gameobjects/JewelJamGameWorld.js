...
function JewelJamGameWorld(layer, id) {
  ...
  // the title screen
  var titleScreen = new SpriteGameObject(
    sprites.title,
    ID.layer_overlays_2,
    ID.title
  );
  ...
  var background = new SpriteGameObject(
    sprites.background,
    ID.layer_background
  );
  this.add(background);
  ...
  var helpFrame = new SpriteGameObject(
    sprites.frame_help,
    ID.layer_overlays,
    ID.help_frame
  );
  ...
  playingField.add(
    new GlitterField(
      2,
      columns * grid.cellWidth,
      rows * grid.cellHeight,
      ID.layer_overlays_1
    )
  );

  // game over overlay
  var gameOver = new SpriteGameObject(
    sprites.gameover,
    ID.layer_overlays_1,
    ID.game_over
  );
  ...
  var doubleTimer = new VisibilityTimer(
    doubleOverlay,
    ID.layer_overlays,
    ID.double_timer
  );
  ...
  var tripleTimer = new VisibilityTimer(
    tripleOverlay,
    ID.layer_overlays,
    ID.triple_timer
  );
...
JewelJamGameWorld.prototype.handleInput = function (delta) {
  ...
  if (titleScreen.visible) {
    if (Mouse.left.pressed || Touch.isPressing) titleScreen.visible = false;
    return;
  }

  // game over
  if (this.gameOver()) {
    if (Mouse.left.pressed || Touch.isPressing) this.reset();
    return;
  }
...
JewelJamGameWorld.prototype.update = function (delta) {
  ...
  if (!helpFrame.visible) GameObjectList.prototype.update.call(this, delta);
};
