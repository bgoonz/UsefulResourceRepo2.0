...
function LevelButton(levelIndex, layer, id) {
  ...
  this._levelSolved = new SpriteGameObject(
    sprites.level_solved,
    ID.layer_overlays
  );
  this._levelUnsolved = new SpriteGameObject(
  ...
  this._levelLocked = new SpriteGameObject(
  ...
  textLabel.position = new Vector2(
...
LevelButton.prototype.handleInput = function (delta) {
  ...
  if (Touch.isTouchDevice)
    this.pressed =
      this.visible && Touch.containsTouchPress(this._levelLocked.boundingBox);
  else
    this.pressed =
      this.visible &&
      Mouse.left.pressed &&
