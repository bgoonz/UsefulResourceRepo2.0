...
//			}
		...
		private bool FindLongestMatch(int curMatch) 
		{
			...
			*/
			...
			if (DeflaterConstants.DEBUGGING && strstart > 2*WSIZE - MIN_LOOKAHEAD) {
			    throw new Exception("need lookahead");
			}
			
			do {
				if (DeflaterConstants.DEBUGGING && curMatch >= strstart) {
					throw new Exception("future match");
				}
		...
		public void SetDictionary(byte[] buffer, int offset, int length) 
		{
			if (DeflaterConstants.DEBUGGING && strstart != 1) {
				throw new Exception("strstart not 1");
			}
...
//				}
		...
		public void SetInput(byte[] buf, int off, int len)
		{
			if (inputOff < inputEnd) {
				throw new Exception("Old input was not completely processed");
			}
