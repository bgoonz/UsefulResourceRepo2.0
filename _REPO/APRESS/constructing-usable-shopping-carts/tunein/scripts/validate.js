...
function validate(form)
{
  ...
  var requirements=new Array("required","alphabetic","address","alphanumeric","nospace",
                             "integer","decimal","minlength","maxlength","ccnumber","uszip",
        ...
        if(context.ccnumber)
        {
          ...
          else
          {
            ...
            if(!isLength)
            {
              if(firstErr<0)
                firstErr=i;
              errMsg += "Wrong number of digits for "+ccType+" credit card.\n";
            }
            else
            {
              var prefix;
              ...
              else
              {
                ...
                if(sum%10 != 0)
                {
                  if(firstErr<0)
                    firstErr=i;
                  errMsg+="The number for the "+ccType+" failed to validate.\n";
                  errMsg+="Please check the number carefully and try again.\n";
                }
                else
                {
                  var today=new Date();
                  ...
                  if(form.Expiration0Year_required==currYear && form.Expiration0Month<currMonth)
                  {
                    errMsg+="Invalid expiration date (you indicated a date in the past).";
                  }
                }
              }
            }
