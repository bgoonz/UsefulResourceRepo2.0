...
function ValidatorHookupControl(control, val) {
  if (
    typeof control.tagName == "undefined" &&
    typeof control.length == "number"
  ) {
    var i;
  ...
  if (
    ...
    for (j = 0; j < control.length; j++) {
      ...
      if (
        ...
        (inner.type != "radio" || inner.status == true)
      ) {
        return inner.value;
      }
    }
  ...
  if (dataType == "Integer") {
    ...
    exp = new RegExp(
      "^\\s*([-\\+])?(((\\d+)\\" +
        ...
        ")*)(\\d+)" +
        (val.digits > 0
          ? "(\\" + val.decimalchar + "(\\d{1," + val.digits + "}))?"
          : "") +
        "\\s*$"
    );
    m = op.match(exp);
    ...
    cleanInput =
      m[1] +
      intermed.replace(new RegExp("(\\" + val.groupchar + ")", "g"), "") +
      (val.digits > 0 ? "." + m[7] : 0);
    num = parseFloat(cleanInput);
    return isNaN(num) ? null : num;
