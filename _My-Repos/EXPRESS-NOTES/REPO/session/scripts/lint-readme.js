...
var BADGE_STAR_LINK_REGEXP =
  /^https:\/\/badgen.net\/github\/stars\/[^/]+\/[^/]+?label=%E2%98%85$/;
    ...
    switch (state) {
      ...
      case 1: // header
        ...
        if (!(header = STORE_HEADER_REGEXP.exec(line))) {
          ...
          prev[2]
            .replace(/^[^/]+\//, "")
            .localeCompare(header[2].replace(/^[^/]+\//, "")) > 0
        ) {
          expect(
            lineidx,
            header[2] + " to be alphabetically before " + prev[2],
            line
          );
          state = 2;
        } else {
      ...
      case 3: // url link
        ...
        if (!urlLink) {
          ...
          urlLink[2] !==
          "https://www.npmjs.com/package/" + header[2]
        ) {
          expect(
            lineidx,
            "link url of https://www.npmjs.com/package/" + header[2],
            line
          );
        } else {
          state = 4;
