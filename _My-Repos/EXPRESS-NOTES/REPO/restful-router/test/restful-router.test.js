...
describe("restful-router.test.js", function () {
  ...
  it("should post /users => user.create", function (done) {
    ...
    request(app)
      ...
      .send({ name: "foo" })
      .expect(
        'POST /users => create, query: {}, params: {}, body: {"name":"foo"}'
      )
      .expect(200, done);
  ...
  it("should put /users/:id => user.update", function (done) {
    request(app)
      ...
      .send({ name: "fooupdate" })
      .expect(
        'PATCH /users/:id => update, query: {}, params: {"id":"123"}, body: {"name":"fooupdate"}'
      )
      .expect(200, done);
  });
  ...
  it("should delete /users/:id => user.destroy", function (done) {
    request(app)
      .del("/users/123")
      .expect(
        'DELETE /users/:id => destroy, query: {}, params: {"id":"123"}, body: {}'
      )
      .expect(200, done);
  });
  ...
  it("should put /users => 404", function (done) {
    request(app).put("/users").send({ name: "fooupdate" }).expect(404, done);
  });

  it("should delete /users => 404", function (done) {
    request(app).del("/users").expect(404, done);
  });

  it("should get /foos => 404", function (done) {
    request(app).get("/foos").expect(404, done);
  });
  ...
  it("should post /users/123/foos => 404", function (done) {
    request(app).post("/users/123/foos").expect(404, done);
  });
  ...
  it("should get /users/123/foos/2013-06-18 => 200", function (done) {
    request(app)
      ...
      .expect(
        'GET /users/:uid/foos/:date => show, params: {"uid":"123","date":"2013-06-18"}'
      )
      .expect(200, done);
  });
});
